...
class DebugServiceClient:
    ...
    def __init__(self, gateway_host: str, service_port: int = 8765, ssh_tunnel: bool = True):
        """
        Initialize debug service client.

        Args:
            gateway_host: Lagerbox IP address
        ...
        if ssh_tunnel:
            ...
            # CLI should establish tunnel: ssh -L 8765:127.0.0.1:8765 <username>@gateway
            # Username is determined from DUT storage (defaults to 'lagerdata' if not set)
            self.base_url = f'http://127.0.0.1:{service_port}'
        else:
            # Direct connection (only works if service binds to 0.0.0.0)
            self.base_url = f'http://{gateway_host}:{service_port}'
    ...
    def connect(self, net: Dict[str, Any], speed: Optional[str] = None,
                force: bool = False, halt: bool = False) -> Dict[str, Any]:
        """
        Connect to debugger.
    ...
    def rtt(self, channel: int = 0, timeout: Optional[int] = None):
        """
    ...
    def start_gdbserver_tunnel(self) -> Dict[str, Any]:
        """
        Start a local GDB server tunnel using the gateway controller endpoint via SSH tunnel.

        The controller runs on localhost:8080 on the gateway, so we need to use
        an SSH tunnel to access it.

        Returns:
            Dict with 'host' and 'port' keys
        """
        # The controller is accessible via the SSH tunnel we already have at localhost:8080
        # Actually, we need to create a separate tunnel for the controller
        # For now, let's try using the gateway's internal hostname
        controller_url = f'http://controller:8080/openocd/local_tunnel'

        # Use the existing SSH tunnel session to reach the controller
        # The controller service runs in Docker and is accessible at 'controller:8080'
        import subprocess
        import socket
        import time

        # Create SSH tunnel for controller (8080 on gateway -> localhost:18080)
        tunnel_proc = None
        local_controller_port = 18080

        try:
            # Build SSH tunnel command
            from cli.ssh_utils import get_ssh_connection_pool
            pool = get_ssh_connection_pool()
            ssh_opts = pool.get_ssh_options(self.gateway_host)

            tunnel_cmd = [
                'ssh',
                '-N',
                '-L', f'{local_controller_port}:controller:8080',
            ] + ssh_opts + [f'lagerdata@{self.gateway_host}']
            ...
            # Make request via tunnel
            controller_url = f'http://127.0.0.1:{local_controller_port}/openocd/local_tunnel'
            response = self.session.post(
                controller_url,
                json={},
                timeout=10
