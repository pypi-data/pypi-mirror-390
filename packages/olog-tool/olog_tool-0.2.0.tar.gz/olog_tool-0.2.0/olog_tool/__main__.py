...
class OlogClient:
    ...
    def get_tags(self) -> List[Dict[str, Any]]:
        """
        Get all tags from the Olog server.
    ...
    def create_tag(self, tag_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        url = f"{self.base_url}/tags/{tag_data.get('name', '')}"
        
        try:
            response = self.session.put(url, json=tag_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def create_tags(self, tags_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            response = self.session.put(url, json=tags_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    # Properties methods
    def get_properties(self, include_inactive: bool = False) -> List[Dict[str, Any]]:
        """
        Get all properties from the Olog server.
        ...
        params = {'inactive': 'true'} if include_inactive else {}
    ...
    def create_property(self, property_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        url = f"{self.base_url}/properties/{property_data.get('name', '')}"
        
        try:
            response = self.session.put(url, json=property_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def create_properties(self, properties_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            List of created property data
        """
        url = f"{self.base_url}/properties"
        
        try:
            response = self.session.put(url, json=properties_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def get_logbooks(self) -> List[Dict[str, Any]]:
        """
        Get all logbooks from the Olog server.
        
        Returns:
            List of logbook dictionaries
        """
        ...
        try:
            ...
            print(f"Error getting logbooks: {e}")
            return []
    
    def create_logbook(self, logbook_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Create a new logbook.
        
        Args:
            logbook_data: Logbook data dictionary
            
        Returns:
            Created logbook data or None if failed
        """
        url = f"{self.base_url}/logbooks/{logbook_data.get('name', '')}"
        
        try:
            response = self.session.put(url, json=logbook_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            print(f"Error creating logbook: {e}")
            return None
    
    def create_logbooks(self, logbooks_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Create multiple logbooks.
        
        Args:
            logbooks_data: List of logbook data dictionaries
            
        Returns:
            List of created logbook data
        """
        url = f"{self.base_url}/logbooks"
        
        try:
            response = self.session.put(url, json=logbooks_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            print(f"Error creating logbooks: {e}")
            return []
    
    # Levels methods
    def get_levels(self) -> List[Dict[str, Any]]:
        """
        Get all levels from the Olog server.
    ...
    def create_level(self, level_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Create a new level.
        ...
        Returns:
            Created level data or None if failed
        """
        url = f"{self.base_url}/levels/{level_data.get('name', '')}"
        
        try:
            response = self.session.put(url, json=level_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def create_levels(self, levels_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Create multiple levels.
        
        Args:
            levels_data: List of level data dictionaries
            
        Returns:
            List of created level data
        """
        url = f"{self.base_url}/levels"
        
        try:
            response = self.session.put(url, json=levels_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def get_templates(self) -> List[Dict[str, Any]]:
        """
        Get all templates from the Olog server.
    ...
    def create_template(self, template_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.session.put(url, json=template_data)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
...
class OlogBackup:
    ...
    def backup_metadata(self, output_dir: Path) -> bool:
        """
        Backup all metadata entities (tags, properties, logbooks, levels, templates).
        ...
        Returns:
            True if successful, False otherwise
        """
        print(f"Starting metadata backup to {output_dir}")
        output_dir.mkdir(parents=True, exist_ok=True)
        
        metadata = {}
        
        # Backup tags
        print("Backing up tags...")
        tags = self.client.get_tags()
        metadata['tags'] = tags
        print(f"Found {len(tags)} tags")
        
        # Backup properties
        print("Backing up properties...")
        properties = self.client.get_properties(include_inactive=True)
        metadata['properties'] = properties
        print(f"Found {len(properties)} properties")
        
        # Backup logbooks
        print("Backing up logbooks...")
        logbooks = self.client.get_logbooks()
        metadata['logbooks'] = logbooks
        print(f"Found {len(logbooks)} logbooks")
        
        # Backup levels
        print("Backing up levels...")
        ...
        print("Metadata backup completed successfully")
        return True
    
    def restore_metadata(self, backup_dir: Path) -> bool:
        """
        Restore metadata entities from backup.
        ...
        Returns:
            True if successful, False otherwise
        """
        print(f"Starting metadata restore from {backup_dir}")
        
        metadata_file = backup_dir / 'metadata.json'
        if not metadata_file.exists():
            print(f"Error: metadata.json not found in {backup_dir}")
            return False
        ...
        if tags:
            print(f"Restoring {len(tags)} tags...")
            created_tags = self.client.create_tags(tags)
            print(f"Created {len(created_tags)} tags")
        
        # Restore properties
        properties = metadata.get('properties', [])
        if properties:
            print(f"Restoring {len(properties)} properties...")
            created_properties = self.client.create_properties(properties)
            print(f"Created {len(created_properties)} properties")
        
        # Restore logbooks
        logbooks = metadata.get('logbooks', [])
        if logbooks:
            print(f"Restoring {len(logbooks)} logbooks...")
            created_logbooks = self.client.create_logbooks(logbooks)
            print(f"Created {len(created_logbooks)} logbooks")
        
        # Restore levels
        levels = metadata.get('levels', [])
        if levels:
            print(f"Restoring {len(levels)} levels...")
            created_levels = self.client.create_levels(levels)
            print(f"Created {len(created_levels)} levels")
        
        # Restore templates
        templates = metadata.get('templates', [])
        if templates:
            print(f"Restoring {len(templates)} templates...")
            for template in templates:
                created_template = self.client.create_template(template)
                if created_template:
                    print(f"Created template: {template.get('name', 'Unknown')}")
        
        print("Metadata restore completed successfully")
        return True
...
def main():
    ...
    # Metadata backup command
    metadata_backup_parser = subparsers.add_parser('backup-metadata', help='Backup metadata (tags, properties, logbooks, levels, templates)')
    metadata_backup_parser.add_argument('--output', '-o', required=True, help='Output directory for metadata backup')
    ...
    metadata_restore_parser = subparsers.add_parser('restore-metadata', help='Restore metadata from backup')
    metadata_restore_parser.add_argument('--input', '-i', required=True, help='Input directory containing metadata backup')
    ...
    create_template_parser = subparsers.add_parser('create-template', help='Create a new template')
    create_template_parser.add_argument('--name', required=True, help='Template name')
    create_template_parser.add_argument('--title', required=True, help='Template title')
    create_template_parser.add_argument('--level', default='Info', help='Log level (default: Info)')
    create_template_parser.add_argument('--source', help='Template source text (markdown)')
    create_template_parser.add_argument('--logbooks', help='Comma-separated list of logbook names')
    create_template_parser.add_argument('--tags', help='Comma-separated list of tag names')
    create_template_parser.add_argument('--properties', help='JSON string of properties')
    
    # Create property command
    create_property_parser = subparsers.add_parser('create-property', help='Create a new property')
    create_property_parser.add_argument('--name', required=True, help='Property name')
    create_property_parser.add_argument('--attributes', required=True, help='JSON string of attributes (e.g., \'[{"name": "attr1", "value": "val1"}]\'')
    
    # Create logbook command
    create_logbook_parser = subparsers.add_parser('create-logbook', help='Create a new logbook')
    create_logbook_parser.add_argument('--name', required=True, help='Logbook name')
    create_logbook_parser.add_argument('--owner', help='Logbook owner')
    ...
    if args.command == 'backup':
    ...
    elif args.command == 'backup-metadata':
        backup = OlogBackup(client)
        success = backup.backup_metadata(Path(args.output))
        return 0 if success else 1
    ...
    elif args.command == 'restore-metadata':
        backup = OlogBackup(client)
        success = backup.restore_metadata(Path(args.input))
        return 0 if success else 1
        ...
        if args.source:
            template_data['source'] = args.source
            
        if args.logbooks:
            template_data['logbooks'] = [{'name': lb.strip()} for lb in args.logbooks.split(',')]
            
        if args.tags:
            template_data['tags'] = [{'name': tag.strip()} for tag in args.tags.split(',')]
            
        if args.properties:
            try:
                template_data['properties'] = json.loads(args.properties)
        ...
        if result:
            print(f"Property '{args.name}' created successfully")
            print(json.dumps(result, indent=2))
            return 0
        else:
            print(f"Failed to create property '{args.name}'")
            return 1
    
    elif args.command == 'create-logbook':
        # Build logbook data
        logbook_data = {
            'name': args.name
        }
        ...
        if args.owner:
            logbook_data['owner'] = args.owner
        
        result = client.create_logbook(logbook_data)
        if result:
            print(f"Logbook '{args.name}' created successfully")
            print(json.dumps(result, indent=2))
            return 0
        else:
            print(f"Failed to create logbook '{args.name}'")
            return 1
