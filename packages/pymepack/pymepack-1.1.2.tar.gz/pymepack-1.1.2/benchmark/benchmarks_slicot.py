...
#slycot solvers not supporting factorization
no_fact_spprt_in_slycot = ["gesylv","gesylv2"]
...
gelyap_setup = {
    "display_order":["slycot", "gelyap_s1", "gelyap_s2", "gelyap_s3"],
    "slycot": {
        "display_name": "sb03md",
        "stmt": "slycot.sb03md(M,x,a,q,'C',"
                "fact=FACT,trana='T',ldwork = max(2*M*M, 3*M)*2)",
        "setup": "A.refresh(); X.refresh(); Q.refresh();"
                 "a = A.get_data().T;q = Q.get_data();x = X.get_data()"
    },
    "gelyap_s1":{
...
gestein_setup = {
    "display_order":["slycot", "gestein_s1", "gestein_s2", "gestein_s3"],
    "slycot": {
        "display_name": "sb03md",
        "stmt": "slycot.sb03md(M,x,a,q,'D',trana='T',"
                "fact=FACT,ldwork=max(2*M*M,3*M)*2)",
        "setup": "A.refresh(); X.refresh(); Q.refresh();"
                 "a = A.get_data();q = Q.get_data();x = X.get_data()"
    },
    "gestein_s1":{
...
gglyap_setup = {
    "display_order":["slycot", "gglyap_s1", "gglyap_s2", "gglyap_s3"],
    "slycot": {
        "display_name": "sg03ad",
        "stmt": """slycot.sg03ad('C','X',FACT,'T','L',
                                 M,a,b,q,z,x,ldwork=max(2*M*M, 8*M+16))""",
        "setup": "A.refresh();B.refresh(),X.refresh();Q.refresh();Z.refresh();"
                 "a = A.get_data();b = B.get_data();x = X.get_data();"
                 "q = Q.get_data();z=Z.get_data()"
...
ggstein_setup = {
    "display_order":["slycot", "ggstein_s1", "ggstein_s2", "ggstein_s3"],
    "slycot": {
        "display_name": "sg03ad",
        "stmt": "slycot.sg03ad('D','X',FACT,'T','U',"
                "M,a,b,q,z,x,ldwork=max(2*M*M, 8*M+16))",
        "setup": "A.refresh();B.refresh();X.refresh();Q.refresh();Z.refresh();"
                 "a = A.get_data();b = B.get_data();x = X.get_data();"
                 "q = Q.get_data();z = Z.get_data()"
...
gesylv_setup = {
    "display_order":["slycot", "gesylv_s1", "gesylv_s2", "gesylv_s3"],
    "slycot": {
        "display_name": "sb04md",
        "stmt": "slycot.sb04md(M,M,a,b,x)",
        "setup": "A.refresh(); B.refresh(); X.refresh();"
                 "a = A.get_data();b = B.get_data();x = X.get_data()"
    },
    "gesylv_s1":{
...
gesylv2_setup = {
    "display_order":["slycot", "gesylv2_s1", "gesylv2_s2", "gesylv2_s3"],
    "slycot": {
        "display_name": "sb04qd",
        "stmt": "slycot.sb04qd(M,M,a,b,x)",
        "setup": "A.refresh(); B.refresh(); X.refresh();"
                 "a = A.get_data();b = B.get_data();x = X.get_data();"
                 "qa = QA.get_data() if FACT == 'F' else None;"
                 "qb = QB.get_data() if FACT == 'F' else None"
...
def slycot_bench(solver, shape="G", repeat=3, precision=np.double, path_to_test_data=None):
    ...
    for key in benchmark_setup['display_order']:
        if (shape == "F" and key=="slycot"
                and solver.lower() in no_fact_spprt_in_slycot):
            continue
        if shape == "H" and key=="slycot":
            continue
        if precision == np.single and key == "slycot":
            continue
    ...
    for M in M_values:
        ...
        if shape == "F"  and solver.lower() in no_fact_spprt_in_slycot:
            benchmark_setup["slycot"]['timings'] = [MIN_BENCH_TIME_SECS] # to skip slycot
        if shape == "H":
            benchmark_setup["slycot"]["timings"] = [MIN_BENCH_TIME_SECS]
        ...
        while min_total_runtime < MIN_BENCH_TIME_SECS:
            ...
            min_total_runtime = min([sum(benchmark_setup[key]['timings'])
                for key in benchmark_setup['display_order']])
        line = "-" * (len(header) - 4)
        print(line)
        ...
        for key in benchmark_setup['display_order']:
            ...
            line.append(
                    ("avg: " + "{:.7f}".format(
                     np.mean(benchmark_setup[key]['timings']))).ljust(17)
                )
            line.append(bound)
        ...
        for key in benchmark_setup['display_order']:
            ...
            line.append(
                    (
                     "min: " + "{:.7f}".format(
                            min(benchmark_setup[key]['timings']))
                    ).ljust(17) + bound
                 )
        print(''.join(line))
