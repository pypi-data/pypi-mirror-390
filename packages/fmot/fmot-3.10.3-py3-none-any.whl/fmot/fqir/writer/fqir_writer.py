...
from fmot.fqir.writer import fftlib
from fmot.fqir.writer.loop_nesting import nest_as_sequential_loop
...
class FQIRWriter:
    ...
    def divide(
        ...
        recip_lo, recip_hi, mask_hi = self._pos_reciprocal_v2_i16_components(den, eps)

        outs = []
        for num_i, quanta_i in zip(num, quanta):
            y_hi = self.multiply(recip_hi, num_i, quanta=quanta_i)
            y_lo = self.multiply(recip_lo, num_i, quanta=quanta_i)
            y = self.masked_construct(mask_hi, y_hi, y_lo, quanta=quanta_i)
            if sign is not None:
                y = self.multiply(y, sign, quanta=quanta_i)
            outs.append(y)
    ...
    def inline_fqir_graph_in_sequential_loop(
        ...
        n_iter: int,
        inputs: list[TensorProto],
        decomp_temp_unfold: bool = True,
    ):
        return nest_as_sequential_loop(self, n_iter, inputs, graph, decomp_temp_unfold)
    ...
    def rfft(self, x: TensorProto, order: int, quanta=None):
        """
        ...
        """
        return fftlib.rfft(self, x, order, quanta)
