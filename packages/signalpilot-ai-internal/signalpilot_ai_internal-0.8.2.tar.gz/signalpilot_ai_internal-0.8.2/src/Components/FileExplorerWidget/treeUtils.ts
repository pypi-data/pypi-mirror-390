import { ISupportedFileEntry, TreeNode, FolderNode, FileNode } from './types';
...
function sortTreeNodes(nodes: TreeNode[]): void {
  ...
  nodes.sort((a, b) => {
    ...
    return a.type === 'folder'
      ? a.name.localeCompare((b as FolderNode).name)
      : (a as FileNode).file.name.localeCompare((b as FileNode).file.name);
  });
