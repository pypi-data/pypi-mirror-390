...
export class DataLoaderService {
   ...
   */
  private static formatNotebookSchema(schema: ISuccessfulDatasetSchema): string {
    ...
    if (schema.sampleData && schema.sampleData.length > 0) {
      ...
      for (let i = 0; i < maxCells; i++) {
        ...
        schemaContent += `\n  Cell ${i + 1} [${cellType}]`;
        if (cellType === 'code' && executionCount !== null && executionCount !== undefined) {
          schemaContent += ` (execution ${executionCount})`;
        }
        schemaContent += ':\n';

        // Truncate source to max 300 chars
        const truncatedSource = source.length > 300 ? source.substring(0, 300) + '...' : source;
        ...
        if (cellType === 'code' && cell.outputs && cell.outputs.length > 0) {
          ...
          for (let j = 0; j < maxOutputs; j++) {
            ...
            if (output.text) {
              const truncatedText = output.text.length > 100 ? output.text.substring(0, 100) + '...' : output.text;
