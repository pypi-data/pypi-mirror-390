...
def get_data_files():
    data_files = []
    # 定义需要打包的文件及其目标目录
    # 1. 打包 config.json：安装到 netease_audiodiff 包目录下
    config_file = "netease_audiodiff/config.json"  # 源文件路径
    if os.path.exists(config_file):
        # 目标路径：netease_audiodiff/（与包目录一致），文件直接放在包根目录
        data_files.append(("netease_audiodiff", [config_file]))
    else:
        print(f"警告：{config_file} 不存在，将不会被打包")
    file_list = [
        "netease_audiodiff/ffmpeg.exe",
        "netease_audiodiff/ffprobe.exe"
    ]
