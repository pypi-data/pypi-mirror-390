    ...
    def mergeBranch(self, targetBranch: str, moduleName: str) -> None:
        ...
        try:
            ...
            if os.path.exists(module_path):
                ...
                success, diff_output = self.gitManager.runCommand(f'git diff --name-only {current_branch}...HEAD -- {moduleName}/')
                if not success:
                ...
                if module_files:
                    ...
                    # 提交更改
                    success, commit_output = self.gitManager.runCommand(f'git commit -m "Merge {targetBranch} changes for {moduleName} module"')
                    if success:
                        print(f"✅ 成功提交模块更改")
                    else:
