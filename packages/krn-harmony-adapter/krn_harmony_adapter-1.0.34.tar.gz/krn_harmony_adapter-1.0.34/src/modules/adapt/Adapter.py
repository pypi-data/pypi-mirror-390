...
class Adapter(Config):
    ...
    def mergeBranch(self, targetBranch: str, moduleName: str) -> None:
        ...
        try:
            ...
            # 获取当前分支和临时分支之间的差异（只针对指定模块）
            success, diff_output = self.gitManager.runCommand(f'git diff --name-only {current_branch}...HEAD -- {target_module_dir}/')
            if not success:
            ...
            # 过滤出属于指定模块的文件
            module_files = [f for f in changed_files if f.startswith(f"{target_module_dir}/")]
