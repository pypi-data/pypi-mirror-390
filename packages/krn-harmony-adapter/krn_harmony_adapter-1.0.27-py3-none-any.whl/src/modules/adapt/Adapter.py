...
class Adapter(Config):
    ...
    def mergeBranch(self, targetBranch: str, moduleName: str) -> None:
        ...
        # 初始化 GitManager
        git_manager = GitManager(self.projectPath)
        
        try:
            # 1. 检查当前分支状态
            current_branch = git_manager.getCurrentBranch()
            print(f"📍 当前分支: {current_branch}")
            
            # 2. 检查目标分支是否存在
            if not git_manager.branchExists(targetBranch):
            ...
            # 创建并切换到临时分支
            success, output = git_manager.runCommand(f'git checkout -b {temp_branch}')
            if not success:
                print(f"❌ 创建临时分支失败: {output}")
                return
            
            # 4. 合并目标分支到临时分支
            success, output = git_manager.runCommand(f'git merge {targetBranch}')
            if not success:
                print(f"❌ 合并失败: {output}")
                # 回滚临时分支
                git_manager.runCommand(f'git checkout {current_branch}')
                git_manager.runCommand(f'git branch -D {temp_branch}')
                return
            ...
            if os.path.exists(module_path):
                # 获取模块目录下的所有更改文件
                success, diff_output = git_manager.runCommand(f'git diff --name-only {current_branch}...HEAD')
                if not success:
                ...
                if module_files:
                    ...
                    # 将这些更改应用到当前分支
                    success, output = git_manager.runCommand(f'git checkout {current_branch}')
                    if not success:
                    ...
                    for file in module_files:
                        success, output = git_manager.checkoutFileFromBranch(temp_branch, file)
                        if success:
                            print(f"  ✅ 应用更改: {file}")
                        else:
            ...
            # 6. 清理临时分支
            success, output = git_manager.runCommand(f'git branch -D {temp_branch}')
            if success:
