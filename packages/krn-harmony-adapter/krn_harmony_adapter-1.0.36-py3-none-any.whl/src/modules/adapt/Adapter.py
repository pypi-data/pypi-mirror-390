...
class Adapter(Config):
    ...
    def mergeBranch(self, targetBranch: str, moduleName: str) -> None:
        ...
        try:
            ...
            # 2. 检查目标分支是否存在
            # 如果是远程分支（包含 origin/），需要特殊处理
            branch_name = targetBranch.replace('origin/', '')
            if not self.gitManager.branchExists(branch_name) and not self.gitManager.branchExists(targetBranch):
            ...
            # 4. 合并目标分支到临时分支
            # 先检查是否有未提交的更改
            success, status_output = self.gitManager.runCommand('git status --porcelain')
            if success and status_output.strip():
                print(f"⚠️  当前分支有未提交的更改，尝试暂存...")
                # 暂存当前更改
                success, output = self.gitManager.runCommand('git stash push -m "kha-temp-stash"')
                if not success:
                    print(f"❌ 无法暂存更改: {output}")
                    # 回滚临时分支
            ...
            try:
                ...
                if targetBranch.startswith('origin/'):
                    # 先获取远程分支的最新内容
                    success, output = self.gitManager.runCommand(f'git fetch origin {targetBranch.split("/", 1)[1]}')
                    if not success:
                        print(f"❌ 获取远程分支失败: {output}")
                        raise Exception("fetch failed")
                    
                    # 使用远程分支的最新提交进行合并
                    success, output = self.gitManager.runCommand(f'git merge origin/{targetBranch.split("/", 1)[1]} --no-commit')
                else:
                    success, output = self.gitManager.runCommand(f'git merge {targetBranch} --no-commit')
                ...
                if "CONFLICT" in output.upper():
                    print("⚠️  检测到合并冲突，尝试自动解决...")
                    # 尝试自动解决冲突
                    success, resolve_output = self.gitManager.runCommand('git merge --abort')
                    if success:
                        print("✅ 已中止合并，将使用 cherry-pick 方式")
                    else:
                        print(f"⚠️  无法中止合并: {resolve_output}")
                
            except Exception as e:
                # 恢复暂存的更改
                if stashed:
                    self.gitManager.runCommand('git stash pop')
            ...
            # 5. 只保留指定模块的更改
            # 检查是否存在以 moduleName 开头的目录
            module_dirs = []
            for item in os.listdir(self.projectPath):
                item_path = os.path.join(self.projectPath, item)
                if os.path.isdir(item_path) and item.startswith(moduleName):
                    module_dirs.append(item)
            
            if not module_dirs:
                print(f"❌ 没有找到以 {moduleName} 开头的模块目录")
                return
            
            target_module_dir = module_dirs[0]  # 使用第一个匹配的目录
            print(f"📍 找到模块目录: {target_module_dir}")
            
            # 在切换回当前分支之前，先获取模块的更改
            # 使用 git diff-tree 来获取更精确的差异
            success, diff_output = self.gitManager.runCommand(f'git diff-tree --no-commit-id --name-only -r HEAD -- {target_module_dir}/')
            if not success:
                print(f"❌ 获取模块更改文件失败: {diff_output}")
                return
            
            module_files = diff_output.strip().split('\n') if diff_output.strip() else []
            
            if module_files:
                print(f"📍 发现 {len(module_files)} 个 {target_module_dir} 模块的更改文件:")
                ...
                # 6. 撤销非指定模块的更改
                print(f"🔄 开始撤销非 {target_module_dir} 模块的更改...")
                
                # 获取所有更改的文件
                success, all_diff_output = self.gitManager.runCommand(f'git diff --name-only HEAD')
                if not success:
                    print(f"⚠️  获取所有更改文件时出错: {all_diff_output}")
                else:
                    all_changed_files = all_diff_output.strip().split('\n') if all_diff_output.strip() else []
                    
                    # 筛选出非指定模块的文件
                    non_module_files = [f for f in all_changed_files if not f.startswith(f"{target_module_dir}/")]
                    
                    if non_module_files:
                        print(f"📍 发现 {len(non_module_files)} 个非 {target_module_dir} 模块的更改文件，开始撤销:")
                        for file in non_module_files:
                            print(f"  - 撤销: {file}")
                        
                        # 使用 git checkout -- <file> 撤销非模块文件的更改
                        for file in non_module_files:
                            success, output = self.gitManager.runCommand(f'git checkout -- {file}')
                            if success:
                                print(f"  ✅ 已撤销: {file}")
                            else:
                                print(f"  ⚠️  撤销失败 {file}: {output}")
                    else:
                        print(f"✅ 没有发现非 {target_module_dir} 模块的更改")
                
                # 提交更改
                success, commit_output = self.gitManager.runCommand(f'git commit -m "Merge {targetBranch} changes for {target_module_dir} module"')
                if success:
                    print(f"✅ 成功提交模块更改")
                else:
                    print(f"⚠️  提交更改时出错: {commit_output}")
            
            else:
                print(f"⚠️  在 {target_module_dir} 模块中没有发现更改")
                ...
                # 恢复暂存的更改
                if stashed:
                    success, stash_output = self.gitManager.runCommand('git stash pop')
                    if success:
                        print("✅ 已恢复暂存的更改")
                    else:
                        print(f"⚠️  恢复暂存更改时出错: {stash_output}")
