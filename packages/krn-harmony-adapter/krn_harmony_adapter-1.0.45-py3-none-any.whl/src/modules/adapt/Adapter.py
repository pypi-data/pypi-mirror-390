...
from util.DocumentationGenerator import DocumentationGenerator
...
class Adapter(Config):
    ...
    def adaptBatchModules(self, moduleType: str = "all") -> bool:
        ...
        for module in modules_to_adapt:
            ...
            if self.adaptSingleModule(module['moduleName']):
                success_count += 1
                adapted_modules.append(module['moduleName'])
        
        # 生成汇总文档
        if adapted_modules:
            print(f"\n📍 正在生成Harmony适配汇总文档...")
            self._generate_summary_documentation(adapted_modules)
    ...
    def adaptSingleModule(self, moduleName: str) -> bool:
        ...
        # 3.1 对于已适配的模块，恢复已适配模块的harmony代码
        if status['is_adapted']:
        ...
        # 3.2 对于未适配的模块，执行适配流程
        if not status['is_adapted']:
            print(f"📍 模块 {moduleName} 未适配，开始执行适配流程...")
        ...
        # 5. 生成Harmony适配文档
        print(f"📍 模块 {moduleName} 适配完成，正在生成Harmony适配文档...")
        self._generate_harmony_documentation(moduleName)
    ...
    def _generate_harmony_documentation(self, moduleName: str):
        """生成Harmony适配文档"""
        try:
            doc_generator = DocumentationGenerator(self.basePath)
            success = doc_generator.generate_bundle_documentation(moduleName)
            if success:
                print(f"  ✅ 已生成 {moduleName} 的Harmony适配文档")
            else:
                print(f"  ⚠️  生成 {moduleName} 的Harmony适配文档失败")
        except Exception as e:
            print(f"  ❌ 生成文档时出错 {moduleName}: {e}")
    
    def _generate_summary_documentation(self, adapted_modules: List[str]):
        """生成适配模块的汇总文档"""
        try:
            doc_generator = DocumentationGenerator(self.basePath)
            success = doc_generator.generate_summary_documentation(adapted_modules)
            if success:
                print(f"  ✅ 已生成 {len(adapted_modules)} 个模块的Harmony适配汇总文档")
            else:
                print(f"  ⚠️  生成汇总文档失败")
        except Exception as e:
            print(f"  ❌ 生成汇总文档时出错: {e}")
    ...
    def _mergeConflictedFiles(self, modulePath: str, backupInfo: Dict[str, Any]) -> None:
        """合并有冲突的文件"""
