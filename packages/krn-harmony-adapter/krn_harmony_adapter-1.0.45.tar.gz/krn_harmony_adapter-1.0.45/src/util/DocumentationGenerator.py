...
class DocumentationGenerator:
    ...
    def _find_harmony_files(self, module_path: Path) -> List[Dict[str, any]]:
        ...
        # 查找src目录下的Harmony相关文件
        src_dir = module_path / "src"
        if src_dir.exists():
            harmony_files.extend(self._find_harmony_files_in_src(src_dir, module_path))
        
        # 查找其他可能包含Harmony代码的目录
        bundles_dir = module_path / "bundles"
        if bundles_dir.exists():
            harmony_files.extend(self._find_harmony_files_in_bundles(bundles_dir, module_path))
    ...
    def _find_harmony_files_in_src(self, src_dir: Path, module_path: Path) -> List[Dict[str, any]]:
        """在src目录中查找Harmony相关文件"""
        harmony_files = []
        ...
        for file_path in src_dir.rglob("*"):
            if file_path.is_file() and file_path.suffix in ['.ts', '.tsx', '.js', '.jsx']:
                try:
                    ...
                    # 检查文件是否包含harmony相关的关键字
                    if self._contains_harmony_keywords(content):
                        harmony_files.append({
                            'type': 'src_file',
                            'path': file_path.relative_to(module_path),
                            'full_path': file_path,
    ...
    def _find_harmony_files_in_bundles(self, bundles_dir: Path, module_path: Path) -> List[Dict[str, any]]:
        """在bundles目录中查找Harmony相关文件"""
        harmony_files = []
    ...
    def _contains_harmony_keywords(self, content: str) -> bool:
        """检查内容是否包含Harmony相关关键字"""
        harmony_patterns = [
            r'\bharmony\b',
            r'\bHARMONY\b',
            r'// HARMONY:',
    ...
    def _analyze_harmony_code(self, module_path: Path, harmony_files: List[Dict]) -> Dict[str, any]:
        ...
        for file_info in harmony_files:
            if file_info['type'] in ['harmony_dir', 'src_file', 'bundles_file'] and file_info['full_path'].is_file():
                stats['total_files'] += 1
                try:
                    with open(file_info['full_path'], 'r', encoding='utf-8') as f:
                        ...
                        # 收集Harmony代码快照 - 从备份文件中提取
                        code_snapshot = self._extract_harmony_code_snapshot_from_backup(file_info['full_path'])
                        if code_snapshot:
    ...
    def _extract_harmony_code_snapshot_from_backup(self, file_path: Path) -> str:
        """
        从备份文件中提取Harmony代码快照
        提取包含harmony的10行代码块
        """
        try:
            ...
            harmony_lines = []
            for i, line in enumerate(lines):
                if re.search(r'\bharmony\b', line, re.IGNORECASE):
                    harmony_lines.append(i)
            
            # 从每个包含harmony的行开始，提取10行代码块
            processed_ranges = set()
            for line_idx in harmony_lines:
                # 找到代码块的起始行
                start_line = max(0, line_idx - 2)  # 包含上下文
                end_line = min(len(lines), line_idx + 8)  # 总共10行
                ...
                for i in range(start_line, end_line):
                    if i < len(lines):
                        line_num = f"{i+1:3d}"
                        code_block.append(f"{line_num}: {lines[i].rstrip()}")
                
                if len(code_block) >= 5:  # 至少5行才有意义
                    snapshots.append("Harmony代码块:\n" + "\n".join(code_block))
            ...
            print(f"⚠️  提取代码快照失败 {file_path}: {e}")
            return ""
