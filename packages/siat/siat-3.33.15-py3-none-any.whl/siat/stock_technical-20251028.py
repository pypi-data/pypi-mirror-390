...
def security_technical(ticker,technical=['MACD'],indicator='Close', \
                       ...
                       resample_freq='1D',smooth=True,
                       
                       ticker_type='auto',source='auto', \
                       
                       graph=['ALL'],linewidth=1.5,annotate=False, \
                       price_line_color='blue',facecolor='papayawhip', \
    ...
    if isinstance(technical,str):
        ...
        if technical.upper() == 'EMA': 
            technical='MACD'; graph='EMA'
            
        if technical.upper() == 'MACD':
            if isinstance(graph,str):
                if not (graph.upper() in ['ALL','MA','EMA','MACD']):
                    graph='MACD'
            elif isinstance(graph,list):
                gf_list=[]
                for gf in graph:
                    if (gf in ['ALL','MA','EMA','MACD']):
                        gf_list=gf_list + [gf]
                        
                if len(gf_list) > 1: graph='ALL'
                elif len(gf_list) == 1: graph=gf_list[0]
                else: graph='MACD'
                        
            technical = 'MACD'
            
        if (technical.upper() == 'RSI'):
            if (graph == 'ALL') or (graph == ['ALL']):
                graph=['RSI1','RSI2','RSI3']
            technical = 'RSI'
            
        if (technical.upper() == 'KDJ'):
            if (graph == 'ALL') or (graph == ['ALL']):
                graph=['K','D','J'] 
            technical = 'KDJ'     
            
        if technical.title() == 'Bollinger':
            technical='Bollinger'
...
if __name__ =="__main__":
    ...
    ticker='600519.SS';ticker_type='auto'
...
def security_technical2(ticker,start='default',end='default',technical='MACD', \
        ...
        #图形上下半区的背景颜色
        facecolor=['papayawhip','papayawhip'], \
        #画布背景颜色
    ...
    #期间内每日涨跌幅分位数：高70%及以上，低30%及以下，中30%~70%
    ...
    if marker_mode == 'none':
        # 不显示
        marker_sizes=[0,0,0]
    elif marker_mode == 'big':
        # 仅显示大涨大跌
        marker_sizes[0]=0; marker_sizes[1]=0
    ...
    for l in tech_line_collist:
        ...
        if labeltxt =='UPPER':
            labeltxt=text_lang('上线','Upper')      
        if labeltxt =='MID':
            labeltxt=text_lang('中线','Mid')      
        if labeltxt =='LOWER':
            labeltxt=text_lang('下线','Lower')      
        ...
        if annotate:
            ...
            #ax.annotate(text=''+labeltxt+' '+srounds(end_value), 
            if annotate_value:
                ann_text=f"{labeltxt}\n({srounds(end_value)})"
            else:
                ann_text=f" {labeltxt}"
            ...
            # 灵活调整va_offset
            if len(va_offset_list) == 1:
                va_offset=va_offset_list[0]
            else:
            ...
            """
            ann=safe_annotate(ax, 
                             text=f" {labeltxt}\n({srounds(end_value)})", 
                             xy=(x_end, y_end), xytext=(x_end, y_end), 
                             color=last_line_color, fontsize=legend_txt_size)
            """
        #判断是否绘制关注线
    ...
    #当日涨跌marker
    if isinstance(price_line_marker,str):
        price_line_marker1=price_line_marker2=price_line_marker
    elif isinstance(price_line_marker,list):
        if len(price_line_marker) >= 2:
            price_line_marker1=price_line_marker[0]
            price_line_marker2=price_line_marker[1]
        elif len(price_line_marker) == 1:
            price_line_marker1=price_line_marker2=price_line_marker[0]
    else:
        price_line_marker1='^'; price_line_marker2='v'
    ...
    for seg in seg_list:
        ...
        ax2.scatter(df1seg.index,df1seg[indicator], \
                    ...
                    #linestyle=':',color=seg_color,lw=price_line_width,marker=seg_marker)   
    ...
    footnote1=text_lang("\n注：","\nNote: ")
    footnote2=text_lang("价格曲线如有端点，其大中小对应当日涨跌幅度的高中低情形。","Node size describes price change amplitude in a day if showing in price curve\n")
    ...
    """
    if technical1.title() == "Bollinger":
        technical1x=technical1+' Bands'
    else:
        technical1x=technical1
    #titletxt2=text_lang(tech_list[technical1]+' ('+technical1x+')',technical1+' ('+tech_list[technical1]+')'
    """
    #titletxt2=text_lang(tech_list[technical1],tech_list[technical1])
