...
def calc_monte_carlo_data(df, df_ends):
    ...
    np_distr : numpy.ndarray
        Array representing probabilistic distributions along the pipeline route.
    np_scen : numpy.ndarray
        Array representing scenario properties along the pipeline route.
    np_ends : numpy.ndarray
        Array representing end properties along the pipeline route.

    Notes
    -----
    np_distr is a numpy array that contains the properties listed below (rows) along the
    mesh of the pipeline route (columns):
        | MUAX_ARRAY = 0
        | MUAX_CDF_ARRAY = 1
        | MULAT_ARRAY_HT = 2
    ...
    np_scen is a numpy array that contains the properties listed below (rows) along the
    mesh of the pipeline route (columns):
        | KP = 0
        | LENGTH = 1
        | ROUTE_TYPE = 2
    ...
    np_ends is a numpy array that contains the properties listed below (rows) along the
    mesh of the pipeline route (columns):
        | ROUTE_TYPE = 0
        | KP_FROM = 1
        | KP_TO = 2
