    ...
    def fetch_messages(
        ...
        try:
            ...
            for uid in uids:
                ...
                # Decodificação segura do campo Subject
                if isinstance(subject_raw, bytes):
                    try:
                        # tenta com o encoding original
                        subject_decoded = subject_raw.decode(enc or "utf-8", errors="ignore")
                    except LookupError:
                        # casos como "unknown-8bit" ou encodings inválidos
                        subject_decoded = subject_raw.decode("utf-8", errors="ignore")
                else:
                    # caso subject_raw não seja bytes
                    subject_decoded = subject_raw

                # fallback final se ainda vier None ou vazio
                if not subject_decoded:
                    subject_decoded = "(Sem assunto)"
