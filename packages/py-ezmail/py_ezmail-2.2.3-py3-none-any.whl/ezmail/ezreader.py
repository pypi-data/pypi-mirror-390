...
class EzReader:
    ...
    def move_to_trash(self, email: EzMail, mailbox: str = "INBOX") -> bool:
        ...
        This method moves the given email from its current mailbox to the Trash folder.
        It automatically detects if the IMAP server is Gmail and applies the correct
        behavior (since Gmail does not support the standard IMAP MOVE command).

        For Gmail:
            - The email is copied to the "[Gmail]/Trash" label.
            - Then it is marked as deleted and expunged from the original mailbox.

        For other servers:
            - Uses `move_email()` to move the message normally.
        ...
        try:
            ...
            # Gmail has a non-standard folder name and no MOVE support
            if "gmail" in server:
                self.mail.select(mailbox)

                # Gmail trash folder label
                trash_folder = "[Gmail]/Trash"

                # Copy to Trash and mark deleted in current folder
            ...
            # For all other servers, use standard move logic
            else:
                # Try to find a valid trash folder
