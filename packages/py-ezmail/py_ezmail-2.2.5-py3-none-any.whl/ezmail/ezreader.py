...
class EzReader:
    ...
    def get_trash_folder(self) -> str:
        ...
        try:
            ...
            for idx, name in enumerate(normalized):
                if any(keyword in name for keyword in trash_keywords):
                    return cleaned[idx]

            # 2️⃣ fallback pro Gmail
            for idx, name in enumerate(normalized):
    ...
    def move_to_trash(self, email: EzMail, mailbox: str = "INBOX") -> bool:
        ...
        try:
            ...
            # Detecta a pasta de lixeira
            trash_folder = self.get_trash_folder()

            # Garante que o nome esteja limpo
            trash_folder = trash_folder.strip().strip('"').strip("'")

            # Seleciona a caixa atual
            self.mail.select(mailbox)

            # Gmail usa labels, precisa de tratamento especial
            if "gmail" in server:
                # Remove o label INBOX e adiciona o label Trash
                self.mail.uid("STORE", uid, "-X-GM-LABELS", "(\\Inbox)")
                self.mail.uid("STORE", uid, "+X-GM-LABELS", "(\\Trash)")
                return True

            # Outros servidores: usa MOVE ou COPY + DELETE
