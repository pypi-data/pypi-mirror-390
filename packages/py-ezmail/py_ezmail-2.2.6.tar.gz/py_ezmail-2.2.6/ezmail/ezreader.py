...
class EzReader:
    ...
    def get_trash_folder(self) -> str:
        """Detects the correct Trash folder name for the connected IMAP account.

        Works for Gmail, Outlook, KingHost, Yahoo and generic IMAP servers.
        Supports nested folders (e.g. 'INBOX.Trash') and multiple languages.
        """
        if not self.mail:
            raise RuntimeError("Not connected to any IMAP server.")
        ...
        try:
            ...
            if not mailboxes:
                raise RuntimeError("No mailboxes found for this account.")

            # Clean up quotes and whitespace
            cleaned = [m.strip().strip('"').strip("'") for m in mailboxes]
            normalized = [m.lower() for m in cleaned]

            # 🔹 1. Try direct match with attribute \Trash (most reliable)
            for idx, raw in enumerate(mailboxes):
                line = raw.lower()
                if "\\trash" in line:
                    return cleaned[idx]

            # 🔹 2. Try keywords (multi-language)
            trash_keywords = [
            ...
            for idx, name in enumerate(normalized):
                if any(k in name for k in trash_keywords):
                    return cleaned[idx]

            # 🔹 3. Gmail fallback
            for idx, name in enumerate(normalized):
                if "[gmail]" in name and ("lixeira" in name or "trash" in name):
                    return cleaned[idx]
            ...
            raise RuntimeError(f"Failed to detect Trash folder: {e}")
    
    def move_to_trash(self, email: EzMail, mailbox: str = "INBOX") -> bool:
        """Moves an EzMail message to the Trash folder (Gmail and IMAP compatible)."""
        if not self.mail:
            raise RuntimeError("Not connected to any IMAP server.")

        try:
            server = self.imap_server.lower()
            uid = str(email.uid)

            trash_folder = self.get_trash_folder().strip().strip('"').strip("'")

            self.mail.select(mailbox)

            # Gmail usa labels (X-GM-LABELS)
            if "gmail" in server:
                self.mail.uid("STORE", uid, "-X-GM-LABELS", "(\\Inbox)")
                self.mail.uid("STORE", uid, "+X-GM-LABELS", "(\\Trash)")
                return True

            # Outros servidores (KingHost, Outlook, etc.)
            if "MOVE" in self.mail.capabilities:
                status, _ = self.mail.uid("MOVE", uid, trash_folder)
                if status != "OK":
                    raise RuntimeError(f"Failed to move {email} to {trash_folder}.")
            else:
                status_copy, _ = self.mail.uid("COPY", uid, trash_folder)
                if status_copy != "OK":
                    raise RuntimeError(f"Failed to copy {email} to {trash_folder}.")
