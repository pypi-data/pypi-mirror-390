...
class MainEngine:
    ...
    def connect(self, setting: dict, gateway_name: str) -> None:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Connection Login -> {}").format(gateway_name))
    ...
    def subscribe(self, req: SubscribeRequest, gateway_name: str) -> None:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Subscribe to market data -> {}：{}").format(gateway_name, req))
    ...
    def send_order(self, req: OrderRequest, gateway_name: str) -> str:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Order placement -> {}：{}").format(gateway_name, req))
    ...
    def cancel_order(self, req: CancelRequest, gateway_name: str) -> None:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Cancellation of Order -> {}：{}").format(gateway_name, req))
    ...
    def send_quote(self, req: QuoteRequest, gateway_name: str) -> str:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Quotation and order placement -> {}：{}").format(gateway_name, req))
    ...
    def cancel_quote(self, req: CancelRequest, gateway_name: str) -> None:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Quotation cancellation -> {}：{}").format(gateway_name, req))
    ...
    def query_history(self, req: HistoryRequest, gateway_name: str) -> list[BarData]:
        ...
        gateway: BaseGateway | None = self.get_gateway(gateway_name)
        if gateway:
            self.write_log(_("Query ohlc chart -> {}：{}").format(gateway_name, req))
...
class OmsEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        #self.full_symbol_contracts: dict[str, ContractData] = {}
        ...
        self.register_event()
        self.channel=broker_identity.channel
    ...
    def process_tick_event(self, event: Event) -> None:
        ...
        self.__set_full_symbol(tick)
        self.ticks[tick.vt_symbol] = tick
        async_publisher_instance.send_topic(self.channel.TICK, broker_data_to_json(tick))

    def process_order_event(self, event: Event) -> None:
        """"""
        order: OrderData = event.data
        self.__set_full_symbol(order)
        ...
        async_publisher_instance.send_topic(self.channel.ORDER, broker_data_to_json(order))

    def process_trade_event(self, event: Event) -> None:
        """"""
        trade: TradeData = event.data
        self.__set_full_symbol(trade)
        ...
        async_publisher_instance.send_topic(self.channel.TRADE, broker_data_to_json(trade))


    def __set_full_symbol(self,data):
        try:
            data.full_symbol=self.contracts[data.vt_symbol].full_symbol
        except:
            pass
    def process_position_event(self, event: Event) -> None:
        ...
        self.__set_full_symbol(position)
        ...
        async_publisher_instance.send_topic(self.channel.POSITION, broker_data_to_json(position))
    ...
    def process_account_event(self, event: Event) -> None:
        ...
        async_publisher_instance.send_topic(self.channel.ACCOUNT, broker_data_to_json(account))
    ...
    def process_contract_event(self, event: Event) -> None:
        ...
        async_publisher_instance.send_topic(self.channel.CONTRACT, broker_data_to_json(contract))

    def process_quote_event(self, event: Event) -> None:
        """"""
        quote: QuoteData = event.data
        self.__set_full_symbol(quote)
        ...
        async_publisher_instance.send_topic(self.channel.QUOTE, broker_data_to_json(quote))
