...
class VisBase(Stream, metaclass=ABCMeta):
    ...
    def subscribe(self, stream:Stream, title=None, clear_after_end=False, clear_after_each=False,
            show:bool=False, history_len=1, dim_history=True, opacity=None, window_width=None, Date=None, window_size=None, useOffset=None, **stream_vis_args):
        # in this ovedrride we don't call base class method
        with self.lock:
            self.layout_dirty = True

            stream_vis = StreamVisInfo(stream, title, clear_after_end,
                clear_after_each, history_len, dim_history, opacity, window_width, window_size, Date, useOffset,
