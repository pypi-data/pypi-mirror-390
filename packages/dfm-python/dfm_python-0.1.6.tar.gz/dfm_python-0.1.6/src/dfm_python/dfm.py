...
def diagnose_series(Res: DFMResult, config: DFMConfig, series_name: Optional[str] = None, 
    ...
    --------
    >>> Res = dfm(X, config)
    >>> diag = diagnose_series(Res, config, series_name="Real GDP")
    ...
    # Note: We don't have access to original X here, so we can't calculate this
...
def print_series_diagnosis(Res: DFMResult, config: DFMConfig, 
    ...
    --------
    >>> Res = dfm(X, config)
    >>> print_series_diagnosis(Res, config, series_name="Real GDP")
    """
