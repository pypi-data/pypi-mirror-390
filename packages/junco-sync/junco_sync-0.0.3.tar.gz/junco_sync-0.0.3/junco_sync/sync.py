...
def init_duckdb(
    ...
    if set_as_default:
        ...
        save_config(config)
        print(f"✓ Set {db_path} as default database")
...
def sync_source(
    ...
    Args:
        ...
        source: Source dictionary from manifest (must have: name, schema, database)
    ...
    if verbose:
        print(f"Syncing: {database}.{schema}.{source_name}")
    
    # Build query
    query = f"SELECT * FROM {database}.{schema}.{source_name}"
    if where_clause:
    ...
    # Load to DuckDB (maintain schema structure)
...
def sync_models(
    ...
    if verbose:
        print(f"🔍 Analyzing dependencies for {len(model_names)} model(s)...\n")
    ...
    if verbose:
        print(f"📊 Found {len(source_to_models)} unique source(s) to sync:\n")
    ...
    if verbose:
        ...
        print(f"✅ Sync complete! Successfully synced {successful}/{len(results)} sources")
...
def get_models_from_selection(
    ...
    for i, line in enumerate(result.stdout.strip().split('\n')):
        ...
        if parts:
            model_name = parts[-1]
            if debug:
                print(f"🐛   -> Parts: {parts}, extracted: '{model_name}'")
            
            # Only add if it looks like a valid identifier (not a number)
            if model_name and not model_name.isdigit():
                models.append(model_name)
                if debug:
                    print(f"🐛   -> ✓ Added to models")
            elif debug:
                print(f"🐛   -> ✗ Rejected (invalid identifier)")
    
    if debug:
        print(f"🐛 Final models list: {models}")
