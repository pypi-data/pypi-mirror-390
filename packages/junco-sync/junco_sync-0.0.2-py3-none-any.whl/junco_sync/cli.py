...
def cli():
    """
    🐦 junco-sync: Sync Snowflake sources to DuckDB for local dbt development
...
def sync(select, exclude, selector, sample, full, days, manifest, profiles_dir, dry_run):
    """
    ...
    if not manifest_path.exists():
        click.echo(f"❌ Manifest not found at {manifest_path}")
    ...
    try:
        ...
        click.echo("🔍 Finding models matching selection criteria...")
    ...
    if not models:
        click.echo("❌ No models matched the selection criteria")
        return
    ...
    if dry_run:
        click.echo("\n🔍 Dry run - would sync dependencies for these models")
        return
    ...
    where_clauses = {}
    if days:
        click.echo(f"\n⏰ Using last {days} days for time-based tables")
    ...
    try:
        ...
        click.echo(f"\n{'='*60}")
        click.echo(f"✅ Successfully synced {successful}/{len(results)} sources")
        click.echo(f"📊 Total rows synced: {total_rows:,}")
        click.echo(f"{'='*60}")
...
def init(path, name):
    ...
    try:
        ...
        click.echo(f"\n🎉 Ready to sync! Use 'junco-sync sync --select <model>' to get started.")
    except Exception as e:
...
def config(action, key, value):
    ...
    if action == 'show':
        ...
        if not key:
            click.echo("❌ Must provide a key to get")
            return
        ...
        if not key or not value:
            click.echo("❌ Must provide both key and value")
            return
