...
class SimpleTranscriptomeDecoder:
    ...
    class MLPDecoder(nn.Module):
        ...
        def __init__(self, latent_dim: int, gene_dim: int, hidden_dims: List[int], dropout_rate: float):
            ...
            # Build encoder part (latent expansion)
            encoder_layers = []
            ...
            # Build decoder part (gene prediction)
            decoder_layers = []
            decoder_dims = hidden_dims[::-1]  # Reverse for decoder
            
            # First decoder layer from last hidden dim to gene prediction
            decoder_layers.extend([
                nn.Linear(decoder_dims[0], decoder_dims[0] // 2),
                nn.BatchNorm1d(decoder_dims[0] // 2),
                nn.GELU(),
                nn.Dropout(dropout_rate)
            ])
            
            # Additional decoder layers
            current_dim = decoder_dims[0] // 2
            for next_dim in decoder_dims[1:]:
                decoder_layers.extend([
            ...
            decoder_layers.append(nn.Linear(current_dim, gene_dim))
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # Encoder: Expand latent representation
            encoded = self.encoder(x)
            
            # Decoder: Predict gene expression
            decoded = self.decoder(encoded)
            
            # Ensure non-negative output with softplus
            output = F.softplus(decoded * self.output_scale + self.output_bias)
    ...
    def _build_mlp_model(self):
        """Build the MLP model based on complexity requirements"""
        # Use residual MLP for better gradient flow
        return self.ResidualMLPDecoder(
            self.latent_dim, 
            self.gene_dim, 
            self.hidden_dims, 
            self.dropout_rate
