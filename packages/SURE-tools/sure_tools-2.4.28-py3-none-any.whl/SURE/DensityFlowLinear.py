...
class DensityFlowLinear(nn.Module):
    ...
    def model1(self, xs):
        ...
        if self.loss_func=='negbinomial':
            inverse_dispersion = pyro.param("inverse_dispersion", self.inverse_dispersion *
        ...
        with pyro.plate('data'):
            ...
            if self.loss_func == 'negbinomial':
                logits = (mu.log()-inverse_dispersion.log()).clamp(min=-15, max=15)
                if self.use_zeroinflate:
                    ...
                    pyro.sample('x', dist.NegativeBinomial(total_count=inverse_dispersion, logits=logits).to_event(1), obs=xs)
    ...
    def model4(self, xs, us, ys, embeds=None):
        ...
        with pyro.plate('data'):
            ...
            if self.loss_func == 'negbinomial':
                logits = (mu.log()-inverse_dispersion.log()).clamp(min=-15, max=15)
                if self.use_zeroinflate:
                    pyro.sample('x', dist.ZeroInflatedDistribution(dist.NegativeBinomial(total_count=inverse_dispersion, logits=logits),gate_logits=gate_logits).to_event(1), obs=xs)
                else:
                    pyro.sample('x', dist.NegativeBinomial(total_count=inverse_dispersion, logits=logits).to_event(1), obs=xs)
            elif self.loss_func == 'poisson':
                if self.use_zeroinflate:
