...
class UPGATTrainer:
    ...
    def __init__(self, data=None, model=None, loss=None, opt=None,
                 early_stop=None, save_path=None, config=None, teacher_model=True):
        ...
        self.gcn_loader = data.gcn_dataloader()
    ...
    def _train_epoch(self) -> float:
        ...
        self.model.train()
        total_loss = 0.0
        train_bar = tqdm(zip(self.train_loader, self.gcn_loader), desc="Train", leave=False,total=len(self.train_loader))

        for batch_train, batch_gcn in train_bar:
            # --- Move tensors to device ---
            pos_sample = batch_train["positive_sample"].to(self.device)
            neg_sample = batch_train["negative_sample"].to(self.device)
            pro = batch_train["probabilities"].to(self.device)
            adj_matrix = batch_gcn["adj_matrix"]

            # --- Forward + Backward ---
            self.optimizer.zero_grad()
            pos_score = self.model.forward_enc_dec(pos_sample, adj_matrix, device=self.device)
            neg_score = self.model.forward_enc_dec(neg_sample, adj_matrix, device=self.device)
            if self.teacher_model:
                loss = self.loss_fn(pos_score, neg_score, pro)
            else:
                pseudo_sample = batch_gcn["pseudo_sample"]
                pseudo_pro = batch_gcn["pseudo_pro"].to(self.device)
                pseudo_score = self.model.forward_enc_dec(pseudo_sample, adj_matrix,device=self.device)
                loss = self.loss_fn(pos_score, neg_score, pro, pseudo_score, pseudo_pro)
            loss.backward()
            self.optimizer.step()
    ...
    def pseudo_tail_predict(self, output_file_path):
        ...
        for idx, value in hr2t.items():
            idx0 = torch.tensor([idx[0]]).to(self.device)
            idx1 = torch.tensor([idx[1]]).to(self.device)
            score = self.model.get_tail_score(idx0, idx1)
            top_scores, top_indices = torch.topk(score.flatten(), k=6, largest=True)
