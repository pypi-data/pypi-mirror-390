...
class HeatingOptimizer(Juham):
    ...
    def publish_heating_plan(self, heatingplan : list[dict[str, Any]]) -> None:
        ...
        for hp in heatingplan:
            if self.new_power_plan == True:
                ...
                self.publish(self.topic_powerplan, json.dumps(hp_to_publish), 0, False)
                self.new_power_plan = False
            else:
                self.publish(self.topic_powerplan, json.dumps(hp), 0, False)
