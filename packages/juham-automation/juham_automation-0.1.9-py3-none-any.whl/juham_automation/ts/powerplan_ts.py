...
class PowerPlanTs(JuhamTs):
    ...
    def on_message(self, client: object, userdata: Any, msg: MqttMsg) -> None:
        ...
        tempForecast : float = float(m.get("NextDayTemperature"))
        solarForecast : float = float(m.get("NextDaySolarpower"))
        minTemp : float = float(m.get("MinTempLimit"))
        maxTemp : float = float(m.get("MaxTempLimit"))
