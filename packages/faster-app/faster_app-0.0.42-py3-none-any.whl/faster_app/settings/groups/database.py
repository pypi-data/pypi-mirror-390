...
class DatabaseSettings(BaseModel):
    ...
    database: str = "faster_app"
    db_schema: Optional[str] = None  # 数据库 schema（PostgreSQL 支持）
    ...
    def validate_type(cls, v):
        ...
        if v_lower not in allowed_types:
            raise ValueError(
                f"database.type 必须是 {allowed_types} 之一，当前值: {v}"
            )
        return v_lower
    ...
    def from_database_url(cls, url: str) -> "DatabaseSettings":
        ...
        if not db_type:
            raise ValueError(
                ...
                f"支持: {list(db_type_map.keys())}"
            )

        # 解析查询参数
        query_params = parse_qs(parsed.query) if parsed.query else {}
        schema = query_params.get("schema", [None])[0]  # 获取 schema 参数
        ...
        if db_type == "sqlite":
            ...
            return cls(
                ...
                database=db_path,
                db_schema=None,  # SQLite 不支持 schema
            )
        ...
        return cls(
            type=db_type,
            engine="tortoise.backends.asyncpg" if db_type == "postgres" else "tortoise.backends.mysql",
            host=parsed.hostname,
            port=port,
            user=unquote(parsed.username) if parsed.username else "postgres",
            password=SecretStr(unquote(parsed.password) if parsed.password else ""),
            database=database,
            db_schema=schema,  # 设置 schema
        )
