...
async def execute_script(
    ...
    try:
        ...
        if not isinstance(resp, dict):
            logger.warning("Unexpected response type from executeScript")
            return ExecuteScriptResult(RequestId="", Results=[], Success=False)
        processed_results = []
        if resp.get('success') and resp.get('root'):
        ...
        return ExecuteScriptResult(RequestId=resp.get('traceId', ""), Results=processed_results,
                                   Success=resp.get('success', False))
    except Exception as e:
        logger.error(f"Error in execute_script: {e}")
        return f"Error in execute_script: {e}"
