"""
shap_yolov8_per_box.py

SHAP for YOLOv8x — per-detection (per-box) explanations.
For each detected object in an image, compute a SHAP heatmap that explains
the confidence of that specific detection. Matching is done by IoU to keep
the same object identity under perturbations.

Requires:
    pip install ultralytics shap opencv-python numpy pillow matplotlib
"""
...
def SHAP(
    ...
    *,
    ...
    """
    Compute SHAP attributions for *each detection* produced by YOLOv8x in every image.
