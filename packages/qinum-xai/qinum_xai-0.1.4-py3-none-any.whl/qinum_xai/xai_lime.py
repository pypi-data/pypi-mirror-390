"""
lime_yolov8_per_box.py

LIME for YOLOv8x — per-detection (per-box) explanations.
For each detected object in an image, compute a LIME heatmap that explains
the confidence of that specific detection. Matching is done by IoU to keep
object identity under perturbations.

Requires:
    pip install ultralytics lime scikit-image opencv-python numpy pillow matplotlib
"""
...
from skimage.segmentation import slic  # segmentation function (options: quickshift/felzenszwalb)
...
def lime(
    ...
    *,
    ...
    """
    Compute LIME attributions for *each detection* produced by YOLOv8x in every image.
