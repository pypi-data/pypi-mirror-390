...
def normalize_image_for_shap(img: np.ndarray) -> np.ndarray:
    """
    Ensure image is RGB float32 in [0,1].
    Accepts uint8 [0,255] or float [0,1]/[0,255].
    """
    if img.dtype == np.uint8:
        ...
        if vmax > 1.0:  # assume range [0,255]
            img_f = np.clip(img_f / 255.0, 0.0, 1.0)
        else:
...
class YoloDetectionScoreWrapper:
    ...
    def __call__(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(X.shape[0]):
            ...
            img = np.clip(X[i], 0, 1)
            rgb = (img * 255.0).round().astype(np.uint8)
    ...
    *,
    ...
    for img_path in jpgs:
        ...
        for d_idx in det_indices:
            ...
            print(f"SHAP mean abs attribution: {np.mean(np.abs(shap_values.values)):.6f}")
