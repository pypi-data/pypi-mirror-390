...
class AlgebraicRecon(Recon):
    ...
    def __init__(self, opti = OptimizerType.MLEM, numIterations = 10000, numSubsets = 1, isSavingEachIteration=True, maxSaves = 5000, alpha = None, denominatorThreshold = 1e-6, useSparseSMatrix=True, sparseType = SparsingType.CSR, sparseThreshold=0.1, device = None, **kwargs):
        ...
        self.device = device
        self.SMatrix = None  # Sparse system matrix
        ...
        if self.useSparseSMatrix:
            self.sparse_SMatrix()
        else:
            self.SMatrix = np.stack([obj.field for obj in self.experiment.AcousticFields], axis=-1)
    ...
    def sparse_SMatrix(self):
        if self.sparseType == SparsingType.CSR:
            self.SMatrix, self.Z_dim = self._sparseSMatrix_CSR(AcousticFields = self.experiment.AcousticFields, threshold_factor=self.sparseThreshold)
    ...
    # PRIVATE METHODS
    @staticmethod
    def _sparseSMatrix_CSR(AcousticFields, threshold_factor=0.1):
        """
