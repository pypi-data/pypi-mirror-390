...
from ..orchestration.utilities import (
    CallInputs,
    CallTypes
)
...
def mi_ses_from_function(delegate:Callable,
                         join_on:list,
                         path_srmi:str="",
...
def _mi_ses_from_function_sequential(delegate,
                                     join_on:list,
                                     path_srmi:str="",
                                     df_implicates:list[IntoFrameT] | DataFrameList | None=None,
                                     index:list|None=None,
                                     ...
                                     implicate_name:str="___implicate___",
                                     rounding:Rounding | None=None,
                                     round_output:bool=True) -> list[ReplicateStats | StatCalculator]:
    ...
    for implicate_number in range(0,len(df_implicates)):
        imp_statsi = _mi_ses_from_function_one_implicate(implicate_number=implicate_number,
                                                         **arguments)
...
def _mi_ses_from_function_parallel(delegate,
                                   join_on:list,
                                   df_implicates:list[IntoFrameT] | DataFrameList | None=None,
                                   path_srmi:str="",
                                   index:list|None=None,
                                   ...
                                   parallel:bool=False,
                                   parallel_inputs:CallInputs | None=None) -> list[ReplicateStats | StatCalculator]:
    ...
    for implicate_number in range(n_implicates):
        ...
        path_temp_implicate = Path(f"{config.path_temp_files}/{random_file_save}_implicate_{implicate_number}").as_posix()
        
        f = FunctionFromPython(
            function=_mi_ses_from_function_one_implicate,
            parameters=dict(
                implicate_number=implicate_number,
                path_save=path_temp_implicate
            )
        )
        
        f.parameters_positional_post = ["**d_implicate"]
        f.add_pre_function("from survey_kit.serializable import SerializableDictionary")
        f.add_pre_function(f"d_implicate = SerializableDictionary.load(path='{path_temp_dict}')")
        
        path_implicates.append(path_temp_implicate)
        function_list.append(f)
        
    log = run_function_list(function_list,
                          call_input=parallel_inputs,
                          run_all=True,
                          testing=False)
...
def _mi_ses_from_function_one_implicate(implicate_number:int,
                                        delegate,
                                        join_on:list,
                                        df_implicates:list[IntoFrameT] | DataFrameList | None=None,
                                        path_srmi:str="",
    ...
    if df_noimputes is not None:
        if index is None:
            ...
            dfi = concat_wrapper([dfi,df_noimputes],
                                 how="horizontal")
        else:
            dfi = join_list([dfi,df_noimputes],
                            how="left",
                            on=index)
    ...
    if type(out) is StatCalculator:
        ...
        imp_statsi = ReplicateStats(df_estimates=out.df_estimates,
                                    df_ses=out.df_ses,
        ...
        imp_statsi = ReplicateStats(df_estimates=df_estimates,
                                    df_ses=df_ses,
