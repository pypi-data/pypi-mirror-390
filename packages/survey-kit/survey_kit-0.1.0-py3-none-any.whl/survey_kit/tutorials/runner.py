...
def run_tutorials_in_path(
    ...
    for item_name in os.listdir(path):
        ...
        if os.path.isfile(item_path):
            valid_type = any([item_path.endswith(suffixi) for suffixi in acceptable_suffixes])
    ...
    if run_files:
        for filei in files_to_run:
            ...
            if f.run:
                ...
                logger.info(f"Not running '{os.path.basename(filei)}' as it is already up to date.")
    else:
        return files_to_run
...
@as_function(outputs=path_to_html,
             args_outputs=["path"],
...
def run_jupyter_to_html(path:str):
    ...
    command_convert = [
        ...
        "--execute", Path(path_name).with_suffix(".ipynb").as_posix()
    ]
    ...
    try:
        ...
        output_docs = (
            Path(output_path)
            .as_posix()
            .replace("/tutorials","/docs/tutorials")
        )
        ...
        logger.error("Please ensure uv is installed and accessible in your system's PATH.")
