...
1. ROOT FINDING METHODS
   - fixed_point_iteration(f, x_init, epsilon=1e-6, n_iter=100): 
   - bisection_method(f, a, b, epsilon=1e-6)
   - regula_falsi(f, a, b, epsilon=1e-6)
   - newton_raphson(f, df, x0, epsilon=1e-6)
   - secant_method(f, x0, x1, epsilon=1e-6)

2. LINEAR SYSTEM SOLVING
   - row_swap(matrix, i, j)
   - row_add(matrix, i, j, alpha)
   - row_reduction(matrix)
   - back_substitution(matrix)
   - forward_substitution(matrix)
   - solve_linear_system(matrix, triangular=None)
...
4. ITERATIVE LINEAR SOLVERS
   - jacobi_iteration(A, B, alpha=1e-10, n_iter=1000)
   - gauss_seidel(A, B, alpha=1e-10, n_iter=1000)

5. POLYNOMIAL EVALUATION (HORNER'S METHOD)
   - horner_poly(a, x)
   - horner_derivative(a, x)
   - horner_integral(a, x, C=0)

6. VANDERMONDE MATRIX
   - vander(x): Generate Vandermonde matrix

7. INTERPOLATION
   - L_k(x, x_nodes, k): Lagrange basis polynomial
   - lagrange_poly(x, x_nodes, y_nodes):
   - divided_diff_table(x, y): Generate divided difference table
   - newton_poly_eval(x, x_nodes, table): 

8. LEAST SQUARES APPROXIMATION
   - least_squares_line(x, y):

9. NUMERICAL DIFFERENTIATION
   - central_difference_2(f, x, h=0.1): 2-point central difference formula
   - central_difference_4(f, x, h=0.1): 4-point central difference formula
   - central_difference_second(f, x, h=0.1, n_iter=20, alpha=1e-8): Richardson extrapolation for derivatives
   - central_difference_extrapolation(f, x, h=0.1, n_iter=5): Richardson extrapolation table
   - central_difference_nodes(f, x_nodes): Derivative at first node using divided differences

10. NUMERICAL INTEGRATION
    - composite_trapezoidal(a, b, n, f): Composite trapezoidal rule
    - composite_simpson(a, b, m, f): Composite Simpson's rule
    - gauss_legendre_quadrature(a, b, N, f, data): Gauss-Legendre quadrature
"""
