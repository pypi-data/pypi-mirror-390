...
class SharePointGraph:
    ...
    def get_drive_id(self, context="cw"):
        try:
            headers = {'Authorization': f'Bearer {self.token}'}
            path = f"/v1.0/sites/{self.site_id}/drives"
            status, _, data = self._request_json("GET", path, headers=headers)
            ...
            if not drives:
                raise RuntimeError(f"Sin drives en sitio: {data}")

            # Selección robusta: Documents/Documentos si existe; si no, el primero.
            preferred_names = {"Documents", "Documentos"}
            chosen = None
            for d in drives:
            ...
            self.drive_id = chosen.get('id')
            if not self.drive_id:
                raise RuntimeError(f"Drive sin id válido: {chosen}")
