...
def highlight_module(name, rawtext, text, lineno, inliner, options={}, content=[]):
    node = nodes.inline(rawtext, text, classes=['highlight-module'])
    return [node], []

roles.register_local_role('hlmod', highlight_module)
...
for root, dirs, files in os.walk(examples_source):
    ...
    for fil in files:
        if os.path.splitext(fil)[1] in [".ipynb", ".md", ".rst", ".png"]:
...
def _convert_internal_and_external_class_to_strings(annotation):
    ...
    if module.split(".")[0] == "qpytorch":
        module = "~" + module
    elif module == "torch.":
        module = "~torch."
    elif module == "gpytorch.":
        module = "~gpytorch."
