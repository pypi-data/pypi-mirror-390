...
class RQKernelGrad(RQKernel):
    ...
    Example:
        ...
        >>> covar_module = qpytorch.kernels.ScaleKernel(qpytorch.kernels.RQKernelGrad())
        ...
        >>> covar_module = qpytorch.kernels.ScaleKernel(qpytorch.kernels.RQKernelGrad(batch_shape=torch.Size([2])))
        >>> covar = covar_module(x)  # Output: LinearOperator of size (2 x 60 x 60)
    """

    def __init__(self, **kwargs):
        super(RQKernelGrad, self).__init__(**kwargs)
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        if not diag:
            ...
            K[..., :n1, n2:] = outer1 * K_11.pow(1+1/self.alpha).repeat([*([1] * (n_batch_dims + 1)), d])

            # 3) Second gradient block
            outer2 = outer.transpose(-1, -3).reshape(*batch_shape, n2, n1 * d)
            outer2 = outer2.transpose(-1, -2)
            K[..., n1:, :n2] = -outer2 * K_11.pow(1+1/self.alpha).repeat([*([1] * n_batch_dims), d, 1])
            ...
            K[..., n1:, n2:] = kp.to_dense() * K_11.pow(1+1/self.alpha).repeat([*([1] * n_batch_dims), d, d]) \
            ...
            kernel_diag = super(RQKernelGrad, self).forward(x1, x2, diag=True)
