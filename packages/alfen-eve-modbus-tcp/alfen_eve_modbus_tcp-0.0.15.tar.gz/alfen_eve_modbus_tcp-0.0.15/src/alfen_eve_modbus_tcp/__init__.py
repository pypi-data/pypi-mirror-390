...
class CarCharger(AlfenEve):
    ...
    #     data = self.read_with_retry("mode_3_state")
    #     raw = (data or {}).get("mode_3_state")
    #
    #     # Normalize / validate
    #     if not raw:
    #         syslog.syslog("Mode 3 State read failed or empty; returning 'Unknown'")
    #         return "Unknown"
    #
    #     key = str(raw).strip()
    #     status = MODE_3_STATE_MAP.get(key, "Unknown")
    #     syslog.syslog(f"Mode 3 State raw '{key}' -> Status: {status}")
    #     return status
    ...
    def get_status(self, *, manage_connection: bool = True) -> str:
        ...
        syslog.syslog(f"Mode 3 State: {mode_3_state}")
        # Harden the mapping a bit so a bad read can't explode.
        raw = (mode_3_state or {}).get("mode_3_state")
        status = MODE_3_STATE_MAP.get(str(raw).strip() if raw else "?", "Unknown")
        syslog.syslog(f"Status: {status}")
        return status
    ...
    def switch_phase(self, phases, *, manage_connection: bool = True) -> None:
    ...
    def set_current(self, current, *, manage_connection: bool = True):
        self.write_with_retry('modbus_slave_max_current', current, manage_connection=manage_connection)
    ...
    def set_charge_profile(self, phases, current, *, manage_connection: bool = True):
        ...
        if (status == "Charging" or status == "Connected"):
            ...
            self.set_current(current, manage_connection=manage_connection)
            time.sleep(1)
