...
from aphrodite.prompt_adapter.models import (
...
class WorkerPromptAdapterManager(AbstractWorkerManager):
    ...
    def _load_adapter(self, prompt_adapter_request: PromptAdapterRequest) -> PromptAdapterModel:
        try:
            prompt_adapter = self._prompt_adapter_model_cls.from_local_checkpoint(
                ...
                num_virtual_tokens=prompt_adapter_request.prompt_adapter_num_virtual_tokens,
                config=self.prompt_adapter_config,
                device=str(self.device),
            )
        except Exception as e:
            raise RuntimeError(
                f"Loading prompt_adapter {prompt_adapter_request.prompt_adapter_local_path} failed"
            ) from e
        return prompt_adapter

    def add_dummy_prompt_adapter(self, prompt_adapter_request: PromptAdapterRequest) -> bool:
        return True
...
class LRUCacheWorkerPromptAdapterManager(WorkerPromptAdapterManager):
    ...
    _prompt_adapter_manager_cls: type[LRUCachePromptAdapterModelManager] = LRUCachePromptAdapterModelManager
    ...
    def create_prompt_adapter_manager(
        ...
        self._adapter_manager: LRUCachePromptAdapterModelManager = prompt_adapter_manager
        return prompt_adapter_manager.model

    def _apply_adapters(self, prompt_adapter_requests: set[PromptAdapterRequest]) -> None:
        prompt_adapters_map = {
        ...
        if len(prompt_adapters_map) > self._adapter_manager.prompt_adapter_slots:
            raise RuntimeError(
    ...
    def add_adapter(self, prompt_adapter_request: PromptAdapterRequest) -> bool:
        if prompt_adapter_request.prompt_adapter_id not in self.list_adapters():
            ...
            loaded = self._adapter_manager.get_adapter(prompt_adapter_request.prompt_adapter_id) is not None
        self._adapter_manager.activate_adapter(prompt_adapter_request.prompt_adapter_id)
        return loaded
