...
from aphrodite.adapter_commons.models import AdapterLRUCache, AdapterModel, AdapterModelManager
from aphrodite.adapter_commons.utils import (
    add_adapter,
    deactivate_adapter,
    get_adapter,
    list_adapters,
...
def convert_mapping(
    ...
    prompt_adapter_index_to_id: list[int | None],
) -> torch.Tensor:
    ...
    pa_indices = [id_to_index.get(id_, -1) if id_ > 0 else -1 for id_ in mapping.index_mapping]
...
class PromptAdapterModel(AdapterModel):
    def __init__(self, prompt_adapter_id=None, num_virtual_tokens=None, prompt_embedding=None) -> None:
    ...
    def from_local_checkpoint(
        ...
        prompt_embedding = adapters_weights["prompt_embeddings"].to(config.prompt_adapter_dtype)
...
class PromptAdapterModelManager(AdapterModelManager):
    ...
    def __init__(
        ...
        self.prompt_adapter_index_to_id: list[int | None] = [None] * self.prompt_adapter_slots
        ...
        self._last_mapping: PromptAdapterMapping | None = None
    ...
    def activate_adapter(
        ...
        first_free_slot = next(
            (
                (i, prompt_adapter_id)
                for i, prompt_adapter_id in enumerate(self.prompt_adapter_index_to_id)
                if prompt_adapter_id is None
            ),
            None,
        )
        ...
        logger.debug("Activating prompt_adapter. int id: %s, slot index: %s", prompt_adapter_model.id, index)
    ...
    def _set_adapter_mapping(self, mapping: PromptAdapterMapping) -> None:
        base_indices, base_embedding_indices = convert_mapping(mapping, self.prompt_adapter_index_to_id)
    ...
    def _create_prompt_adapter_modules(self):
        for module_name, module in self.model.named_modules(remove_duplicate=False):
            if "VocabParallel" in module.__class__.__name__:
                ...
                replaced_module = self.replace_submodule(self.model, module_name, new_module)
                self.register_module(module.__class__.__name__, replaced_module)
                replaced_module.set_mapping(self.base_indices, self.base_embedding_indices)
                break

    def replace_submodule(self, model: nn.Module, module_name: str, new_module: nn.Module) -> nn.Module:
    ...
    def pin_adapter(self, prompt_adapter_id: int) -> bool:
        ...
        raise NotImplementedError(
            "Pinning is not supported in PromptAdapterModelManager. Use LRUCachePromptAdapterModelManager for pinning"
        )  # type: ignore
...
class PromptAdapterLRUCache(AdapterLRUCache[PromptAdapterModel]):
    def __init__(self, capacity: int, deactivate_prompt_adapter_fn: Callable[[int], bool]):
...
class LRUCachePromptAdapterModelManager(PromptAdapterModelManager):
    ...
    def __init__(
        ...
        self.prompt_adapter_config = prompt_adapter_config
        super().__init__(model, max_num_seqs, max_num_batched_tokens, prompt_adapter_config)
        self._registered_adapters = PromptAdapterLRUCache(self.capacity, self.deactivate_adapter)
        self._active_adapters = PromptAdapterLRUCache(self.prompt_adapter_slots, self._deactivate_adapter)
    ...
    def activate_adapter(
        ...
        if prompt_adapter_id not in self._active_adapters and len(self._active_adapters) >= self.prompt_adapter_slots:
    ...
    def _pin_prompt_adapter_in_cpu_cache(self, prompt_adapter_id: int):
        try:
            ...
            raise ValueError(f"Pinning failed. Prompt Adapter {prompt_adapter_id} is not registered.") from err
...
def create_prompt_adapter_manager(
    ...
    max_num_batched_tokens: int,
    prompt_adapter_config: PromptAdapterConfig,
    prompt_adapter_manager_cls: type[PromptAdapterModelManager] = PromptAdapterModelManager,
    **kwargs,
