...
class Idefics3ProcessingInfo(BaseProcessingInfo):
    ...
    def _get_image_token(self, processor: Idefics3Processor | None) -> tuple[str, str, str]:
        if processor is None:
            processor = self.get_hf_processor()
...
class Idefics3MultiModalProcessor(BaseMultiModalProcessor[Idefics3ProcessingInfo]):
    def _call_hf_processor(
        ...
        parsed_images = self._get_data_parser().parse_mm_data({"image": images}).get_items("image", ImageProcessorItems)
...
class Idefics3ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsLoRA):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = Idefics3Model(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        self.image_token_id = self.config.image_token_id
