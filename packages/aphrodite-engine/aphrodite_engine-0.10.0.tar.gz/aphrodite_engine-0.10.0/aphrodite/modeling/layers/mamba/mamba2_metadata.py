...
from aphrodite.attention.backends.placeholder_attn import PlaceholderAttentionMetadata
...
def get_platform_metadata_classes() -> tuple[type[AttentionMetadata], ...]:
    ...
    if current_platform.is_rocm():
        from aphrodite.attention.backends.rocm_flash_attn import ROCmFlashAttentionMetadata
...
def prepare_mamba2_metadata(
    ...
    if num_prefills > 0:
        ...
        seq_idx = torch.repeat_interleave(
            torch.arange(num_prefills, dtype=torch.int32, device=query_start_loc.device),
...
def update_metadata(
    x: torch.Tensor, query_start_loc: torch.Tensor, mamba2_metadata: Mamba2Metadata | Mamba2AttentionMetadata
):
    """
    ...
    for BLOCK_M in [8]:  # cover all BLOCK_M values
        ...
        nums_dict[BLOCK_M]["token_chunk_offset_ptr"] = mamba2_metadata.token_chunk_offset_ptr  # type: ignore
