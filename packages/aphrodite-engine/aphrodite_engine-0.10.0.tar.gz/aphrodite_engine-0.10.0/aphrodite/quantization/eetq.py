...
class EETQConfig(QuantizationConfig):
    ...
    def __init__(
        ...
        if self.weight_bits != 8:
            raise ValueError(
                f"Currently, only 8-bit weight quantization is supported for EETQ, but got {self.weight_bits} bits."
            )
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional["EETQLinearMethod"]:
...
class EETQLinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        qweight = Parameter(
            torch.empty(input_size_per_partition, output_size_per_partition, dtype=torch.int8), requires_grad=False
        )
        weight_scales = Parameter(torch.empty(output_size_per_partition, dtype=torch.float16), requires_grad=False)
        set_weight_attrs(
            qweight,
            {
                "input_dim": 0,
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if HAS_EETQ:
            ...
            raise ImportError("You have not installed EETQ. Please refer to https://github.com/NetEase-FuXi/EETQ")
