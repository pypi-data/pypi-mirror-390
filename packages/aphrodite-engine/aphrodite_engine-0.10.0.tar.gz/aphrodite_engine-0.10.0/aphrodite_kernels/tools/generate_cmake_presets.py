...
def generate_presets(output_path="CMakeUserPresets.json"):
    """Generates the CMakeUserPresets.json file for aphrodite_kernels."""
    ...
    if python_executable:
        ...
        python_executable_prompt = (
            ...
            "the full path to your Python executable for Aphrodite kernels development "
    ...
    # Get aphrodite_kernels project root
    # (assuming this script is in aphrodite_kernels/tools/)
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    print(f"Aphrodite kernels project root detected as: {project_root}")
    ...
    try:
        ...
        print("\nTo use this preset:")
        print(f"1. Ensure you are in the aphrodite_kernels directory: cd {project_root}")
        print("2. Initialize CMake: cmake --preset release")
        print("3. Build+install: cmake --build --preset release --target install")
        print("\nNote: For building with setuptools, use:")
        print(f"   cd {project_root} && pip install -e .")
