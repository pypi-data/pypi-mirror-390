...
class Test(unittest.TestCase):
    ...
    def test_fused_quantization(self):
        ...
        def _absmax_nv_case(rot_size: int, global_scale_value: float):
            ...
            xh_e4m3 = xh_e4m3.reshape(2, 4096, 4096 // 16)  # FIXME:
            ...
            a_dq, *_ = _dq_fp4(a_e2m1, a_e4m3[:m, :k//16], alpha=1.)
            b_dq, *_ = _dq_fp4(b_e2m1, b_e4m3[:n, :k//16], alpha=1.)
            out_ref = a_dq @ b_dq.transpose(-2, -1)

            a_scale_block = to_blocked(a_e4m3, True)
            b_scale_block = to_blocked(b_e4m3, True)
            alpha = torch.tensor([1.], device=device)
            out = matmul_nvf4_bf16_tn(a_e2m1, b_e2m1, a_scale_block, b_scale_block, alpha)
            assert out.equal(out_ref.to(dtype=out.dtype))

        for rs in (16, 32, 64, 128):
            _absmax_nv_case(rot_size=rs, global_scale_value=6.0)
    ...
    def run_problem_cudagraph(self, m, n, k, rot_size):
        print("(CG)", m, n, k)
        a = torch.randn(m, k, dtype=self.dtype, device=self.device) * 25.
        b = torch.randn(n, k, dtype=self.dtype, device=self.device) * 25.
        hadamard_matrix = get_hadamard_matrix(rot_size, self.dtype, self.device)
        ...
        def _matmul_call():
            ...
            a_scale_block = to_blocked(a_e4m3, True)
            b_scale_block = to_blocked(b_e4m3, True)
            return matmul_nvf4_bf16_tn(a_e2m1, b_e2m1, a_scale_block, b_scale_block, alpha)
            ...
            for layer in layers:
                for batch in [1, 16]:
                    for rot_size in [16, 32, 64, 128]:
                        ...
                        self.run_problem_cudagraph(batch, layer[1], layer[0], rot_size)
