...
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE_PARAM) __global__
    void topkGatingSoftmax(const InputType* input, const bool* finished,
...
void topkGatingSoftmaxLauncherHelper(
  ...
  topkGatingSoftmax<VPT, EXPERTS, WARPS_PER_TB, BYTES_PER_LDG, WARP_SIZE_PARAM,
                    IndType, InputType><<<num_blocks, block_dim, 0, stream>>>(
      input, finished, output, num_rows, indices, source_row, k, start_expert,
      end_expert, renormalize);
}
  ...
  switch (num_experts) {
    ...
    default: {
      ...
      moeTopK<TPB><<<num_tokens, TPB, 0, stream>>>(
          ...
          token_expert_indices, num_experts, topk, 0, num_experts, renormalize);
    }
  }
}
...
void dispatch_topk_softmax_launch(torch::Tensor& gating_output,
...
void topk_softmax(torch::Tensor& topk_weights,          // [num_tokens, topk]
  ...
  if (gating_output.scalar_type() == at::ScalarType::Float) {
    dispatch_topk_softmax_launch<float>(
        ...
        softmax_workspace, num_tokens, num_experts, topk, renormalize, stream);
