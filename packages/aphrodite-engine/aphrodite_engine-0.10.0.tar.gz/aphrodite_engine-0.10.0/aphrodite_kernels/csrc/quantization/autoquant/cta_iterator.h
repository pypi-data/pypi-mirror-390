...
template <typename T>
__inline__ __device__ void cp_async_ca(uint32_t smem_int_ptr,
                                       const T* __restrict__ src, bool mask) {
#if APHRODITE_ARCH_SM80
  constexpr int cp_size = sizeof(T);
  // clang-format off
...
struct IteratorA {
  ...
  static_assert(CTA_M % 32 == 0 && CTA_K % 32 == 0,
  ...
  __device__ IteratorA(const uint* src, void* smem, int m, int k, int cta_m,
                       int cta_k, int warp_id, int lane_id)
      : src_(src),
...
struct IteratorQ {
  ...
  static constexpr int kWarpIterK =
      SLICE_K / kWarpAccessK;  // SLICE_K / GROUP_SIZE, maybe 0
  ...
  static constexpr int kSizePerStage =
      std::max(SLICE_K / GROUP_SIZE, 1) * CTA_M;
  ...
  __device__ IteratorQ(const T_Q* src, T_Q* smem, int m, int k, int cta_m,
                       int cta_k, int warp_id, int lane_id)
      : src_(src),
        smem_(smem),
        smem_int_ptr_(cast_smem_ptr_to_uint(smem)),
    ...
    src_step_m_ = kWarpAccessM;
    src_step_k_ =
        m_ - kIterM * kWarpAccessM;  // valid only when SLICE_K >= GROUP_SIZE
    ...
    dst_step_m_ = kWarpAccessM;
    dst_step_k_ =
        CTA_M - kIterM * kWarpAccessM;  // valid only when SLICE_K >= GROUP_SIZE
  ...
  __device__ void prefetch(bool mask) {
    cp_async_ca(smem_int_ptr_ + dst_offset_,
                (const AccessType*)src_ + src_offset_, mask);
  }
};
...
struct IteratorB {
  ...
  __device__ IteratorB(const T_BC* src, void* smem, int k, int n, int cta_n,
                       int cta_k, int warp_id, int lane_id)
      : src_(src),
        smem_((AccessType*)smem),
        smem_int_ptr_(cast_smem_ptr_to_uint(smem)),
    ...
    const int cta_thread_offset_k =
        kWarpFootprintK * warp_offset_k + warp_thread_offset_k * kAccessK;
    const int cta_thread_offset_n =
        kWarpFootprintN * warp_offset_n + warp_thread_offset_n;
