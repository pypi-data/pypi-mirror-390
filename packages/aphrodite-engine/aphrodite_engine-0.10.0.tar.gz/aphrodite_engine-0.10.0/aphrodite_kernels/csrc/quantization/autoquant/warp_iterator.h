...
template <int CTA_M, int CTA_K, int WARP_M, int WARP_K, int OP_M, int OP_K,
          int GROUP_SIZE, int STAGES, int kSizePerStageA, int kSizePerStageQ,
          typename T_BC, typename T_Q>
struct WarpIteratorA {
  ...
  //__device__ WarpIteratorA(uint4* smem_A, half2* smem_Q, int warp_id, int
  // lane_id, int offset_m, int offset_k):
  __device__ WarpIteratorA(uint4* smem_A, T_Q* smem_Q, int warp_id, int lane_id,
                           int offset_m, int offset_k)
      : smem_A_(smem_A),
        smem_Q_(smem_Q),
        offset_m_(offset_m),
        offset_m_Q_(offset_m / 32 * 32 + lane_id / 4) {}
  ...
  __device__ void load(Array<T_BC, 8>* data, int iter_k) {
    ...
    if (iter_k % 2 == 0) {
      ...
      for (int x = 0; x < ITER_X; ++x) {
        frag_A4_[x] =
            smem_A_[offset_A_ + (iter_k / 2) * CTA_M + x * 32 + offset_m_];
      }
    }
...
template <int CTA_N, int CTA_K, int WARP_N, int WARP_K, int OP_N, int OP_K,
          int SMEM_STRIDE, int STAGES, typename T_BC>
