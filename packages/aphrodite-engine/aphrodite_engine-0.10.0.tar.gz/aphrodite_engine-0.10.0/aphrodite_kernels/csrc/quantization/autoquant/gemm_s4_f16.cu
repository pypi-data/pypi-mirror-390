...
struct Identity {
  static __inline__ __device__ void apply(uint data, int m, int n, half* C,
                                          int M, int N) {
    if (n < N) {
  ...
  static __inline__ __device__ void apply(uint data, int m, int n,
                                          __nv_bfloat16* C, int M, int N) {
    if (n < N) {
      (uint&)C[n * M + m] = (uint&)data;
    }
  }
...
void Impl<T_BC, T_Q>::Generate(std::vector<Kernels>& kernels) {
  ...
  k.emplace_back(new GemmKernel<Shape<256, 128, 32>, Shape<32, 128, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<256, 64, 64>, Shape<64, 64, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<256, 64, 32>, Shape<64, 64, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<256, 32, 64>, Shape<64, 32, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<256, 16, 256>, Shape<32, 16, 128>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<256, 8, 256>, Shape<32, 8, 128>, 3, GS,
                                Op, T_BC, T_Q>{});
  // 128
  k.emplace_back(new GemmKernel<Shape<128, 128, 64>, Shape<32, 128, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<128, 128, 32>, Shape<32, 128, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<128, 96, 64>, Shape<32, 96, 32>, 3, GS,
                                Op, T_BC, T_Q>{});
  ...
  k.emplace_back(new GemmKernel<Shape<128, 16, 256>, Shape<32, 16, 64>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<128, 8, 512>, Shape<32, 8, 128>, 3, GS,
                                Op, T_BC, T_Q>{});
  k.emplace_back(new GemmKernel<Shape<128, 8, 512>, Shape<32, 8, 128>, 2, GS,
                                Op, T_BC, T_Q>{});  // for 86/89
  // 64
...
template <typename T_BC, typename T_Q>
void Impl<T_BC, T_Q>::Measure(T_BC* C, const uint* A, const T_BC* B,
                              const T_Q* Q, int m, int n, int k, int group_size,
                              Type type, std::vector<Metric>& metrics,
                              cudaStream_t st, std::vector<Kernels>& _kernels) {
  int gid = -1;
  ...
  std::stable_sort(indices.begin(), indices.end(), [&](int i, int j) {
    return metrics[i].time < metrics[j].time;
  });
...
int Impl<T_BC, T_Q>::Estimate(int m, int n, int k, Kernels& kernels) {
  ...
  if (g_dump_kernel_info_once) {
    ...
    std::iota(indices.begin(), indices.end(), 0);
    std::stable_sort(indices.begin(), indices.end(), [&](int i, int j) {
      return Compare(metrics[i], metrics[j]);
    });
    DumpMetrics(std::cerr, metrics, indices);
    g_dump_kernel_info_once = 0;
  }
...
template <typename T_BC, typename T_Q>
void Impl<T_BC, T_Q>::Run(T_BC* C, const uint* A, const T_BC* B, const T_Q* Q,
                          int m, int n, int k, int group_size, Type type,
...
template <typename T_BC, typename T_Q>
GemmS4F16<T_BC, T_Q>::GemmS4F16()
    : impl_(std::make_unique<Impl<T_BC, T_Q>>()) {}

template <typename T_BC, typename T_Q>
GemmS4F16<T_BC, T_Q>::~GemmS4F16() = default;

template <typename T_BC, typename T_Q>
void GemmS4F16<T_BC, T_Q>::Measure(T_BC* C, const uint* A, const T_BC* B,
                                   const T_Q* Q, int m, int n, int k,
                                   int group_size, Type type,
                                   std::vector<Metric>& metrics,
                                   cudaStream_t st) {
  impl_->Measure(C, A, B, Q, m, n, k, group_size, type, metrics, st,
                 impl_->kernels_);
}

template <typename T_BC, typename T_Q>
void GemmS4F16<T_BC, T_Q>::Run(T_BC* C, const uint* A, const T_BC* B,
                               const T_Q* Q, int m, int n, int k,
                               int group_size, Type type, int algo_id,
                               cudaStream_t st) {
  impl_->Run(C, A, B, Q, m, n, k, group_size, type, algo_id, st,
             impl_->kernels_);
}
