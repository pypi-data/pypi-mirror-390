...
__inline__ __device__ uint4 dequantize_s4_to_fp16x2_v2(uint32_t const& source) {
  ...
  if (0) {  // 1024 & 64
    ...
    h[0] <<= 4;
    h[2] <<= 4;
    // we don't need to subtract the magic nums because zeros will go through
    // the same dequant function and carry the same magic constant, the magic
    // num will be canceled out after subtracting zeros
  }
...
inline __device__ __nv_bfloat162 bf16hsub2(const __nv_bfloat162 a,
...
inline __device__ __nv_bfloat162 bf16hmul2(const __nv_bfloat162 a,
                                           const __nv_bfloat162 b) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 800
  assert(false);
#else
...
inline __device__ __nv_bfloat162 halves2bfloat162(const __nv_bfloat16 a,
...
template <>
__inline__ __device__ __nv_bfloat162 apply_Q(const __nv_bfloat162& x,
...
__device__ __forceinline__ float clamp_inf_for_half(const float input) {
  ...
  return input > 0.0f ? min(input, (HALF_FLT_MAX - 1000) / 2.0)
                      : max(input, (-HALF_FLT_MAX + 1000) / 2.0);
}
...
struct Array {
  ...
  __device__ __host__ constexpr T& operator[](int i) noexcept { return a[i]; }
...
struct Shape {
  ...
  constexpr int count() const noexcept { return (Ns * ...); }
};
