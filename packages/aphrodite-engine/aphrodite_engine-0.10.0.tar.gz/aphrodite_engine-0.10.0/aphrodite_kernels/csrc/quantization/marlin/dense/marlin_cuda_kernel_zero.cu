...
__device__ inline void scale(FragB& frag_b, FragS& frag_s, FragS& frag_z,
                             int i) {
...
__global__ void Marlin(
    ...
    const int4* __restrict__ s,  // fp16 quantization scales of shape
                                 // (k/groupsize)xn
    const int4* __restrict__ z,  // fp16 quantization zeros of shape
                                 // (k/groupsize)xn
  ...
  auto init_slice = [&]() {
    ...
    if (slice_iters < 0 || slice_col >= n_tiles) slice_iters = 0;
  ...
  auto fetch_to_registers = [&](int k, int pipe) {
    ...
    if (group_blocks != -1) {
      ...
      int4* sh_z_stage =
          sh_z + s_sh_stage * ((group_blocks / thread_k_blocks) *
                               (pipe / (group_blocks / thread_k_blocks)));
...
const int SHARED_MEM =
  ...
  #define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,         \
      ...
      Marlin<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,     \
          ...
          A_ptr, B_ptr, C_ptr, s_ptr, z_ptr, prob_m, prob_n, prob_k, locks); \
    }

const int ERR_PROB_SHAPE = 1;
const int ERR_KERN_SHAPE = 2;

int marlin_cuda(const void* A, const void* B, void* C, void* s, void* z,
...
__global__ void dequant_marlin(const uint32_t* qweight, const half* scales,
                               const half* zeros, half* out, int m, int n,
                               int groupsize) {
  int t = threadIdx.x;
  int group = blockIdx.x * 16 / groupsize;
  int4 pack = *reinterpret_cast<const int4*>(qweight + blockIdx.x * n * 2 +
                                             blockIdx.y * 128 + t * 4);
  const FragS* sscale = reinterpret_cast<const FragS*>(
      scales + group * n + blockIdx.y * 64 + t / 4 * 8);
  const FragS* szero = reinterpret_cast<const FragS*>(
      zeros + group * n + blockIdx.y * 64 + t / 4 * 8);
...
__global__ void awq_to_marlin(uint32_t* in, uint32_t* out, int m, int n) {
  ...
  #pragma unroll
    for (int i = 0; i < 8; i += 1) {
      block[col_offset / 2][offset][8 * (col_offset % 2) + order_map[i]] =
          v & 0xf;
      v >>= 4;
    }
  }
  ...
  // repack
  // ref: _get_perms @
  // https://github.com/IST-DASLab/marlin/blob/master/marlin/__init__.py
  uint32_t srow = (t % 4) * 2;
  uint32_t scol = t / 4;
...
__global__ void gptq_to_marlin(uint32_t* in, uint32_t* out, int* g_idx, int m,
                               int n) {
  ...
  for (int i = 0; i < 16; i += 1) {
    uint32_t source_row =
        g_idx ? g_idx[blockIdx.x * 16 + i] : (blockIdx.x * 16 + i);
...
// input:     `torch.half` input matrix of shape `(m, k)` in standard row-major
// layout weights:   `torch.int` weight matrix of original shape `(k, n)` in
// Marlin format; see `Layer.pack()` output:    `torch.half` out matrix of shape
// `(m, n)` in standard row-major layout scales:    `torch.half` scales of shape
// `(m / groupsize, n)` workspace: `torch.int` tensor with at least `n / 128`
// entries that are all zero

void marlin_gemm_zero(const torch::Tensor& input, const torch::Tensor& weights,
                      torch::Tensor& output, const torch::Tensor& scales,
                      const torch::Tensor& zeros, torch::Tensor& workspace) {
  ...
  if (prob_m >= 256) {
    ...
    aphrodite::marlin::dequant_marlin<<<gridDim, blockDim, 0, stream>>>(
        (const uint32_t*)weights.data_ptr(), (const half*)scales.data_ptr(),
        (const half*)zeros.data_ptr(), (half*)temp_dq.data_ptr(), prob_k,
        prob_n, groupsize);

    const half alpha = __float2half(1.0f);
    const half beta = __float2half(0.0f);
    cublasHgemm(at::cuda::getCurrentCUDABlasHandle(), CUBLAS_OP_N, CUBLAS_OP_N,
                prob_n, prob_m, prob_k, &alpha, (half*)temp_dq.data_ptr(),
                prob_n, (const half*)input.data_ptr(), prob_k, &beta,
                (half*)output.data_ptr(), prob_n);
  } else {
    int err = aphrodite::marlin::marlin_cuda(
        input.data_ptr(), weights.data_ptr(), output.data_ptr(),
        scales.data_ptr(), zeros.data_ptr(), prob_m, prob_n, prob_k,
...
torch::Tensor gptq_to_marlin(torch::Tensor W, torch::Tensor g_idx) {
  int m = W.sizes()[0];
  int n = W.sizes()[1];
  ...
  aphrodite::marlin::gptq_to_marlin<<<blocks, threads, 0, stream>>>(
      (uint32_t*)W.data_ptr(), (uint32_t*)result.data_ptr(),
      g_idx.device().is_meta() ? NULL : (int*)g_idx.data_ptr(), m, n);
  return result;
}

torch::Tensor awq_to_marlin(torch::Tensor W) {
  int m = W.sizes()[0];
  int n = W.sizes()[1];
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
  aphrodite::marlin::awq_to_marlin<<<blocks, threads, 0, stream>>>(
      (uint32_t*)W.data_ptr(), (uint32_t*)result.data_ptr(), m, n);
  return result;
}
#endif