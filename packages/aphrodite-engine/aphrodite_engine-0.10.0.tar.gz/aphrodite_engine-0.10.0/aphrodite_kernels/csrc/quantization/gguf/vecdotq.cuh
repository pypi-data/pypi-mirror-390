...
static __device__ __forceinline__ int get_int_from_int8_aligned(
    const int8_t* x8, const int& i32) {
  return *((const int*)(x8 + sizeof(int) *
                                 i32));  // assume at least 4 byte alignment
}

static __device__ __forceinline__ int get_int_from_uint8_aligned(
...
static __device__ __forceinline__ float vec_dot_q5_0_q8_1_impl(
    const int* vl, const int* vh, const int* u, const float& d5,
    const half2& ds8) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
  int sumi = 0;
...
static __device__ __forceinline__ float vec_dot_q5_1_q8_1_impl(
    const int* vl, const int* vh, const int* u, const half2& dm5,
    const half2& ds8) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
  int sumi = 0;
...
template <int vdr>
static __device__ __forceinline__ float vec_dot_q8_1_q8_1_impl(
...
static __device__ __forceinline__ float vec_dot_q2_K_q8_1_impl_mmq(
    ...
    const uint8_t* __restrict__ scales, const half2& dm2, const float& d8) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
  int sumi_d = 0;
  int sumi_m = 0;
...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1_impl_vmmq(
  ...
  for (int i = 0; i < QR4_K; ++i) {
    ...
    const int dot1 = __dp4a(v1i, u[2 * i + 1],
                            __dp4a(v0i, u[2 * i + 0], 0));  // SIMD dot product
    const int dot2 = __dp4a(0x01010101, u[2 * i + 1],
...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1_impl_mmq(
  ...
  #pragma unroll
    for (int j = 0; j < QI8_1; ++j) {
      sumi_d = __dp4a((v[j] >> (4 * i)) & 0x0F0F0F0F, u[i * QI8_1 + j],
...
static __device__ __forceinline__ float vec_dot_q5_K_q8_1_impl_mmq(
  ...
  #pragma unroll
    for (int j = 0; j < QI8_1; ++j) {
      sumi_d = __dp4a(v[i * QI8_1 + j], u[i * QI8_1 + j],
                      sumi_d);  // SIMD dot product
    }
...
static __device__ __forceinline__ float vec_dot_q6_K_q8_1_impl_mmq(
  ...
  #pragma unroll
    for (int i = i0; i < i0 + 2; ++i) {
      sumi_d.x =
          __dp4a(v[2 * i + 0], u[2 * i + 0], sumi_d.x);  // SIMD dot product
      sumi_d.x =
          __dp4a(v[2 * i + 1], u[2 * i + 1], sumi_d.x);  // SIMD dot product

      sumi_d.y =
          __dp4a(v[2 * i + 4], u[2 * i + 4], sumi_d.y);  // SIMD dot product
      sumi_d.y =
          __dp4a(v[2 * i + 5], u[2 * i + 5], sumi_d.y);  // SIMD dot product
    }

    sumf_d +=
        d8[i0 / 4] * (sc[i0 / 2 + 0] * sumi_d.x + sc[i0 / 2 + 1] * sumi_d.y);
  }
...
#pragma unroll
  ...
  return vec_dot_q4_0_q8_1_impl<VDR_Q4_0_Q8_1_MMVQ>(
      v, u, __half2float(bq4_0->d), bq8_1->ds);
}

template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q4_0(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI4_0) {
    ...
    x_dmf[i * (WARP_SIZE_GGUF / QI4_0) + i / QI4_0 + kbxd] =
        __half2float(bxi->d);
  }
}
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q4_1(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_qs[mmq_y * (WARP_SIZE_GGUF) + +mmq_y];
...
#pragma unroll
  for (int l = 0; l < VDR_Q4_1_Q8_1_MMQ; ++l) {
    u[2 * l + 0] = y_qs[j * WARP_SIZE_GGUF + (kyqs + l) % WARP_SIZE_GGUF];
    u[2 * l + 1] =
        y_qs[j * WARP_SIZE_GGUF + (kyqs + l + QI4_1) % WARP_SIZE_GGUF];
  }
...
#pragma unroll
  ...
  return vec_dot_q5_0_q8_1_impl<VDR_Q5_0_Q8_1_MMVQ>(
      vl, vh, u, __half2float(bq5_0->d), bq8_1->ds);
}

template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q5_0(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI5_0) {
    ...
    x_dmf[i * (WARP_SIZE_GGUF / QI5_0) + i / QI5_0 + kbxd] =
        __half2float(bxi->d);
  }
}
...
#pragma unroll
  for (int l = 0; l < VDR_Q5_0_Q8_1_MMQ; ++l) {
    u[2 * l + 0] = y_qs[j * WARP_SIZE_GGUF + (kyqs + l) % WARP_SIZE_GGUF];
    u[2 * l + 1] =
        y_qs[j * WARP_SIZE_GGUF + (kyqs + l + QI5_0) % WARP_SIZE_GGUF];
  }

  return vec_dot_q8_0_q8_1_impl<QR5_0 * VDR_Q5_0_Q8_1_MMQ>(
      &x_ql[i * (2 * WARP_SIZE_GGUF + 1) + 2 * k], u, x_dmf[index_bx],
      y_df[j * (WARP_SIZE_GGUF / QI8_1) +
           (2 * k / QI8_1) % (WARP_SIZE_GGUF / QI8_1)]);
}
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q5_1(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_ql[mmq_y * (2 * WARP_SIZE_GGUF) + mmq_y];
...
#pragma unroll
  for (int l = 0; l < VDR_Q5_1_Q8_1_MMQ; ++l) {
    u[2 * l + 0] = y_qs[j * WARP_SIZE_GGUF + (kyqs + l) % WARP_SIZE_GGUF];
    u[2 * l + 1] =
        y_qs[j * WARP_SIZE_GGUF + (kyqs + l + QI5_1) % WARP_SIZE_GGUF];
  }

  return vec_dot_q8_1_q8_1_impl<QR5_1 * VDR_Q5_1_Q8_1_MMQ>(
      &x_ql[i * (2 * WARP_SIZE_GGUF + 1) + 2 * k], u, x_dm[index_bx],
      y_ds[j * (WARP_SIZE_GGUF / QI8_1) +
           (2 * k / QI8_1) % (WARP_SIZE_GGUF / QI8_1)]);
}
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q8_0(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_qs[mmq_y * (WARP_SIZE_GGUF) + mmq_y];
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI8_0) {
    ...
    x_dmf[i * (WARP_SIZE_GGUF / QI8_0) + i / QI8_0 + kbxd] =
        __half2float(bxi->d);
  }
}
...
static __device__ __forceinline__ float vec_dot_q8_0_q8_1_mul_mat(
  ...
  return vec_dot_q8_0_q8_1_impl<VDR_Q8_0_Q8_1_MMQ>(
      &x_ql[i * (WARP_SIZE_GGUF + 1) + k], &y_qs[j * WARP_SIZE_GGUF + k],
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q2_K(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
...
static __device__ __forceinline__ float vec_dot_q2_K_q8_1_mul_mat(
  ...
  const int kqsx = i * (WARP_SIZE_GGUF + 1) + kbx * QI2_K +
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q3_K(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_ql[mmq_y * (WARP_SIZE_GGUF) + mmq_y];
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI3_K) {
    ...
    x_dmf[i * (WARP_SIZE_GGUF / QI3_K) + i / QI3_K + kbxd] =
        __half2float(bxi->d);
  }
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 2) {
    ...
    const block_q3_K* bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE_GGUF / 2)) / (QI3_K / 2);
    // invert the mask with ~ so that a 0/1 results in 4/0 being subtracted
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 4) {
    ...
    const block_q3_K* bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE_GGUF / 4)) / (QI3_K / 4);
...
static __device__ __forceinline__ float vec_dot_q3_K_q8_1_mul_mat(
  ...
  const int8_t* scales =
      ((const int8_t*)(x_sc + i * (WARP_SIZE_GGUF / 4) + i / 4 + kbx * 4)) +
      ky / 4;
...
#pragma unroll
  for (int l = 0; l < QR3_K * VDR_Q3_K_Q8_1_MMQ; ++l) {
    const int kqsx = i * (WARP_SIZE_GGUF + 1) + kbx * QI3_K +
                     (QI3_K / 2) * (ky / (2 * QI3_K)) + ky % (QI3_K / 2);
    const int shift = 2 * ((ky % 32) / 8);
    const int vll = (x_ql[kqsx + l] >> shift) & 0x03030303;

    const int vh = x_qh[i * (WARP_SIZE_GGUF / 2) + i / 2 + kbx * (QI3_K / 2) +
                        (ky + l) % 8] >>
                   ((ky + l) / 8);
  ...
  return vec_dot_q3_K_q8_1_impl_mmq(
      v, &y_qs[index_y], scales,
      x_dmf[i * (WARP_SIZE_GGUF / QI3_K) + i / QI3_K + kbx],
      y_df[index_y / QI8_1]);
}
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q4_K(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_ql[mmq_y * (WARP_SIZE_GGUF) + mmq_y];
...
#pragma unroll
  ...
  const int blocks_per_tile_x_row =
      WARP_SIZE_GGUF / QI4_K;                  // == 1 if QK_K == 256
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 8) {
    ...
    const block_q4_K* bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE_GGUF / 8)) / (QI4_K / 8);
...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1_mul_mat(
  ...
  const uint8_t* sc =
      ((const uint8_t*)&x_sc[i * (WARP_SIZE_GGUF / 8) + i / 8 + k / 16]) +
      2 * ((k % 16) / 8);

  const int index_y = j * WARP_SIZE_GGUF + (QR4_K * k) % WARP_SIZE_GGUF;
  return vec_dot_q4_K_q8_1_impl_mmq(
      &x_ql[i * (WARP_SIZE_GGUF + 1) + k], &y_qs[index_y], sc, sc + 8,
...
template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q5_K(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_ql[mmq_y * (2 * WARP_SIZE_GGUF) + mmq_y];
...
#pragma unroll
  ...
  const int blocks_per_tile_x_row =
      WARP_SIZE_GGUF / QI5_K;                  // == 1 if QK_K == 256
  const int kbxd = k % blocks_per_tile_x_row;  // == 0 if QK_K == 256
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 8) {
    ...
    const block_q5_K* bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE_GGUF / 8)) / (QI5_K / 8);
...
static __device__ __forceinline__ float vec_dot_q5_K_q8_1_mul_mat(
  ...
  return vec_dot_q5_K_q8_1_impl_mmq(
      &x_ql[index_x], &y_qs[index_y], sc, sc + 8,
...
#pragma unroll
  ...
  return vec_dot_q6_K_q8_1_impl_mmvq(vl, vh, u, scales, __half2float(bq6_K->d),
                                     d8);
}

template <int mmq_y>
static __device__ __forceinline__ void allocate_tiles_q6_K(int** x_ql,
                                                           half2** x_dm,
                                                           int** x_qh,
                                                           int** x_sc) {
  __shared__ int tile_x_ql[mmq_y * (2 * WARP_SIZE_GGUF) + mmq_y];
...
#pragma unroll
  ...
  const int blocks_per_tile_x_row =
      WARP_SIZE_GGUF / QI6_K;                  // == 1 if QK_K == 256
  const int kbxd = k % blocks_per_tile_x_row;  // == 0 if QK_K == 256
  float* x_dmf = (float*)x_dm;
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI6_K) {
    ...
    x_dmf[i * (WARP_SIZE_GGUF / QI6_K) + i / QI6_K + kbxd] =
        __half2float(bxi->d);
  }
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 8) {
    ...
    const block_q6_K* bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE_GGUF / 8)) / 4;

    x_sc[i * (WARP_SIZE_GGUF / 8) + i / 8 + k % (WARP_SIZE_GGUF / 8)] =
...
static __device__ __forceinline__ float vec_dot_q6_K_q8_1_mul_mat(
  ...
  const int8_t* sc =
      ((const int8_t*)&x_sc[i * (WARP_SIZE_GGUF / 8) + i / 8 + k / 8]);
  ...
  return vec_dot_q6_K_q8_1_impl_mmq(
      ...
      x_dmf[i * (WARP_SIZE_GGUF / QI6_K) + i / QI6_K], &y_df[index_y / QI8_1]);
}
...
static __device__ __forceinline__ float vec_dot_iq2_xxs_q8_1(
  ...
  const float d = __half2float(bq2->d) * (0.5f + aux32) *
                  __half2float(bq8_1[ib32].ds.x) * 0.25f;
  return d * sumi;
}
...
static __device__ __forceinline__ float vec_dot_iq2_s_q8_1(
  ...
  for (int l = 0; l < 2; ++l) {
    const uint32_t* grid =
        (const uint32_t*)(iq2s_grid +
                          (bq2->qs[4 * ib32 + l] |
                           ((bq2->qh[ib32] << (8 - 2 * l)) & 0x300)));
    const uint32_t signs0 =
        __vcmpeq4(((signs[l] & 0xf) * 0x01010101) & 0x08040201, 0x08040201);
    const uint32_t signs1 =
        __vcmpeq4(((signs[l] >> 4) * 0x01010101) & 0x08040201, 0x08040201);
...
static __device__ __forceinline__ float vec_dot_iq3_s_q8_1(
  ...
  for (int l = 0; l < 4; ++l) {
    const uint32_t* grid1 =
        iq3xs_grid + (qs[2 * l + 0] | ((bq2->qh[ib32] << (8 - 2 * l)) & 256));
    const uint32_t* grid2 =
        iq3xs_grid + (qs[2 * l + 1] | ((bq2->qh[ib32] << (7 - 2 * l)) & 256));
    uint32_t signs0 =
        __vcmpeq4(((bq2->signs[4 * ib32 + l] & 0xf) * 0x01010101) & 0x08040201,
                  0x08040201);
    uint32_t signs1 =
        __vcmpeq4(((bq2->signs[4 * ib32 + l] >> 4) * 0x01010101) & 0x08040201,
                  0x08040201);
  ...
  const float d = __half2float(bq2->d) *
                  (0.5f + ((bq2->scales[ib32 / 2] >> 4 * (ib32 % 2)) & 0xf)) *
                  __low2float(bq8_1[ib32].ds) * 0.5f;
  return d * sumi;
#endif
}
...
static __device__ __forceinline__ float vec_dot_iq1_s_q8_1(
  ...
  for (int l0 = 0; l0 < 8; l0 += 2) {
    const int grid =
        iq1s_grid_gpu[qs[l0 / 2] | (((qh >> 3 * (l0 / 2)) & 0x07) << 8)];
  ...
  const float delta =
      -1.0f + IQ1S_DELTA - (qh & 0x8000) * (2.0f * IQ1S_DELTA / 0x8000);
...
static __device__ __forceinline__ float vec_dot_iq1_m_q8_1(
  ...
  for (int l0 = 0; l0 < 8; l0 += 2) {
    ...
    const float delta =
        -1.0f + IQ1M_DELTA - (qhl & 0x08) * (2.0f * IQ1M_DELTA / 0x08);
    int sumy = 0;
...
static __device__ __forceinline__ void get_int_from_table_16(
...
static __device__ __forceinline__ float vec_dot_iq4_xs_q8_1(
  ...
  const int8_t ls = ((bq4->scales_l[ib32 / 2] >> 4 * (ib32 % 2)) & 0xf) |
                    (((bq4->scales_h >> 2 * ib32) & 3) << 4);
  const float d =
      __half2float(bq4->d) * (ls - 32) * __low2float(bq8_1[ib32].ds);
  int v1, v2;
  int sumi1 = 0, sumi2 = 0;
  for (int j = 0; j < 4; ++j) {
