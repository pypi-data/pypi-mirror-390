...
void xqa_paged_attention_launcher(
  ...
  auto seqLenPtr =
      reinterpret_cast<SeqLenDataType const*>(seq_lens.data_ptr<int>());
  ...
  KVCacheListParams kvcacheList(cacheHeads, pageListPtr, seqLenPtr,
                                maxNbPagesPerSeq);

  if (true) {
    printf("cacheHeads= %p q= %p output= %p\n", cacheHeads, qHeads, output);
    printf("maxSeqLen= %u, \n", maxSeqLen);
  ...
  DecoderXQARunner runner(io_type, num_heads, nbKHeads, head_size,
                          use_multi_block);

  size_t const nbSemaphores = nbKHeads * batchSize;
  auto const semaphores = ManagedMemBuf<uint32_t>(nbSemaphores);
  size_t workspace_size =
      runner.getWorkspaceSize(0);  // max_num_tokens is only useful when medusa
  ...
  torch::Tensor ws_tr = torch::empty(
...
void xqa_paged_attention(torch::Tensor& out, torch::Tensor& query,
                         ...
                         double k_scale, double v_scale) {
  DISPATCH_BY_KV_CACHE_ELEM_ENUM(query.dtype(), kv_cache_dtype,
                                 CALL_XQA_LAUNCHER);
}
...
//     CompileMacroOption('TOKENS_PER_PAGE', 'pagedKV', [0, 16, 32, 64, 128]), #
//     0 denotes contiguous kv cache. CompileMacroOption('HEAD_GRP_SIZE',
//     'nqpkv', [8]), CompileMacroOption('M_TILESIZE', 'm', [8]),
// ],
