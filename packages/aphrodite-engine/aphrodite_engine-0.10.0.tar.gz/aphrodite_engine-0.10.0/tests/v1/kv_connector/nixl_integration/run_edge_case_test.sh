...
run_tests_for_model() {
  ...
  BASE_CMD="CUDA_VISIBLE_DEVICES=$PREFILL_GPU_ID APHRODITE_NIXL_SIDE_CHANNEL_PORT=5559 aphrodite run $model_name \
  --port $PREFILL_PORT \
  --enforce-eager \
  ...
  BASE_CMD="CUDA_VISIBLE_DEVICES=$DECODE_GPU_ID APHRODITE_NIXL_SIDE_CHANNEL_PORT=6000 aphrodite run $model_name \
  --port $DECODE_PORT \
  --enforce-eager \
  --gpu-memory-utilization 0.2 \
  --disable-hybrid-kv-cache-manager \
