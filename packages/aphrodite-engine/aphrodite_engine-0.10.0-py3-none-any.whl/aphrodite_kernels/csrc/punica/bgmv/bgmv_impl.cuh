...
#ifdef USE_ROCM
template <size_t len>
__host__ __device__ inline void* memcpy_blocking(void* dst, const void* src) {
  // does not handle the case of long dtypes
...
__global__ void bgmv_shrink_kernel(out_T* __restrict__ Y,
  ...
  #pragma unroll
    for (size_t offset = tx / 2; offset > 0; offset /= 2) {
      sum += APHRODITE_SHFL_DOWN_SYNC(sum, offset);
    }
  ...
  if (threadIdx.x == 0 && threadIdx.y == 0) {
    ...
    Y[y_idx] =
        aphrodite_add<out_T>(Y[y_idx], convert_type<float, out_T>(y_write));
  }
}
...
#pragma unroll
  ...
  if (threadIdx.x == 0) {
    ...
    Y[y_idx] = aphrodite_add<out_T>(Y[y_idx], convert_type<float, out_T>(sum));
#endif
  }
}
...
void bgmv_kernel(out_T* __restrict__ Y, const in_T* __restrict__ X,
  ...
  #define LAUNCH_BGMV_SHRINK_KERNELS_ROCM(factor_, vec_size_, tx_, ty_)     \
    ...
    static_assert(CHECK_INPUT_TILEABLE_BY(32) || CHECK_INPUT_TILEABLE_BY(16) ||
                  CHECK_INPUT_TILEABLE_BY(8) || CHECK_INPUT_TILEABLE_BY(4) ||
                  CHECK_INPUT_TILEABLE_BY(2) || CHECK_INPUT_TILEABLE_BY(1));
