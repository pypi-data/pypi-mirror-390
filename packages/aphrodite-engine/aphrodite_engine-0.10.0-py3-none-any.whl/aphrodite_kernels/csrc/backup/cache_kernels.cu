  ...
  APHRODITE_DISPATCH_FLOATING_AND_BYTE_TYPES(
      key_caches[0].scalar_type(), "copy_blocks_kernel", ([&] {
        aphrodite::copy_blocks_kernel<scalar_t><<<grid, block, 0, stream>>>(
            ...
            block_mapping_tensor.data_ptr<int64_t>(), numel_per_block);
      }));
}
...
__global__ void reshape_and_cache_kernel(
    ...
    const float k_zp, const float v_scale, const float v_zp) {
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    if constexpr (KV_CACHE_DTYPE == INT8) {
      ...
      key_cache[tgt_key_idx] =
          fp8_e5m2_unscaled::vec_conversion<uint8_t, scalar_t>(tgt_key);
      value_cache[tgt_value_idx] =
          fp8_e5m2_unscaled::vec_conversion<uint8_t, scalar_t>(tgt_value);
#endif
    } else {
...
#define CALL_RESHAPE_AND_CACHE(KV_T, CACHE_T, KV_CACHE_DTYPE)         \
  aphrodite::reshape_and_cache_kernel<KV_T, CACHE_T, KV_CACHE_DTYPE>  \
      <<<grid, block, 0, stream>>>(                                   \
          ...
          num_heads, head_size, block_size, x, k_scale, k_zp, v_scale, v_zp);
    ...
    torch::Tensor& slot_mapping)  // [in]  [num_tokens]
{
  ...
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  APHRODITE_DISPATCH_FLOATING_AND_BYTE_TYPES(
      key.scalar_type(), "gather_cached_kv_kernel_optimized", [&] {
        aphrodite::gather_cached_kv_kernel_optimized<scalar_t>
            <<<grid, block, 0, stream>>>(
                key.data_ptr<scalar_t>(), value.data_ptr<scalar_t>(),
                key_cache.data_ptr<scalar_t>(),
                value_cache.data_ptr<scalar_t>(), slot_mapping.data_ptr<int>(),
...
__global__ void convert_fp8_e5m2_kernel(const Tin* __restrict__ src_cache,
...
#ifdef ENABLE_FP8_E5M2
    dst_cache[idx] =
        fp8_e5m2_unscaled::vec_conversion<Tout, Tin>(src_cache[idx]);
#else
    assert(false);
#endif
  }
