...
__inline__ __device__ void mma_m16n8k16_row_col(
    Array<float, 4>& d, const Array<__nv_bfloat16, 8>& a,
    const Array<__nv_bfloat16, 4>& b, Array<float, 4>& c) {
#if APHRODITE_ARCH_SM80
  uint32_t const* A = reinterpret_cast<uint32_t const*>(&a);
  uint32_t const* B = reinterpret_cast<uint32_t const*>(&b);
  float const* C = reinterpret_cast<float const*>(&c);
...
template <int CTA_M, int CTA_N, int CTA_K, int WARP_M, int WARP_N, int WARP_K,
          int STAGES, int GROUP_SIZE, typename OutputOps, typename T_BC,
          typename T_Q>
struct Gemm {
  ...
  using IteratorA = aphrodite::autoquant::IteratorA<kWarpCountMN, CTA_M, CTA_N,
                                                    CTA_K, STAGES, SLICES>;
  using IteratorQ =
      aphrodite::autoquant::IteratorQ<kWarpCountMN, CTA_M, CTA_N, CTA_K, STAGES,
                                      SLICES, GROUP_SIZE, T_Q>;
  using IteratorB =
      aphrodite::autoquant::IteratorB<kWarpCountMN, CTA_M, CTA_N, CTA_K, STAGES,
                                      SLICES, T_BC>;
  ...
  using WarpIterA = aphrodite::autoquant::WarpIteratorA<
      CTA_M, CTA_K, WARP_M, WARP_K, OP_M, OP_K, GROUP_SIZE, STAGES,
      IteratorA::kSizePerStage, IteratorQ::kSizePerStage, T_BC, T_Q>;

  using WarpIterB =
      aphrodite::autoquant::WarpIteratorB<CTA_N, CTA_K, WARP_N, WARP_K, OP_N,
                                          OP_K, IteratorB::kSmemPadCtaK, STAGES,
                                          T_BC>;

  __device__ void warp_mma(IteratorA& iter_A, IteratorQ& iter_Q,
                           IteratorB& iter_B, WarpIterA& warp_iter_A,
                           WarpIterB& warp_iter_B, float* accum, int slice_id,
                           int& gemm_iter) {
  ...
  __device__ void load_partial(float* tb_frag_C, const float* partial_C,
                               int cta, int slice_id) {
    if (slice_id == 0) {
      PRAGMA_UNROLL
      for (int i = 0; i < CTA_N; ++i) {
        tb_frag_C[i] +=
            partial_C[cta * CTA_N * CTA_M + i * CTA_M + threadIdx.x];
      }
    }
  }

  __device__ void store_partial(float* partial_C, const float* tb_frag_C,
                                int cta, int slice_id) {
    if (slice_id == 0) {
      PRAGMA_UNROLL
      for (int i = 0; i < CTA_N; ++i) {
  ...
  template <int Index>
  __device__ void store_accum(float* tb_frag_C, float* tb_smem_C, T_BC* C,
                              int m, int n, int cta_m, int cta_n, int warp_id_m,
                              int warp_id_n, int lane_id, int slice_id) {
    if (slice_id != 0) {
      return;
    }
    ...
    for (int i = 0; i < WARP_N / OP_N; ++i) {
      ...
      for (int j = 0; j < WARP_M / OP_M; ++j) {
        ...
        for (int x = 0; x < 2; ++x) {
          const int mm =
              cta_m + warp_id_m * WARP_M + j * OP_M + x * 8 + lane_id % 4 * 2;
  ...
  __device__ void sum_slices(float* tb_frag_C, float* tb_smem_C, int warp_id_m,
                             int warp_id_n, int lane_id, int slice_id) {
  ...
  __device__ void run_v2(T_BC* __restrict__ C, const uint* __restrict__ A,
                         const T_BC* __restrict__ B, const T_Q* __restrict__ Q,
                         int M, int N, int K, int output_op_idx) {
    ...
    uint4* const tb_smem_A =
        (uint4*)(smem + IteratorA::kSmemByteSize * slice_id);
    T_BC* const tb_smem_B = (T_BC*)(smem + IteratorA::kSmemByteSize * SLICES +
                                    IteratorB::kSmemByteSize * slice_id);
    ...
    WarpIterA warp_iter_A(iter_A.smem_, iter_Q.smem_, warp_id, lane_id,
                          offset_m, cta_k);
    ...
    for (; gemm_iter > -STAGES + 1;) {
      warp_mma(iter_A, iter_Q, iter_B, warp_iter_A, warp_iter_B, tb_frag_C,
               slice_id, gemm_iter);
    }
    ...
    switch (output_op_idx) {
      case 0:
        store_accum<0>(tb_frag_C, tb_smem_C, C, M, N, cta_m, cta_n, warp_id_m,
                       warp_id_n, lane_id, slice_id);
        break;
      case 1:
        store_accum<1>(tb_frag_C, tb_smem_C, C, M, N, cta_m, cta_n, warp_id_m,
                       warp_id_n, lane_id, slice_id);
        break;
      default:
        return;
...
__global__ void gemm_s4_f16_nn(T_BC* __restrict__ C, const uint* __restrict__ A,
                               const T_BC* __restrict__ B,
                               const T_Q* __restrict__ Q, int M, int N, int K,
                               int output_op_idx) {
