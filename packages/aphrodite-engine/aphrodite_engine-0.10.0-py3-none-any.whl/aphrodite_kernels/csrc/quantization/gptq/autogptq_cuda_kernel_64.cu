...
__global__ void VecQuant2MatMulKernel(
    const scalar_t* __restrict__ vec, const int* __restrict__ mat,
...
__global__ void VecQuant2MatMulKernel_old(
    const scalar_t* __restrict__ vec, const int* __restrict__ mat,
    scalar_t* __restrict__ mul, const scalar_t* __restrict__ scales,
    const int* __restrict__ zeros, int batch, int vec_height, int height,
    int width, int zero_width, int groupsize);
...
__global__ void VecQuant2MatMulKernelFaster_old(
    ...
    float* __restrict__ mul, const float* __restrict__ scales,
    const int* __restrict__ zeros, int batch, int vec_height, int height,
    int width, int zero_width, int groupsize);
...
void vecquant2matmul_cuda(torch::Tensor vec, torch::Tensor mat,
                          torch::Tensor mul, torch::Tensor scales,
                          torch::Tensor zeros, torch::Tensor g_idx) {
  ...
  dim3 blocks((height + BLOCKHEIGHT2 - 1) / BLOCKHEIGHT2,
...
__global__ void VecQuant2MatMulKernel(
  ...
  for (k = 0; k < BLOCKWIDTH; ++k) {
    ...
    scalar_t zero =
        scalar_t((as_unsigned(zeros[g * zero_width + z_w]) >> z_mod & 0x3) + 1);
  ...
  for (int b = 0; b < batch; ++b) {
    ...
    blockvec[threadIdx.x] =
        vec[b * vec_height + blockIdx.x * BLOCKWIDTH + threadIdx.x];
    __syncthreads();
    for (k = 0; k < BLOCKWIDTH; ++k) {
...
void vecquant3matmul_cuda(torch::Tensor vec, torch::Tensor mat,
  ...
  dim3 blocks((height + BLOCKHEIGHT3 - 1) / BLOCKHEIGHT3,
              (width + BLOCKWIDTH - 1) / BLOCKWIDTH);
  dim3 threads(BLOCKWIDTH);
...
__global__ void VecQuant3MatMulKernel(
  ...
  for (k = 0; k < BLOCKWIDTH; ++k) {
    ...
    if (z_mod == 10) {
      ...
      zero = scalar_t(
          ((as_unsigned(zeros[g * zero_width + z_w]) >> z_bit) & 0x7) + 1);
    }
...
void vecquant4matmul_cuda(torch::Tensor vec, torch::Tensor mat,
                          torch::Tensor mul, torch::Tensor scales,
                          torch::Tensor zeros, torch::Tensor g_idx) {
  int batch = vec.size(0);
  int vec_height = vec.size(1);
...
void vecquant8matmul_cuda(torch::Tensor vec, torch::Tensor mat,
                          torch::Tensor mul, torch::Tensor scales,
                          torch::Tensor zeros, torch::Tensor g_idx) {
  int batch = vec.size(0);
  int vec_height = vec.size(1);
  ...
  dim3 blocks((height + BLOCKHEIGHT8 - 1) / BLOCKHEIGHT8,
              (width + BLOCKWIDTH - 1) / BLOCKWIDTH);
  dim3 threads(BLOCKWIDTH);
...
__global__ void VecQuant8MatMulKernel(
  ...
  for (k = 0; k < BLOCKWIDTH; ++k) {
    ...
    scalar_t scale = scales[g * width + w];
    scalar_t zero = scalar_t(
        ((as_unsigned(zeros[g * zero_width + z_w]) >> z_mod) & 0xFF) + 1);
