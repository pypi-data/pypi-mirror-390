...
def dequantize_weight(codes: torch.Tensor, codebooks: torch.Tensor, scales: torch.Tensor | None = None) -> torch.Tensor:
    """
...
class AQLMConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional["AQLMLinearMethod"]:
...
class AQLMLinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        set_weight_attrs(
            scales,
            {"output_dim": 0, "packed_dim": 0, "pack_factor": self.quant_config.out_group_size},
        )
    ...
    def apply(
        ...
        # kernels.
        if ingroups == 8 and outgroups == 1 and ((bits == 256 and nbooks == 2) or (bits == 65536 and nbooks == 1)):
