...
from aphrodite.modeling.models.utils import AutoWeightsLoader, flatten_bn, init_aphrodite_registered_model, maybe_prefix
from aphrodite.modeling.sampling_metadata import SamplingMetadata
from aphrodite.multimodal import MULTIMODAL_REGISTRY
from aphrodite.multimodal.inputs import MultiModalDataDict, MultiModalFieldConfig, MultiModalKwargs
from aphrodite.multimodal.parse import ImageSize, MultiModalDataItems
from aphrodite.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, PromptReplacement
...
class Ovis2ProcessingInfo(BaseProcessingInfo):
    ...
    def get_supported_mm_limits(self) -> Mapping[str, int | None]:
        return {  # 32k is model token limit at the moment
            "image": self.get_hf_config().multimodal_max_length // ((9 + 1) * NUMBER_OF_TOKEN_TO_RESERVE_FOR_SEGMENT)
        }

    def get_image_size_with_most_features(self) -> ImageSize:
        image_processor = self.get_image_processor()
        return ImageSize(
            width=image_processor.size["shortest_edge"] * 9 * 2, height=image_processor.size["shortest_edge"] * 9 * 2
        )
...
@MULTIMODAL_REGISTRY.register_processor(
    Ovis2MultiModalProcessor, info=Ovis2ProcessingInfo, dummy_inputs=Ovis2DummyInputsBuilder
)
class Ovis2ForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Ovis2ImagePatchInputs | None:
    ...
    def _process_image_input(self, image_input: Ovis2ImagePatchInputs) -> MultiModalEmbeddings:
    ...
    def get_input_embeddings(
        ...
        if multimodal_embeddings is not None:
            inputs_embeds = merge_multimodal_embeddings(
                input_ids, inputs_embeds, multimodal_embeddings, [IMAGE_PAD_TOKEN_ID]
            )
    ...
    def compute_logits(
        ...
        logits = self.llm.logits_processor(self.llm.lm_head, hidden_states, sampling_metadata)
        return logits
