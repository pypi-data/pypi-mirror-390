...
from aphrodite.modeling.models.bart import BartDecoder, BartEncoder, BartParallelLMHead, BartScaledWordEmbedding
...
from .interfaces import MultiModalEmbeddings, SupportsMultiModal, SupportsV0Only
...
class Florence2LanguageModel(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.encoder = BartEncoder(
            config, cache_config=cache_config, quant_config=quant_config, prefix=f"{prefix}.encoder"
        )
        self.decoder = BartDecoder(
            config, cache_config=cache_config, quant_config=quant_config, prefix=f"{prefix}.decoder"
        )
    ...
    def forward(
        ...
        if (inputs_embeds is not None and inputs_embeds.numel() > 0) or encoder_input_ids.numel() > 0:
...
class Florence2LanguageForConditionalGeneration(nn.Module, SupportsV0Only):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.config = config
        self.model = Florence2LanguageModel(aphrodite_config=aphrodite_config, prefix=f"{prefix}.model")
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self.model(input_ids, positions, encoder_input_ids, encoder_positions, inputs_embeds=inputs_embeds)
        ...
        for name, loaded_weight in weights:
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if "final_logits_bias" in name:
                    continue
                if self.config.tie_word_embeddings and ("embed_tokens" in name or "lm_head" in name):
                    continue
                param = params_dict[name]
...
@MULTIMODAL_REGISTRY.register_processor(
    Florence2MultiModalProcessor, info=Florence2ProcessingInfo, dummy_inputs=Florence2DummyInputsBuilder
)
class Florence2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsV0Only):
    @classmethod
    ...
    def get_input_embeddings(
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(
                input_ids, inputs_embeds, multimodal_embeddings, self.pad_token_id
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.language_model(
            input_ids, positions, encoder_input_ids, encoder_positions, inputs_embeds=inputs_embeds
        )
