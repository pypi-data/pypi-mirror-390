...
from augment.agent import Agent, Model
from augment.exceptions import (
    AugmentCLIError,
...
def make_type_inference_response(value, type_name="str", message="Response"):
    ...
    Args:
        value: The value to return (will be JSON-encoded)
...
class TestAgent:
    ...
    def test_init_invalid_workspace(self):
        ...
        with pytest.raises(AugmentWorkspaceError):
            Agent(workspace_root="/nonexistent/path")

    @patch("augment.agent.AuggieACPClient")
...
"Agent response message"
        ...
        agent = Agent()
        result, inferred_type = agent.run("Test instruction")
...
42
        ...
        agent = Agent()
        result, chosen_type = agent.run("What is 2 + 2?")  # No return_type

        assert result == 42
        assert chosen_type == int
        assert isinstance(result, int)
...
"hello"
        ...
        agent = Agent()
        result, chosen_type = agent.run("Say hello")  # No return_type

        assert result == "hello"
        assert chosen_type == str
        assert isinstance(result, str)

    @patch("augment.agent.AuggieACPClient")
    def test_run_automatic_type_inference_missing_type_tag(self, mock_acp_client_class):
        """Test automatic type inference error when type tag is missing."""
        # Mock ACP client
        mock_client = MagicMock()
        mock_client.session_id = "test-session-123"
...
42
        ...
        with pytest.raises(AugmentParseError, match="No type classification found"):
            agent.run(
                "Test", max_retries=0
...
42
    ...
    def test_run_automatic_type_inference_with_retry(self, mock_acp_client_class):
        """Test automatic type inference with retry on parse failure."""
        # Mock ACP client
        mock_client = MagicMock()
        mock_client.session_id = "test-session-123"
        ...
        # First: missing type tag, Second: correct
        mock_client.send_message.side_effect = [
            """<augment-agent-message>
Missing type tag.
</augment-agent-message>
...
42
    ...
    def test_session_context_manager(self, mock_acp_client_class):
        ...
        with agent.session() as session:
            ...
            result, inferred_type = session.run("Test instruction")
    ...
    def test_multiple_run_calls_are_independent(self, mock_acp_client_class):
        ...
        result1, type1 = agent.run("First instruction")
        result2, type2 = agent.run("Second instruction")
...
class TestGetAvailableModels:
    ...
    @patch("augment.agent.subprocess.run")
...
class TestProvidedACPClient:
    ...
    def test_run_with_provided_running_client(self):
        ...
        # Run a command
        result, inferred_type = agent.run("Test")
    ...
    def test_session_with_provided_client(self):
        ...
        # Use session
        with agent.session() as session:
            result1, type1 = session.run("First message")
            result2, type2 = session.run("Second message")
    ...
    def test_session_with_provided_running_client(self):
        ...
        # Use session
        with agent.session() as session:
            result, inferred_type = session.run("Test")
