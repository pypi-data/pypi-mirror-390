...
To run these tests: pytest tests/test_agent_e2e.py -m ""
"""
...
from augment.exceptions import AugmentParseError
...
def test_simple_query():
    ...
    # No return_type specified, so it returns (result, inferred_type)
    result, inferred_type = agent.run(
    ...
    assert inferred_type in [int, str]
...
def test_context_manager():
    ...
    with agent.session() as session:
        result, inferred_type = session.run(
...
def test_session_context_shares_context():
    ...
    with agent.session() as session:
        ...
        result, _ = session.run("What is my favorite color? Return just the color.")
        response = str(result).lower()
...
def test_invalid_workspace_error():
    ...
    from augment.exceptions import AugmentWorkspaceError

    with pytest.raises(AugmentWorkspaceError):
        Agent(workspace_root="/nonexistent/path/that/does/not/exist")
