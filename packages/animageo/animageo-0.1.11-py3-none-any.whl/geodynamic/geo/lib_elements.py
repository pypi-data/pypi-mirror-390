...
class Element:
    def __init__(self, name, data = None, visible = True, fixed = False, alpha = None):
        self.name = name
        self.data = data
        self.fixed = fixed
        self.alpha = alpha
...
def get_direction(alpha = None):
    return cpx_to_a(np.exp((alpha if alpha is not None else np.random.random()) * 2*np.pi * 1j))
def square_norm(x):
    return np.dot(x,x)
