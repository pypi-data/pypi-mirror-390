from __future__ import annotations

from typing import TYPE_CHECKING

from fortytwo.request.parameter.pagination import with_pagination
from fortytwo.resources.cursus.resource import GetCursusById, GetCursuses


if TYPE_CHECKING:
    from fortytwo.client import Client
    from fortytwo.request.parameter.parameter import Parameter
    from fortytwo.resources.cursus.cursus import Cursus


class CursusManager:
    """
    Manager for cursus-related API operations.
    """

    def __init__(self, client: Client) -> None:
        self._client = client

    def get_by_id(self, cursus_id: int, *params: Parameter) -> Cursus:
        """
        Get a cursus by ID.

        Args:
            cursus_id: The cursus ID to fetch
            *params: Additional request parameters

        Returns:
            Cursus object

        Raises:
            FortyTwoNotFoundException: If cursus is not found
            FortyTwoRequestException: If the request fails
        """
        return self._client.request(GetCursusById(cursus_id), *params)

    @with_pagination
    def get_all(
        self,
        *params: Parameter,
        page: int | None = None,
        page_size: int | None = None,
    ) -> list[Cursus]:
        """
        Get all cursuses.

        Args:
            *params: Additional request parameters
            page: Page number to fetch (1-indexed)
            page_size: Number of items per page (1-100)

        Returns:
            List of Cursus objects

        Raises:
            FortyTwoRequestException: If the request fails
        """
        return self._client.request(GetCursuses(), *params)
