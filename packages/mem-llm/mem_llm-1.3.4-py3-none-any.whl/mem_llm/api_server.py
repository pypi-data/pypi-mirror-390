...
app = FastAPI(
    title="Mem-LLM API",
    description="REST API for Mem-LLM - Memory-enabled AI Assistant",
    version="1.3.3",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
class AgentConfigRequest(BaseModel):
    ...
    backend: Optional[str] = Field(None, description="LLM backend (ollama/lmstudio/gemini)")
...
async def get_memory_stats():
    ...
    try:
        # Use any agent to get global stats
        if agents:
...
if web_ui_path.exists():
    ...
    async def root():
        ...
        return {"message": "Mem-LLM API Server", "version": "1.3.3"}
    ...
    async def memory_page():
        ...
        if memory_path.exists():
            return FileResponse(str(memory_path))
