...
def strain_and_relax(pdict, mdict):
    ...
    structure_files = [
        copy_labeled_structure(p, p.replace(DIR_MAKE_STRUCT, DIR_SUPERCELL))
...
def compute_stress_single_structure(work_dir, pdict, mdict):
    ...
    ref_crystal = read(f"{work_dir}/{FILE_FRAME_LABEL}")
    ...
    for idx, atoms in enumerate(deform_crystals):
        write_extxyz(f"{work_dir}/deform_{idx:03d}/{FILE_FRAME_unLABEL}", atoms)
...
def compute_elastic_tensor_single_structure(work_dir, pdict: dict, mdict: dict):
    ...
    deform_crystals = [read(f"{p}/{FILE_FRAME_LABEL}") for p in deform_paths]
...
def compute_elastic(pdict: dict, mdict: dict):
    ...
    ### work_dir
    struct_startdir = mk_struct_dir(pdict)
    work_dir = f"{struct_startdir}/{DIR_ELASTIC}"
    make_dir(work_dir, backup=False)

    compute_arg = pdict["elastic"].get("compute", {})
    ### Copy elastic_tensor.txt
    structure_paths = read_yaml(f"{struct_startdir}/{DIR_SUPERCELL}/structure_dirs.yml")

    if compute_arg.get("elastic_tensor", False):
        _ = [
            move_file(
                f"{p}/elastic_tensor.txt",
                f"{p}/elastic_tensor.txt".replace(DIR_SUPERCELL, DIR_ELASTIC),
            )
            for p in structure_paths
        ]
    if compute_arg.get("BM_EOS", False):
        _ = [
            move_file(
                f"{p}/BMeos.txt",
                f"{p}/BMeos.txt".replace(DIR_SUPERCELL, DIR_ELASTIC),
            )
            for p in structure_paths
        ]
        _ = [
            move_file(
                ...
                f"{p}/BMeos_pv_data.txt".replace(DIR_SUPERCELL, DIR_ELASTIC),
            )
            for p in structure_paths
        ]
...
class WorkflowElastic(Workflow):
    ...
    def __init__(self, param_file: str, machine_file: str):
        super().__init__(param_file, machine_file, SCHEMA_ELASTIC)
        self.stage_map = {
