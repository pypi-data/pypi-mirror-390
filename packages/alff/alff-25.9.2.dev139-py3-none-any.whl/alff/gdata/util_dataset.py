...
def split_extxyz_dataset(
    ...
    if len(train_idxs) > 0:
        ...
        write(f"{outfile_prefix}_trainset.extxyz", train_data, format="extxyz")
    if len(valid_idxs) > 0:
        valid_data = [struct_list[i] for i in valid_idxs]
        write(f"{outfile_prefix}_validset.extxyz", valid_data, format="extxyz")
    if len(test_idxs) > 0:
        test_data = [struct_list[i] for i in test_idxs]
        write(f"{outfile_prefix}_testset.extxyz", test_data, format="extxyz")
    return
...
def read_list_extxyz(extxyz_files: list[str]) -> list[Atoms]:
    ...
    struct_list = []
    [struct_list.extend(read(file, format="extxyz", index=":")) for file in extxyz_files]
    return struct_list
...
def merge_extxyz_files(
    ...
    sort_by_composition: bool = True,
    sort_pbc_len: bool = True,
):
    """
    ...
    Args:
        ...
        sort_by_natoms (bool): Sort by number of atoms. Defaults to True.
        sort_by_composition (bool): Sort by chemical composition. Defaults to True.
    ...
    """
    assert len(extxyz_files) > 0, "Must provide a list of EXTXYZ files."
    ...
    if sort_by_natoms and sort_by_composition and sort_pbc_len:
    ...
    write(outfile, sorted_struct_list, format="extxyz")
    return
...
def select_structs_from_extxyz(
    extxyz_file: str,
    has_symbols: list = None,
    only_symbols: list = None,
    exact_symbols: list = None,
    has_properties: list = None,
    only_properties: list = None,
    has_columns: list = None,
    ...
    if len(selected_index) > 0:
        ...
        write(extxyz_file.replace(".extxyz", "_selected.extxyz"), selected_structs, format="extxyz")
...
def remove_duplicate_structs_serial(extxyz_file: str, tol=1e-6) -> None:
    ...
    if len(unique_structs) < len(struct_list):
        ...
        write(out_file, unique_structs, format="extxyz")
...
def _struct_unique_fingerprint(struct, tol=1e-6):
    ...
    cell = tuple(map(tuple, np.round(s.get_cell() / tol).astype(int)))
...
def remove_duplicate_structs_hash(extxyz_file: str, tol=1e-6) -> None:
