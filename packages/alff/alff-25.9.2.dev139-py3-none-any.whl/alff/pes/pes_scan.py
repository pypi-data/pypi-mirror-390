...
from thkit.path import ask_user_action, make_dir
...
def scanning_space(pdict, mdict):
    ...
    ### Define work_dir
    struct_startdir = mk_struct_dir(pdict)
    work_dir = f"{struct_startdir}/{DIR_SCAN}"
    ...
    structure_files = [
        copy_labeled_structure(p, p.replace(f"{DIR_MAKE_STRUCT}", f"{DIR_SCAN}"))
    ...
    if scan_arg.get("scan_x", []) and scan_arg.get("scan_y", []) and scan_arg.get("scan_x", []):
        ans = ask_user_action(
            "WARNING: Scanning in all three dimensions is extremely time-consuming. \
            It is recommended to scan xy-plane or z-direction only. Are you sure want to continue?",
        )
        if ans == "no":
...
def compute_pes(pdict, mdict):
    ...
    ### work_dir
    struct_startdir = mk_struct_dir(pdict)
    work_dir = f"{struct_startdir}/{DIR_PES}"
    make_dir(work_dir, backup=False)

    ### Collect energy files
    task_dirs = read_yaml(f"{struct_startdir}/{DIR_SCAN}/task_dirs.yml")
    rawdata = [None] * len(task_dirs)
    for i, tdir in enumerate(task_dirs):
        struct = read_extxyz(f"{tdir}/{FILE_FRAME_LABEL}")[-1]
        energy = struct.calc.results.get("energy", None)
        stress = struct.calc.results.get("stress", None)
        dx, dy, dz = _extract_dxdydz(mystring=Path(tdir).name)
        rawdata[i] = [dx, dy, dz, energy]
        rawdata[i].extend(stress)
    ...
    ### natoms
    struct0 = read_extxyz(f"{task_dirs[0]}/{FILE_FRAME_LABEL}")[-1]
    n_atoms = len(struct0)
    ...
    fix_idxs = read_yaml(f"{struct_startdir}/{DIR_SCAN}/fix_idxs.yml")
...
class WorkflowPes(Workflow):
    ...
    def __init__(self, param_file: str, machine_file: str):
        super().__init__(param_file, machine_file, SCHEMA_PES_SCAN)
        self.stage_map = {
