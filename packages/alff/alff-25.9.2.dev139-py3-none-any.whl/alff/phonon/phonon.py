...
from thkit.path import ask_user_action, make_dir, move_file
...
def strain_and_relax(pdict, mdict):
    ...
    ### Define work_dir
    struct_startdir = mk_struct_dir(pdict)
    work_dir = f"{struct_startdir}/{DIR_SUPERCELL}"
    make_dir(work_dir, backup=False)

    ### Copy/symlink files from structure_dirs (unlabeled or labeled)
    structure_dirs = read_yaml(f"{struct_startdir}/{DIR_MAKE_STRUCT}/task_dirs.yml")
    structure_files = [
        copy_labeled_structure(p, p.replace(DIR_MAKE_STRUCT, DIR_SUPERCELL)) for p in structure_dirs
    ]
...
def compute_force_one_scaledstruct(work_dir: str, pdict, mdict):
    ...
    struct = read(f"{work_dir}/{FILE_FRAME_LABEL}")
    ...
    if auto_primitive_arg:
        ...
        if not (len(prim_struct) < len(struct)) and any(np.array(build_cell) > 1):
            ans = ask_user_action(
                "Primitive cell detection may be inaccurate. Try again with lower value of `auto_primitive_cell.symprec`, or continue anyway? (y/n):"
            )
            if ans in ["yes", "y"]:
                pass
            elif ans in ["no", "n"]:
    ...
    supercell_list = phonon.supercells_with_displacements
    ...
    for idx, struct_ph in enumerate(supercell_list):
        ...
        write_extxyz(f"{work_dir}/displace_{idx:03d}/{FILE_FRAME_unLABEL}", struct)
    ...
    phonon.save(
        filename=f"{work_dir}/{FILE_PHONOPYwFORCES}",
...
def compute_phonon(pdict, mdict):
    ...
    ### work_dir
    struct_startdir = mk_struct_dir(pdict)
    work_dir = f"{struct_startdir}/{DIR_PHONON}"
    make_dir(work_dir, backup=False)

    ### Copy phonopy_param.yml from the previous stage
    task_dirs = read_yaml(f"{struct_startdir}/{DIR_SUPERCELL}/task_dirs.yml")
    phonopy_files = [
        move_file(
            f"{p}/{FILE_PHONOPYwFORCES}",
            f"{p}/{FILE_PHONOPYwFORCES}".replace(DIR_SUPERCELL, DIR_PHONON),
        )
        for p in task_dirs
    ]
...
class WorkflowPhonon(Workflow):
    ...
    def __init__(self, param_file: str, machine_file: str):
        super().__init__(param_file, machine_file, SCHEMA_PHONON)
        self.stage_map = {
