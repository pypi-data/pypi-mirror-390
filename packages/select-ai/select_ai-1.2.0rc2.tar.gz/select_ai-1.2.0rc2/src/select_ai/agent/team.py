...
class AsyncTeam(BaseTeam):
    ...
    async def _get_description(team_name: str) -> Union[str, None]:
        async with async_cursor() as cr:
            await cr.execute(GET_USER_AI_AGENT_TEAM, team_name=team_name.upper())
    ...
    @classmethod
    async def list(cls, team_name_pattern: Optional[str] = ".*") -> AsyncGenerator["AsyncTeam", None]:
        """
        List AI Agent Teams
    ...
    async def set_attribute(self, attribute_name: str, attribute_value: Any) -> None:
        """
        Set the attribute of the AI Agent team specified by
        `attribute_name` and `attribute_value`.
        """
