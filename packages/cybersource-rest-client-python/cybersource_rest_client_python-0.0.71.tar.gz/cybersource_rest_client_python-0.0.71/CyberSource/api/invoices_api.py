...
class InvoicesApi(object):
    ...
    def perform_publish_action(self, id, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.perform_publish_action(id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.perform_publish_action_with_http_info(id, **kwargs)
        else:
            (data) = self.perform_publish_action_with_http_info(id, **kwargs)
            return data

    def perform_publish_action_with_http_info(self, id, **kwargs):
        """
        Publish an Invoice
        You can publish an invoice in DRAFT status. After invoking this method, the invoice status is changed to CREATED.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.perform_publish_action_with_http_info(id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method perform_publish_action" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        ...
        return self.api_client.call_api(f'/invoicing/v2/invoices/{id}/publication', 'POST',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
                                        post_params=form_params,
                                        files=local_var_files,
                                        response_type='InvoicingV2InvoicesPublish200Response',
                                        auth_settings=auth_settings,
                                        callback=params.get('callback'),
                                        _return_http_data_only=params.get('_return_http_data_only'),
                                        _preload_content=params.get('_preload_content', True),
