...
- Support for internal library logs via get_library_logger(), shown only when level="ALL"
...
Note: level="ALL" enables internal library logs (via get_library_logger()) at DEBUG level.
Get Loggers
get_logger() -> DevopsLogger: Get the main logger for scripts/main use.
get_library_logger() -> Any: Get a wrapper for internal library logs (hidden unless level="ALL").
Core Logging
...
Internal logs (via get_library_logger()) are only shown if init_logger(level="ALL").
"""
    )
def _is_tty() -> bool:
    try:
...
class DevopsLogger:
    ...
    def __init__(
        ...
        # Handle special "ALL" level for internal logs
...
def get_library_logger():
    ...
    Logs from this are hidden unless init_logger(level="ALL").
    """
