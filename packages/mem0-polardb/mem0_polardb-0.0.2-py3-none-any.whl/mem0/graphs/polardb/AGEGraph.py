...
class AGEGraph():
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def __init__(
        self, graph_name: str, conf: Dict[str, Any], create: bool = True, dimension: int = 1536
    ) -> None:
            ...
            if data is None:
                if create:
                    create_statement = """
                        ...
                        ALTER table {}._ag_label_vertex alter column vec TYPE vector({});
                    """.format(graph_name, graph_name, dimension)
    ...
    def _check_label_vertex(self, label: str, graph_name: str, create: bool = True) -> None:
        """
        ...
        with self._get_cursor() as curs:
            # check if graph with name graph_name exists
            label_relation_query = (
                """select relation
                    from ag_catalog.ag_label 
                    where graph =  (SELECT graphid from ag_catalog.ag_graph where name = '{}')
                        and name = '{}'
                        and kind = 'v'
                """.format(
                    graph_name, _label
            ...
            curs.execute(label_relation_query)
            data = curs.fetchone()
    ...
    def _check_label_edge(self, label: str, graph_name: str, create: bool = True) -> None:
        """
        Check if a edgelabel exists in the graph.
        """
        _label = label.replace('"', "") 
        with self._get_cursor() as curs:
            # check if graph with name graph_name exists
            ...
            index_query = """
                CREATE UNIQUE INDEX IF NOT EXISTS "{}_idx" ON {}(start_id, end_id);
            """.format(
                _label, data.relation
            )
    ...
    def _get_cursor(self) -> psycopg2.extras.NamedTupleCursor:
        ...
        cursor.execute("""select * from ag_catalog.get_cypher_keywords() limit 0; """)
    ...
    @staticmethod
    def _record_to_dict_simple(record: NamedTuple) -> Dict[str, Any]:
        """
    ...
    def _query(self, query: str, params: dict = {}) -> List[Dict[str, Any]]:
        """
        ...
        with self._get_cursor() as curs:
            try:
                _query = query.format_map(params)
                curs.execute(_query)
                self.connection.commit()
            except psycopg2.Error as e:
            ...
            try:
                ...
                if data is None:
                    ...
                    result = [self._record_to_dict_simple(d) for d in data]
            except psycopg2.Error as e:
                # maybe no result to fetch
                result = []
    ...
    def query(self, query: str, params: dict = {}) -> List[Dict[str, Any]]:
        ...
        _query = query.format_map(params)
        wrapped_query = self._wrap_query(_query, self.graph_name)

        # execute the query, rolling back on an error
        with self._get_cursor() as curs:
            try:
                curs.execute(wrapped_query, params)
                self.connection.commit()
            except psycopg2.Error as e:
                self.connection.rollback()
                raise AGEQueryException(
