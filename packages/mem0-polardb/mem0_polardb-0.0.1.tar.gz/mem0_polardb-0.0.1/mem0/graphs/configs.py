...
class PolarDBConfig(BaseModel):
    ...
    password: Optional[str] = Field(None, description="Password for the graph database")
    database: Optional[str] = Field(None, description="Database for the graph database")
    port: Optional[int] = Field(None, description="port number for the graph database")
    graphname: Optional[str] = Field(None, description="port number for the graph database")

    @model_validator(mode="before")
    def check_host_port_or_path(cls, values):
        url, username, password, database, port, graphname = (
            values.get("url"),
            values.get("username"),
            values.get("password"),
            values.get("database"),
            values.get("port"),
            values.get("graphname"),
        )
        if not url or not username or not password:
            raise ValueError("Please provide 'url', 'username' and 'password'.")
        
        if database is None:
            values.set("database", "mem0")
            
        if port is None:
            values.set("port", 5432);
            
        if graphname is None:
            values.set("graphname", "memory");
...
class GraphStoreConfig(BaseModel):
    provider: str = Field(
        description="Provider of the data store (e.g., 'neo4j', 'memgraph', 'neptune', 'kuzu', 'polardb')",
        default="neo4j",
    )
    config: Union[Neo4jConfig, MemgraphConfig, NeptuneConfig, KuzuConfig, PolarDBConfig] = Field(
    ...
    def validate_config(cls, v, values):
        ...
        elif provider == "polardb":
            return PolarDBConfig(**v.model_dump())
        else:
