...
def make_parser() -> argparse.ArgumentParser:
    ...
    s2.add_argument("subject", choices=SUBJECTS, type=str.upper)
    ...
    s3.add_argument("--subject", choices=SUBJECTS, type=str.upper, help="Limit to a single subject")
    ...
    s4.add_argument("-s", "--subject", choices=SUBJECTS, type=str.upper, help="Subject name")
    ...
    s5.add_argument("--subject", choices=SUBJECTS, type=str.upper, help="Show details for one subject")
    ...
    # version
    s7 = sub.add_parser("ver", help="Show version; optionally check or upgrade")
    s7.add_argument("--check", action="store_true", help="Check latest version on PyPI")
    s7.add_argument("--upgrade", action="store_true", help="Upgrade to latest via pip")
    s7.set_defaults(func=cmd_ver)
    return p
...
def _print_help() -> int:
    print(
        ...
        "  gc subjects\n"
        "  gc info --subject DSA\n"
        "  gc dbms            # list DBMS codes (shorthand)\n"
        "  gc dbms ddl        # fetch best match for 'ddl' in DBMS (shorthand)\n"
        "  gc DSA/v3/B6_BSTops.cpp  # fetch by repo path (shorthand)\n"
        "  gc search bst --subject DSA\n"
        "  gc get -s DSA -m bst --copy   # copy best match to clipboard\n"
        "  gc get DSA/v3/B6_BSTops.cpp --to ./out\n\n"
        "Tips:\n"
        "  - Subjects are case-insensitive (e.g., dbms == DBMS).\n"
        "  - Use --all-versions to include older implementations.\n"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    subcmds = {"subjects", "list", "search", "get", "info", "help"}
    ...
    if new_argv is None and raw and not raw[0].startswith("-") and raw[0].upper() in SUBJECTS:
        subj = raw[0].upper()
        if len(raw) > 1:
            rest = raw[1:]
