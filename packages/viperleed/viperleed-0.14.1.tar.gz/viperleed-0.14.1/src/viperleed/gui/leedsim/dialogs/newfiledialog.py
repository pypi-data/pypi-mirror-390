"""Module newfiledialog of viperleed.gui.leedsim.dialogs.

Defines the NewFileDialog class, which allows user input of bulk
and surface periodicities and space groups. Multiple structural
domains are supported (all with the same bulk cell).
"""
...
class NewFileDialog(qtw.QDialog):
    ...
    def _compose(self):
        ...
        self._ctrls['add_surface_tab'].setDefaultAction(
            qtw.QAction(
                qtg.QIcon(
                    os.path.join(resources_path('gui/icons'),
