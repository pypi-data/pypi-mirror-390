...
window.addEventListener("load", function() {
	...
	let $light = document.querySelector("input[value='light']");
	$light.addEventListener("change", function(ev) {
		if (event.target.checked) {
			setTheme("light");
		}
	});
	let $dark = document.querySelector("input[value='dark']");
	$dark.addEventListener("change", function(ev) {
		if (event.target.checked) {
			setTheme("dark");
		}
	});
	document.querySelector("input[value='auto']").addEventListener("change", function(ev) {
		if (event.target.checked) {
			setTheme("");
		}
	});
...
navigator.mediaSession.setActionHandler("seekbackward", (param) => {
	var $player = document.querySelector("#player");
	var newtime = $player.currentTime - param.seekOffset;
...
navigator.mediaSession.setActionHandler("seekto", (param) => {
	var $player = document.querySelector("#player");
	$player.currentTime = Math.min($player.duration, param.seekTime);
});
