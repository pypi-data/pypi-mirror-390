...
from biosimulators_utils.utils.core import (
    validate_str_value,
    parse_value,
...
def exec_sedml_docs_in_combine_archive(archive_filename, out_dir, config=None):
    ...
    results, log = exec_sedml_docs_in_archive(
        exec_sed_doc,
        archive_filename,
        out_dir,
    ...
    try:
        ...
        exec_biosimularium(
            working_dir=temp_archive_root, output_dir=out_dir, use_json=True
        )
...
def exec_sed_task(task, variables, preprocessed_task=None, log=None, config=None):
    ...
    if preprocessed_task is None:
        ...
        sed_model_changes = list(
            filter(
                lambda change: change.target
                in preprocessed_task["sed_smoldyn_simulation_change_map"],
                sed_model_changes,
            )
        )
    ...
    for change in sed_model_changes:
        ...
        if (
            smoldyn_change is None
            or smoldyn_change.execution != SimulationChangeExecution.simulation
        ):
            raise NotImplementedError(
                "Target `{}` can only be changed during simulation preprocessing.".format(
                    change.target
                )
            )
        apply_change_to_smoldyn_simulation(smoldyn_simulation, change, smoldyn_change)
    ...
    smoldyn_simulation.run(
        **smoldyn_run_args, overwrite=True, display=False, quit_at_end=False
    )
...
def preprocess_sed_task(task, variables, config=None):
    ...
    if config.VALIDATE_SEDML:
        ...
        raise_errors_warnings(
            validation.validate_model_language(
                sed_model.language, ModelLanguage.Smoldyn
            ),
        ...
        raise_errors_warnings(
            validation.validate_model_change_types(
                sed_model.changes, (ModelAttributeChange,)
            ),
        ...
        raise_errors_warnings(
            validation.validate_simulation_type(
                sed_simulation, (UniformTimeCourseSimulation,)
            ),
            error_summary="{} `{}` is not supported.".format(
                sed_simulation.__class__.__name__, sed_simulation.id
            ),
        )
        raise_errors_warnings(
        ...
        raise_errors_warnings(
            *validation.validate_data_generator_variables(variables),
            error_summary="Data generator variables for task `{}` are invalid.".format(
                task.id
            ),
        )
    ...
    write_smoldyn_simulation_configuration(
        simulation_configuration, smoldyn_configuration_filename
    )
...
def validate_model_change(sed_model_change):
    ...
    #     execution = SimulationChangeExecution.preprocessing
        ...
        raise NotImplementedError(
            "Target `{}` is not supported.".format(sed_model_change.target)
        )
...
def apply_change_to_smoldyn_simulation_configuration(
...
def get_smoldyn_instance_attr_or_run_algorithm_parameter_arg(
    sed_algorithm_parameter_change,
):
    """Get the Smoldyn representation of a SED uniform time course simulation
    ...
    """
    parameter_props = KISAO_ALGORITHM_PARAMETERS_MAP.get(
        sed_algorithm_parameter_change.kisao_id, None
    )
    if parameter_props:
        if not validate_str_value(
            sed_algorithm_parameter_change.new_value, parameter_props["data_type"]
        ):
            msg = "{} ({}) must be a {}, not `{}`.".format(
                parameter_props["name"],
                sed_algorithm_parameter_change.kisao_id,
                parameter_props["data_type"].name,
                sed_algorithm_parameter_change.new_value,
            )
            raise ValueError(msg)
...
def validate_variables(variables):
    ...
    for variable in variables:
        if variable.symbol:
            ...
            output_command, _, output_args = re.sub(
                r" +", " ", variable.target
            ).partition(" ")

            if output_command in [
                "molcount",
                "molcountinbox",
                "molcountincmpt",
                "molcountincmpt2",
...
def add_smoldyn_output_files_for_sed_variables(
    configuration_dirname, variables, variable_output_cmd_map, smoldyn_simulation
):
    ...
    for command, include_header, _, _ in output_cmds:
        add_smoldyn_output_file_for_output(
...
def add_smoldyn_output_file_for_output(
    ...
    """
    smoldyn_output_files[smoldyn_output_command] = add_smoldyn_output_file(
        configuration_dirname, smoldyn_simulation
    )
...
def get_variable_results(
    number_of_steps, variables, variable_output_cmd_map, smoldyn_output_files
):
    """Get the result of each SED variable
    ...
    for variable in variables:
        output_command_args, _, shape, results_slicer = variable_output_cmd_map[
            (variable.target, variable.symbol)
        ]
        variable_result = get_smoldyn_output(
            output_command_args, True, shape, smoldyn_output_files, smoldyn_results
        )
        if results_slicer:
        ...
        if variable_result is None:
            missing_variables.append(
                "{}: {}: {}".format(variable.id, "target", variable.target)
            )
        else:
...
def get_smoldyn_output(
    ...
    if smoldyn_output_command not in smoldyn_results:
        ...
        if three_d_shape:
            ...
            smoldyn_results[smoldyn_output_command] = numpy.array(data_list).transpose(
                (0, 2, 1)
            )

        else:
            smoldyn_results[smoldyn_output_command] = pandas.read_csv(
                smoldyn_output_file.filename, sep=" "
            )
