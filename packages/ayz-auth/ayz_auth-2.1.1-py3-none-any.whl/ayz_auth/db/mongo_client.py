...
class MongoClient:
    ...
    async def _get_client(self) -> Optional[AsyncIOMotorDatabase]:
        """
    ...
    async def get_user(self, stytch_member_id: str) -> Optional[Dict[str, Any]]:
        """
        Get user data by Stytch member ID (DEPRECATED - uses stale field).

        DEPRECATED: This method queries users.stytch_member_id which may be stale.
        Use get_user_by_membership() instead for accurate lookups via user_organization_memberships.
    ...
    async def get_user_by_membership(
        ...
        This is the correct way to lookup users, as it uses the active membership
        record which contains the current stytch_member_id from Stytch.
        ...
        Args:
            stytch_member_id: Stytch member identifier from the authenticated token
            stytch_org_id: Optional Stytch organization ID to scope the lookup
        ...
        try:
            ...
            # Step 1: Find active membership by stytch_member_id
            membership_query = {
            ...
            if not membership:
                logger.debug(
                    f"No active membership found for stytch_member_id: {stytch_member_id}"
            ...
            if not user_id:
                logger.warning(
                    f"Membership found but missing user_id for stytch_member_id: {stytch_member_id}"
                )
                return None

            # Step 2: Lookup user by _id
            user = await db.users.find_one({"_id": user_id})

            if user:
                logger.debug(
                    f"Found user via membership lookup for stytch_member_id: {stytch_member_id}"
                )
            else:
                logger.warning(
                    f"Membership found but user not found for user_id: {user_id}"
                )
            ...
            logger.warning(
                f"Failed to fetch user via membership from MongoDB: {str(e)}"
            )
            return None
