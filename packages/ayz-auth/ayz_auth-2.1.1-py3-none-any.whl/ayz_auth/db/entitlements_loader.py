...
class EntitlementsLoader:
    ...
    async def load_user_context(
        ...
        try:
            ...
            # Load user doc from MongoDB using the correct lookup via user_organization_memberships
    ...
    async def _get_first_team_in_org(
        ...
        """
        try:
            # Get user document using correct lookup via user_organization_memberships
            user_doc = await mongo_client.get_user_by_membership(
                stytch_member_id, stytch_org_id
            )
            if not user_doc:
