...
class TestMongoClient:
    ...
    # =====================================================================
    # get_user_by_membership() Tests
    # =====================================================================

    @pytest.mark.asyncio
    async def test_get_user_by_membership_success(
        self, mock_mongo_client, mock_settings_with_mongodb
    ):
        """Test successful user retrieval via user_organization_memberships lookup."""
        membership_data = {
            "_id": "membership_mongo_id",
            "stytch_member_id": "member-live-xyz789",
        ...
        mock_memberships_collection.find_one = AsyncMock(return_value=membership_data)
        ...
        with patch.object(mock_mongo_client, "_get_client", return_value=mock_db):
            result = await mock_mongo_client.get_user_by_membership(
                "member-live-xyz789"
            )
            ...
            mock_memberships_collection.find_one.assert_called_once_with(
                {"stytch_member_id": "member-live-xyz789", "status": "active"}
            )
            # Verify user query
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_membership_with_org_scope(
        self, mock_mongo_client, mock_settings_with_mongodb
    ):
        """Test user retrieval with organization scoping."""
        org_data = {
            "_id": "org_mongo_id",
            "stytch_org_id": "organization-live-abc123",
        }
        ...
        with patch.object(mock_mongo_client, "_get_client", return_value=mock_db):
            ...
            assert result == user_data
            # Verify organization query was made
            mock_orgs_collection.find_one.assert_called_once()
            # Verify membership query included organization_id
            mock_memberships_collection.find_one.assert_called_once_with(
                {
                    "stytch_member_id": "member-live-xyz789",
                    "status": "active",
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_membership_no_membership_found(
        self, mock_mongo_client, mock_settings_with_mongodb
    ):
        """Test when no active membership exists."""
        mock_db = MagicMock()
        mock_memberships_collection = AsyncMock()
        ...
        with patch.object(mock_mongo_client, "_get_client", return_value=mock_db):
            result = await mock_mongo_client.get_user_by_membership(
                "member-live-nonexistent"
            )

            assert result is None

    @pytest.mark.asyncio
    async def test_get_user_by_membership_missing_user_id(
        self, mock_mongo_client, mock_settings_with_mongodb
    ):
        """Test when membership exists but has no user_id."""
        membership_data = {
            "_id": "membership_mongo_id",
            "stytch_member_id": "member-live-xyz789",
            # user_id is missing
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_membership_user_not_found(
        self, mock_mongo_client, mock_settings_with_mongodb
    ):
        """Test when membership exists but user document doesn't."""
        membership_data = {
            "_id": "membership_mongo_id",
            "stytch_member_id": "member-live-xyz789",
            "user_id": "user_mongo_id",
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_membership_mongodb_not_configured(
        self, mock_mongo_client, mock_settings_without_mongodb
    ):
        """Test get_user_by_membership returns None when MongoDB not configured."""
        result = await mock_mongo_client.get_user_by_membership("member-live-xyz789")
        assert result is None

    @pytest.mark.asyncio
    async def test_get_user_by_membership_handles_exception(
        self, mock_mongo_client, mock_settings_with_mongodb
    ):
        """Test graceful handling of exceptions during membership lookup."""
        mock_db = MagicMock()
        mock_memberships_collection = AsyncMock()
