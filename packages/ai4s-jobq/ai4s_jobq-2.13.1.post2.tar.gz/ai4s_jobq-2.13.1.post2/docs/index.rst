...
The ``ai4s.jobq`` package enables multiple users to push work items to an `Azure Queue <https://azure.microsoft.com/en-us/products/storage/queues/>`_, while one or more workers pull and process tasks asynchronously. This approach is useful in scenarios where:
