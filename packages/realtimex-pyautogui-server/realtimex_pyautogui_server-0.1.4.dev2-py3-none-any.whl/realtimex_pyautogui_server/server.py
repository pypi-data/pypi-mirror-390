...
def _release_modifiers() -> None:
    ...
    for key in MODIFIER_KEYS:
        try:
            ...
            continue
    # Brief delay to ensure all modifiers are processed
    time.sleep(0.03)
...
def press_key(
    ...
    interval: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Delay between key down and key up. Higher values help prevent stuck keys.",
    ),
) -> Dict[str, str]:
    """Press a single key."""
    try:
...
def type_text(
    ...
    interval: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Delay between characters. Higher values (0.15-0.2) help prevent stuck shift keys.",
    ),
) -> Dict[str, str]:
