...
def press_key(
    ...
    interval: float = Field(
        ...
        ge=0.5,
        le=1.0,
        description="Delay between key down and key up in seconds. MUST be 0.5 or higher to prevent stuck keys. Default 0.5 is recommended.",
    ),
) -> Dict[str, str]:
    """Press a single key."""
    try:
...
def type_text(
    ...
    interval: float = Field(
        ...
        ge=0.5,
        le=1.0,
        description="Delay between characters in seconds. MUST be 0.5 or higher to prevent stuck shift keys. Default 0.5 is recommended.",
    ),
) -> Dict[str, str]:
    ...
    try:
        ...
        # Hardcode interval to 2.0 seconds to debug stuck key issues
        pyautogui.typewrite(text, interval=2.0)
