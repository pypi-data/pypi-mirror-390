...
def split_space(
    ...
    Однако это поведение уже реализовано по умолчанию в Segmenter.segment(),
    поэтому функция избыточна и никогда не вызывается.

    Сохранена для 100% совместимости кода с оригинальным razdel.
    """
    if split.delimiter:
        return SPLIT
