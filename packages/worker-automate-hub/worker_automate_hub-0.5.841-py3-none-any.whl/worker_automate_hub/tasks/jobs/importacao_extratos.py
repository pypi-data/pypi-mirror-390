...
DESTINO_BASE = fr"Z:\Nexera\Extrato\{ano_atual}"
...
async def importacao_extratos(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    try:
        ...
        EXT_PERMITIDA = ".ret"                                # filtrar só arquivos .ret
        TEXTO_ALVO = "SIM REDE DE POSTOS LTDA"                # texto a procurar no conteúdo
        ...
        log("Parâmetros -> pasta: %s | ignorar se contém: %s | extensão: %s | texto alvo: %s",
            PASTA, IGNORAR_SE_CONTEM, EXT_PERMITIDA, TEXTO_ALVO)
        ...
        for entry in os.scandir(PASTA):
            ...
            # 1️⃣ Ignora se o nome contiver qualquer padrão da lista IGNORAR_SE_CONTEM
            padrao_batido = next((p for p in IGNORAR_SE_CONTEM if p.upper() in upper_name), None)
            if padrao_batido is not None:
                ignorados_nome += 1
                log("Ignorado por nome: %s (contém '%s')", nome, padrao_batido)
                continue

            # 2️⃣ Considera apenas arquivos com extensão .ret (case-insensitive)
            if not lower_name.endswith(EXT_PERMITIDA.lower()):
                nao_ret += 1
                continue

            avaliados += 1

            # 3️⃣ Procura o texto alvo dentro do arquivo (case-insensitive)
            ...
            # 4️⃣ Se encontrou o texto alvo, adiciona à lista de selecionados
            if encontrado:
                selecionados.append({
                    "arquivo": nome,
                    "caminho": caminho,
        ...
        log("Avaliados (.ret): %s | Ignorados por nome: %s | Ignorados por !.ret: %s | Lidos: %s | Erros leitura: %s",
            avaliados, ignorados_nome, nao_ret, lidos, erros)

        # ======== VERIFICA RESULTADO ========
        if not selecionados:
            if ignorados_nome > 0:
                msg = f"Arquivos foram ignorados por nome conforme os padrões definidos: {', '.join(IGNORAR_SE_CONTEM)}"
            else:
                msg = f"Nenhum arquivo .ret contendo '{TEXTO_ALVO}' encontrado."
        ...
        log("[green]Credenciais carregadas[/green]. Tarefa recebida | empresa fixa = %s", EMPRESA)
        ...
        try:
            type_text_into_field("Conciliador Bancario 2.0", app["TFrmMenuPrincipal"]["Edit"], True, "50")
        ...
        for idx, sel in enumerate(selecionados, 1):
            log("[bold cyan]Iniciando importação (%s/%s)[/bold cyan]: %s", idx, len(selecionados), sel["arquivo"])
            ...
            try:
                ...
                app_imp = Application(backend="win32").connect(class_name="TFrmImportarArquivoConciliadorBancario2", timeout=30)
            ...
            try:
                ...
                try:
                    app_err = Application(backend="win32").connect(title="Erro", found_index=0)
                ...
                try:
                    ...
                    destino_arquivo = os.path.join(DESTINO_BASE, sel["arquivo"])
                    shutil.move(sel["caminho"], destino_arquivo)
                    movidos.append(destino_arquivo)
                    importados.append(sel["arquivo"])
                    log("[green]Arquivo movido[/green]: %s -> %s", sel["caminho"], destino_arquivo)
                except Exception as e:
                    msg = f"Falha ao mover '{sel['arquivo']}': {e}"
                    log(f"[red]{msg}[/red]")
                    falhas.append({"arquivo": sel["arquivo"], "motivo": msg})
                    continue
        ...
        if importados:
            ...
            retorno_payload = {
                ...
                "importados_count": 0,
                "falhas": falhas,
                "mensagem": "Nenhum arquivo foi importado/movido com sucesso."
            }
            retorno_str = json.dumps(retorno_payload, ensure_ascii=False, indent=2)
            return RpaRetornoProcessoDTO(
                sucesso=False,
