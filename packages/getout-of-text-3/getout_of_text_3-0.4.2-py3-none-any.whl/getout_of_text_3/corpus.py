...
class LegalCorpus:
    ...
    def _detect_corpus_type(self, dir_of_text_files):
        """Detect corpus type by analyzing folder naming patterns.
        
        Returns:
            str: 'coca', 'glowbe', or 'generic'
        """
        folders = [f for f in os.listdir(dir_of_text_files) if f.startswith('text_')]
        if not folders:
            return 'generic'
        
        # Sample multiple folders to determine pattern
        sample = folders[0]
        parts = sample.split('_')
        
        # COCA pattern: text_GENRE_YEAR (e.g., text_acad_2016)
        # Check if last part is a 4-digit year
        if len(parts) == 3 and parts[2].isdigit() and len(parts[2]) == 4:
            return 'coca'
        
        # COCA web/blog pattern: text_GENRE_ID (e.g., text_web_ywv, text_blog_wih)
        # Check if it's a known COCA genre with 3-letter ID
        if len(parts) == 3 and len(parts[2]) == 3:
            known_coca_genres = {'acad', 'blog', 'fic', 'mag', 'news', 'spok', 'tvm', 'web'}
            if parts[1] in known_coca_genres:
                return 'coca'
        
        # GloWbE pattern: text_COUNTRY_SECTION_* (e.g., text_jm_bsh, text_us_genl_ksl)
        # Check if we have country code (2 letters) as second component
    ...
    def _read_coca_structure(self, dir_of_text_files, show_progress, show_file_progress, log_every):
        """Parse COCA corpus structure: text_GENRE_YEAR or text_GENRE_ID formats.
        ...
        for genre_folder in genre_iter:
            parts = genre_folder.split('_')
            genre = parts[1] if len(parts) > 1 else 'unknown'
            year_or_id = parts[2] if len(parts) > 2 else 'unknown'
            
            print(f"Processing COCA genre: {genre} ({year_or_id})")
            ...
            for filename in file_iter:
                file_rows = self._parse_corpus_file(os.path.join(genre_path, filename), log_every, genre, 0)
                if file_rows:
                    text_rows.extend(file_rows)
            
            if text_rows:
                if genre not in coca_dict:
                    coca_dict[genre] = {}
                coca_dict[genre][year_or_id] = pd.DataFrame(text_rows)
                print(f"Finished genre: {genre} ({year_or_id}) - {len(text_rows):,} texts")
        
        return coca_dict
    
    def _read_glowbe_structure(self, dir_of_text_files, show_progress, show_file_progress, log_every):
        """Parse GloWbE corpus structure: text_COUNTRY_SECTION_* format.
        
        Returns: {country_section: DataFrame} (flat structure)
        """
        glowbe_dict = {}
        folders = [f for f in os.listdir(dir_of_text_files) if f.startswith('text_')]
        
        folder_iter = tqdm(folders, desc="GloWbE Sections", unit="section") if show_progress else folders
        
        for folder in folder_iter:
            parts = folder.split('_')
            # text_jm_bsh -> country='jm', section='bsh'
            # text_us_genl_ksl -> country='us', section='genl_ksl'
            country = parts[1] if len(parts) > 1 else 'unknown'
            section = '_'.join(parts[2:]) if len(parts) > 2 else 'unknown'
            composite_key = f"{country}_{section}"
            
            print(f"Processing GloWbE: {composite_key}")
            folder_path = os.path.join(dir_of_text_files, folder)
            
            # GloWbE files use 'w_*.txt' naming pattern (e.g., w_hk_b01.txt)
            files = [fn for fn in os.listdir(folder_path) if fn.startswith('w_') and fn.endswith('.txt')]
            file_iter = tqdm(files, desc=f"{composite_key} files", unit="file", leave=False) if (show_file_progress and show_progress) else files
            
            all_text_rows = []
            for filename in file_iter:
                text_rows = self._parse_corpus_file(os.path.join(folder_path, filename), log_every, composite_key, len(all_text_rows))
                all_text_rows.extend(text_rows)
            
            if all_text_rows:
                glowbe_dict[composite_key] = pd.DataFrame(all_text_rows)
                print(f"Finished {composite_key}: {len(all_text_rows)} texts")
        
        return glowbe_dict
    
    def _parse_corpus_file(self, file_path, log_every, label, starting_count):
        """Parse a single corpus text file (works for both COCA and GloWbE).
        
        Returns: List of dicts with 'text_id' and 'text' keys
        """
        text_rows = []
        captured_lines = starting_count
    ...
    def read_corpus(self, dir_of_text_files=None, show_progress=True, show_file_progress=True, log_every=0):
        """Build a structured dictionary from corpus text folders (auto-detects format).
        
        Supports multiple corpus types:
        - COCA: text_GENRE_YEAR → Returns {genre: {year: DataFrame}}
        - GloWbE: text_COUNTRY_SECTION_* → Returns {country_section: DataFrame}
        
        Parameters:
            dir_of_text_files (str|None): Root directory containing subfolders (defaults to self.data_dir)
            show_progress (bool): Show top-level progress bar.
            show_file_progress (bool): Show per-folder file progress bar (requires tqdm).
            log_every (int): If > 0, print a running line-count every N captured lines.
        
        Returns:
            dict: Structure depends on corpus type (see above)
        """
        ...
        corpus_type = self._detect_corpus_type(dir_of_text_files)
