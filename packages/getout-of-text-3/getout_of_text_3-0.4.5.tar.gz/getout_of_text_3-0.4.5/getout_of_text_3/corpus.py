...
class LegalCorpus:
    ...
    def read_corpus(self, dir_of_text_files=None, corpus_name='coca', show_progress=True, show_file_progress=True, log_every=0):
        ...
        Structure returned depends on corpus_name:
        - 'coca'/'diy': {genre -> (year OR file_num) -> DataFrame(['text_id', 'text'])}
        - 'glowbe': {country_code -> DataFrame(['text_id', 'text'])}
    ...
    def _read_glowbe_structure(self, dir_of_text_files, show_progress, show_file_progress, log_every):
        ...
        Aggregates by country code: {country_code: DataFrame}
        """
        glowbe_dict = {}
        ...
        for folder in folder_iter:
            ...
            # GloWbE files use w_*.txt naming
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                        for line in f:
                            ...
                            captured_lines += 1
                            if log_every and captured_lines % log_every == 0:
                                print(f"  {country}: {captured_lines} lines captured so far...")
                except Exception as e:
                    print(f"  ⚠️ Failed reading {file_path}: {e}")
                
                # Aggregate all texts for this country
                if text_rows:
                    if country not in glowbe_dict:
                        glowbe_dict[country] = []
                    glowbe_dict[country].extend(text_rows)
        
        # Convert aggregated lists to DataFrames
        for country in glowbe_dict:
            glowbe_dict[country] = pd.DataFrame(glowbe_dict[country])
            print(f"Finished country: {country} ({len(glowbe_dict[country]):,} texts)")
