...
class Parser(ABC):
    ...
    _default_ifstates = {
        'parameters': {
            ...
            'ignore': {
                'ipaddr_builtin': [
                    # ignore proto kernel_ll
                    {'proto': 3},
                    # it was observed that not always proto kernel_ll is set,
                    # so this is an ulgy hack by looking also for mod. EUI-64
                    # addresses :-(
                    {'prefix': '__MATCH_MOD_EU64_LLA__'}
                ],
                'ipaddr_dynamic': True,
                'ifname_builtin': [
                    r'^br-[\da-f]{12}',
