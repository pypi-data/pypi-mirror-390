...
@click.command("run_blast")
@click.argument("reads_bam", type=click.Path(exists=True))
...
@click.option(
    ...
    show_default=True,
    type=int,
    help="Number of threads to use during the blast search.",
)
@click.option(
    "-p",
    "--prefix",
...
def run_blast(
    reads: click.Path,
    fasta: click.Path,
    blastn_path: click.Path,
    prefix: str,
    threads: int,
):
    """
    Run BLAST to search for adapters in a given FASTA file in a set
    of reads.
    """
    if not blastn_path:
        ...
        if blastn_path is None:
            raise FileNotFoundError(
                "Error: blastn not found in PATH! Check your environment or pass the full path with --blastn_path"
            )
