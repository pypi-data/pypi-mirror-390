...
class UPACParser(Parser):
    ...
    def parse_digital_data_old(self, in_data) -> dict:
        ...
        available_channels = np.unique(channels)
        packets_per_channel = num_packet // len(available_channels)
        for idx, chan in enumerate(available_channels):
            curr_event["data"][chan] = (
                event_matrix[
                    1:packet_size,
                    idx * packets_per_channel : ((idx + 1) * packets_per_channel),
                ]
            ).flatten("F")
            curr_event["window_labels"][chan] = list(
                (
                    raw_event_matrix[
                        0, idx * packets_per_channel : ((idx + 1) * packets_per_channel)
                    ]
