...
_C4='openfmea_content_based_keys'
_C3='openfmea_include_cme_unlinked'
_C2='openfmea_include_cme_orphans'
_C1='openfmea_expand_compoundfields'
_C0='remote_procedure_call_api_endpoint'
_B_='remote_procedure_call_api_key'
_Bz='remote_procedure_call_enable'
_By='remote_resouce_fetch_enable'
_Bx='sbdl_disable_pretty_print'
_Bw='sbdl_disable_pretty_print_environ'
_Bv='manual_command'
_Bu='add_description_bsci_property'
_Bt='elision_placeholder'
_Bs='reference_maximum_length'
_Br='usecase_diagram_style'
_Bq='state_diagram_descriptions'
_Bp='state_diagram_style'
_Bo='uml_text_block_wrap_element'
_Bn='process_diagram_descriptions'
_Bm='function_diagram_aspect'
_Bl='function_diagram_descriptions'
_Bk='aspect_diagram_descriptions'
_Bj='aspect_diagram_element'
_Bi='aspect_diagram_aspect'
_Bh='requirement_diagram_legend'
_Bg='requirement_diagram_style'
_Bf='requirement_diagram_title_node_always'
_Be='requirement_diagram_toplevel_depth'
_Bd='requirement_diagram_aspect_groups'
_Bc='network_diagram_hide_types'
_Bb='network_diagram_dpi'
_Ba='network_diagram_layout'
_BZ='network_diagram_labels'
_BY='network_diagram_descriptions'
_BX='network_diagram_height'
_BW='network_diagram_width'
_BV='parser_allow_load_config'
_BU='parser_allow_custom_directive_import'
_BT='parser_custom_attributes'
_BS='parser_allow_preparse_directives'
_BR='plantuml_generate_footer'
_BQ='plantuml_output_type'
_BP='plantuml_limit_size'
_BO='plantuml_command'
_BN='wsl_auto_correct_paths'
_BM='sbdl_caching_suffix'
_BL='sbdl_caching_enabled'
...
def allow_pretty_print():return not get_config_value(_Bw)in os.environ and not get_config_value(_Bx,number=_A)
...
def fix_wsl_path(path):
	result=path
	if get_config_value(_BN,number=_A):
...
class SBDL_Parser:
	...
	class Parser_Element:
		...
		def shorten_reference(self,reference):
			reference_result=reference;max_length=get_config_value(_Bs,number=_A)
			if max_length!=_B and max_length>0:reference_result=(reference_result[::-1][:max_length]+(reference_result[::-1][max_length:]and get_config_value(_Bt)[::-1]))[::-1]
	...
	def builtin_directive_dict(self):
		...
		def prolog_startup(_,*inp):
			...
			def prolog_element_to_fact(element):
				...
				attr_list.append(prolog_attr(_v,_d+prolog_separator.join(inter_prop)+']',quote=_C))
				def handle_link_list(link_list,link_type):
					...
					attr_list.append(prolog_attr(link_type,_d+prolog_separator.join(inter_list)+']',quote=_C))
	...
	def attribute_string(self,statement_object):
		...
		if get_config_value(_V,number=_A):indent_str='    ';newline_str=(self.Tokens.line_continuation if not get_config_value(_p,number=_A)else'')+_D;spacing_str=''
	...
	def declaration_string(self,declaration_object):
		newline_str=''
		if get_config_value(_V,number=_A):newline_str=(self.Tokens.line_continuation if not get_config_value(_p,number=_A)else'')+_D
...
class SBDL_Semantics:
	...
	def get_language_information_struct(self):
		lang_info_struct={_e:{}};lang_info_struct[_Z]=self.type_information;lang_type_struct=lang_info_struct[_Z];categories=lang_info_struct[_e];skip_properties=[SBDL_Parser.Attributes.pragma,SBDL_Parser.Attributes.reference]
	...
	def calc_bsci(self,statement_text,language):
		bsci_value=.0;sentence_tokens=[_N];clausal_tokens=[_S,';','(',_d];conjunction_tokens=['for','and','nor','but','or','yet','so','either','neither','both','whether','rather','after','as long as','as soon as','before','now','since','till','until','when','whenever','while','within'];exclude_length_threshold=3;processed_statement_text=_I.join([x for x in statement_text.split()if len(x)>exclude_length_threshold or x in conjunction_tokens+clausal_tokens]);sentences=[processed_statement_text]
				...
				if isinstance(parsed_object,SBDL_Using_Statement):print_l('USING+=: {}'.format(parsed_object.data()),debug=_A);handle_using(parsed_object.data(),inherit_data)
					...
					try:
						...
						if not statement_element.identifier()in sbdl_elements:
							if get_config_value(_Bu,number=_A)and statement_element.description():statement_element.set_property(SBDL_Parser.Attributes.description_bsci,SBDL_Semantics.calc_bsci(statement_element.description(),'english'))
...
def make_remote_file_local(file_reference):
	...
	if parsed_file_reference.scheme in['http','https','ftp','file://']and get_config_value(_By,number=_A):
...
def plantuml_writer(file_opener,output_file,content,_):
	...
	if not get_config_value(_AN,number=_A):
		try:
			plantuml_env=os.environ.copy();plantuml_env.pop('LD_LIBRARY_PATH',_B);plantuml_env['PLANTUML_LIMIT_SIZE']=str(get_config_value(_BP));plantuml_command=get_config_value(_BO)
			if isinstance(plantuml_command,str):plantuml_command=shlex.split(plantuml_command)
			plantuml_command_full=plantuml_command+['-t',get_config_value(_BQ),'-p'];debug_out('EXECUTE_PLANTUML: '+str(plantuml_command_full));exec_result=subprocess.run(plantuml_command_full,input=content.encode(),stdout=subprocess.PIPE,stderr=subprocess.STDOUT,env=plantuml_env,check=_C);content_to_output=exec_result.stdout
...
def write_requirement_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	if len(top_level_requirements)==1 and not get_config_value(_Bf):root_requirement=requirements[top_level_requirements[0][A]]
	else:
	...
	def plantuml_requirement_diagram_wrap(content):
		header=plantuml_footer_generator();legend_text='';legend=get_config_value(_Bh)
		if legend!=_B:legend_text='legend\n{legend_content}\nendlegend\n'.format(legend_content=legend)
		plant_uml_prepost='@startwbs\n{style}\n\n{header}\n{content}\n{legend}@endwbs\n';return plant_uml_prepost.format(style=get_config_value(_W)+get_config_value(_Bg),header=header,content=content,legend=legend_text)
...
def write_decomposition_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_decomposition_diagram():
		header=plantuml_footer_generator();title=_y.format(arguments.title)if arguments.title!=''else'';plant_uml_prepost=_z;content='';notes='';note_count=0;interface_count=0;relations='';descriptions=get_config_value(_Bk,number=_A);interface_mem={};handled_elems=[];indentation_prefix=_M
...
def write_element_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_decomposition_diagram():
		...
		def detailed_content(elem_id):
			...
			for prop in elem.properties():props.append('\t{}:\n\t  {}'.format(prop,textwrap.shorten(elem.get_property(prop),width=get_config_value(_Bo,number=_A),placeholder='...')))
		...
		def handle_func(func_id,depth=1):
			...
			def define_aspect(aspect_name):
				...
				if not aspect_name in entities:
					...
					content+='{}{} "{}{}" as {} {}{}\n'.format(_M*depth,get_config_value(_Bm),aspect_name,_H+elements[aspect_name].reference()if arguments.source else'',puml_id(aspect_name),aspect_stereotype,aspect_color)
...
def write_process_graph_output(sbdl_ast,output_file,file_opener,_,print_l):
	errors=0;elements,errors=sbdl_ast.elements(print_l,prune_missing_references=_A);show_descriptions=get_config_value(_Bn,number=_A);first_event=_A;g_process_elem=_B;note_stagger=_C
	def elem_colour_prefix(elem):
		colour=''
		if SBDL_Parser.Attributes.color in elem.properties():colour=f"#{elem.get_property(SBDL_Parser.Attributes.color)} "
		return colour
	...
	def plantuml_process_diagram():
		process_dia_cont='';process_dia_cont+='@startuml\n';process_dia_cont+=get_config_value(_W)+get_config_value(_AU)+_f
...
def write_state_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	elements,errors=sbdl_ast.elements(print_l,prune_missing_references=_A);show_descriptions=get_config_value(_Bq,number=_A)
	def plantuml_state_diagram():
		...
		return plant_uml_prepost.format(style=get_config_value(_W)+get_config_value(_Bp),header=header+title,content=statedefs+content+_D+relations)
...
def write_usecase_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_usecase_diagram():
		...
		for aspect in aspects:
			...
			content+='}\n'
		content+=relations;return plant_uml_prepost.format(style=get_config_value(_W)+get_config_value(_Br),header=header+title,content=content)
...
def write_network_graph_output(sbdl_ast,output_file,_,arguments,print_l):
	...
	elements,errors=sbdl_ast.elements(print_l,prune_missing_references=_A);sbdl_graph=nx.MultiDiGraph();skip_types={x for x in get_config_value(_Bc,number=_C).split(_S)};node_name_sep=_m;node_text_wrap_width=25
	def node_name(elem_id):
		...
		if get_config_value(_BY,number=_A):node_text+=_f+_D.join(textwrap.wrap(elements[elem_id].description(),width=node_text_wrap_width))
...
class OpenFMEA:
	...
	def generate_sbdl_table_from_ast(self,sbdl_ast,id_in_body=_C,source_in_body=_C,print_l=print_null,multi_field_separator=_B):
		...
		def generate_table(element,row=_B,path=_B,add_single=_C):
			...
			def add_row():
				...
				rows_to_add=[row];expand_attrs=get_config_value(_C1)
	...
	def get_id_from_string(self,string_content,raw=_C):
		...
		if string_content and len(string_content)>2 and string_content[0]==_d:
			level=1;i=1
			while i<len(string_content)and level>0:
				if string_content[i]==_d:level+=1
	...
	def elements_from_table(self,sbdl_table,_):
		multi_field_separator=get_config_value(_AW);elems_to_consider={_c:SBDL_Parser.Types.aspect,_Y:SBDL_Parser.Types.requirement,_L:SBDL_Parser.Types.mode,_a:SBDL_Parser.Types.effect,_R:SBDL_Parser.Types.cause,_o:SBDL_Parser.Types.current_control,_g:SBDL_Parser.Types.current_detection,_A0:SBDL_Parser.Types.action_control,_h:SBDL_Parser.Types.action_detection};elem_rating={_a:_t,_R:_u,_L:_s};elem_rating_post={_R:_A7,_L:_A6};elem_link={_R:[_L,_o,_A0,_g,_h],_L:[_c,_a,_g,_Y,_h,_R],_Y:[_c,_L],_a:[_L],_o:[_R],_g:[_L],_A0:[_R],_h:[_L]};elem_present_every_row={_g,_h};replace_str=SBDL_Parser.Tokens.replacement_string_default;elements={};id_lookup={}
		if not get_config_value(_C7,number=_A):elem_link[_L].remove(_R);elem_link[_a].remove(_L)
...
def write_template_fill(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	if get_config_value(_C6,number=_A):openfmea,errors_fmea=OpenFMEA.generate_sbdl_table_from_ast(sbdl_ast,id_in_body=arguments.identifier,source_in_body=arguments.source,print_l=print_l);errors+=errors_fmea;template_context[_AD]=openfmea
...
def affinity_graph():
	...
	for category in lang_info_struct[_e]:
		graph_output+='frame "{}" {{\n'.format(category)
		for e_type in lang_info_struct[_e][category][_Z]:
...
def get_cache_file_path(input_file):
	cache_file_path=_B
	if input_file!=SBDL_Parser.Tokens.stdio_name:cache_file_path=f"{input_file}{get_config_value(_BM)}"
...
def show_manual(_):
	...
	sbdl_info_struct=SBDL_Semantics.get_language_information_struct();sbdl_properties=sbdl_info_struct[_v];sbdl_types=sbdl_info_struct[_Z];sbdl_directives=sbdl_info_struct[_Ai]
	...
	try:
		manual_command=get_config_value(_Bv)
