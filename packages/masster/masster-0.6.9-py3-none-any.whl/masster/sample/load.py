...
def load(
    ...
    Parameters:
        ...
        sample_idx (int, optional): For WIFF files with multiple samples, specifies which sample to load (0-indexed).
    ...
    Notes:
        The function determines the appropriate internal loading mechanism based on the file extension:
            ...
            - ".wiff": Calls _load_wiff(filename, sample_idx)
...
def _load_wiff(
    ...
    if sample_idx > num_samples - 1:
        raise ValueError(f"sample_idx {sample_idx} out of range. File contains {num_samples} samples.")    
    if num_samples > 1:
        self.logger.warning(
