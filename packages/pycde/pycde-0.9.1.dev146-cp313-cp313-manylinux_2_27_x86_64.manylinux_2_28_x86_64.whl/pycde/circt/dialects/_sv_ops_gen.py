...
class AlwaysOp(_ods_ir.OpView):
  ...
  def __init__(self, events, clocks, *, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_601')) else
      _ods_ir.AttrBuilder.get('anonymous_601')(events, context=_ods_context))
    results = []
    _ods_successors = None
    super().__init__(self.OPERATION_NAME, self._ODS_REGIONS, self._ODS_OPERAND_SEGMENTS, self._ODS_RESULT_SEGMENTS, attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip)
...
class FuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, module_type, *, per_argument_attrs=None, input_locs=None, result_locs=None, verilogName=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_673')) else
      _ods_ir.AttrBuilder.get('anonymous_673')(module_type, context=_ods_context))
...
class SVVerbatimModuleOp(_ods_ir.OpView):
  r"""
  The `sv.verbatim.module` operation represents a module with verbatim content.
  It is a `HWModuleLike` (like `hw.module` and `hw.module.extern`).
  To support verbatim content split accross multiple files, there is an optional
  additional_files field which can reference additional emitted files.
  """
  ...
  def __init__(self, sym_name, module_type, content, output_file, parameters, *, per_port_attrs=None, port_locs=None, additional_files=None, verilogName=None, loc=None, ip=None):
    operands = []
    attributes = {}
    regions = None
    _ods_context = _ods_get_default_loc_context(loc)
...
def verbatim_module(sym_name, module_type, content, output_file, parameters, *, per_port_attrs=None, port_locs=None, additional_files=None, verilog_name=None, loc=None, ip=None) -> SVVerbatimModuleOp:
  return SVVerbatimModuleOp(sym_name=sym_name, module_type=module_type, content=content, output_file=output_file, parameters=parameters, per_port_attrs=per_port_attrs, port_locs=port_locs, additional_files=additional_files, verilogName=verilog_name, loc=loc, ip=ip)
