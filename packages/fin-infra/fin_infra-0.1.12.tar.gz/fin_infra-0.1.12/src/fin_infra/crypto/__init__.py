...
def easy_crypto(
    ...
    **config,
    ...
    if provider_name == "coingecko":
        ...
        raise ValueError(
            f"Unknown crypto data provider: {provider_name}. "
            f"Supported: coingecko"
        )
...
def add_crypto_data(
    ...
    **config
    ...
    if isinstance(provider, str):
        crypto_provider = easy_crypto(provider=provider, **config)
    ...
    async def get_ticker(symbol: str):
        ...
        try:
            ...
            return {
                ...
                "as_of": ticker.as_of.isoformat() if ticker.as_of else datetime.now(timezone.utc).isoformat(),
            }
    ...
    async def get_ohlcv(
        ...
        try:
            ...
            return {
                ...
                "candles": [
                    {
                        "timestamp": c.ts if isinstance(c.ts, int) else int(c.ts.timestamp() * 1000),
                        "open": float(c.open),
                        "high": float(c.high),
                        "low": float(c.low),
                        "close": float(c.close),
            ...
            raise HTTPException(status_code=400, detail=f"Error fetching OHLCV for {symbol}: {str(e)}")
