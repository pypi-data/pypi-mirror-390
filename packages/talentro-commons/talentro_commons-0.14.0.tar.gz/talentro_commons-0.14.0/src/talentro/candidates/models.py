...
class Document(CandidatesOrganizationModel, table=True):
    ...
    candidate_id: Optional[UUID] = Field(foreign_key="candidate.id", ondelete="SET NULL", index=True)
    candidate: "Candidate" = Relationship(back_populates="documents")

    application_id: Optional[UUID] = Field(foreign_key="application.id", ondelete="SET NULL", index=True)
    application: "Candidate" = Relationship(back_populates="documents")
...
class Application(CandidatesOrganizationModel, table=True):
    ...
    source_id: Optional[UUID] = Field(foreign_key="source.id", ondelete="SET NULL", index=True)
