...
///
/// @brief LWE Secret key class
///

class HEAAN_API SecretKey {
public:
    using Coefficients = int *;
    /// @brief Generate random secret key provided with dimension
    explicit SecretKey(const Context &context);
    /// @brief Load secret key from stream
    /// @details The key can be loaded regardless of whether the stream is
    /// saving the full key or its seed only.
    ...
    LevelledVector &getSx();
    /// @brief Get const sx part of secret key
    const LevelledVector &getSx() const;
