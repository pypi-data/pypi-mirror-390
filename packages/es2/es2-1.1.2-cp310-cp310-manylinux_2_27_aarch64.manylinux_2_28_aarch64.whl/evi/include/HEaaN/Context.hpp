...
HEAAN_API Context makeContext(const ParameterPreset &preset,
                              const std::string &param_name, const u64 num_sk,
                              const bool is_ci_param, const bool is_emb_param,
                              const CudaDeviceIds &cuda_device_ids = {});

HEAAN_API Context genMultiSecretContext(const Context &base_context,
                                        const std::string &param_name,
                                        const u64 num_secret);

HEAAN_API std::vector<Context>
genMultiSecretContextVector(const Context &base_context,
                            const u64 log_num_secret,
...
HEAAN_API Context makeContext(const u64 log_degree, const u64 chain_length,
                              const u64 bpsize, const u64 qpsize,
                              const u64 tpsize, const u64 gadget_rank,
                              const u64 rank, const u64 num_secret,
                              const CudaDeviceIds &cuda_device_ids = {});
...
HEAAN_API u64 getNumSecret(const Context &context);
HEAAN_API u64 getLength(const Context &context);
HEAAN_API u64 getNumTP(const Context &context);
HEAAN_API u64 getGadgetRank(const Context &context);
...
HEAAN_API u64 getRemoveIBeginLevel(const Context &context);
HEAAN_API u64 getRemoveIEndLevel(const Context &context);

HEAAN_API u64 getPow(const Context &context, u64 rot);
...
HEAAN_API bool isDoubleHoistingSupported(const Context &context);

///@brief Get whether the given context supports outsourced bootstrap or
/// not.
///@param[in] context
///@details Returns true for the context using a parameter, which can be
/// temporilily switched into another parameter with RNS moduli using
/// near-word-size primes, so bootstrapping can be accelerated.
HEAAN_API bool isOutsourcedBootstrapSupported(const Context &context);

HEAAN_API bool isFuseModRaiseSupported(const Context &context);
...
///@brief Get the list of rotation key index for the bootstrapping process for
/// given radix decomposition
///@param[in] context
///@param[in] log_slots
HEAAN_API std::set<i64>
getRotIndicesForBootstrap(const Context &context,
...
HEAAN_API ParameterPreset getParameterPreset(const Context &context);
HEAAN_API std::string getParameterName(const Context &context);
HEAAN_API bool isReRemoveI(const Context &context);
HEAAN_API bool isCoeffBoot(const Context &context);
HEAAN_API bool isFullPackedBaseModulus(const Context &context);
HEAAN_API u64 getGapLevel(const Context &context);
HEAAN_API bool isAutomaticGapResolve(const Context &context);
HEAAN_API u64 getHammingWeight(const Context &context);
} // namespace HEaaN
