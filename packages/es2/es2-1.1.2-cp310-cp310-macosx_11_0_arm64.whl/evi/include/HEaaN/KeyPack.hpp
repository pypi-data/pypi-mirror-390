...
public:
    ...
    /// key file, this function does nothing.
    /// If stream is specified, load from the stream.
    void loadLeftRotKey(const u64 rot, const u64 max_use_dnum = 0);
    void loadLeftRotKey(const u64 rot, std::istream &stream,
                        const u64 max_use_dnum = 0);
    ...
    /// key file, this function does nothing.
    /// If stream is specified, load from the stream.
    void loadRightRotKey(const u64 rot, const u64 max_use_dnum = 0);
    ...
    bool isLeftRotKeyLoaded(const u64 rot, const u64 max_use_dnum = 0) const;

    ///@brief Check whether the Right Rotation Key is loaded in memory
    ///@param[in] rot Rotation Key index.
    bool isRightRotKeyLoaded(const u64 rot, const u64 max_use_dnum = 0) const;
    ...
    getLeftRotKey(const u64 rot, const u64 max_use_dnum = 0) const;
    ...
    /// finally returns nullptr.
    std::shared_ptr<EvaluationKey>
    getRightRotKey(const u64 rot, const u64 max_use_dnum = 0) const;
    ...
    std::shared_ptr<FuseRotationKey> getFuseRotKey(const u64 rot) const;
    ...
    /// Creates /PK under @p key_dir_path .
    ///@throws RuntimeException if @p key_dir_path is not a valid directory
    ...
    ///@brief Save multiplication, conjugation, encryption, sparse secret
    /// encapsulation, and all rotation keys under @p dir_path + "/PK".
    /// Creates /PK under @p dir_path .
    ///@throws RuntimeException if @p dir_path is not a valid directory
    void save(const std::string &dir_path) const;

    ///@brief Make all the keys usable at `device`.
    void copyTo(const Device &device);

    /// @brief Given a target rotation index, returns a list of rotation indices
    ///   where each rotation index key exists.
    /// @param[in] target_idx Target rotation index
    /// @details Returns a vector of rotation indices whose summation is equal
    /// to the target index.
    std::vector<u64> getRotKeyDecomposition(u64 target_idx);

    ///@brief Precompute decomposition of rotation indices.
    ///@param[in] get_loaded_key Compute decomposition with rotation keys which
    /// are loaded in memory or whose files are available
    ///@details Computes decomposition of rotation indices with optimal number
    /// of rotations
    void computeRotationKeyDecomposition(bool get_loaded_key = true);

    void saveList(const std::string &dir_path) const;
    void loadMinRotKey(const std::string &key_dir_path);
    void setMinimalKeyLoad(bool set);
    bool getMinimalKeyLoad() const;
