...
/// RingPacker::modPack
///
class HEAAN_API IncreaseNSKeyBundle {
public:
    explicit IncreaseNSKeyBundle() = default;
    ///@brief Create a IncreaseNSKeyBundle object
    ///@param[in] context
    ///@param[in] num_keys
    explicit IncreaseNSKeyBundle(const Context &context, const u64 num_keys);

    ///@brief Get the single switching key at given index
    ///@param[in] idx
    std::shared_ptr<EvaluationKey> getIncreaseNSKey(const u64 idx) const;
...
private:
    Context context_;
    std::vector<std::shared_ptr<EvaluationKey>> increase_ns_keys_;
};
