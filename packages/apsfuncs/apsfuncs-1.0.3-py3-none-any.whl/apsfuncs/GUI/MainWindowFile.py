...
class MainWindowAPSBase(QMainWindow):
    ...
    def __init__(self, program_title, main_screen, data_log, log_filename, config_dict, default_screen_name):
        super().__init__()
        self.logger = data_log
        ...
        self.crash_screen = CrashScreen(config_dict=config_dict, logger=data_log, current_log_name=log_filename)
        self.screen_dict["Crash"] = len(self.screen_dict)
        self.stack_dict["Crash"] = self.crash_screen
        self.stack.addWidget(self.crash_screen)

        # Add a crash screen for error handling
        self.feedback_screen = FeedbackScreen(config_dict=config_dict, main_window=self, logger=data_log, current_version=self.current_version)
        ...
        # Add the main program loading screen
        self.loading_screen = LoadingScreen(main_window=self, config_dict=config_dict, data_logger=self.logger)
    ...
    def show_previous_screen(self):
        ...
        self.stack.setCurrentIndex(self.data_share.page_history[-2])
        self.data_share.page_history.appendleft(self.screen_dict[self.default_screen_name])
    ...
    def update_scene_content(self, new_index):
        ...
        if not self.screen_returning:
            self.data_share.page_history.append(new_index)
        else:
            self.screen_returning = False

        # Open the target screen
        self.logger.info("Opening {}".format(list(self.screen_dict.keys())[list(self.screen_dict.values()).index(self.stack.currentIndex())]))
    ...
    def closeEvent(self, event):
         self.logger.info('Log closed')

    # Method if an unhandled exception was caught (crash screen)
    def display_crash_screen(self):
        self.logger.info("Crash detected")
        self.logger.info("Logged user: {}".format(os.getlogin()))
        self.logger.info("Software version: {}".format(self.current_version))
        self.logger.info("Crashed on the {} screen".format(list(self.screen_dict.keys())[list(self.screen_dict.values()).index(self.stack.currentIndex())]))
    ...
    def load_complete(self, current_version):
        ...
        self.setWindowTitle("{} {}".format(self.program_title, self.current_version))
    ...
    def closeEvent(self, *args, **kwargs):
        self.logger.info("Window exited by user")
        if self.stack.currentIndex() == self.screen_dict["Crash"]:
            self.crash_screen.post_crash_log()

        # Close the timer thread in the control screen
        self.control_screen.close_timer_thread()

        # Close the comms thread in the connection screen
        self.connection_screen.clear_comms_thread()
