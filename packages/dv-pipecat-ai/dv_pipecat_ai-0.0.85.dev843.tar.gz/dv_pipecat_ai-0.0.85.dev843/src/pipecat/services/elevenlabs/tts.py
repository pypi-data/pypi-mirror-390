...
def build_elevenlabs_voice_settings(
    ...
    if (speed := settings.get("speed")) is not None:
        # Clamp the speed to be within the accepted range [0.7, 1.2].
        voice_settings["speed"] = max(0.7, min(speed, 1.2))
...
class ElevenLabsTTSService(AudioContextWordTTSService):
    ...
    async def _connect_websocket(self):
        try:
            ...
            if model in ELEVENLABS_MULTILINGUAL_MODELS and language is not None:
                ...
                self.logger.debug(f"Using language code: {language}")
    ...
    async def _disconnect_websocket(self):
        try:
            ...
            if self._websocket:
                self.logger.debug("Disconnecting from ElevenLabs")
    ...
    async def _keepalive_task_handler(self):
        ...
        while True:
            ...
            try:
                ...
                self.logger.warning(f"{self} keepalive error: {e}")
                break
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"{self} error sending message: {e}")
        ...
        try:
            ...
            async with self._session.post(
                ...
                if response.status != 200:
                    ...
                    self.logger.error(f"{self} error: {error_text}")
