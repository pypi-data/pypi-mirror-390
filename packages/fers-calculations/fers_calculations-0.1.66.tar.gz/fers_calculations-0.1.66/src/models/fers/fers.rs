...
impl FERS {
    fn unit_factors(&self) -> UnitFactors {
    ...
    pub fn denormalize_results(mut bundle: ResultsBundle, uf: &UnitFactors) -> ResultsBundle {
            ...
            .chain(bundle.loadcombinations.values_mut())
        {
            ...
            for d in res.displacement_nodes.values_mut() {
                ...
                d.dy *= inv_l;
                d.dz *= inv_l;
                // rotations assumed in radians → no scaling
            }
    ...
    pub fn assemble_global_stiffness_matrix(
        ...
        // Increase/decrease as needed (e.g., 1e-8 .. 1e-4).
        let off_scale: f64 = 1.0e-3;
    ...
    pub fn solve_for_load_case(&mut self, load_case_id: u32) -> Result<Results, String> {
        ...
        let load_vector = m.assemble_load_vector_for_case(load_case_id);
        let load_case = m
            .load_cases
            .iter()
        ...
        let res_si = m.solve_first_order_common(
            load_vector,
            load_case.name.clone(),
            ResultType::Loadcase(load_case_id),
        )?;

        if let Some(bundle_si) = m.results.take() {
            let bundle_user = FERS::denormalize_results(bundle_si, &uf);
            self.results = Some(bundle_user);

            let results_ref = self
                .results
                .as_ref()
                .and_then(|b| b.loadcases.get(&res_si.name))
                .ok_or_else(|| "Internal error: missing denormalized results".to_string())?;
            Ok(results_ref.clone())
        } else {
            Err("Internal error: solver produced no results".to_string())
        }
    }
    ...
    pub fn solve_for_load_case_second_order(
        ...
        let res_si = m.solve_second_order_common(
            load_vector,
            load_case.name.clone(),
            ResultType::Loadcase(load_case_id),
            max_iterations,
    ...
    pub fn solve_for_load_combination(&mut self, combination_id: u32) -> Result<Results, String> {
        ...
        let load_vector = m.assemble_load_vector_for_combination(combination_id)?;
        let combo = m
            .load_combinations
            .iter()
            .find(|lc| lc.id == combination_id)
        ...
        let (initial_u_full, initial_active_map) = if let Some(mut u_user) = warm_user {
            ...
            let active = m.active_map_from_trial_displacement(&u_user);
        ...
        let res_si = if initial_u_full.is_some() {
            m.solve_first_order_common_warm(
                load_vector,
                combo.name.clone(),
        ...
        // 6) Bring SI results back to user units and attach to self
        if let Some(bundle_si) = m.results.take() {
            let bundle_user = FERS::denormalize_results(bundle_si, &uf);
            self.results = Some(bundle_user);
            ...
            let results_ref = self
                .results
                .as_ref()
                .and_then(|b| b.loadcombinations.get(&res_si.name))
                .ok_or_else(|| "Internal error: missing denormalized results".to_string())?;
    ...
    pub fn solve_for_load_combination_second_order(
        ...
        // Denormalize and expose on self
        if let Some(bundle_si) = m.results.take() {
            let bundle_user = FERS::denormalize_results(bundle_si, &uf);
            self.results = Some(bundle_user);
