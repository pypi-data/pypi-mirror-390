...
class FindingsMergeStrategy(MergeStrategy):
    ...
    def merge(
        ...
        for key, value in origin_value.items():
            destination_value[key] = FindingTypeConfig(**value.model_dump())
        return destination_value
...
class UserDBConfigParser(ConfigParser):
    ...
    def get_config_from_dict(
        self, config_delta: dict[str, Any]
    ) -> tuple[UserConfig, Optional[str]]:
        """
        Parse and validate a config delta from the database.

        Args:
            config_delta: Dictionary containing the configuration delta from DB

        Returns:
            Tuple of (merged_config, warning_message)
        """
        # Map data from dict to UserDBConfig
        config_data = _UserDBConfig(**config_delta)
