...
class PlaywrightCrawler:
    ...
    async def goto(self, url: str, max_proxy_delay: timedelta = timedelta(minutes=10), timeout: int = 30_000) -> Response:
        """
    ...
    async def get_data(
        ...
        if r.status in ignored_status_codes:
            raise ManagedHTTPError(r, url=url, message=f"Status {r.status} in ignored_status_codes for URL {url}")

        error_handlers = {
            404: lambda: NotFoundError(f"404 Not Found error for {url}"),
            400: lambda: BadRequestError(f"400 Bad Request error for {url}"),
        }

        if (code := r.status) in error_handlers:
            raise error_handlers[code]()

        if r and r.status >= 400:
            raise HTTPError(f"Navigation failed: {r.status} - {url}")

        if not body_content:
            raise EmptyResponseError(f"Empty body received from {url} (status {r.status})")
