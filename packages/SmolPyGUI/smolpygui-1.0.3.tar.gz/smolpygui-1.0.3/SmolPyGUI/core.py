...
class Button():
    def __init__(self, width:int, height:int, x:int, y:int, texture:pygame.Color|pygame.surface.Surface, method:Callable, stroke:int=0, scene:int=0):
        """
...
class HoverButton():
    def __init__(self, width:int, height:int, x:int, y:int, texture:pygame.Color|pygame.surface.Surface, method, stroke=0, scene:int|str=0):
        self.visible = True
        self.x=x
        self.y=y
        self.height=height
...
class DrawRect():
    def __init__(self, width:int, height:int, x:int, y:int, texture:pygame.Color|pygame.surface.Surface, stroke:int=0,scene:int|str=0):
        self.visible = True
        self.x=x
        self.y=y
        self.height=height
...
class TextBox():
    def __init__(self, x:int, y:int, width:int, size:int, bg="#ffffff", bgActive="#9999ff",outline="#444444", textColor="#000000", onUpdate=lambda x: None, onEnter=lambda x: None,scene:int|str=0):
        ...
        self.button = Button(self.width, self.height, self.x, self.y, self.bg, lambda box=self:box.inputStart(), scene=scene)
        self.outBox = DrawRect(self.width, self.height, self.x, self.y, self.outline, 4, scene=scene)
        self.text = Text(self.x+5,self.y,self.value,self.size,self.textColor, scene=scene)
        self.indic = DrawRect(2, self.height-10, self.x+10, self.y+5, self.bg, 0, scene=scene)
        ...
        while True:
            for e in pygame.event.get():
                if e.type == 256:
                    ...
                    if(len(self.value)*(0.8*self.size)>(self.width)):
                        self.drawVal = self.value[-int(self.width//(self.size*0.8)):]
                    else:
                        self.drawVal = self.value
                    self.text.text = self.drawVal
                    self.indic.x = (1+len(self.drawVal))*(0.825*self.size)
...
class TextDisplay():
    def __init__(self, x:int, y:int, width:int, lines:int, size:int, value:str="", bg:pygame.Color|pygame.surface.Surface="#ffffff",outline:pygame.Color="#444444", textColor:pygame.Color="#000000",scene:int|str=0):
        ...
        self.charHgt = size*1.55
        self.height = size*1.55*lines
        self.lines = lines
        self.size = size
        self.drawVal = value
        self.bg = bg
        ...
        self.box = DrawRect(self.width, self.height, self.x, self.y, self.bg, scene=scene) if bg else False
        self.outBox = DrawRect(self.width, self.height, self.x, self.y, self.outline, 4, scene=scene) if outline else False
...
class globals:
    ...
    width = None
    height = None
    text = pygame.font.SysFont("Courier", 32, True)
    ...
    def drawTexts():
        for tex in draw.texts:
            if(tex.visible and tex.scene == globals.scene.name):
                globals.screen.blit(globals.text.render(tex.text, True, tex.color), (tex.x,tex.y))
...
def initialize(framerate:int=60, size:tuple[int,int]=(600,600), screenFlags:int=0, runtimeFuncs:list[Callable]=[]):
    """
