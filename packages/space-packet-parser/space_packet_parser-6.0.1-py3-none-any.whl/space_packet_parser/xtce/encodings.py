...
class DataEncoding(common.AttrComparable, common.XmlObject, metaclass=ABCMeta):
    ...
    def get_context_calibrators(
        ...
        if (context_calibrators_elements := data_encoding_element.find("ContextCalibratorList")) is not None:
            return [calibrators.ContextCalibrator.from_xml(el) for el in context_calibrators_elements]
        return None
...
class StringDataEncoding(DataEncoding):
    ...
    def __init__(
        ...
        *,
        ...
        if encoding not in self._supported_encodings:
            raise ValueError(
                f"Got encoding={encoding} (uppercased). Encoding must be one of {self._supported_encodings}."
            )
        ...
        if encoding not in ["US-ASCII", "ISO-8859-1", "Windows-1252", "UTF-8"]:  # for these, byte order doesn't matter
            ...
            if self.byte_order and self.byte_order not in ("leastSignificantByteFirst", "mostSignificantByteFirst"):
                raise ValueError(
                    "If specified, byte order must be one of `leastSignificantByteFirst`, `mostSignificantByteFirst`."
                )
        ...
        if length_linear_adjuster and not dynamic_length_reference:
            raise ValueError(
                "Got a length linear adjuster for a string whose length is not specified by a dynamic length reference."
            )
        ...
        if termination_character:
            ...
            if len(self.termination_character.decode(encoding)) != 1:
                raise ValueError(
                    f"Termination character {termination_character} appears to be malformed. "
    ...
    def _calculate_size(self, packet: spp.SpacePacket) -> int:
        ...
        if self.fixed_length:
            ...
            for discrete_lookup in self.discrete_lookup_length:
                ...
                raise ValueError(
                    "List of discrete lookup values being used for determining length of "
    ...
    def _get_raw_buffer(self, packet: spp.SpacePacket) -> bytes:
        ...
        if (buflen_bits + pad_bits) % 8 != 0:
            raise ValueError(
                f"Error in buffer length math in _get_raw_buffer. "
        ...
        raw_string_buffer = (packet._read_from_binary_as_int(buflen_bits) << pad_bits).to_bytes(buflen_bytes, "big")
    ...
    def parse_value(self, packet: spp.SpacePacket) -> common.StrParameter:
        ...
        if self.leading_length_size:
            ...
            if strlen_bits % 8 != 0:
                raise ValueError(
                    f"String length (in bits) is {strlen_bits}, which is not a multiple of 8. "
        ...
        *,
        ...
        if encoding not in ("US-ASCII", "ISO-8859-1", "Windows-1252", "UTF-8"):  # single-byte chars
            if not (encoding.endswith("BE") or encoding.endswith("LE")):
                ...
                if byte_order is None:
                    raise ValueError(
                        "For multi-byte character encodings, byte order must be specified "
    ...
    def to_xml(self, *, elmaker: ElementMaker) -> ElementTree.Element:
        ...
        if self.termination_character:
            size_element.append(elmaker.TerminationChar(self.termination_character.hex()))
...
class NumericDataEncoding(DataEncoding, metaclass=ABCMeta):
    ...
    def to_xml(self, *, elmaker: ElementMaker) -> ElementTree.Element:
        ...
        if self.default_calibrator:
            element.append(elmaker.DefaultCalibrator(self.default_calibrator.to_xml(elmaker=elmaker)))

        if self.context_calibrators:
            element.append(
                elmaker.ContextCalibratorList(*(cal.to_xml(elmaker=elmaker) for cal in self.context_calibrators))
            )
...
class IntegerDataEncoding(NumericDataEncoding):
    ...
    def _get_raw_value(self, packet: spp.SpacePacket) -> int:
        ...
        if self.byte_order == "leastSignificantByteFirst":
            ...
            val = int.from_bytes(val.to_bytes(length=(self.size_in_bits + 7) // 8, byteorder="little"), byteorder="big")
    ...
    def __init__(
        ...
        *,
        ...
        if encoding == "IEEE-754":
            warnings.warn(
                "Float encoding 'IEEE-754' (with a dash) is not supported by the XTCE spec; use 'IEEE754' instead"
            )
            encoding = "IEEE754"
        elif encoding == "MIL-1750A":
            warnings.warn("Float encoding 'MIL-1750A' is not supported by the XTCE spec; use 'MILSTD_1750A' instead")
            encoding = "MILSTD_1750A"
        if encoding not in self._allowed_encodings:
            raise ValueError(
                f"Invalid encoding type {encoding} for float data. Must be one of {self._allowed_encodings}."
            )
        if encoding not in self._supported_encodings:
            raise NotImplementedError(
                f"Although the XTCE spec allows {encoding}-encoded floats, parsing them is not currently supported."
            )
        if encoding == "MILSTD_1750A" and size_in_bits != 32:
            raise ValueError(
                "MIL-1750A encoded floats must be 32 bits, per the MIL-1750A spec. See "
        ...
        if encoding in ("IEEE754", "IEEE754_1985") and size_in_bits not in (16, 32, 64):
            raise ValueError(
                f"Invalid size_in_bits value for IEEE754 FloatDataEncoding, {size_in_bits}. Must be 16, 32, or 64."
            )
        super().__init__(
            size_in_bits=size_in_bits,
            encoding=encoding,
...
class BinaryDataEncoding(DataEncoding):
    ...
    def __init__(
        ...
        *,
        ...
        if not any([fixed_size_in_bits, size_reference_parameter, size_discrete_lookup_list]):
            raise ValueError(
                "Binary data encoding initialized with no way to determine a size. "
    ...
    def from_xml(
        ...
        *,
        ...
        if (dynamic_value_element := element.find("SizeInBits/DynamicValue")) is not None:
            ...
            linear_adjuster = cls._get_linear_adjuster(dynamic_value_element)
            return cls(
                size_reference_parameter=referenced_parameter,
                use_calibrated_value=use_calibrated_value,
                linear_adjuster=linear_adjuster,
            )
        ...
        raise ValueError(
            "Tried parsing a binary parameter length using Fixed, Dynamic, and DiscreteLookupList "
    ...
    def to_xml(self, *, elmaker: ElementMaker) -> ElementTree.Element:
        ...
        if self.size_discrete_lookup_list:
            size_in_bits.append(
                elmaker.DiscreteLookupList(*(dl.to_xml(elmaker=elmaker) for dl in self.size_discrete_lookup_list))
            )
