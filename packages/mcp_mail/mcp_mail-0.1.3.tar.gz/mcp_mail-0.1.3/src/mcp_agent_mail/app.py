...
async def _log_agent_name_conflicts(
    *,
    name: str,
    project: Project,
    stage: str,
    ...
    """Emit structured diagnostics when a global agent-name conflict occurs."""
    ...
    try:
        async with get_session() as session:
            result = await session.execute(
                ...
                .join(Project, Project.id == Agent.project_id, isouter=True)
            ...
            conflicts = [
                {
                    ...
                    "deleted_ts": row[4],
                    "last_active_ts": row[5],
                    "project_slug": row[6],
                    "project_human_key": row[7],
                }
                for row in result.all()
            ]
    except Exception:  # pragma: no cover - logging should never raise
        logger.exception(
            "agent_register.conflict_logging_failed",
            extra={
                "name": name,
    ...
    logger.warning(
        "agent_register.name_conflict",
        extra={
            "stage": stage,
            "requested_name": desired_name or name,
            "normalized_name": name,
            "project_id": project.id,
            "project_slug": getattr(project, "slug", None),
            "project_human_key": getattr(project, "human_key", None),
            "conflicts": conflicts,
...
async def _create_agent_record(
    ...
    async with get_session() as session:
        ...
        try:
            ...
            await _log_agent_name_conflicts(
                name=name.lower(),
                desired_name=name,
                project=project,
                stage="create_agent_record",
                error=exc,
            )
            raise ToolExecutionError(
                "NAME_TAKEN",
