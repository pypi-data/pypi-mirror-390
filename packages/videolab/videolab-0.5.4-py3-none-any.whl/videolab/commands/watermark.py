...
def _parse_font_color(font_color_str: str) -> tuple[int, int, int, int]:
    """Parses an RGBA color string and validates component ranges."""
    try:
        components = list(map(int, font_color_str.split(",")))
        if len(components) != 4:
            raise ValueError("Color string must contain exactly 4 components (R,G,B,A).")
        ...
        logging.warning("Invalid font color format '%s': %s. Using default (255, 255, 255, 128).", font_color_str, e)
            ...
            with tqdm(total=total_frames, unit="frame", desc="Watermarking") as pbar:
                for packet in in_container.demux(in_video_stream):
                    for frame in packet.decode():
                        new_frame = _add_watermark(frame, watermark_img, watermark_position)
