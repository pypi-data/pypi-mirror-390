...
class PandasApp(BaseWorker):
    """Minimal worker wiring for the Pandas app template."""
        ...
        **kwargs: ArgsOverrides,
        ...
        if args is None:
            ...
            if extra := set(kwargs) - allowed:
                logger.debug("Ignoring extra PandasAppArgs keys: %s", sorted(extra))
            args = PandasAppArgs(**clean)
