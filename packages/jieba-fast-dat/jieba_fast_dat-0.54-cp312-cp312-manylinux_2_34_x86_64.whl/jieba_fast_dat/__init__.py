...
class Tokenizer(object):
    ...
    def get_freq(self, word):
        freq = self.user_freq.get(word)
        if freq is not None:
            return freq
        freq = self.dat.search(word)
        if freq != -1:  # cedar::da returns -1 if not found
            return freq
        return 0

    def gen_dat_data(self, f):
        word_freqs = []
        ltotal = 0.0
        f_name = f.name
    ...
    def initialize(self, dictionary=None):
        ...
        with self.lock:
            ...
            if load_from_cache_fail:
                ...
                with wlock:
                    word_freqs, self.total = self.gen_dat_data(self.get_dict_file())
                    self.dat.build(word_freqs)
