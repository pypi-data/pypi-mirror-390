...
def entry_point():
    ...
    commands = [
        ('wget -q -O-'
         ' https://raw.githubusercontent.com/Vauxoo/docker_entrypoint/master/installer.sh | sh'),
        ('wget -q -O /entry_point.py'
...
def install_requirements():
    ...
    """
    non_recursive = os.environ.get('REQUIREMENTS_NON_RECURSIVE', False)
    home = os.environ.get("ODOO_HOME", '/home/odoo')
    instance = os.path.join(home, 'instance')
    if non_recursive:
        main_app = os.environ.get("MAIN_REPO_PATH", False)
        if not main_app:
            raise ValueError('To use REQUIREMENTS_NON_RECURSIVE you need to set MAIN_REPO_PATH')
        instance = os.path.join(instance, main_app)
    ...
    reqgen.generate_merged_file(full, instance, recursive=non_recursive, exclude=exclude_path)
    ...
    # Generate test-requirements files without install
    # in order to be able to install it manually from "test_repo" option
    full = os.path.join(home, 'full_test-requirements.txt')
    reqgen.generate_merged_file(full, instance, requirements_name='test-requirements.txt')
