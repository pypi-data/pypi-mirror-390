...
def interpret_drug_design(
    ...
    try:
        ...
        if mol:
            ...
            oral_bioavailability = "High" if (qed_score > 0.5 and tpsa < 140 and rotatable_bonds < 10) else "Medium" if qed_score > 0.3 else "Low"

            # BBB permeability (simple rule)
            bbb_permeability = "High" if (tpsa < 90 and best_candidate.properties.get('logP', 0) > 1) else "Low"

            # P-gp substrate prediction
            pgp_substrate = "No" if best_candidate.properties.get('molecular_weight', 500) < 400 else "Possible"
    ...
    cardiotoxicity = "High" if (mw > 500 and logp > 5) else "Medium" if (mw > 400 or logp > 4) else "Low"
    ...
    total_h_capability = best_candidate.properties.get('hbd', 0) + best_candidate.properties.get('hba', 0)
    h_bonds = f"{int(total_h_capability / 2)}-{total_h_capability}" if num_poses > 0 else f"~{int(total_h_capability / 2)}"
...
def interpret_drug_docking(
    ...
    if smiles:
        try:
            ...
            print(f"   Lipinski Rule of 5: {'✅ PASS' if pharma_scores.get('lipinski_compliant') else '❌ FAIL'} ({pharma_scores.get('lipinski_violations', 0)} violations)")
            print(f"   QED (Drug-likeness): {pharma_scores.get('qed_score', 0):.3f} ({pharma_scores.get('qed_rating', 'N/A')})")
            print(f"   SA Score (Synthesis): {pharma_scores.get('sa_score', 0):.2f}/10 ({pharma_scores.get('sa_rating', 'N/A')})")
            print(f"   PAINS Alerts: {pharma_scores.get('pains_alerts', 0)} ({'✅ Clean' if pharma_scores.get('pains_alerts', 0) == 0 else '⚠️  Flagged'})")
