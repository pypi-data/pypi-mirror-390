        ...
        try:
            ...
            for operation in parsed_circuit.operations:
                ...
                if not operation.target_qubits:
                    self.logger.warning(f"Skipping {operation.gate_type} operation with no target qubits")
                    continue
                ...
                if operation.gate_type == QuantumGateType.HADAMARD:
                    ...
                    if len(operation.control_qubits) < 2:
                        raise CompilerError(f"TOFFOLI gate requires 2 control qubits, got {len(operation.control_qubits)}")
