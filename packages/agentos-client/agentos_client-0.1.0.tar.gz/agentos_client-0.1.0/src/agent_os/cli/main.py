#!/usr/bin/env python3
"""
Agent‑OS CLI - SDK+CLI (remote‑only): build, deploy, invoke, logs, list.
...
@click.option('--platform', envvar='AGENTOS_PLATFORM_URL', help='Platform URL (default: https://app.agentosai.org)')
def cli(platform):
    """Agent‑OS CLI (remote‑only)."""
    if platform:
...
@click.option('--platform-url', envvar='AGENTOS_PLATFORM_URL', help='Platform URL')
def login(email, password, platform_url):
    """Authenticate with the Agent‑OS Platform."""
    click.echo("🔐 Logging in to Agent‑OS Platform...")
    if platform_url:
...
@cli.command()
def logout():
    """Log out from Agent‑OS Platform."""
...
@click.option('--tags', multiple=True, help='Tags for the agent')
def deploy(agent_tar, name, description, tags):
    """Deploy an agent tarball to the Platform."""
    if not config.is_authenticated():
        click.echo("Error: Not authenticated. Run 'agent-os login' first.", err=True)
        sys.exit(1)
    ...
    try:
        with click.progressbar(length=100, label='Uploading') as bar:
            bar.update(30)
            result = api_client.upload_agent(name=name, agent_tar_path=agent_path, description=description, tags=list(tags))
            bar.update(70)
        click.echo()
        ...
        click.echo("  agent-os invoke <agent-id> --payload '{""message"": ""Hello!""}'")
    except PlatformAPIError as e:
...
def invoke(agent_id, payload, sync_mode, timeout):
    ...
    try:
        ...
        if sync_mode:
            ...
            click.echo("Use: agent-os logs <invocation-id> --follow")
    except PlatformAPIError as e:
...
def list_command(page, per_page):
    ...
    try:
        ...
        if not agents:
            ...
            click.echo("  agent-os build ./my-agent")
            click.echo("  agent-os deploy my-agent.tar.gz --name my-agent")
            return
...
def main():
    ...
    message = input_data.get('message', 'Hello, Agent‑OS!')
...
description: A new Agent‑OS agent
version: 1.0.0
template: {template}
...
An Agent‑OS agent created with the {template} template.
...
Deploy to platform:
```bash
agent-os deploy {name}.tar.gz --name {name}
```
...
Invoke the agent:
```bash
agent-os invoke <agent-id> --payload '{{"message": "Hello!"}}'
```
'''
    ...
    click.echo("  agent-os build .")
    click.echo(f"  agent-os deploy {name}.tar.gz --name {name}")


# ---- Stubbed VM commands (disabled in this package) ----

def _vm_stub(cmd: str):
    click.echo(f"'{cmd}' is not available in the agent-os package. Use platform remote workflow.", err=True)
    sys.exit(1)


@cli.command()
def version():
    """Show Agent‑OS SDK/CLI version."""
    click.echo("Agent‑OS SDK/CLI v0.1.0 (remote‑only)")
