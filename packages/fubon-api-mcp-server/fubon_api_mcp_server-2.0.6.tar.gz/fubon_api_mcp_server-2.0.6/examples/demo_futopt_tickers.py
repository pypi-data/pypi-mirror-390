#!/usr/bin/env python3
"""
Demo: 呼叫富邦期貨/選擇權 intraday/tickers 取得期權商品列表
...
執行範例：
  python examples/demo_futopt_tickers.py --type FUTURE --exchange TAIFEX --session REGULAR --contractType E
...
def print_summary(resp: Dict[str, Any]) -> None:
    ...
    print(f"查詢日期: {date}")
    print(f"查詢條件: type={t}, exchange={ex}, session={sess}, contractType={ctype}")
    ...
    for i, item in enumerate(data[:10], start=1):
        ...
        print(
            ...
            f"contractType={item.get('contractType')} referencePrice={item.get('referencePrice')} "
            f"startDate={item.get('startDate')} endDate={item.get('endDate')} "
            f"settlementDate={item.get('settlementDate')} isDynamicBanding={item.get('isDynamicBanding')}"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Fubon intraday/tickers demo")
    ...
    parser.add_argument("--product", default=None, help="期權商品代碼")
    ...
    try:
        ...
        if args.contractType:
            params["contractType"] = args.contractType
        
        resp = restfutopt.intraday.tickers(**params)
