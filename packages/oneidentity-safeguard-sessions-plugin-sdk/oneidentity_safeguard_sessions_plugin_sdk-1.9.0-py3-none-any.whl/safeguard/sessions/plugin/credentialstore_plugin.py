...
class CredentialStorePlugin(PluginBase):
    ...
    *New in version: 1.7.0*
    ...
    def do_get_totp_codes(self):
        """
        *New in version: 1.9.0*

        The :meth:`do_get_totp_codes` method should return with a list of TOTP
        codes with their expiration and refresh period.

        :return: list of dicts of {'unix_time': 1761749392, 'period': 30, "code": "123456"}
        """
        raise NotImplementedError("do_get_totp_codes")
    ...
    def get_totp_codes(
        ...
        try:
            result = self.do_get_totp_codes()
            return self.finalize_hook(
                self._add_cookies(
                    {
                        "totp_codes": [
                            TotpCode(**totp_code).as_dict() for totp_code in result
                        ]
                    }
                )
            )
            ...
            self.logger.info("This plugin does not support fetching TOTP code")

        return self._add_cookies({})
