!! Intermolecular interactions, specific to molecular systems
module interaction_intermolecular
...
contains
  ...
  subroutine energies(zero,box,pos,ids,rad,mol,epot)
    ...
    integer                         :: i, j, isp, jsp, mol_i, mol_j
    logical                         :: zero_ij
    if (zero) then
    ...
    do i = 1,size(pos,2)
       ...
       do j = i+1,size(pos,2)
          mol_i = mol(i)
          mol_j = mol(j)
          if (mol_i.gt.0 .and. mol_j.gt.0 .and. mol_i==mol_j) cycle
          jsp = ids(j)
  ...
  subroutine forces(zero,box,pos,ids,rad,mol,for,epot,virial)
    logical,          intent(in)    :: zero ! to set variables to zero
    double precision, intent(in)    :: box(:)
    double precision, intent(in)    :: pos(:,:)
    integer,          intent(in)    :: ids(:)
    ...
    do i = 1,size(pos,2)
       ...
       do j = i+1,size(pos,2)
          mol_i = mol(i)
          mol_j = mol(j)
          !print*, mol_i.gt.0 .and. mol_j.gt.0 .and. mol_i==mol_j, i, j
          if (mol_i.gt.0 .and. mol_j.gt.0 .and. mol_i==mol_j) cycle
          jsp = ids(j)
          call distance(i,j,pos,rij)
          call pbc(rij,box,hbox)
  ...
  subroutine hessian(zero,box,pos,ids,rad,mol,hes)
    logical,          intent(in)    :: zero ! to set variables to zero
    double precision, intent(in)    :: box(:)
    double precision, intent(in)    :: pos(:,:)
    integer,          intent(in)    :: ids(:)
  ...
  subroutine gradw(zero,box,pos,ids,rad,mol,grad_w)
    logical,          intent(in)    :: zero ! to set variables to zero
    double precision, intent(in)    :: box(:)
    double precision, intent(in)    :: pos(:,:)
    integer,  intent(in)            :: ids(:)
    ...
    integer                         :: i, j, isp, jsp, mol_i, mol_j
    logical                         :: zero_ij
    call forces(.TRUE.,box,pos,ids,rad,mol,forc,epot,virial)
    if (zero) grad_w = 0.d0
    hbox = box / 2
    loop_i: do i = 1,size(pos,2)
       isp = ids(i)
...
end module interaction_intermolecular
