...
class TrajectoryRUMD(TrajectoryXYZ):
    ...
    def _read_comment(self, frame):
        meta = super()._read_comment(frame)
    ...
    def write_system(self, system, step):
        ...
        for p in system.particle:
            ...
            pos = ' '.join([str(x) for x in p.position])
            vel = ' '.join([str(x) for x in p.velocity])
            self._file.write(f"{isp} {pos} {vel}\n")
