...
from .utils import _PossiblyZippedFile
...
class TrajectoryXYZ(TrajectoryBase):
    ...
    def __init__(self, filename, mode='r', alias=None, fields=None):
        ...
        self.callback_read = {'particle.species': _update_species,
        ...
        if self.mode == 'a':
            _mode += '+'
        self._file = _PossiblyZippedFile(self.filename, _mode)
        ...
        if self.mode[0] in ['r', 'a']:
            ...
            # self._sanitize_frames()

        if self.mode[0] == 'r':
            # Sanity checks
            assert len(self._index_frame) > 0 and \
                len(self._index_header) > 0, f'empty file {self.filename}'
    ...
    def _setup_format(self):
        # This private method has been removed, but might have been used in client code.
        # If so, we raise an exception to at least avoid unexpected results.
        raise AttributeError('method _setup_fmt() was removed')
    ...
    def _setup_index(self):
        ...
        while True:
            ...
            if len(data) > 0:
                ...
                raise IOError(f'malformed xyz file {self.filename}')
    ...
    def _sanitize_frames(self):
        steps, redundant_frames = [], []
        for frame in range(len(self._index_frame)):
            meta = self._read_comment(frame)
            if meta['step'] in steps:
                redundant_frames.append(steps.index(meta['step']))
                print('found existing step', meta['step'])
            steps.append(meta['step'])
        self._index_frame = [_ for i, _ in enumerate(self._index_frame) if i not in redundant_frames]
        self._index_header = [_ for i, _ in enumerate(self._index_header) if i not in redundant_frames]
...
""" % key, namespace)
    ...
    def read_system(self, frame):
        ...
        if 'mass' in db:
            if isinstance(db['mass'], (list, tuple)):
    ...
    def write_system(self, system, step):
        ...
        def array_str(arr):
            """Format numpy arrays or scalars"""
        ...
        # In numpy 2 we can only override repr() to get a reasonable efficiency.
        # But this means we must use a switch for non-array variables.
        def str_or_repr(x):
        ...
        variables = [v.split('particle.')[-1] for v in self.variables]
        for i, p in enumerate(system.particle):
            # TODO: why this? who needs it?
            p._index = i
            p._step = step
            line = []
            for v in variables:
                line.append(str_or_repr(getattr(p, v)))
            self._file.write(' '.join(line) + '\n')

    def close(self):
        self._file.close()
        # Remove the _file instance, to allow pickling/unpickling of trajectory
        # objects (_PossiblyZippedFile raises recursion issues when unpickling)
        try:
            del self._file
