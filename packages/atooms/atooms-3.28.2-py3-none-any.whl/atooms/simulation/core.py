...
class Simulation:
    ...
    def write_checkpoint(self):
        ...
        if hasattr(self.backend, 'write_checkpoint'):
            ...
            _log.debug(f'writing checkpoint with native backend method')
            ...
            _log.debug(f'writing checkpoint with fallback format')
    ...
    def read_checkpoint(self):
        ...
        if hasattr(self.backend, 'read_checkpoint'):
            # Use native backend checkpoint method
            _log.debug(f'reading checkpoint with native backend')
            self.backend.read_checkpoint(self.output_path)
        else:
            # Fallback to backend trajectory class with high precision
            _log.debug(f'reading checkpoint with fallback format')
    ...
    def _elapsed_wall_time(self):
        ...
        if self._stop_time < 0 and self._start_time > 0:
    ...
    def run(self, steps=None):
        ...
        if steps is None:
            ...
            if self.restart and self.current_step > 0:
                # If we are restarting, we do not allow changing
                # target steps on the fly, because it might have side
                # effects like non-constant writing interval.
                _log.warning('attempt to change steps while restarting')
                steps = self.steps
            else:
                # Keep track of the number of steps
                self.steps = steps

        # Make sure we have an integer number of steps
        steps = int(round(steps))

        # Targeter for steps. We first replace the existing one
        self.remove(self._targeter_steps)
        self.add(self._targeter_steps, Scheduler(self.current_step + steps),
                 self.current_step + steps)
        ...
        if atooms.core.progress.active:
            ...
            min_iters = 10
            if len(intervals) == 0 or (min(intervals) > (self.current_step + steps) / min_iters and
                                       (self.current_step + steps) / min_iters > 10):
                def flush(sim):
                    pass
                self.add(flush, Scheduler((self.current_step + steps) // min_iters))
        ...
        try:
            ...
            _log.warning('issues reporting system; this is mostly harmless but check client code')
        ...
        # Timing can be refactored with core.utils.Timer()
        # Restart timing if the simulation is started again
        ...
        try:
            ...
            # Do not show traceback upon kill, but still return an error
            _log.warning('simulation terminated')
            sys.exit(1)
