...
class TrajectoryBase:
    ...
    The `read()` method is a template composed of the two following steps:

    - `read_init()`: called only once to initialize internal data structures,
      grab metadata, etc. Need *not* be implemented by subclasses.

    - `read_system()`: actually return the system at a given frame. It must
    ...
    def __init__(self, filename, mode='r', cache=False):
        ...
        # In append mode, the default behavior is to allow overwrite existing
        # steps. This is the expected behavior when restarting a simulation from
        # a checkpoint.
        if self.mode[0] == 'a':
