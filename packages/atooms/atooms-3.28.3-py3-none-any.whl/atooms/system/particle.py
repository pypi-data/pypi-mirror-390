...
def cm(particle, what):
    """General center-of-mass attribute of a list of particles."""
    rcm = numpy.zeros_like(getattr(particle[0], what))
    mtot = 0.0
    for p in particle:
        rcm += getattr(p, what) * p.mass
        mtot += p.mass
    return rcm / mtot
...
def _lattice(N, d=3, spacing=-1.0):
    """
