    ...
    def __init__(self, filename, mode='r'):
        ...
        if mode == 'r':
            ...
            if ext == '.bz2':
                cmd = 'bunzip2'
            if ext in ['.gz', '.bz2']:
                subprocess.check_call(f'{cmd} -c {filename} > {self._file_tmp}', shell=True)  # pylint:disable=possibly-used-before-assignment
...
def split(inp, path='{base}-{step:09}{ext}', archive=False, index=None):
    """
