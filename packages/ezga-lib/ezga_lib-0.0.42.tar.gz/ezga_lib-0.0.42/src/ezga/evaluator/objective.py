...
def objective_formation_energy(reference_potentials: dict | None = None,
    ...
    If `reference_potentials` is None, estimate μ by linear least squares
    (no intercept) using E ≈ X μ, where X is the per-structure count matrix.

    Dataset API expected:
      - dataset.get_all_energies() -> (N,)
      - dataset.get_all_compositions(return_species=True) -> (X_all, species_order)
        * X_all: (N, S_all) integer counts per species in dataset order
        * species_order: list[str] length S_all
      - dataset.get_species_mapping(order="stored") -> {label: col_index_in_X_all}

    Parameters
    ----------
    reference_potentials : dict | None
        {label: μ}. If None, μ is fitted from the dataset by least squares.
        Labels absent from X (never present) get μ = 0 in the fitted vector.
    unique_labels : list[str] | None
        Optional ordered list of species to define the composition space and μ order.
        If None, use the dataset's stored species order.
