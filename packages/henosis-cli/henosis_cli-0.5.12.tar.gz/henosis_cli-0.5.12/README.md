...
- Tools include read_file, write_file, append_file, list_dir, apply_patch, string_replace (fallback-only), run_command.
- string_replace is a guarded, bounded find/replace across files. Prefer apply_patch for deterministic edits; use replace only when patching fails repeatedly.
...
string_replace CLI examples
- Dry-run literal replacement (expect exactly 3 replacements):
  henosis-tools replace apply --pattern "foo" --replacement "bar" --glob "workspace/**/*.txt" --expected 3 --dry-run
- Regex replacement with multiple globs, excluding dist/:
  henosis-tools replace apply --regex --pattern "\bacme\b" --replacement "Acme" --glob "**/*.md" --glob "**/*.py" --exclude "dist/**" --expected 5
