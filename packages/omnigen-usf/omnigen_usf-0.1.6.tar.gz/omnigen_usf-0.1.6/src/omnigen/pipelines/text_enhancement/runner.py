...
class Runner:
    ...
    def _generate_parallel(self, num_texts: int, num_workers: int):
        ...
        with ThreadPoolExecutor(max_workers=num_workers, thread_name_prefix="Worker") as executor:
            ...
            submitted = 0
            text_stream = self.data_loader.stream_texts(skip_positions=skip_positions)
            
            # Initial batch submission - keep executor saturated
            initial_batch = min(num_workers * 2, num_texts)  # 2x workers for saturation
            ...
            batch_count = 0
            auto_save_freq = self.config.get('checkpoint.auto_save_frequency', 100)
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    pbar.set_postfix_str(
                        f"✓{complete} ⚠{partial} ✗{failed} ~{skipped} | {metric_str}"
                    )
        ...
        if total > 0:
            print(f"⚡ Speed:   {total/total_time:>5.2f} text/s")
        print("="*60)
