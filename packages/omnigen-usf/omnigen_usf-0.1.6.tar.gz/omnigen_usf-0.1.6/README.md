...
- ✨ **Text Enhancement** - Corpus generation and text quality improvement  
- 🤖 **Agentic Datasets** - Tool use, function calling, multi-step reasoning
- 🎨 **Multimodal Datasets** - Text, images, audio, video combinations (coming soon)
- 🖼️ **Images** - Synthetic image generation and editing (coming soon)
- 🎵 **Audio** - Speech, music, sound effects (coming soon)
- 🎬 **Video** - Synthetic video sequences (coming soon)
...
- **Continual Pre-Training** - Enhance and expand existing corpora
- **Offline Reinforcement Learning** - Preference datasets with rewards
- **Online Reinforcement Learning** - Ground truth with reward checking
- **Machine Learning** - Synthetic training data for any ML task

### 🏗️ Why OmniGen?
- ✅ **Enterprise-Ready** - Built for production at scale
- ✅ **Unlimited Parallelism** - Scale workers based on your infrastructure (100, 1000, 10000+)
- ✅ **Auto-Resume** - Zero duplicates, seamless resume from any interruption
- ✅ **Graceful Shutdown** - Clean stop in ~7 seconds with real-time progress
- ✅ **Fully Customizable** - Configure every aspect of generation
- ✅ **Secure** - Complete workspace isolation, no data mixing
- ✅ **Easy to Use** - Simple API with comprehensive documentation
- ✅ **Modular** - Independent pipelines for different data types
- ✅ **Fault Tolerant** - Checkpoint system prevents data loss
...
### 2. **text_enhancement** - Corpus Generation & Text Quality Improvement ✨ NEW!
...
#### 1. **Text Enhancement Pipeline** ✨
Brand new pipeline for large-scale text generation and enhancement - corpus generation for pre-training, text quality improvement, dataset expansion.

#### 2. **Auto-Resume Without Duplicates** 🔄
Both pipelines guarantee zero duplicates on resume:
- Position-based tracking (every item has unique position)
- Content hash verification (detects dataset changes)
- Zero duplicates - Mathematical guarantee (impossible to process same position twice)
- Automatic skip of processed items
- O(1) performance even for millions of items

```bash
# Just restart - auto-resumes!
$ python run_pipeline.py
🔄 Resuming from checkpoint... ✓ Skipping 500 already processed positions
```

#### 3. **Graceful Shutdown with Real-Time Status** 🛑
Press Ctrl+C and see exactly what's happening:
- 3-step progress (Cancel → Checkpoint → Wait)
...
```
[1/3] Canceling 1950 pending tasks... ✓ 0.08s
[2/3] Saving checkpoint... ✓ 1.42s
[3/3] Waiting for in-flight... 48 done, 3.2s left
✓ Total shutdown time: 6.82s
```

#### 4. **Default Validation Enabled** ✅
Smart defaults for quality control - Text Enhancement (not_empty, not_identical, length_ratio), Conversation Extension (empty_content_check, duplicate_check, tool_calls_validation).

#### 5. **Multiple Pattern Support** 🔢
Validation rules now support lists for OR logic:
```yaml
validation:
  rules:
    - type: regex_pattern_match
      patterns: ["\\bcorrect\\b", "\\baccurate\\b"]  # OR logic
```

#### 6. **Tool Calls Support** 🛠️
Full OpenAI/Anthropic API compliance - Assistant messages can have empty content if `tool_calls` present.

#### 7. **Real-Time Token Tracking** 💰
Captures actual token usage from API responses, tracks per generation call, aggregates per conversation/dataset, saves detailed token data in output files.
...
✅ **Simple** - One command to generate thousands of items
✅ **Unlimited Scalability** - Scale workers based on your infrastructure (100, 1000, 10000+)
✅ **Flexible** - Mix different AI providers (OpenAI, Anthropic, Ultrasafe AI)
✅ **Production Ready** - Built for enterprise SaaS platforms with multi-tenant support
✅ **Fault Tolerant** - Checkpoint/resume system prevents data loss from any interruption
✅ **Auto-Resume** - Zero duplicates guaranteed, seamless resume from any interruption
✅ **Graceful Shutdown** - Clean stop in ~7 seconds with real-time progress tracking
...
#### Option A: Conversation Extension

**Prepare Base Data** (`base_data.jsonl`):
```jsonl
...
config = (ConversationExtensionConfigBuilder()
    .add_provider('user_followup', 'ultrasafe', api_key)
    .add_provider('assistant_response', 'ultrasafe', api_key)
...
#### Option B: Text Enhancement

**Prepare Input** (`input.jsonl`):
```jsonl
...
config = (TextEnhancementConfigBuilder()
    .add_provider('text_enhancement', 'ultrasafe', api_key)
    .set_generation(num_texts=1000, parallel_workers=100)
