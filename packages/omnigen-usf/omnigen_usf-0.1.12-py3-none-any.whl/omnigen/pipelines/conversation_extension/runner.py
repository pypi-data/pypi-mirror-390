...
from omnigen.monitoring.mongodb import MongoDBMonitor
...
class Runner:
    ...
    def _generate_parallel(self, num_conversations: int, num_workers: int):
        ...
        pbar = tqdm(
            ...
            ncols=140,         # Fixed width for consistency
        ...
        pbar.set_postfix_str(
            f"✓{complete} ⚠{partial} ✗{failed} ~{skipped} 🔍{filtered} | Workers:0/{num_workers} {self._get_rate_limit_metric()}"
        )
                ...
                for future in done_futures:
                    ...
                    try:
                        ...
                        if result is None:
                            filtered += 1
                            pbar.update(1)
                            active_workers = len(futures) - 1  # Exclude current completed future
                            pbar.set_postfix_str(
                                f"✓{complete} ⚠{partial} ✗{failed} ~{skipped} 🔍{filtered} | Workers:{active_workers}/{num_workers} {self._get_rate_limit_metric()}"
                            )
                        ...
                        pbar.update(1)
                        pbar.set_postfix_str(
                            f"✓{complete} ⚠{partial} ✗{failed} ~{skipped} 🔍{filtered} | Workers:{active_workers}/{num_workers} {metric_str}"
                        )
