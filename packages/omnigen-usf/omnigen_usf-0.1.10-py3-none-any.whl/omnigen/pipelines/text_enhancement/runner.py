...
class Runner:
    ...
    def _generate_parallel(self, num_texts: int, num_workers: int):
        ...
        pbar.set_postfix_str(
            f"✓{complete} ⚠{partial} ✗{failed} ~{skipped} | Workers:0/{num_workers} {self._get_rate_limit_metric()}"
        )
            ...
            while futures or submitted < num_texts:
                ...
                for future in done_futures:
                    ...
                    try:
                        ...
                        pbar.update(1)
                        pbar.set_postfix_str(
                            f"✓{complete} ⚠{partial} ✗{failed} ~{skipped} | Workers:{active_workers}/{num_workers} {metric_str}"
                        )
