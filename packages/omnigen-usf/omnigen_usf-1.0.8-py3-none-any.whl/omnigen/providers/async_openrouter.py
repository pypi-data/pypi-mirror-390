...
class AsyncOpenRouterProvider(BaseLLMProvider):
    """
    Async OpenRouter provider (OpenAI-compatible API).
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            if not OPENAI_AVAILABLE:
                raise ImportError("openai and httpx required for async OpenRouter provider")
            ...
            if not self.api_key:
                raise AuthenticationError("OpenRouter API key required")
            ...
            try:
                ...
                logger.info(f"Async OpenRouter provider initialized: {self.model}")
            except Exception as e:
                raise AuthenticationError(f"Failed to initialize async OpenRouter: {e}")
                
        except Exception as e:
            logger.critical(f"Critical error initializing async OpenRouter: {e}", exc_info=True)
            raise ProviderError(f"Failed to initialize async OpenRouter: {e}")
