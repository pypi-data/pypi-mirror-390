...
class WrmEnv(BaseEnv):
    ...
    def __init__(self,
        ...
        if checkparams:
            ...
            # 校验脚本输入参数, 分类存储
            # self.login     保存登录方式，{'ip':'', 'port':'', 'username':'','password':''}
        ...
        for login_param in self.login_list:
            ...
            if proxy_list:
                ...
                for proxy_param in proxy_list:
                    ...
                    encodestr_pwd = b64encode(pwd.encode('utf-8')).decode()
                    self.log('  password: ' + str(encodestr_pwd), tasklog=False)
            socks5_proxy = login_param.get('socks5Proxy')
            if socks5_proxy:
                self.log('Login proxy socks5 parameter: ', tasklog=False)
                self.log('  ip: ' + str(socks5_proxy['ip']), tasklog=False)
                self.log('  port: ' + str(socks5_proxy['port']), tasklog=False)
                self.log('  username: ' + str(socks5_proxy.get('username', "")), tasklog=False)
                pwd = str(socks5_proxy.get('password', ""))
                encodestr_pwd = b64encode(pwd.encode('utf-8')).decode()
                self.log('  password: ' + str(encodestr_pwd), tasklog=False)
        ...
        if self.parameter:
            ...
            for key, val in self.parameter.items():
                self.log(f'  {key}: {val}', tasklog=False)
        else:
    ...
    def verify_script_define_login_socks5_param(self, socks5_param):
        """
        验证脚本中预定义登录参数中SockS5代理服务参数格式是否正确
        """
        # for key in ["ip", "port", "username", "password"]:
        for key in ["ip", "port"]:
            self._check_param_define("upParaStruct.login.socks5Proxy", socks5_param, key)
            self._check_param_define(f"upParaStruct.login.socks5Proxy.{key}", socks5_param[key], "required", ["Y"])
            if socks5_param[key].get('values', False):
                self.verify_script_define_param_values(f"upParaStruct.login.socks5Proxy.{key}.values", socks5_param[key]['values'])
        for key in ["username", "password"]:
            self._check_param_define("upParaStruct.login.socks5Proxy", socks5_param, key)
            self._check_param_define(f"upParaStruct.login.socks5Proxy.{key}", socks5_param[key], "required", ["Y", "N"])
            if socks5_param[key].get('values', False):
                self.verify_script_define_param_values(f"upParaStruct.login.socks5Proxy.{key}.values", socks5_param[key]['values'])
    ...
    def verify_script_define_login_param(self, login_param):
        ...
        host_role = login_param.get("role")
        if host_role:
        ...
        # SockS5 代理参数
        socks5_param = login_param.get("socks5Proxy")
        if socks5_param:
            self._check_login_socks5_param(socks5_param)

    def verify_script_define_param(self):
        """
        验证脚本中预定义参数格式是否正确
        参数为json格式的字符串, 包括:
          upParaStruct: <login>/[other parameter]/[scriptTaskId]
            - 必须参数login保存登录信息
        ...
        login_param = up_param.get('login') or up_param.get('loginWinRM')
    ...
    def _check_login_socks5_param(self, socks5_param):
        """
        验证脚本参数中登录参数中 SockS5代理服务参数 信息是否完整
        """
        # for key in ["ip", "port", "username", "password"]:
        for key in ["ip", "port"]:
            self._check_param_define("upParaStruct.login.socks5Proxy", socks5_param, key)
    ...
    def check_script_input_param(self):
        ...
        # 使用参数之前需先通过格式较验
        login_param = paramjson.get('login') or paramjson.get('loginWinRM')
        if not login_param:
