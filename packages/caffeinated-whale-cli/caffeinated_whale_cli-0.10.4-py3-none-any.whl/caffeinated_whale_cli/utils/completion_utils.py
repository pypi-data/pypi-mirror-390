...
def complete_project_names(incomplete: str) -> list[str]:
    ...
    Args:
        incomplete: Partial string being completed.
...
def complete_app_names(ctx: typer.Context, incomplete: str) -> list[str]:
    """
    ...
    """
    project_name = ctx.obj.get("project_name") if ctx.obj else None
    if not project_name:
        return []
    ...
    try:
        ...
        filtered_result = [app_name for app_name in result if app_name.startswith(incomplete)]

        # Exclude already chosen apps if the option allows multiple values
        # The 'app' parameter in ctx.params will be a list if multiple values are provided
        already_chosen_apps: list[str] = ctx.params.get("app") or []
        final_result = [
            app_name for app_name in filtered_result if app_name not in already_chosen_apps
        ]

        _set_cached(cache_key, final_result)
        return final_result
    except Exception:
        return []


def complete_site_names(ctx: typer.Context, incomplete: str) -> list[str]:
    """
