...
# Заменено для совместимости с mawo_pymorphy3
# import pymorphy2
# from utils import assert_parse_is_correct


def assert_parse_is_correct(parse_results, word, expected_normal_form, expected_tag):
    """
    Проверяет, что в результатах parse есть разбор с нужной нормальной формой и тегом.
    """
    assert len(parse_results) > 0, f"No parse results for word '{word}'"

    # Разбираем ожидаемый тег
    tag_parts = expected_tag.split()
    expected_pos = tag_parts[0].split(",")[0]
    expected_grammemes = set()
    for part in tag_parts:
        expected_grammemes.update(part.split(","))
    expected_grammemes.discard(expected_pos)

    # Ищем подходящий разбор
    for p in parse_results:
        if p.normal_form == expected_normal_form:
            if p.tag.POS == expected_pos:
                # Проверяем граммемы
                parse_grammemes = p.tag.grammemes
                if expected_grammemes.issubset(parse_grammemes):
                    return  # Нашли подходящий разбор

    # Если не нашли, формируем информативное сообщение
    actual_parses = [f"{p.normal_form} {p.tag}" for p in parse_results[:3]]
    raise AssertionError(
        f"No matching parse for '{word}'.\n"
        f"Expected: {expected_normal_form} {expected_tag}\n"
        f"Actual parses: {actual_parses}"
    )
