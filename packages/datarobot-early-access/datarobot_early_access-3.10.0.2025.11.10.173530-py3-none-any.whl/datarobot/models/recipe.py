...
class Recipe(APIObject, HumanReadable):
    ...
    def update(
        ...
        inputs: List[JDBCTableDataSourceInput | RecipeDatasetInput] | None = None,
        operations: List[WranglingOperation] | None = None,
        downsampling: DownsamplingOperation | None | object = NOT_SET,
        **kwargs: Any,
    ) -> None:
        """Update the recipe.
        ...
        downsampling:
            The new downsampling
             or None if you wouldn't like to apply any final downsampling on publishing.
        """
        kwargs.update({'name': name, 'description': description, 'sql': sql, 'recipe_type': recipe_type})
        recipe_fields = {k: v for k, v in kwargs.items() if v is not None}
        new_recipe = None

        if recipe_fields:
            new_recipe = self.set_recipe_metadata(self.id, metadata=recipe_fields)
        ...
        if downsampling is not NOT_SET:
            new_recipe = self.update_downsampling(self.id, cast(DownsamplingOperation, downsampling))
    ...
    @classmethod
    def update_downsampling(cls, recipe_id: str, downsampling: DownsamplingOperation | None) -> Recipe:
