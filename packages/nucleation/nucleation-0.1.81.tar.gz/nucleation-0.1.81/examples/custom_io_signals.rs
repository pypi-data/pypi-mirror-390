...
fn main() {
    ...
    println!("Created schematic with dimensions: {:?}", schematic.get_dimensions());
    ...
    let mut world = MchprsWorld::with_options(schematic, options)
        .expect("Failed to create simulation world");
    ...
    println!("Signal at (3,1,0): {}", world.get_signal_strength(probe_pos1));
    println!("Signal at (6,1,0): {}", world.get_signal_strength(probe_pos2));
