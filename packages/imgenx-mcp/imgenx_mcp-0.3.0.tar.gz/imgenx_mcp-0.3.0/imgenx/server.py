...
def upload_to_oss(file_path: str, business_dir: str = 'images') -> Dict[str, str]:
    ...
    Returns:
        Dict[str, str]: 包含 object_key（存储路径）、oss_url（OSS地址）、cdn_url（CDN加速地址）
    '''
    try:
        oss_service = get_oss_service()
        result = oss_service.upload_file(file_path, business_dir=business_dir)
    except Exception as e:
...
def download_and_upload_to_oss(url: str, business_dir: str = 'images') -> Dict[str, str]:
    ...
    try:
        ...
        raise ToolError(f'下载并上传到 OSS 失败: {e}')
