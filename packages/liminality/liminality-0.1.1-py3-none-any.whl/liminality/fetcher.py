...
class Fetcher:
    ...
    def _parse_time(self, time_str: str | int | datetime) -> int:
        ...
        try:
            ...
            raise LiminalError(f"Invalid time format: {time_str}. Use ISO format (YYYY-MM-DD) or datetime object") from e
            ...
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    if self.show_progress:
                        tqdm.write(f"[Fetcher] Request failed, retrying in {delay}s... ({attempt + 1}/{self.max_retries})")
                    time.sleep(delay)
                else:
                    raise LiminalError(f"Failed after {self.max_retries} attempts: {e}") from e
    ...
    def fetch_klines(
        ...
        if self.show_progress and num_requests > 1:
            progress_bar = tqdm(
                total=num_requests,
                desc=f"Fetching {symbol} {interval}",
                unit="req"
            )
    ...
    def fetch_multi_klines(
        ...
        if self.show_progress:
            progress_bar = tqdm(
                total=len(symbols),
                desc="Fetching multiple symbols",
                unit="symbol"
            )
    ...
    def get_available_date_range(self, symbol: str, interval: str) -> tuple[datetime, datetime]:
        """
