...
class Signal(BaseSignal):
    ...
    def connect(self, receiver, sender=None, weak=True, dispatch_uid=None, priority=50):
        ...
        super(Signal, self).connect(
            receiver, sender=sender, weak=weak, dispatch_uid=inner_uid
        )
        self.receivers.sort()
    ...
    if signal_name in REGISTERED_SERVICE_SIGNALS:
        if signal.is_signal_registered():
            raise AttributeError(
                f"Signal name {signal_name} clashes with already existing "
                f"signal name {REGISTERED_SERVICE_SIGNALS[signal_name]} and can't be registered."
            )
        else:
            signal.register_signal(providing_args)
    else:
        REGISTERED_SERVICE_SIGNALS[signal_name] = RegisteredServiceSignal(
            providing_args
        )
...
def __raise_unregistered_signal_exception(signal_name):
    raise ValueError(
        f"Signal for name {signal_name} not found. Was it registered?"
    ...
    def decorator(func):
        ...
        def wrapper_propagate_signal(*args, **kwargs):
            ...
            cls_, func_args = (
                args[0],
                args[1:],
            )  # First element of args is self/cls. Moved to separate variable.
            signal_call_args = {
                "cls_": cls_,
                "data": [func_args, kwargs],
                "context": context,
            }
