...
class CustomFilterWizardStorage:
    ...
    @classmethod
    def build_custom_filters_definition(
        cls, module_name: str, object_type: str, **kwargs
    ) -> List[namedtuple]:
        """
        ...
        if module_name in registered_filter_wizards:
            for registered_filter_wizard in registered_filter_wizards[module_name]:
                if cls.__KEY_FOR_OBTAINING_CLASS not in registered_filter_wizard:
                    continue
                wizard_filter_class = cls.__create_instance_of_wizard_class(
                    registered_filter_wizard
                )
                if not cls.__check_object_type(wizard_filter_class, object_type):
                    continue
                output_of_possible_filters.extend(
                    cls.__run_load_definition_object_in_wizard(
                        wizard_filter_class, **kwargs
                    )
                )
        ...
        **kwargs
        ...
        if module_name in registered_filter_wizards:
            for registered_filter_wizard in registered_filter_wizards[module_name]:
                ...
                if not cls.__check_object_type(wizard_filter_class, object_type):
                    continue
                query = wizard_filter_class.apply_filter_to_queryset(
                    custom_filters, query, relation
                )
        return query

    @classmethod
    def __run_load_definition_object_in_wizard(
        cls, wizard_filter_class: CustomFilterWizardInterface, **kwargs
    ) -> List[namedtuple]:
        wizard_filter_tuple_type = namedtuple(
            wizard_filter_class.get_type_of_object(),
            [cls.__FIELD, cls.__FILTER, cls.__TYPE],
        )
        tuple_list_result = wizard_filter_class.load_definition(
            wizard_filter_tuple_type, **kwargs
        )
        return tuple_list_result

    @classmethod
    def __create_instance_of_wizard_class(
        cls, registered_filter_wizard: dict
    ) -> CustomFilterWizardInterface:
        return registered_filter_wizard[cls.__KEY_FOR_OBTAINING_CLASS]()

    @classmethod
    def __check_object_type(
        cls, wizard_filter_class: CustomFilterWizardInterface, object_type: str
    ) -> bool:
