"""
Econ Benefit Calculation Connected Module (Cleaned Version)

This module calculates benefit scores for road segments based on fuzzy clustering membership values,
optimized for connected routing with economic considerations.
...
- Includes routing scores for connected path optimization
- Ranks segments for optimal sensor placement with connectivity
...
def calculate_routing_scores(all_segments, k=5):
    """
    Calculate a routing-friendly score for each segment and add it as 'routing_score'.
    The score = segment length + mean distance to k nearest segments.
    
    This is used for economic routing optimization to favor segments that:
    1. Are longer (more coverage)
    2. Are close to other high-value segments (connectivity)
        
    Modifies the segments in-place.
        
    Args:
        all_segments (list of dict): List of segments from analyze_segment_information().
        k (int): Number of nearest neighbors to consider for proximity.
    """
    from scipy.spatial import distance
    import numpy as np

    if not all_segments or len(all_segments) < 2:
        # Not enough segments to compute routing score
        return

    # Step 1: Compute center coordinates for each segment
    centers = []
    for seg in all_segments:
        start = np.array(seg["segment_start_coordinate"])
        end = np.array(seg["segment_end_coordinate"])
        center = ((start + end) / 2).tolist()
        seg["segment_center"] = center
        centers.append(center)

    centers = np.array(centers)

    # Step 2: Compute pairwise distances between centers
    dists = distance.cdist(centers, centers)

    # Step 3: Assign routing score
    for i, seg in enumerate(all_segments):
        dists[i, i] = np.inf  # avoid self in nearest neighbor search
        nearest_indices = np.argsort(dists[i])[:k]
        mean_neighbor_distance = float(np.mean(dists[i][nearest_indices]))
        seg_len = seg.get("segment_length_in_m", 0.0)
        seg["routing_score"] = seg_len + mean_neighbor_distance
...
def analyze_segment_information(structured_data):
    ...
    This is the main analysis function for the econ version that:
    ...
    5. Includes all segments (no filtering) for economic routing
    6. Adds routing scores for connectivity optimization
    7. Tracks visited point coordinates for route validation
    
    Args:
        structured_data: Dictionary of roads with segments and points
        
    Returns:
        list: All analyzed segments with metrics attached (including routing_score)
    """
    all_segments = []
    road_types = [
        "primary",
    ...
    for road_id, road_data in structured_data.items():
        ...
        for segment_id, segment_data in road_data['segments'].items():
            ...
            #     continue
            if segment_data['number_of_points'] != 0:
                for i in range(1,segment_data['number_of_points']+1):
                    visited_point_coordinates.append(segment_data['points'][str(i)]['point_coordinates'])
            segment_info = {
                "segment_start_id": segment_data["segment_start_id"],
                ...
                "number_of_visited_points": segment_data['number_of_points'],
                'visited_point_coordinates': visited_point_coordinates,
    ...
    #all_segments = sorted(all_segments, key=lambda x: x['number_of_visited_points'], reverse=True)
    calculate_routing_scores(all_segments)
    # Sort segments by routing score
    # all_segments = sorted(all_segments, key=lambda x: x['routing_score'], reverse=True)
    return all_segments
...
def benefit_calculation(working_directory, lower_benefit_limit,number_of_classes,benefit_type,route_type):
    """
    Main benefit calculation pipeline for economic routing optimization.
    
    This function:
    1. Loads econ_pm_output.json data with FCM membership values
    ...
    6. Calculates routing scores for connectivity
    7. Ranks and outputs segments for economic route planning
    
    Key difference from standard benefit_calculation:
    - Reads from econ_pm_output.json (connected components only)
    - Writes to econ_bc_* files
    - Includes routing_score for path optimization
    - No score filtering (keeps all segments for routing)
    
    Args:
        working_directory: Path to work directory with transient data
        lower_benefit_limit: Minimum benefit threshold (not used in econ version)
        number_of_classes: Number of FCM clusters (e.g., 15)
        benefit_type: 't' for total, 'm' for max
        route_type: 'g' for good (sort by routing_score), 'b' for bad (sort by info_weight)
    ...
    # Read from econ_pm_output.json (output from econ_point_mapping_connected.py)
    read_file_path = os.path.join(transient_dir, "econ_pm_output.json")
    
    # Write to econ_bc_* files
    benefits_path = os.path.join(transient_dir, "econ_bc_benefits_output.json")
    top_benefits_path = os.path.join(transient_dir, "econ_bc_top_benefits_output.json")
    ...
    if DEBUG:
        with open(os.path.join(debug_path,'econ_bc_benefits_output_tabbed.json'), 'w') as f:
            json.dump(structured_data, f, indent=4)
    
    
    # Prepare data for top roads output
  
    
    if route_type == 'g':
        # Descending order
        all_data_sorted=sorted(top_segments_by_class, key=lambda x: x['routing_score'])
    ...
    if DEBUG:
        with open(os.path.join(debug_path,'econ_bc_top_benefits_output_tabbed.json'), 'w') as f:
