...
# Constants
# MAX_DISTANCE = 50
optimization_objective = 'd'  # 'i' for information, 'd' for distance, 't' for time
...
def build_information_grid(structured_data,route_type, cell_size=10):
...
def astar_generator(road_network, start_id, end_id, info_grid, route_type,optimization_objective, heuristic=calculate_distance, weight='distance', timeout=9999, cell_size=10):
...
def find_critical_segments(segments):
    point_to_segments = {}  # Maps each point to the list of segments that cover it

    for i, seg in enumerate(segments):
        for pt in map(tuple, seg['visited_point_coordinates']):
            if pt not in point_to_segments:
                point_to_segments[pt] = []
            point_to_segments[pt].append(i)

    # Points covered by only one segment → that segment is critical
    critical_segment_indices = set()
    for pt, seg_list in point_to_segments.items():
        if len(seg_list) == 1:
            critical_segment_indices.add(seg_list[0])

    return list(critical_segment_indices)
    

from collections import defaultdict

def select_segments(segments, points,critical_segments):
    """
    Select segments with the lowest routing scores to cover all given points.

    segments: list of dicts, each with 'visited_point_coordinates' and 'routing_score'
    points: list of all target points to cover (each point as [x, y] or tuple)
    """
    all_points = set(map(tuple, points))
    covered_points = set()
    selected_segments = {}
    all_points_covered = False
    # Add critical segments first
    for seg_idx in critical_segments:
        seg = segments[seg_idx]
        seg_points = set(map(tuple, seg['visited_point_coordinates']))
        if not seg_points - covered_points:
            continue
        selected_segments[seg['segment_start_id'], seg['segment_end_id']] = seg
        
        covered_points.update(seg_points)
        if len(covered_points) >= len(all_points):
            all_points_covered = True
            break
    # Sort segments by ascending routing score
    sorted_segments = sorted(segments, key=lambda s: s.get("routing_score", float("inf")))

    if not all_points_covered:
        for seg in sorted_segments:
            seg_points = set(map(tuple, seg['visited_point_coordinates']))
            if not seg_points - covered_points:
                continue  # This segment adds nothing new
            selected_segments[seg['segment_start_id'], seg['segment_end_id']] = seg
            covered_points.update(seg_points)
            if len(covered_points) >= len(all_points):
                break
    ...
    # Return best effort if can't cover all points (instead of None)
    if len(covered_points) >= len(all_points):
        if DEBUG:
            print(f"✓ All {len(all_points)} points covered by {len(selected_segments)} segments")
        return selected_segments
    else:
        if DEBUG:
            print(f"⚠ Warning: Only {len(covered_points)}/{len(all_points)} points covered by {len(selected_segments)} segments")
            print(f"  Uncovered points: {len(all_points - covered_points)}")
        # Return what we have instead of None to avoid crashes
        return selected_segments if selected_segments else {}


# def select_segments(segments,points):
#     """
#     segments: list of segment dicts, each with 'visited_point_coordinates' key (list of points)
#     critical_segments: list of segment indices that must be included first
#     taboo_segments: list of segment indices to avoid
#     points: list of all points to cover (each point as [x,y] or tuple)
#     """
#     critical_segments= find_critical_segments(segments)
#     covered_points = set()
#     selected_segments = critical_segments.copy()
    
    
#     # Add points covered by critical segments first
#     for seg_idx in selected_segments:
#         covered_points.update(map(tuple, segments[seg_idx]['visited_point_coordinates']))

#     all_points = set(map(tuple, points))
#     if covered_points >= all_points:
#         return selected_segments

#     available = [i for i in range(len(segments)) if i not in selected_segments]
    
#     while available:
#         # Select segment covering most new uncovered points
#         next_seg = min(
#             available,
#             key=lambda i: len(set(map(tuple, segments[i]['routing_score'])) - covered_points)
#         )
#         selected_segments.append(next_seg)
#         covered_points.update(map(tuple, segments[next_seg]['visited_point_coordinates']))
#         available.remove(next_seg)
...
def get_segment_dict(segments, selected_indices):
    segment_dict = {}
    for seg_idx in selected_indices:
        segment = segments[seg_idx]
        key = (segment['segment_start_id'], segment['segment_end_id'])
...
def calculate_paths(selected_segments, road_network, problematic_segments, no_path_count, no_path_count_per_id,
                total_number_of_classes, is_calculated, calculated_path, calculated_distance,
                calculated_info_weight, calculated_time, route_type, info_grid,
                optimization_objective, segment_scores, failed_connections,
                already_reported, critical_segments):
    ...
    for (start_id, end_id), start_data in selected_segments.items():
        if (start_id, end_id) not in no_path_count_per_id:
        ...
        start_segment_route_score = start_data["routing_score"]
        start_segment_visited_points = start_data["visited_point_coordinates"]
        j = 1
        for (end_start_id, end_end_id), end_data in selected_segments.items():
            if (end_start_id, end_end_id) not in no_path_count_per_id:
                no_path_count_per_id[(end_start_id, end_end_id)] = 0

            if (start_class_id == end_data["class"] and
                start_id == end_start_id and end_id == end_end_id):
                j += 1
                continue
            ...
            end_segment_route_score = end_data["routing_score"]
            end_segment_visited_points = end_data["visited_point_coordinates"]

            shortest_path, s, e = find_shortest_path_with_fallback(
                road_network, start_nodes, end_nodes,
                is_calculated, calculated_path, calculated_distance,
                calculated_info_weight, calculated_time,
                route_type, info_grid, optimization_objective
            )

            current_start_pair = (start_id, end_id)
            current_end_pair = (end_start_id, end_end_id)

            if not shortest_path:
                is_wend = False
                no_path = True
                no_path_count += 1
                # if current_start_pair not in critical_segments.values() and current_end_pair not in critical_segments.values():
                    # if current_start_pair not in already_reported:
                    #     already_reported.add(current_start_pair)
                    #     print(f"Abort for segment: {current_start_pair}, i={i}, j={j}")
                    #     print(f"Segment score: {segment_scores[current_start_pair]}")
                    #     print(f"Failed connections from {start_id}: {failed_connections[start_id]}")
                    # if current_end_pair not in already_reported:
                    #     already_reported.add(current_end_pair)
                    #     print(f"Abort for segment: {current_end_pair}, i={i}, j={j}")
                    #     print(f"Segment score: {segment_scores[current_end_pair]}")
                    #     print(f"Failed connections from {end_start_id}: {failed_connections[end_start_id]}")
                if current_start_pair not in problematic_segments or current_end_pair not in problematic_segments:
                    failed_connections[start_id].add(end_id)
                    segment_scores[current_start_pair] -= 1
                    segment_scores[current_end_pair] -= 1

                    if DEBUG:
                        print(f"DEBUG: Current score for {current_start_pair}: {segment_scores[current_start_pair]}")
                        print(f"DEBUG: Current score for {current_end_pair}: {segment_scores[current_end_pair]}")
                    if segment_scores[current_start_pair] <= -2:
                        if start_data['segment_id'] in critical_segments:
                            segment_to_add = (end_start_id, end_end_id)
                            if DEBUG:
                                print(f"[CRITICAL] Protected start segment, replacing end segment instead")
                        
                        elif end_data['segment_id'] in critical_segments:   
                            segment_to_add = (start_id, end_id)
                            if DEBUG:
                                print(f"[CRITICAL] Protected end segment, replacing start segment instead")
                    elif end_data['segment_id'] in critical_segments:
                        segment_to_add = (start_id, end_id)
                        if DEBUG:
                            print(f"[CRITICAL] Protected end segment, replacing start segment instead")
                        ...
                        segment_to_add = (end_start_id, end_end_id)
                    problematic_segments.append(segment_to_add)
                no_path_count_per_id[segment_to_add] += 1
                        
                        
                        
                        
                # if current_pair not in problematic_segments.values():
                #     no_path_count += 1
                #     failed_connections[start_id].add(end_id)
                #     segment_scores[current_pair] -= 1

                #     print(f"DEBUG: Current score for {current_pair}: {segment_scores[current_pair]}")

                #     if current_pair not in already_reported or segment_scores[current_pair] <= -2:
                #         print(f"Abort for segment: {current_pair}, i={i}, j={j}")
                #         print(f"Segment score: {segment_scores[current_pair]}")
                #         print(f"Failed connections from {start_id}: {failed_connections[start_id]}")
                #         already_reported.add(current_pair)

                #     if segment_scores[current_pair] <= -2:
                #         if start_data['segment_id'] in critical_segments:
                #             segment_to_add = (end_start_id, end_end_id)
                #             print(f"[CRITICAL] Protected start segment, replacing end segment instead")
                #         elif end_data['segment_id'] in critical_segments:
                ...
                #             print(f"[CRITICAL] Protected end segment, replacing start segment instead")
                #         else:
                #             segment_to_add = (start_id, end_id) if start_segment_route_score >= end_segment_route_score else (end_start_id, end_end_id)
                #     if segment_to_add and segment_to_add not in problematic_segments.values():
                #         problematic_segments[no_path_count] = segment_to_add
                #         no_path_count_per_id[segment_to_add] += 1
                #         already_reported.discard(current_pair)
                #         print(f"[!] Segment moved to problematic_segments: {segment_to_add}")

                abort_time = time.time()
                if DEBUG:
                    print(f"Abort time: {abort_time - individual_path_time_start:.2f} seconds")
                    print(f"Path not found, i={i}, j={j}")
                    print(f"Segment to replace: {segment_to_add}")
                    print(f"Problematic_segments: {problematic_segments}")
                return (all_paths, problematic_segments, is_wend, no_path_count, calculated_paths,
                calculated_id_data, no_path_count_per_id, is_calculated, calculated_path,
                calculated_distance, calculated_info_weight, calculated_time,
                segment_scores, failed_connections, already_reported)
                

            actual_start_id = start_id if s == 0 else end_id
            actual_end_id = end_start_id if e == 0 else end_end_id

            path_distance, path_time_cost, path_information_weight, path_information_value = calculate_path_metrics(
                road_network, shortest_path)
            ...
            if actual_start_id not in calculated_ids:
                ...
                calculated_id_data[actual_start_id] = copy.deepcopy(start_data)
                calculated_id_data[actual_start_id]['used'] = 'start' if actual_start_id == start_data['segment_start_id'] else 'end'

            if actual_end_id not in calculated_ids:
                calculated_ids.append(actual_end_id)
                calculated_id_data[actual_end_id] = copy.deepcopy(end_data)
                calculated_id_data[actual_end_id]['used'] = 'start' if actual_end_id == end_data['segment_start_id'] else 'end'

            calculated_paths.append((actual_start_id, actual_end_id))
...
#     print('selected segments',len(selected_segments))
#     i=1
#     # Initialize outside loop if not done yet:
#     segment_scores = defaultdict(int)  # defaults to 0
#     failed_connections = defaultdict(set)
#     already_reported = set()  # To avoid repeated print spam per failure pair
#     for (start_id, end_id), start_data in selected_segments.items():
...
#             # Define this once, outside of the main loop (e.g., at the start of your path-finding function)
#             # segment_scores = defaultdict(int)  
#             # failed_connections = defaultdict(set)
#             # already_reported = set()

#             # Then inside your loop, when path fails:
#             current_pair = (start_id, end_id)

#             # Make sure this block does NOT reinitialize segment_scores
...
#                     segment_scores[current_pair] -= 1  # decrement score, persists over calls
...
#                     if segment_scores[current_pair] <= -2:
#                         problematic_segments[no_path_count] = current_pair
#                         already_reported.discard(current_pair)
#                         print(f"[!] Segment moved to problematic_segments: {current_pair}")
...
def path_finding(working_directory,segment_number_per_class, total_number_of_classes,route_type,optimization_objective):
    ...
    roads_data_file = os.path.join(transient_dir, "econ_bc_benefits_output.json")
    benefits_data_file = os.path.join(transient_dir, "econ_bc_top_benefits_output.json")
    ...
    points_file_path = os.path.join(input_dir, "points.csv")
    # segments_file_path = os.path.join(input_dir, "segments.json")
    # segments_data = load_data(segments_file_path)
    segment_point_list = []
    # segments_data = segments_data[1:]
    for segment in benefits_data:
        segment_info=segment.copy()
        segment_point_list.append(copy.deepcopy(segment_info))
    osm_file=glob.glob(os.path.join(input_dir, '*.geojson'))
    with open(points_file_path, "r") as f:
        reader = csv.reader(f)
        next(reader)  # Skip the first row
        points_data = [[float(row[0]), float(row[1])] for row in reader]
    osm_file=osm_file[0]
    ...
    failed_connections = defaultdict(set)
    already_reported = set()
    critical_segments = find_critical_segments(segment_point_list)
    selected_segments = select_segments(segment_point_list,points_data,critical_segments)
    #selected_segments = get_segment_dict(segment_point_list,selected_segments)
        
    #selected_segments = hull_construction(segments_hull_lookup,selected_segments, benefit_type)
        
    

    critical_segments = list(selected_segments.values())[0:len(critical_segments)]
    ...
    while True:
        ...
        # Then call calculate_paths like this:
        result=calculate_paths(
            selected_segments, road_network, problematic_segments, no_path_count, no_path_count_per_id,
            total_number_of_classes, is_calculated, calculated_path, calculated_distance,
            calculated_info_weight, calculated_time, route_type, info_grid, optimization_objective,
            segment_scores=segment_scores, failed_connections=failed_connections, already_reported=already_reported, critical_segments=critical_segments)


        (all_paths, problematic_segments, is_wend, no_path_count, calculated_paths, calculated_id_data,
         no_path_count_per_id, is_calculated, calculated_path, calculated_distance,
         calculated_info_weight, calculated_time, segment_scores, failed_connections, already_reported) = result
        ...
        replacement_segment=find_replacement_segment_by_routing_score(
            segment_point_list,segment_to_replace_key=problematic_segments[-1],critical_segments=critical_segments,problematic_segments=problematic_segments
        )
        replacement_segment_key = (replacement_segment['segment_start_id'], replacement_segment['segment_end_id'])
        selected_segments.pop(problematic_segments[-1])
        selected_segments[replacement_segment_key] = replacement_segment
    # all_paths.append(selected_segments)
    write_path = os.path.join(transient_dir, "econ_pf_output.json")
    write_output(write_path, all_paths)
    if DEBUG:
        debug_path = os.path.join(transient_dir, "debug")
        with open(os.path.join(debug_path,'econ_pf_output_tabbed.json'), 'w') as f:
...
def find_replacement_segment_by_routing_score(segments, segment_to_replace_key, critical_segments, problematic_segments):
    """
    Find a replacement segment for the one being replaced, using routing_score from segment data.

    Args:
        segments (dict): Mapping from (start_id, end_id) to segment data.
        segment_to_replace_key (tuple): (start_id, end_id) of the segment to be replaced.
        critical_segments (set): Set of segment keys (start_id, end_id) that are critical.
        problematic_segments (dict): Mapping of index → segment key (start_id, end_id) for problematic segments.

    Returns:
        tuple or None: Replacement segment key (start_id, end_id), or None if no suitable found.
    """
    # if segment_to_replace_key not in segments.values():
    #     print(f"[WARN] Segment {segment_to_replace_key} not found in current segments")
    #     return None
    critical_segments_set_by_ids = []
    #original_points = set(map(tuple, segments[["segment_start_id"]=segment_to_replace_key]['visited_point_coordinates']))
    for seg in segments:
        if seg['segment_start_id'] == segment_to_replace_key[0] and seg['segment_end_id'] == segment_to_replace_key[1]:
            original_points = set(map(tuple, seg['visited_point_coordinates']))
            break
    for seg in critical_segments:
        critical_segments_set_by_ids.append((seg['segment_start_id'], seg['segment_end_id']))
        #critical_segments_set_by_ids= set(map(tuple, critical_segments[]))
    # Build exclusion set
    excluded_segments = set(critical_segments_set_by_ids).union(problematic_segments)

    # Filter and rank candidate segments
    candidates = []
    for seg in segments:
        key = ((seg['segment_start_id'], seg['segment_end_id']))
        #key = ((10291845529, 2443443119))
        if key in excluded_segments or key == segment_to_replace_key:
            continue
        covered_points = set(map(tuple, seg['visited_point_coordinates']))
        if covered_points == original_points:
            print(f"[INFO] Exact match found for {segment_to_replace_key}.")
        if original_points.issubset(covered_points):
            score = seg.get('routing_score', 0)
            candidates.append((score, key))

    if not candidates:
        print(f"[INFO] No full-cover candidate for {segment_to_replace_key}. Trying partial matches.")

        # Fall back to partial match (some shared points)
        for key, seg in segments.items():
            if key in excluded_segments or key == segment_to_replace_key:
                continue
            covered_points = set(map(tuple, seg['visited_point_coordinates']))
            if original_points & covered_points:
                score = seg.get('routing_score', 0)
                candidates.append((score, key))
    ...
    # Sort candidates by routing_score descending
    candidates.sort(reverse=True)
    best_score, best_key = candidates[0]
    for seg in segments:
        if seg['segment_start_id'] == best_key[0] and seg['segment_end_id'] == best_key[1]:
            best_segment = seg
            break
    # best_segment = segments[best_key]
    print(f"[REPLACEMENT] Selected {best_key} with score {best_score} to replace {segment_to_replace_key}")
    return best_segment
...
if __name__ == "__main__":
    path_finding(
        ...
        segment_number_per_class=10, #This is the number of segments per class
        ...
        optimization_objective='d'  #This is the optimization objective, 'd' for distance, 't' for time, 'i' for information weight
        )
