    ...
    def run(self):
        ...
        for i in range(self.n_iterations):
            ...
            if short_check:
                ...
                shortest_path = ([int(item) if isinstance(item, (np.int64, float)) else item for item in shortest_path[0]], shortest_path[1])
                if DEBUG:
                    print(i)
                    print(shortest_path)
...
def extract_unique_main_segment_coords(path_matrix_index):
    """
    Extracts the first and last two coordinates for each unique index in path_matrix_index.
    Returns a dictionary: {(i, j): {"first_two": [...], "last_two": [...]}, ...}
    """
    result = {}
    found_keys=[]
    all_coords=[]
    for key, coords in path_matrix_index.items():
        if not coords or len(coords) < 2:
            continue
        start_key=key[0]
        end_key = key[1]
        # Get first two and last two coordinates
        first_two = coords[:2] if len(coords) >= 2 else coords
        last_two = coords[-2:] if len(coords) >= 2 else coords
        rev_first_two = first_two[::-1]
        rev_last_two = last_two[::-1]
        if start_key not in found_keys:
            found_keys.append(start_key)
            if first_two not in all_coords and rev_first_two not in all_coords:
                all_coords.append(first_two)
        if end_key not in found_keys:
            found_keys.append(end_key)
            if last_two not in all_coords and rev_last_two not in all_coords:
                all_coords.append(last_two)
...
def route_finding(working_directory, number_of_classes, time_limit, optimization_objective, max_iter, ant_no,segments_number_per_class, benefit_type, route_type):
    ...
    network_file_path = os.path.join(transient_dir, "econ_pf_output.json")
    ...
    alpha = 0.5 # Pheromone factor, >=0, default=1, best so far=0.35
    ...
    slow_coords=extract_unique_main_segment_coords(path_matrix)
    # Save slow_coords to a JSON file
    slow_coords_file_path = os.path.join(transient_dir, "slow_coords.json")
    with open(slow_coords_file_path, "w") as f:
        json.dump(slow_coords, f, indent=4)
...
if __name__ == "__main__":
    route_finding(
        working_directory='work_dir/test_pipeline',
        number_of_classes=15,
        time_limit=80,
        optimization_objective='d',
        max_iter=200,
        ant_no=50,
        segments_number_per_class=10,
        benefit_type='t',
        route_type='g'
    )