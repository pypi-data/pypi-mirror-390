...
class Circle:
	...
	def draw(self):
		...
		c = self.app.create_surface(self.radius*2, self.radius*2, color=None, blend=True)
...
class Pointer_App(App):
	...
	def __init__(self):
		super().__init__()
		self.add_mode(Mode_Pointer(app=self))
		self.set_mode('Pointer')								# note: first mode is selected by default

class Mode_Pointer(Mode):
	""" Pointer mode. """

	def __init__(self, app):
		super().__init__(app, name='Pointer')
		self.circles = []

	def click_handler(self, x, y):
		""" Click event handler for Pointer mode. """

		# super().on_click(x, y)								# not needed since there are no user controls
		self.circles.append(Circle(self.app, x, y))

	def process(self):
		""" Process method for Hello mode. """

		# super().process()											# not needed since there are no submodes
