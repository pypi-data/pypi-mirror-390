...
import __main__, random_user_agent, requests
	...
	def load(cls, filename: str=None, defaults: dict=None) -> dict:
		...
		try:
			...
			if os.path.isfile(filename) is False:
				Log.error('Config', 'load', f'{filename} does not exist')
			else:
...
class Misc:
	...
	def http_request(cls, url: str, method: str=None, headers: dict=None,
		...
		status_code = None
		try:
			if method.upper() == 'POST' or data is None:
