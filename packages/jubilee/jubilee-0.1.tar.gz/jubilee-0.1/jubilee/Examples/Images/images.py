...
class Images_App(App):
	...
	def __init__(self):
		super().__init__()
		self.add_mode(Mode_Images(app=self))

class Mode_Images(Mode):
	""" Images mode. """

	def __init__(self, app):
		super().__init__(app, name='Images')
		self.bubbles = []
		self.robots = list(Robot(app, self) for _ in range(5))			# create 5 robots
		self.player = Player(app, self)															# create player
	...
	def draw(self):
		...
		super().draw()
		self.draw_sprites()											# draw robots and player
