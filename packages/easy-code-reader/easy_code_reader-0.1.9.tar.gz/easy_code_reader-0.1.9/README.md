...
或者参考 [uv 官网](https://github.com/astral-sh/uv) 进行安装，并配置 uv 的安装路径添加到系统 PATH 中，以便可以直接使用 `uvx` 命令。[uv](https://github.com/astral-sh/uv) 是一个极快的 Python 包和项目管理工具。使用 `uvx` 可以无需预先安装，直接运行：

```bash
# 无需安装，直接运行（读取默认本地 Maven 仓库，如果要读取本地项目源码需每次调用时声明仓库路径 project_dir）
uvx easy-code-reader

# 指定自定义 Maven 仓库路径（读取配置的 Maven 仓库，如果要读取本地项目源码需每次调用时声明仓库路径 project_dir）
uvx easy-code-reader --maven-repo /path/to/maven/repository

# 指定项目目录路径（读取默认本地 Maven 仓库，读取本地项目目录以配置的 project_dir 为准）
...
**流程 1: 探索未知项目**
```
1. list_all_project → 获取所有项目列表
2. list_project_files → 了解项目结构和文件
3. read_project_code → 读取感兴趣的具体文件
```

**流程 2: 分析特定模块**
```
1. list_project_files (指定 sub_path) → 获取模块文件列表
2. read_project_code → 读取相关类的源代码
```

**流程 3: 直接读取已知类**
```
1. read_project_code (使用类名或路径) → 直接读取
```
...
1. **CFR (C# Friendly Recompiler)**
   - 兼容 Java 8 及更高版本
   - 适合低版本 JVM 环境
   - 已包含在包中：`src/easy_code_reader/decompilers/cfr.jar`

2. **Fernflower**
   - IntelliJ IDEA 使用的反编译器
   - 需要 Java 21 或更高版本
   - 已包含在包中：`src/easy_code_reader/decompilers/fernflower.jar`
...
- `MAVEN_HOME`: Maven 主目录（将使用 `$MAVEN_HOME/repository`，优先级最高）
- `M2_HOME`: Maven 主目录（将使用 `$M2_HOME/repository`，优先级次之）
- `MAVEN_REPO`: 直接指定 Maven 仓库路径（优先级最低）

**优先级顺序：** `MAVEN_HOME` > `M2_HOME` > `MAVEN_REPO` > 默认路径（`~/.m2/repository`）
