...
class EasyCodeReaderServer:
        ...
        async def handle_list_resources() -> List[Resource]:
            ...
            return [
                Resource(
                    ...
                    description="介绍 Easy Code Reader 的功能和配置参数说明",
                    mimeType="text/plain"
                )
            ]
    ...
    def _get_guide_content(self) -> str:
        ...
        return f"""# Easy Code Reader 使用指南

## 功能介绍

Easy Code Reader 是一个强大的 MCP (Model Context Protocol) 服务器，专为智能读取 Java 源代码而设计，能从本地 Maven 仓库和项目目录中提取源码。
...
### 1. maven_repo（Maven 仓库路径）

- **当前配置：** `{maven_repo}`，**用途：** 指定本地 Maven 仓库的位置，用于查找和读取 JAR 包。

**配置优先级：**
1. 启动参数 `--maven-repo`（最高优先级）
2. 环境变量 `MAVEN_HOME`（使用 $MAVEN_HOME/repository）
3. 环境变量 `M2_HOME`（使用 $M2_HOME/repository）
4. 环境变量 `MAVEN_REPO`
5. 默认路径 `~/.m2/repository`（最低优先级）

### 2. project_dir（项目目录路径）

- **当前配置：** `{project_dir}`，**用途：** 指定本地项目代码的根目录，用于读取本地项目源码。

## 提供的工具

1. **read_jar_source** - 读取 Maven 依赖中的 Java 类源代码
2. **read_project_code** - 读取本地项目中的源代码
3. **list_all_project** - 列举项目目录下的所有项目
4. **list_project_files** - 列出项目中的源代码和配置文件

## 技术细节

- **反编译缓存位置：** `{maven_repo}/.../easy-code-reader/`
- **日志文件位置：** `src/easy_code_reader/easy_code_reader.log`
- **支持的文件类型：** .java, .xml, .properties, .yaml, .json, .gradle 等

---

💡 **提示：** 使用 AI 助手时，可以直接描述你想读取的代码，AI 会自动选择合适的工具来获取源码。
"""
    ...
    async def _read_project_code(self, project_name: str, class_name: str, 
        ...
        if '/' in class_name or class_name.endswith('.java'):
            ...
            if result:
                ...
                logger.warning(f"在项目 {project_name} 的子模块中未找到: {class_name}")
