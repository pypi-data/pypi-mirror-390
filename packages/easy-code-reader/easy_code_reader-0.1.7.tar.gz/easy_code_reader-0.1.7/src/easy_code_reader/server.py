...
class EasyCodeReaderServer:
        ...
        async def handle_list_tools() -> List[Tool]:
            ...
            return [
                ...
                Tool(
                    ...
                    description="列出指定 Java 项目中的所有源代码文件和配置文件的完整路径列表（已过滤测试目录和编译产物）",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "project_name": {
    ...
    async def _list_project_files(self, project_name: str, project_dir: Optional[str] = None) -> List[TextContent]:
        """
        列出指定 Java 项目中的所有源代码文件和配置文件的完整路径列表
        已过滤测试目录（src/test）、编译产物和不必要的文件
