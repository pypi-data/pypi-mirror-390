...
class NetworkValidator:
    ...
    def check_dns(hostname: str) -> DNSCheckResult:
        ...
        try:
            ...
            return DNSCheckResult(hostname=hostname, resolved=True, ip_addresses=ip_addresses)
        except socket.gaierror as e:
            return DNSCheckResult(hostname=hostname, resolved=False, error_message=f"DNS resolution failed: {e}")
        except Exception as e:
            return DNSCheckResult(hostname=hostname, resolved=False, error_message=f"Unexpected error: {e}")
    ...
    def check_ssl(hostname: str, port: int = 443) -> SSLCheckResult:
        ...
        try:
            ...
            return SSLCheckResult(hostname=hostname, port=port, valid=False, error_message="Connection timeout")
        except Exception as e:
            return SSLCheckResult(hostname=hostname, port=port, valid=False, error_message=f"Unexpected error: {e}")
    ...
    def check_connectivity(url: str, timeout: int = 10) -> ConnectivityCheckResult:
        ...
        try:
            ...
            return ConnectivityCheckResult(
                url=url, reachable=True, response_time_ms=response_time_ms, status_code=response.status_code
            )

        except requests.exceptions.Timeout:
            return ConnectivityCheckResult(
                url=url, reachable=False, error_message=f"Connection timeout after {timeout} seconds"
            )
        except requests.exceptions.ConnectionError as e:
            return ConnectivityCheckResult(url=url, reachable=False, error_message=f"Connection error: {str(e)[:200]}")
        except Exception as e:
            return ConnectivityCheckResult(url=url, reachable=False, error_message=f"Unexpected error: {str(e)[:200]}")
    ...
    def validate_sharepoint_connectivity(site_url: str) -> NetworkValidationResult:
        ...
        sp_dns = NetworkValidator.check_dns(sharepoint_host)
        ...
        sp_ssl = NetworkValidator.check_ssl(sharepoint_host)
        ...
        sp_connectivity = NetworkValidator.check_connectivity(site_url)
        ...
        azure_connectivity = NetworkValidator.check_connectivity(azure_ad_url)
