...
class CacheInspectionResult(BaseModel):
    ...
    access_tokens: list[CachedToken] = Field(default_factory=list, description="Cached access tokens")
    refresh_tokens: list[CachedToken] = Field(default_factory=list, description="Cached refresh tokens")
    id_tokens: list[CachedToken] = Field(default_factory=list, description="Cached ID tokens")
    accounts: list[dict[str, Any]] = Field(default_factory=list, description="Cached accounts")
    has_valid_token: bool = Field(description="Whether any valid (non-expired) access token exists")
    warnings: list[str] = Field(default_factory=list, description="Cache-related warnings")
...
class CacheInspector:
    ...
    def inspect_cache(cache: TokenCache) -> CacheInspectionResult:
        ...
        cache_data_str = cache.serialize()
        cache_data = json.loads(cache_data_str) if cache_data_str else {}
