...
class Storage:
    def __init__(self, json_path: str = "videos.json", csv_path: str = "video_index.csv", token_path="token.csv", next_page_token_path="next_page_token.csv"):
        ...
        if not os.path.exists(self.token_path):
            with open(self.token_path, "w", newline='', encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["token", "videoCount"])
        
        # Ensure next page token file exists
        if not os.path.exists(self.next_page_token_path):
            with open(self.next_page_token_path, "w", encoding="utf-8") as f:
                f.write("keyword,token\n")
    ...
    def store_next_page_token(self, token: str, keyword: str):
        # Store the next page token associated with the keyword in CSV format with headers "keyword" and "token"
        # If the file exists, append to it; otherwise, create it with headers
        with open(self.next_page_token_path, "a", encoding="utf-8") as f:
            if f.tell() == 0:
                f.write(f"keyword,token\n")
            f.write(f"{keyword},{token}\n")
        self.logger.info(f"Next page token stored for keyword '{keyword}'.")

    def load_next_page_token(self, keyword: str) -> str | None:
        if os.path.exists(self.next_page_token_path):
            with open(self.next_page_token_path, "r", encoding="utf-8") as f:
                next(f)  # Skip header
                for line in f:
                    kw, token = line.strip().split(",", 1)
                    if kw == keyword:
                        return token
        return None