...
class YouTubeCrawler:
    ...
    def _crawl_for_keyword(
        self, keyword: str, existing_ids: set, max_per_keyword: Optional[int], fetch_comments: bool, total_fetched: int = 0
    ):
        if total_fetched > 0:
            self.logger.info(f"Starting crawl for keyword: '{keyword}' (already fetched {total_fetched})")
        else:
