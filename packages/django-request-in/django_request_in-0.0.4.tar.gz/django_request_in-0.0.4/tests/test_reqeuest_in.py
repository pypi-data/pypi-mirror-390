...
@function_decorator(
    ...
    dog=AnyField(source=SourceEnum.data, data_type=Dog,
                parser=dog_parser, allow_none=True, validators=[lambda x: x.name=="Husky"])
)
def request_in(request_data, x=None, y=None, z=None, dog=None):
    ...
    Args:
        ...
        y: 字符串。
        z: 布尔值。
        dog: 狗对象。 允许用户使用自定义的parser函数来解析数据。
    Returns:
        dict: 返回数据。
    """
    assert x == 1, "x must be 1"
    assert y == 'xx', "y must be 'xx'"
    assert z == False, "z must be False"
    assert isinstance(dog, Dog), "dog must be a Dog object"
