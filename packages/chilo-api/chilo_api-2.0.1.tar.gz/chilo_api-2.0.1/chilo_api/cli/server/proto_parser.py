...
# Our utilities
def optional_string(s): return string(s).times(0, 1).concat()


convert_decimal = int
def convert_octal(s): return int(s, 8)
def convert_hex(s): return int(s, 16)
def exclude_none(l): return [i for i in l if i is not None]
...
def signed_number_change(s, num): return (-1) if s == "-" else (+1)


sign = regex("[-+]?")
signedIntLit = seq(sign, intLit).combine(signed_number_change)
signedFloatLit = seq(sign, floatLit).combine(signed_number_change)
