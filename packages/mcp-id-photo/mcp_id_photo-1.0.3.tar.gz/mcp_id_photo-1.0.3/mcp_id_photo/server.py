...
def make_id_photo(
    ...
    # 保存到路径
    ts = int(time.time())
    # 根据 output_path 推断保存格式（默认PNG）
    save_format = "PNG"
    ext = ".png"
    if output_path:
        raw = output_path.strip()
        _, op_ext = os.path.splitext(raw)
        op_ext = op_ext.lower()
        if op_ext in (".jpg", ".jpeg"):
            save_format = "JPEG"
            ext = ".jpg"
        elif op_ext == ".png":
            save_format = "PNG"
    ...
    default_name = f"id_photo_{size_label}_{ts}{ext}"
    file_path = _resolve_output_file(output_path, default_name)
    try:
        img_to_save = final_img
        if save_format == "JPEG":
            img_to_save = img_to_save.convert("RGB")
            img_to_save.save(file_path, format=save_format, quality=90)
        else:
