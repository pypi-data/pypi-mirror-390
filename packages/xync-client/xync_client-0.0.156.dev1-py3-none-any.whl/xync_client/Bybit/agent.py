    ...
    async def watch_payeer(self, mcs: dict[int, "AgentClient"]):
        ...
        while True:
            try:
                ...
                bs = [b for b in bs if float(b.price) < 89.56 or int(b.userId) == list(mcs.keys())[-1]]
                if bs:
                    ad: Ad = bs[0]
                    ...
                    am = min(float(ad.maxAmount), max(800 + i, float(ad.minAmount)))
                    req = TakeAdReq(
                        ad_id=ad.id,
                        amount=am,
                        pm_id=14,
...
async def main():
    ...
    cl: AgentClient = agent.client(ecl, filebot, abot, "http://hmxelnzd:zaw8ied2qdjc@64.137.96.74:6641")
    ...
    ms = await models.Agent.filter(
        actor__ex_id=4, auth__isnull=False, status__gt=AgentStatus.off, actor__person__user__id__in=[2, 1]
    ).prefetch_related(
        "actor__ex",
        "actor__person__user__gmail",
        "actor__my_ads__my_ad__race",
