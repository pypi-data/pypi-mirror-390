            ...
            with patch("keynet_train.cli.commands.push.select_project", return_value=1):
                with patch(
                    ...
                    with patch(
                        ...
                        assert "📋 Step 5/10: Requesting upload key..." in captured.out
                        assert "✅ Upload key:" in captured.out  # Mock 값 검증은 생략

                        assert (
                            "📋 Step 6/10: Resolving dependencies..." in captured.out
                            or "📋 Step 6/10: Using custom Dockerfile..."
                            in captured.out
                        )

                        assert (
                            "📋 Step 7/10: Building container image..." in captured.out
                        )
                        assert "✅ Built image: sha256:abc12" in captured.out

                        assert "📋 Step 8/10: Tagging image..." in captured.out
                        ...
                        assert "📋 Step 9/10: Pushing to Harbor..." in captured.out
