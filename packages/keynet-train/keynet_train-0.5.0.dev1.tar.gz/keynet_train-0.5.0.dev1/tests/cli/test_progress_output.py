            ...
            with patch("keynet_train.cli.commands.push.select_project", return_value=1):
                with patch(
                    ...
                    with patch(
                        ...
                        assert (
                            "📋 Step 7/11: Resolving dependencies..." in captured.out
                            or "📋 Step 7/11: Using custom Dockerfile..."
                            in captured.out
                        )

                        assert (
                            "📋 Step 8/11: Building container image..." in captured.out
                        )
                        assert "✅ Built image: sha256:abc12" in captured.out

                        assert "📋 Step 9/11: Tagging image..." in captured.out
                        ...
                        assert "📋 Step 10/11: Pushing to Harbor..." in captured.out
            ...
            with patch("keynet_train.cli.commands.push.select_project", return_value=1):
                with patch(
                    ...
                    with patch(
                        ...
                        # 최종 메시지에 하이퍼파라미터 개수 표시 확인
                        assert (
                            "Hyperparameters: 3 arguments sent to Backend"
                            in captured.out
                        )
