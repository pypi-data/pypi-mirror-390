...
def test_push_image_success(client):
    ...
    with patch.object(client._client.images, "push") as mock_push:
        ...
        # images.push() 호출 확인 (repository와 tag 분리)
...
def test_push_image_without_tag(client):
    ...
    with patch.object(client._client.images, "push") as mock_push:
        ...
        # tag=None으로 호출되어야 함
        mock_push.assert_called_once_with(
            repository="harbor.example.com/ml-models/my-model", tag=None
        )
...
def test_push_image_with_port_in_registry(client):
    ...
    with patch.object(client._client.images, "push") as mock_push:
        ...
        mock_push.assert_called_once_with(
            repository="registry.example.com:8443/project/model", tag="v2.0"
        )
...
def test_push_image_with_multiple_colons(client):
    ...
    with patch.object(client._client.images, "push") as mock_push:
        ...
        mock_push.assert_called_once_with(
            repository="harbor.example.com:8443/project/model", tag="latest"
        )
