...
class TestEdgeCases:
    ...
    def test_special_characters_in_experiment_name(self, setup_mlflow):
        ...
        @trace_pytorch(
            ...
            torch.randn(1, 10),
            model_name="special-chars-test-model",
            enable_autolog=False,
        )
        def train_model():
            return nn.Linear(10, 5)
    ...
    def test_interrupted_upload(self, setup_mlflow):
        ...
        @trace_pytorch(
            "interrupt_exp",
            torch.randn(1, 10),
            model_name="interrupt-test-model",
            enable_autolog=False,
        )
        def train_model():
            return nn.Linear(10, 5)
    ...
    def test_invalid_dynamic_axes(self, setup_mlflow):
        ...
        @trace_pytorch(
            "invalid_axes_exp",
            torch.randn(1, 10),
            model_name="invalid-axes-test-model",
        ...
        def run_experiment(exp_name, idx):
            try:
                ...
                @trace_pytorch(
                    exp_name,
                    torch.randn(1, 10),
                    model_name=f"concurrent-test-model-{idx}",
                    enable_autolog=False,
                )
                def train():
                    return nn.Linear(10, 5)
