...
from keynet_train.clients.models import UploadKeyCommand, UploadKeyResponse
...
def sample_upload_response():
    ...
    command = UploadKeyCommand(
        tag="docker tag <YOUR_IMAGE:TAG> kitech-harbor.wimcorp.dev/kitech-model/test-upload-key:latest",
        push="docker push kitech-harbor.wimcorp.dev/kitech-model/test-upload-key:latest",
    )
...
def test_progress_output_shows_all_steps(
    ...
    """성공 워크플로우에서 Step 1-10 프로그레스 출력 검증"""
    # 임시 train.py 생성
            ...
            with patch("keynet_train.cli.commands.push.select_project", return_value=1):
                with patch(
                    ...
                    with patch(
                        ...
                        # Step 1-10 프로그레스 출력 검증
                        assert (
                            "📋 Step 1/10: Checking authentication..." in captured.out
                        )
                        assert "✅ Authenticated" in captured.out

                        assert "📋 Step 2/10: Validating entrypoint..." in captured.out
                        assert "✅ Validation passed" in captured.out

                        assert (
                            "📋 Step 3/10: Extracting hyperparameters..."
                            in captured.out
                        )
                        assert "✅ Found 0 hyperparameters" in captured.out

                        assert "📋 Step 4/10: Selecting project..." in captured.out
                        assert "✅ Selected project ID: 1" in captured.out

                        assert "📋 Step 5/10: Determining model name..." in captured.out

                        assert "📋 Step 6/10: Requesting upload key..." in captured.out
                        assert "✅ Upload key:" in captured.out  # Mock 값 검증은 생략

                        assert (
                            "📋 Step 7/10: Resolving dependencies..." in captured.out
                            or "📋 Step 7/10: Using custom Dockerfile..."
                            in captured.out
                        )

                        assert (
                            "📋 Step 8/10: Building container image..." in captured.out
                        )
                        assert "✅ Built image: sha256:abc12" in captured.out

                        assert "📋 Step 9/10: Tagging image..." in captured.out
                        # Updated to match the actual image reference from sample_upload_response
                        assert (
                            "✅ Tagged: kitech-harbor.wimcorp.dev/kitech-model/test-upload-key:latest"
                            in captured.out
                        )

                        assert "📋 Step 10/10: Pushing to Harbor..." in captured.out
                        ...
                        # 하이퍼파라미터가 없을 때 Panel 내부에 "None" 표시
                        assert "Hyperparameters: None" in captured.out
            ...
            with patch("keynet_train.cli.commands.push.select_project", return_value=1):
                with patch(
                    ...
                    with patch(
                        ...
                        # 하이퍼파라미터 개수가 제대로 표시되는지 확인
                        assert "✅ Found 3 hyperparameters" in captured.out
