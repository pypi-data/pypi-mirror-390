...
def trace_pytorch(
    ...
    사용 예시:
        ...
        @trace_pytorch(
            "multi_experiment",
            {"image": torch.randn(1, 3, 224, 224), "mask": torch.randn(1, 1, 224, 224)},
        ...
        # CLI에서 model_name override:
        # $ keynet-train push train.py --model-name custom-model-v2
        ```
                ...
                try:
                    ...
                    if training_match_end_point and train_id and api_key:
                        try:
                            ...
                            logger.debug("🔗 MLflow run_id 서버 전송 시도: " + api_url)
                            ...
                            logger.error(
                                f"⚠️ MLflow run_id 서버 전송 실패 (계속 진행): {e}"
                            )
                    else:
