"""Databricks notebook creation and export utilities."""

import base64
import os
from dataclasses import dataclass

from ..types.comment_language import (
    MessageKey,
    MessageManager,
    CommentLanguage,
)
...
class NotebookExportHelper:
    ...
    def create_notebook_content(self, output_file_path: str, ex_in: ExportInput) -> str:
        ...
        messages = MessageManager.get_all_messages(CommentLanguage(ex_in.comment_lang))
        notebook_content = (
        ...
        if ex_in.python_parse_error or ex_in.sql_parse_error:
            notebook_content += f"# MAGIC {messages[MessageKey.ERRORS_FROM_CHECKS]}\n"
            ...
            if ex_in.sql_parse_error:
                notebook_content += f"# MAGIC ### {messages[MessageKey.SPARK_SQL_SYNTAX_ERRORS]}\n" f"# MAGIC ```\n"
                for error in ex_in.sql_parse_error:
                    notebook_content += f"# MAGIC {error}\n"
                notebook_content += "# MAGIC ```\n"
        else:
            notebook_content += (
