"""Python code analysis and cell splitting utilities for notebook conversion."""
...
class CellSplitHelper:
    ...
    def split_cells(self, code: str | None) -> str | None:
        """
        ...
        Args:
            code (str | None): The input Python code to be split into cells.

        Returns:
            str | None: The code split into Databricks notebook cells, with `# COMMAND ----------` separating each cell.
        ...
        """
        self.logger.debug("Starting cell splitting process.")

        # If the code is None or not a string, return None
        if code is None or not isinstance(code, str):
            self.logger.warning("Invalid input: code is None or not a string.")
            return None

        # If stripping the code results in an empty string, return None
        code_stripped = code.strip()
        if not code_stripped:
            self.logger.warning("Invalid input: code is empty after stripping.")
            return None
        ...
        try:
            ...
            self.logger.error(f"Unexpected error during AST parsing: {type(e).__name__} - {e}")
            return self._create_single_cell(code)
    ...
    def _extend_and_format_blocks(self, adjusted_blocks: list[tuple[int, int]], lines: list[str]) -> list[str]:
        ...
        for block_start, block_end in adjusted_blocks:
            ...
            extended_end = self._extend_block_downward_for_blank_lines(block_end, lines)

            block_end = max(block_end, extended_end)

            if block_start <= block_end:
                output_lines.append(self.COMMAND_MARKER)
                for line_num in range(block_start, block_end + 1):
                    output_lines.append(lines[line_num - 1])
    ...
    def _adjust_block_start_lines(
        self, merged_blocks: list[tuple[int, int]], lines: list[str]
    ) -> list[tuple[int, int]]:
        """
        Adjust block start lines to include preceding comments and blank lines.
    ...
    def _get_node_end_line(self, node: ast.AST) -> int | None:
        """
        ...
        Returns:
            int | None: The end line number of the node, or None if not found.
        """
    ...
    def _pull_up_leading_comments_and_blank_lines(self, start_line: int, lines: list[str], limit_line: int) -> int:
        """
        ...
        while idx - 1 >= limit_line - 1:
            ...
            if content == "" or content.startswith("#"):
                ...
                break
        idx = max(idx, limit_line - 1)
        return idx + 1

    def _extend_block_downward_for_blank_lines(self, block_end: int, lines: list[str]) -> int:
        """
