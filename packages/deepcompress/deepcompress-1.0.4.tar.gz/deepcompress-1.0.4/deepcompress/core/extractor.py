...
class OCRExtractor:
    ...
    async def _extract_page(self, image: Any, page_number: int) -> Page:
        ...
        # DeepSeek-OCR uses a vision-language model format
        # Format prompt with image placeholder token
        prompt_text = "Extract all text, entities, tables, and structured information from this document image. Return the results in JSON format."
        
        # Prepare inputs with text prompt and image
        # The processor handles tokenization and image preprocessing
