...
class OCRExtractor:
    ...
    async def _extract_page(self, image: Any, page_number: int) -> Page:
        ...
        try:
            ...
            # Prompt format for structured extraction - be specific to avoid hallucination
            prompt = "<image>\nExtract the text content from this document page. Focus on accuracy and completeness. Output only the extracted text without repetition or elaboration."
            ...
            if result is None:
                raise OCRError(
                    f"Model inference returned None for page {page_number}",
                    details={
