                                            ...
                                            try:
                                                ...
                                                if position_ids is None:
                                                    if cache_position is not None:
                                                        position_ids = cache_position.view(1, -1).to(hidden_states.device)
                                                    else:
                                                        past_length = 0
                                                        if past_key_value is not None:
                                                            try:
                                                                past_length = int(getattr(past_key_value, "seen_tokens", 0) or 0)
                                                            except Exception:
                                                                past_length = 0
                                                ...
                                                rotary_callable = rotary_emb
                                                if hasattr(rotary_emb, 'forward'):
                                                    rotary_callable = rotary_emb.forward

                                                rotary_sig = _inspect.signature(rotary_callable)
                                                if 'cache_position' in rotary_sig.parameters:
                                                    cos, sin = rotary_emb(value_states, position_ids, cache_position=cache_position)
                                                else:
