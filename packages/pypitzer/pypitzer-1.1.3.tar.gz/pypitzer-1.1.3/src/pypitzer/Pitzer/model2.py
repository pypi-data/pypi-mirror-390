...
class FluidPitzer:
    def __init__(self, x0, species, database='spencer', t=None, neutral=False, solids=None):
        """
        Initiate the solution.
        :param x0: initiate values of x(x1, x2), namely value of meta1 and meta2.
        :param species: define species of a solution.
        :param t: melting temperature, in Kelvin.
    ...
    def get_molalities(self, x):
        molalities = dict(zip(self.species, list(x)))

        if 'Cl-' not in molalities.keys():
            tvalue = 0
            for key, value in molalities.items():
                tvalue += get_charge_number(key) * value
            molalities['Cl-'] = tvalue
        return molalities

    def charge_balance(self, x):
        x = tuple(x)
        balance = mt.calculate_charge_balance(x, self.get_molalities(x))
        return balance
    ...
    def get_ionic_strength(self, x):
        ...
        """
        i = mt.calculate_ionic_strength(self.get_molalities(x))
        return i
    ...
    def get_a_phi(self, x):
        return pm.a_phi_moller(self.get_t(x))
        ...
        if self.get_component_groups(x)['anion_pairs']:
            ...
            for anion_pair in anion_pairs:
                ...
                for cation in cations:
                    rd = pm.ternary_parameters_ready((anion1, anion2, cation), self.get_t(x))
    ...
    def get_lambdas(self, x):
        ...
        for pair in neutral_ion_pairs:
            rd = pm.binary_parameters_ready(pair, self.get_t(x))
            lambda_value = pm.get_parameter(pair=pair, name='lambda', data=rd, t=self.get_t(x))
    ...
    def get_zetas(self, x):
        ...
        for pair in nca_pairs:
            rd = pm.ternary_parameters_ready(pair, self.get_t(x))
            zeta_value = pm.get_parameter(pair=pair, name='zeta', data=rd, t=self.get_t(x))
    ...
    def total_gibbs_energy(self, x):
        ...
        for species in molalities.keys():
            ...
            aqu_cmp += m_i * (0 + r * self.get_t(x) * (np.log(m_i) + ln_gamma))

        # gibbs energy of water
        ln_a_w = self.get_water_activity(x)
        std_cp_water = mt.get_chemical_potential(species='H2O(l)', t=self.get_t(x))

        cp_water = (1000 / 18.015) * (std_cp_water + r * self.get_t(x) * ln_a_w)

        solid_cmp = 0
        for solid in self.solids:
            solid_gibbs = mt.get_chemical_potential(solid, self.get_t(x))
            solid_cmp += solid_gibbs

        total_g = aqu_cmp + cp_water + solid_cmp
        return total_g

    def solubility_equilibrium(self, y):
        ...
        def solubility_equilibrium_single(x):
            ...
            if target_species == 'H2O(S)':
                ...
                # lnk_ice = monnin(self.get_t(x))
                lnk_ice = spencer(self.get_t(x))
                f = lnk_ice - lna_pitzer
            else:
                molalities = self.get_molalities(x)

                lnk_potential = mt.get_chemical_potential(species=target_species, t=self.get_t(x))
        ...
        return solubility_equilibrium_single
    ...
    def optimize(self):
        ...
        for i in range(len(self.solids)):
            cons_eqm += ({'type': 'eq', 'fun': self.solubility_equilibrium(i)},)
        constraints = cons_eqm
