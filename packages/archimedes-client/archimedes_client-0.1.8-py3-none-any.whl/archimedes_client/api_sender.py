        ...
        for attempt in range(self.max_retries):
            try:
                if method == 'GET':
                    ...
                    response = requests.post(url, headers=self.headers, json=data, timeout=self.timeout, verify=False)
                elif method == 'PATCH':
                    response = requests.patch(url, headers=self.headers, json=data, timeout=self.timeout, verify=False)
                elif method == 'PUT': 
                    response = requests.put(url, headers=self.headers, json=data, timeout=self.timeout, verify=False)
                else:
                ...
                try:
                    if response.text.strip():
                        ...
                        logger.warning(f"[API] Resposta vazia de {url} (status={response.status_code})")
                        return None
                except ValueError:
                    logger.warning(f"[API] Resposta não JSON de {url}: {response.text[:200]}")
                    return None
