...
class OrchestratorAPIClient:
    ...
    def send_logs(self, logs: List[Dict[str, Any]]) -> bool:
        ...
        for attempt in range(3):
            try:
                response = self.__make_request('POST', 'logs/batch', logs)
                if response:
...
a = OrchestratorAPIClient( 'http://127.0.0.1:8000', '8baaa93c-6e48-408e-8ca3-172bc4f5b153')
a.autenticar('@.com', '')
aa = a.retornar_tarefa_run_agente()
a.criar_sub_tarefa('teste', aa['id'])
