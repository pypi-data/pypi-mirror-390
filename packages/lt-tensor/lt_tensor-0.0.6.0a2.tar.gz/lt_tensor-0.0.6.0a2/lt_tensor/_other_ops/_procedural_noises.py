...
def generate_perlin_noise(size: Tuple[int, int], grid_size: Tuple[int, int]):
    """
...
def generate_perlin_noise_3d(
    size: Tuple[int, int, int], grid_size: Tuple[int, int, int]
):
    """
...
def warped3(x: int, y: int, z: int, seed: int = 1337):
    perm = make_perm(seed=seed)
    return domain_warped_fbm_3d(
        x, y, z, base_scale=0.02, warp_scale=0.04, warp_amplitude=15.0, perm=perm
    )


def fbm3(x, y, z):
    return fbm_3d(x, y, z, octaves=5, scale=0.02, persistence=0.5, lacunarity=2.0)
