...
def to_numpy_array(
    ...
    if isinstance(inp, Tensor):
        return np.asarray(inp.clone().detach().tolist(), dtype=dtype)
...
def time_weighted_ema(
    ...
    """
    Differentiable Time-Weighted Exponential Moving Average (EMA).

    Args:
        x (torch.Tensor): Input tensor of shape (B, T, ...) or (T, ...)
        time_weights (torch.Tensor, optional): Relative weights per time step.
            Shape (T,) or broadcastable to time dimension. If None, uniform.
        alpha (float): Base smoothing factor ∈ [0, 1). Higher = more smoothing.
        normalize (bool): Whether to normalize EMA output (zero mean, unit std).
        eps (float): Small value for numerical stability.

    Returns:
        torch.Tensor: EMA-smoothed version of x, same shape as input.
    """
    if x.ndim < 2:
        x = x.view(1, -1, 1)  # -> (1, T, 1)
    elif x.ndim == 2:
    ...
    # Compute variable decays (broadcast to shape (B, T, C))
    decays = (alpha**time_weights).view(1, T, 1).expand(B, T, C)

    # Recursive-like differentiable formulation using cumulative products
    # numerator_t = sum_{i<=t} x_i * prod_{j=i+1..t}(decay_j)
    # denom_t = sum_{i<=t} prod_{j=i+1..t}(decay_j)
    w = torch.cumprod(
        torch.cat([torch.ones(B, 1, C, device=x.device), decays[:, :-1]], dim=1), dim=1
    )
    num = torch.cumsum(w * x, dim=1)
    ...
    return ema.squeeze(-1)  # remove feature dim if 1D
