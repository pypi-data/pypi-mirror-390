...
def eng_to_telugu_base(text: str, rules: dict) -> str:
    ...
    while i < len(text):
        ...
        chunk3 = text[i:i+3]
        chunk2 = text[i:i+2]
        single = text[i]        # Handle 'r' + vowel combinations (r+i, r+u, r+a, r+e, r+o)
        # These should be: consonant ర + matra, NOT vocalic ృ
        if single == 'r' and i + 1 < len(text):
            next_char = text[i + 1]
            # r + vowel = consonant ర + matra
            if next_char == 'i':
                if prev_cons:
                    result.append("్")
                result.append("రి")
        ...
        # CRITICAL FIX: Handle nasal + consonant clusters
        # These should become:ం + consonant (not న్ + consonant)
        # Check for nasal clusters (n + consonant) in priority order
        nasal_mappings = {
            # 4-char nasal clusters
            "nchh": "ంచ", "njh": "ంజ", "nTh": "ంఠ", "nDh": "ంఢ",
            "nkh": "ంఖ", "ngh": "ంఘ", "nth": "ంథ", "ndh": "ంధ",
            "mph": "ంఫ", "mbh": "ంభ",
            # 3-char nasal clusters
            "nch": "ంచ", "nT": "ంట", "nD": "ండ",
            "nk": "ంక", "ng": "ంగ", "nj": "ంజ",
            "nt": "ంత", "nd": "ండ", "mp": "ంప", "mb": "ంబ",
        }

        # Check for nasal clusters (longest first for 3-char, then 2-char)
        matched_nasal = False
        for length in [3, 2]:
            if i + length <= len(text):
                substr = text[i:i+length]
                if substr in nasal_mappings:
                    result.append(nasal_mappings[substr])
                    i += length
                    prev_cons = True
                    matched_nasal = True
                    break
        ...
        # Note: Only use vocalic ృ for Sanskrit words in clusters, not for ri/ru
...
def get_clusters(style="modern"):
    ...
    # CRITICAL FIX: Add nasal+consonant clusters FIRST (highest priority)
    clusters = {
        # 4-letter nasal+consonant clusters
        "nchh": ["n", "chh"],
        "njh": ["n", "jh"],
        "nTh": ["n", "Th"],
        "nDh": ["n", "Dh"],
        ...
        "mph": ["n", "ph"],
        "mbh": ["n", "bh"],
        # 3-letter nasal+consonant clusters
        "nch": ["n", "ch"],
        "nT": ["n", "tt"],
        "nD": ["n", "dd"],
        "nk": ["n", "k"],
    ...
    if style == "classical":
        # Classical style has additional archaic nasal clusters
        clusters.update({
            "nga": ["nga"],  # Velar nasal (archaic)
            "nya": ["nya"],  # Palatal nasal (archaic)
        })
