...
class AlexaLogin:
    ...
    async def test_loggedin(self, cookies: Union[dict[str, str], None] = None) -> bool:
        ...
        await self.get_csrf()
        path = self._prefix + "amazon.com" + f"/api/users/me?platform=ios&version={CALL_VERSION}"
        ...
        if self.url.lower() != "amazon.com":
            self._headers["authority"] = f"www.{self._url}"
            path = self._prefix + self._url + f"/api/users/me?platform=ios&version={CALL_VERSION}"
            self._log_cookies_for_url(path)
