...
from .const import GQL_SMARTHOME_QUERY, CALL_VERSION
...
class AlexaAPI:
    ...
    async def get_authentication(login: AlexaLogin) -> Optional[dict[str, Any]]:
        ...
        response = await AlexaAPI._static_request(
            "get", login, f"/api/users/me?platform=ios&version={CALL_VERSION}", query=None
        )
        if response:
            json_resp = await response.json(content_type=None)
            return {
                "authenticated": True,
                "canAccessPrimeMusicContent": json_resp.get("canAccessPrimeMusicContent", True),
