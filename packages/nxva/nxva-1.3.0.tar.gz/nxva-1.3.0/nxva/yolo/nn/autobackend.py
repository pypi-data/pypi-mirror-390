...
class HefHandler(AutoBackend, exts=['.hef']):
    def __init__(self, model_path, device, config:dict):
        ...
        self.task = self.config['task']
        self.size = self.config['size']
        self.decode_head = DecodeHead(version=self.version, task=self.task, nc=self.config['nc'])
    ...
    def __call__(self, imgs):
        ...
        m = self.model.models_pool[self.model.active_model_id]
        input_data = {m["input_info"][0]: imgs}
        input_data = self.preprocess_feat(input_data)
        results = self.model.infer_pipeline.infer(input_data)
        return [self.decode_head(self.postprocess_feat(results, m))]

    def load_model(self):
        from nxva.nxhailo import HefInference
        return HefInference(self.config['weights'])
