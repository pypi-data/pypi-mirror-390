...
class YOLOToolBox(YOLO):
    ...
    def export(
        ...
        if task == 'torch2engine':
            ...
            Convert.torch_to_onnx(model, shape, onnx_output_path=output_path, dynamic=dynamic, type=precision)
        elif task == 'onnx2engine':
            Convert.onnx_to_engine(model, shape, output_path=output_path, dynamic=dynamic, dynamic_batch=dynamic_batch, type=precision)
        # elif task == 'onnx2jit':
