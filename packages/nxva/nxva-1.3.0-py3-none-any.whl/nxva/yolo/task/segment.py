...
class SegmentationPredictor(DetectionPredictor):
    ...
    def postprocess(
        ...
        for i, (pred, orig_img) in enumerate(zip(preds, orig_imgs)):
            ...
            masks = np_ops.process_mask(proto[i], pred[:, 6:], pred[:, :4], (self.size, self.size), upsample=True)  # HWC
            pred[:, :4] = np_ops.scale_boxes((self.size, self.size), pred[:, :4], orig_img.shape)
