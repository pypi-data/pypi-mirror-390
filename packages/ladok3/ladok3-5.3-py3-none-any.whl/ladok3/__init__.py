...
class LadokSession:
    ...
    def grade_scales_JSON(self):
        ...
        raise LadokAPIError(f"can't fetch grading scales: {response.text}")
    ...
    def search_course_rounds_JSON(self, /, **kwargs):
        ...
        raise LadokAPIError(f"searching for course rounds failed: {response.text}")
    ...
    def course_round_components_JSON(self, round_id):
        ...
        raise LadokServerError(response.json()["Meddelande"])
    ...
    def result_reporters_JSON(self, organization_id):
        ...
        if response.status_code == 200:
            return response.json()["Anvandare"]
        raise LadokAPIError(response.text)
