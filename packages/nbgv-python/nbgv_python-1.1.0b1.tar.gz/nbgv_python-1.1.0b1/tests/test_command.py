...
from nbgv_python.command import ENV_COMMAND_OVERRIDE, discover_command, parse_command_tokens
...
def test_discover_command_prefers_environment_override(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_discover_command_falls_back_to_dotnet_tool(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_discover_command_raises_when_not_found(monkeypatch: pytest.MonkeyPatch) -> None:
