...
class Test_SMA_class:
    ...
    async def test_newsession_pos(self, mock_aioresponse):  
        ...
        mock_aioresponse.get(
            f"https://localhost/api/v1/plants/Plant:1",
            payload="",
            repeat=True
            
        )
        mock_aioresponse.get(
            f"https://localhost/api/v1/plants/Plant:1/devices",
            payload="",
            repeat=True
    ...
    async def test_unknown_device(self, mock_aioresponse):
        ...
        mock_aioresponse.get(
            "https://localhost/api/v1/plants/Plant:1/devices/IGULD:SELF",
            payload= self.changeExistingDeviceInfo("TripowerX15-deviceinfo.json", {"productTagId": 9999999, "product": "Unknown", "vendor": "SomeoneElse" })
        )
        mock_aioresponse.post(
            "https://localhost/api/v1/parameters/search",
            payload= self.loadJson("TripowerX15-parameters.json"),
        ...
        mock_aioresponse.get(
            "https://localhost/api/v1/plants/Plant:1",
            payload= self.changeExistingDeviceInfo("TripowerX15-plant1.json", {"product": "Unknown", "vendor": "SomeoneElse" }),
            #payload=self.loadJson("TripowerX15-plant1.json"),
            repeat = True
        )
        mock_aioresponse.get(
            "https://localhost/api/v1/plants/Plant:1/devices",
            payload= self.changeExistingDeviceInfo("TripowerX15-plant1-devices.json", {"product": "Unknown", "vendor": "SomeoneElse" }),
#            payload= self.loadJson("TripowerX15-plant1-devices.json"),
            repeat=True
        )
        session = aiohttp.ClientSession()
