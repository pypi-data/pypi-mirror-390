...
class PlackettLuce:
    def __init__(
            ...
            n_shuffle:int=2,
            pl_best:bool=True,
            n_random_perms:int=5_0000 # how many random permutations to use for alignment computation
            ):
        """
    ...
    def get_sampled_combined_orderings(self):
        for idx in range(self.sample_count):
            ...
            self.sampled_energies[idx] = self.pl_energy(order)
    
    def compute_alignment(self) -> Tuple[np.ndarray, np.ndarray, float, float, float, float]:
        ...
        n_sampled=len(self.sampled_combined_orderings)
        taus = np.zeros(len(random_perms), dtype=np.float64)
        energies = np.zeros(len(random_perms), dtype=np.float64)
        for i, random_perm in enumerate(random_perms):
            energies[i] = self.pl_energy(random_perm)
            inner_total = 0.0 
            a = np.argsort(random_perm)
            for sampled_combined_ranking in self.sampled_combined_orderings:
                b = np.argsort(sampled_combined_ranking)
                # tau, p = kendalltau(a, b)
                # inner_total += (1-tau)/2
                inner_total += normalized_kendalls_tau_distance(a, b) 
            taus[i] = inner_total/n_sampled
        # Pearson's r
        pearson_r, pearson_p = pearsonr(taus, energies)
        # Spearman correlation
        spearman_rho, spearman_p = spearmanr(taus, energies)
        print("Pearson r:", pearson_r, "p-value:", pearson_p)
        print("Spearman rho:", spearman_rho, "p-value:", spearman_p)
        return taus, energies, pearson_r, pearson_p, spearman_rho, spearman_p
...
class MCMC:
    ...
    def compute_alignment(self) -> Tuple[np.ndarray, np.ndarray, float, float, float, float]:
        ...
        energies = np.zeros(len(random_perms), dtype=np.float64)
        for i, random_perm in enumerate(random_perms):
            energies[i] = self.get_energy(random_perm)
            inner_total = 0.0 
            a = np.argsort(random_perm)
            for sampled_combined_ranking in self.sampled_combined_orderings:
                b = np.argsort(sampled_combined_ranking)
