...
def get_combined_order(
    ...
    if method == 'PL':
        ...
        sampler = MCMC(ordering_array=padded_partial_orders, rng=rng, method=method, mcmc_iterations=mcmc_iterations, temperature=mallows_temperature)
...
class PlackettLuce:
    ...
    def check_model_validity(self):
        ...
        return taus, self.random_perm_energies, pearson_r, pearson_p, spearman_rho, spearman_p
...
@njit(fastmath=False, parallel=False) 
def mallows_energy_numba(ordering:np.ndarray, central_ordering:np.ndarray, dist_metric:str, temperature:float) -> float:
...
def alignment_metric(algo_energies, random_energies):
    """Calculates the proportion of algo energies below the minimum random energy."""
    if len(algo_energies) == 0:
        return 0.0
    min_random = np.min(random_energies)
    count_below = np.sum(algo_energies < min_random)
    return count_below / len(algo_energies)
