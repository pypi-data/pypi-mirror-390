...
@njit
def pl_energy_numba(ordering, theta):
    """
    ordering: ID int64 array of IDs
    theta: 1D float64 array
    Returns: scalar energy
    """
    ordering_idx = np.argsort(ordering) # ordering_idx: 1D int64 array of 0-based indices into theta
    total_energy = 0.0 
    n = len(ordering_idx)
    for i in range(n):
...
@njit 
def pl_sample_one_random(n:int, unique_elements:np.ndarray, theta:np.ndarray) -> np.ndarray:
    ...
    for pos in range(n):
        ...
        chosen_index = np.random.choice(remaining_len, p=probs)
        # update res 
...
def pl_sample_one_best(current_order:np.ndarray, theta:np.ndarray, mcmc_iterations:int, n_shuffle:int) -> np.ndarray:
    """Use MCMC to sample PL
    """
    # Sample one first 
    current_energy = pl_energy_numba(current_order, theta)
...
class PlackettLuce:
    ...
    def pl_energy(self, ordering):
        ...
        return pl_energy_numba(ordering, self.theta)

    def _sample_one_random(self) -> np.ndarray:
        """Sample a single complete ordering (integer indices into theta)."""
        return pl_sample_one_random(self.n_unique_elements, self.unique_elements, self.theta)
    
    def _sample_one_best(self) -> np.ndarray:
        current_order = self._sample_one_random()
