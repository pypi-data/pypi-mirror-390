...
def run_mpebm(
    ...
    try:
        best_order, best_log_likelihood, best_theta_phi = metropolis_hastings(
            ...
            # theta_phi_use_matrix = theta_phi_use_matrix, 
            # energy_influence=energy_influence,
            # use_scaling=use_scaling, percentile=percentile, use_percentile=use_percentile
        )
    except Exception as e:
    ...
    post_burnin_accepted_orders = accepted_orders[burn_in::thinning]
    post_burnin_log_likelihoods = log_likelihoods[burn_in::thinning]
    order_with_highest_ll = post_burnin_accepted_orders[
        post_burnin_log_likelihoods.index(max(post_burnin_log_likelihoods))]
    ...
    if save_results:
        final_stage_post, ml_stages, updated_pi = utils.stage_with_plugin_pi_em(
