"""Convert Clang-Tidy log output to JUnit XML"""
    ...
    with open(sys.argv[1], "r", encoding="utf-8") as f:
        for line in f:
            if error_regex.match(line):
                if error:
                    errors.append(process_error(error))
                error = [line]
...
def generate_test_suite(errors: list[tuple[str, Error]]) -> TestSuite:
    """generate testsuite of errors"""
    test_suite = TestSuite("Clang-Tidy")  # type: ignore
    if errors:
        sorted_errors = sorted(errors, key=lambda e: e[0])
        for file, it in itertools.groupby(sorted_errors, key=lambda e: e[0]):
            errors = list(it)
            test = generate_test_case(file, "Clang-Tidy error", errors)
