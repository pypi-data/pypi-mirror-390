...
def playwright_executor(args: JobArgs) -> ExecuteResult:
    ...
    try:
        ...
        response = ExecutorResponse.failure(message=f"Git操作失败：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)

    except RequirementNotFoundError as e:
        logger.REMOTE.error(f"❌ 业务逻辑文件夹错误: {str(e)}")
        response = ExecutorResponse.failure(message=f"业务逻辑文件夹错误：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)

    except DependencyInstallError as e:
        logger.REMOTE.error(f"❌ 依赖安装失败: {str(e)}")
        response = ExecutorResponse.failure(message=f"依赖安装失败：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)
    except asyncio.CancelledError as e:
        logger.REMOTE.error(f"❌ 任务被中断: {str(e)}")
        response = ExecutorResponse.failure(message=f"任务被中断：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)
    except ScriptExecutionError as e:
        logger.REMOTE.error(f"❌ 脚本执行失败: {str(e)}")
        response = ExecutorResponse.failure(message=f"脚本执行失败：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)

    except ExecutorError as e:
        logger.REMOTE.error(f"❌ 执行器错误: {str(e)}")
        response = ExecutorResponse.failure(message=f"执行器错误：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)
        ...
        response = ExecutorResponse.failure(message=f"未知错误：{str(e)}", data=result)
        return ExecuteResult.failure(result=response)
