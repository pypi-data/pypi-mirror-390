    ...
    try:
        ...
        if success:
            ...
            if result is not None:
                logger.REMOTE.info(f"返回结果: {result}")
                return sj.ExecuteResult.success(result=result)
            else:
                return sj.ExecuteResult.success()
        else:
            logger.REMOTE.error(f"❌ 脚本执行失败: {result}")
            return sj.ExecuteResult.failure(result=result if result else "执行失败")

    except GitOperationError as e:
        logger.REMOTE.error(f"❌ Git 操作失败: {str(e)}")
        return sj.ExecuteResult.failure(result=f"Git操作失败: {str(e)}")

    except RequirementNotFoundError as e:
        logger.REMOTE.error(f"❌ 业务逻辑文件夹错误: {str(e)}")
        return sj.ExecuteResult.failure(result=f"业务逻辑文件夹错误: {str(e)}")

    except DependencyInstallError as e:
        logger.REMOTE.error(f"❌ 依赖安装失败: {str(e)}")
        return sj.ExecuteResult.failure(result=f"依赖安装失败: {str(e)}")

    except ScriptExecutionError as e:
        logger.REMOTE.error(f"❌ 脚本执行失败: {str(e)}")
        return sj.ExecuteResult.failure(result=f"脚本执行失败: {str(e)}")

    except PlaywrightExecutorError as e:
        logger.REMOTE.error(f"❌ 执行器错误: {str(e)}")
        return sj.ExecuteResult.failure(result=f"执行器错误: {str(e)}")
        ...
        return sj.ExecuteResult.failure(result=f"未知错误: {str(e)}")
