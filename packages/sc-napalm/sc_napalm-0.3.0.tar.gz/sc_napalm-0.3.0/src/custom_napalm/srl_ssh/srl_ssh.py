...
class NokiaSRLSSHDriver(NetworkDriver):
    ...
    def get_config(self, retrieve="all", full=False, sanitized=False, format="text"):
        ...
        if retrieve in ["all", "running"]:
            result["running"] = self.send_command("info from running")
        if retrieve in ["all", "startup"]:
            result["startup"] = self.send_command("info from startup")
        if retrieve in ["all", "candidate"]:
            result["candidate"] = self.send_command("info from candidate")
        return result
