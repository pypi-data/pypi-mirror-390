    ...
    async def authenticate_dataflow(self, handler, data):
        ...
        try:
            ...
            valid_user = (
                ...
                .filter(
                    m_otp.UserOtp.email == user_email,
            ...
            user = (
                self.db.query(m_user.User)
                .filter(m_user.User.email == user_email)
                .first()
            )
            ...
            if not valid_user:
                if user:
                    ...
                    self.login_error(handler, f"login-{user_email}")
                else:
                    self.log.warning(f"User not found: {user_email}")
                    self.login_error(handler, f"signup-{user_email}")
                return None
            ...
            if not user:
              ...
              if not db_user:
                ...
                self.login_error(handler, f"Authentication error occurred. Please try again.")
                return None
              user = db_user
...
class DataflowGoogleAuthenticator(DataflowBaseAuthenticator, GoogleOAuthenticator):
    ...
    block_personal_emails = Bool(
        default_value=True,
        config=True,
        help="Whether to block personal email providers like Gmail, Yahoo, etc."
    )
    ...
    def __init__(self, **kwargs):
        ...
        self.log.info(f"DataflowGoogleAuthenticator initialized with google_client_id={self.google_client_id}, "
                      ...
                      f"block_personal_emails={self.block_personal_emails}")
    ...
    async def authenticate(self, handler, data):
        ...
        try:
            ...
            if self.block_personal_emails:
                email_domain = email.split('@')[-1].lower()
                if email_domain in blocked_domains:
                    raise Exception(
                        "Please use your organizational email to log in."
                    )
