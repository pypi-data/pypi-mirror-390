...
提供统一的 `THS` 客户端用于连接行情服务器、查询K线/盘口/板块等数据，并封装
响应解析与错误信息，便于第三方集成调用。
...
快速开始:
    ...
    with THS() as ths:
        r = ths.klines("USZA300033", count=100)
        if r.is_success():
            print(r.get_result())
...
标准使用案例:
    ...
    def main():
        ths = THS({"username": "your_username", "password": "your_password"})
        try:
            ths.connect()
            # 示例：获取K线数据
            resp = ths.klines("USZA300033", count=10)
            if resp.is_success():
                print("K线数据：", resp.get_result())
            else:
                print("错误信息：", resp.err_info)
        finally:
            ths.disconnect()
...
class Response:
    ...
    def __init__(self, json_str: str):
        ...
        try:
            try:
                import orjson
                data_dict = orjson.loads(json_str.encode('utf-8'))
            except ImportError:
                import json
        ...
        if isinstance(result, list):
            result = self._convert_data_keys_list(result) if result else []
        ...
        dict_extra_converted = self._convert_data_keys_dict(dict_extra) if isinstance(dict_extra,
                                                                                      dict) and dict_extra else {}

        self.payload = Payload(result, dict_extra_converted)
...
class THS:
    ...
    def __init__(self, ops: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            raise Exception(f"加载动态链接库 {lib_path} 失败: {e}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.err_info == "":
                    self._login = True
                    logger.info(f"✅ TCP成功连接到服务器 account:{self.ops.get('username', '')}")
                    if str(self.ops.get('username', '')).startswith("thsguest_"):
                        logger.info("✅ 当前为临时游客账户，如需更稳定请使用自己的账户登陆")
                    return response
                else:
