...
class ModuloBrowser(AbstractModuloBrowser):
	...
	def scrollIntoView(self,element:WebElement):
		"""
		@param element: must be attached to dom, if you query an element, then reload page and use it, it will throw StaleElementReferenceException
		"""
		command="window.scrollTo(%s,%s); window.scrollBy(0, -150);"%(element.location["x"],element.location["y"])
