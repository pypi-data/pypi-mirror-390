...
jackEventHandlingQueueLock = threading.RLock()
...
class PortInfo:
    def __init__(
        ...
        self.is_output: bool = not isInput
        self.is_input: bool = isInput
        self.is_audio: bool = isAudio

        self.name: str = name
        self.shortname: str = sname
        self.clientName: str = name[: -len(":" + sname)]
        portInfoByID[id(self)] = self
        self.aliases: list[str] = aliases or []

    def __repr__(self):
        return f"PortInfo(name={self.name}, isInput={self.is_input}, sname={self.shortname}, isAudio={self.is_audio}, aliases={self.aliases})"
...
def Airwire(f, t, force_combining=False):
    ...
    elif ((":" in t) and (":" not in f)) or ((":" in f) and (":" not in t)):
        return CombiningAirwire(f, t)
    elif (":" in f) and (":" in t):
...
def get_ports(*a, max_wait=10, **k) -> list[PortInfo]:
