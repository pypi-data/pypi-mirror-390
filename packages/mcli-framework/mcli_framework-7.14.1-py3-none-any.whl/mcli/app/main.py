...
def _add_lazy_commands(app: click.Group):
    ...
    # Top-level rm command
    try:
        from mcli.app.rm_cmd import rm
    ...
    # Store commands moved to repo group

    # Repo management - load immediately for fast access (renamed from 'commands' -> 'workflow' -> 'repo')
    try:
        from mcli.app.commands_cmd import commands, repo, workflow

        app.add_command(repo, name="repo")
        # Backward compatibility aliases
        app.add_command(workflow, name="workflow")
        app.add_command(commands, name="commands")
        logger.debug("Added repo management group (with backward aliases: workflow, commands)")
    except ImportError as e:
        logger.debug(f"Could not load repo management group: {e}")
    ...
    # Note: lib group removed - secrets moved to run
    # Previous: mcli lib secrets -> Now: mcli run secrets

    # Add run group directly (not lazy-loaded) to preserve -g/--global option
    try:
        from mcli.workflow.workflow import run as run_group
        from mcli.workflow.workflow import workflows as workflows_group

        app.add_command(run_group, name="run")
        # Backward compatibility alias
        app.add_command(workflows_group, name="workflows")
        logger.debug("Added run group with -g/--global support (with backward alias: workflows)")
    except ImportError as e:
        logger.error(f"Could not load run group: {e}")
        # Fallback to lazy loading if import fails
        try:
            from mcli.app.completion_helpers import create_completion_aware_lazy_group

            run_group = create_completion_aware_lazy_group(
                "run",
                "mcli.workflow.workflow.run",
            ...
            run_group = LazyGroup(
                "run",
                "mcli.workflow.workflow.run",
                help="Run workflows for automation, video processing, and daemon management",
            )
            app.add_command(run_group, name="run")
    ...
    for cmd_name, cmd_info in lazy_commands.items():
        # Skip run since we already added it with completion support
        if cmd_name == "run":
            continue
