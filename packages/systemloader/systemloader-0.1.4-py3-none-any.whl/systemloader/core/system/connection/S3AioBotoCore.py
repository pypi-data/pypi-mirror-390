...
class S3AioBotoCore(BaseConnection):
    ...
    @client_context  # Список объектов в корзине
    async def list_objects_v2(self, client, bucket_name=None, is_entire_obj=False, request_args: dict | None = None):
        if bucket_name is None:
            bucket_name = self.values['bucket_name']
        request_args = {**request_args} if isinstance(request_args, dict) else {}
        request_args['Bucket'] = bucket_name
        if not ('MaxKeys' in request_args):
            request_args['MaxKeys'] = 1_000
        continuation_token = None
        list_obj = []
        while True:
            if continuation_token:
                request_args['ContinuationToken'] = continuation_token
            result = await client.list_objects_v2(**request_args)
            # Обработка объектов в текущей странице
            for obj in result.get('Contents', []):
                list_obj.append(obj if is_entire_obj else obj['Key'])
            # Проверка на наличие дополнительных страниц
            if not (continuation_token := result.get('NextContinuationToken')):
                break
        return list_obj
