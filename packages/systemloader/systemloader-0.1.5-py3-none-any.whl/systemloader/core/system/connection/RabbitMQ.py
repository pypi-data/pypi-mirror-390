...
class RabbitMQ(BaseConnection):
    ...
    def gen_msg(self, system_to: str, object: str, action: str, reference_id=None, msg_id=None,
                params=None, msg_update=None) -> dict:
        if not msg_id:
            msg_id = str(uuid4())
        if not reference_id:
    ...
    async def publish_reply(self, msg, error_list, params_reply, msg_update=None):
        ...
        rabbit_json = self.gen_msg(system_to=msg['system_from'], object=msg['object'], action=f"{msg['action']}_reply",
                                   reference_id=msg['reference_id'], params=params, msg_update=msg_update)
    ...
    async def get_connection(self):
        if rabbitmq_url := self.values.get('rabbitmq_url'):
            return await aio_pika.connect_robust(rabbitmq_url, loop=self.loop)
        else:
            return await aio_pika.connect_robust(url=None, **self.values['connect'], loop=self.loop)
    ...
    async def connection(self):
        ...
        self.queue = {}
        if rabbitmq_url := self.values.get('rabbitmq_url'):
            self.logger.info(f"Подключение к RabbitMQ ({dsn_hide_password(rabbitmq_url)})")
        else:
            connect_dict = {key: value for key, value in self.values['connect'].items() if key != 'password'}
            self.logger.info(f"Подключение к RabbitMQ ({connect_dict})")
        ...
        try:
            async with self.channel_pool.acquire() as channel:
                ...
                if exchange_type := self.values['exchange'].get('type'):
                    self.exchange = await channel.declare_exchange(
                        self.values['exchange']['name'], type=exchange_type,
                        durable=self.values['exchange']['durable'],
                        auto_delete=self.values['exchange']['auto_delete'])
                else:
                    self.exchange = await channel.get_exchange(self.values['exchange']['name'])
                ...
                for queue_name, queue_bindings in dispatcher.queue_bindings.items():
                    if exchange_type:
                        ...
                        self.queue[queue_name] = await channel.get_queue(queue_name)
                    ...
                    self.logger.info(f'RabbitMQ: успешно инициализирован!')
                    return True
        except ConnectionError as err:
            self.logger.error(f"RabbitMQ: Нет подключения к пулу каналов или очереди! "
