...
ACTIONS: {actions}
            ...
            if isinstance(action_data, dict):
                try:
                    ...
                    raise ValueError(f"Unknown action: {action_data}, only DONE is supported") from e
    ...
    def _get_cached_image(self, screenshot_path: str) -> str:
        ...
        try:
            request = requests.get(f"{settings.dojo_http_endpoint}/image?path={screenshot_path}")
            screenshot_base64 = base64.b64encode(request.content).decode("utf-8")

            # Calculate size for monitoring
            size_mb = len(screenshot_base64) / (1024 * 1024)
