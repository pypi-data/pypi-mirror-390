...
from .base_dojo_client import BaseDojoClient, NoRunnersAvailableError, TaskStatus
...
class DojoReward(vf.RewardFunc):
    def __init__(self, client: BaseDojoClient, task_loader: RemoteTaskLoader, verbose: bool = False):
        self.client = client
...
def load_environment(API_KEY: str, system_prompt: str, tasks: List[str], **kwargs):
    ...
    rubric = vf.Rubric(
        funcs=[DojoReward(client, task_loader)],
        weights=[1.0],
    )
