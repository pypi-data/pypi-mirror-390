...
from .base_dojo_client import BaseDojoClient, NoRunnersAvailableError
...
class DojoEvalClient:
    ...
    def __init__(self, agent: BaseAgent, api_key: str, verbose: bool = False):
        self.agent = agent
    ...
    async def _submit_action(self, task_def, metadata, exec_id, action, reasoning, raw_response) -> (TaskResponse, float):
        ...
        task_state = await self.client.get_task_status(exec_id)
        ...
        await self.client.submit_step_score(exec_id, metadata.current_step, reward)
    ...
    async def _try_terminate_task(self, exec_id: str, reason: TaskStatus) -> None:
        ...
        """
        try:
            await self.client.stop_task(exec_id, reason)
