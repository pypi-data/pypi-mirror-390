...
sub handler_master_dbx_preprocess_file
{
	...
	# Converts double single quotes within a string to double quotes.
	$cont =~ s/\'\'(\w+)\'\'/"$1"/g;
	...
	$cont =~ s/\bBEGIN\s+TRANSACTION\b//gis;
...
sub convert_select_into
{
	...
	$sql  =~ s/(\bSELECT.*?)INTO\s+(\bTEMP_TABLE_\w+)(.*)/CREATE TEMPORARY TABLE $2 AS\n$1$3/is;
...
sub convert_select_into_real_table
{
	...
	$sql  =~ s/(\bSELECT.*?)INTO\s+(\`?\w+\`?\.\`?\w+\`?\.?\`?\w*\`?)(.*)/CREATE OR REPLACE TABLE $2 AS\n$1$3/is;
...
sub mark_separators
{
    ...
    my @prior_keywords = ('MERGE', 'ALTER', 'DROP', 'TRUNCATE', 'IF', 'GO', 'WHILE', 'RETURN', 'BREAK', 'BEGIN\s+TRANSACTION', 'CREATE\s+TABLE', 'CREATE\s+INDEX', 'COMMIT', 'ROLLBACK', 'SAVE\s+TRANSACTION',
