...
# Third-party imports
import requests
from mastodon.return_types import Account, MediaAttachment, Status, StatusMention
...
print(masto_instance().instance().domain)
...
def mention_to_email_address(mention: StatusMention) -> str:
    username = mention.username
    address = mention.acct

    return formataddr((username, address))

def calculate_to(toot: Status) -> str:
    if toot.visibility == PUBLIC_VISIBILITY:
        return UNDISCLOSED_RECIPIENTS
    if toot.visibility == UNLISTED_VISIBILITY:
        return UNDISCLOSED_RECIPIENTS
    if toot.visibility == PRIVATE_VISIBILITY:
        return UNDISCLOSED_RECIPIENTS
    if toot.visibility == DIRECT_VISIBILITY:
        return ", ".join([mention_to_email_address(mention)for mention in toot.mentions])

    return "ERROR"

def calculate_cc(toot: Status) -> str | None:
    if toot.visibility == DIRECT_VISIBILITY:
        return None
...
def create_new_message(toot, folder):
    ...
    msg.add_header('To', calculate_to(toot=toot))
    msg.add_header('Date', utils.format_datetime(toot.created_at))

    cc = calculate_cc(toot=toot)
    if cc:
