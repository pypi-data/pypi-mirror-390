...
from thkit.display import DynamicBarColumn, TextDecor, ThangBar, _index2color
from thkit.jobman.helper import (
    init_jobman_logger,
    loadconfig_multi_machines,  # noqa # this is alias for easy import from outside
    remote_info,
)
...
def submit_job_chunk(
    ...
    bar1 = thangbar.add_task(
        description=TextDecor(f"{_strtime_now()} Machine {machine_index}").make_color(color),
        total=num_tasks,
        eta_text="[ETA --:--]",  # add custom field
    ...
    for idx, task_list_current_chunk in enumerate(chunks):
        ...
        try:
            ...
            logger.error(TextDecor(f"Machine {machine_index} error: \n\t{e}").make_color(color))
        ...
        thangbar.update(
            ...
            description=f"{_strtime_now()} Machine {machine_index}",
    ...
    ### Finish message
    thangbar.hide_bar()
    logger.info(TextDecor(f"Machine {machine_index} finished all jobs !").make_color(color))
    thangbar.show_bar()
