...
from .smart_router import AutoRouterOptions, run_auto_router
...
def _build_prompt_session() -> PromptSession:
    ...
    def _shift_pressed() -> bool:
        try:
            return bool(get_app().key_processor.shift_pressed)
        except Exception:  # pragma: no cover - extremely unlikely
            return False

    @bindings.add("enter", filter=~_shift_pressed)
    def _(event) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    autorouter_parser = subparsers.add_parser(
        "autorouter",
        help="Interactive chat that auto-routes web search before querying deepseek-reasoner.",
    )
    autorouter_parser.add_argument(
        "--force-search",
        action="store_true",
        help="Force Tavily web search ON for all turns.",
    )
    autorouter_parser.add_argument(
        "--no-search",
        action="store_true",
        help="Force Tavily web search OFF for all turns.",
    )
    autorouter_parser.add_argument(
        "--max-results",
        type=int,
        default=8,
        help="Maximum Tavily results when search is enabled (default: 8).",
    )
    autorouter_parser.add_argument(
        "--search-depth",
        choices=["basic", "advanced"],
        default="advanced",
        help="Search depth to request from Tavily (default: advanced).",
    )
    autorouter_parser.add_argument(
        "--time-range",
        default=None,
        help="Optional Tavily time_range filter (d,w,m,y) if your plan supports it.",
    )
    autorouter_parser.add_argument(
        "--verbose-router",
        action="store_true",
        help="Print the heuristic decision about running web search.",
    )
...
def handle_autorouter(args: argparse.Namespace, resolved: ResolvedConfig) -> int:
    ...
    options = AutoRouterOptions(
        ...
        verbose_router=args.verbose_router,
    )
    return run_auto_router(client, options, tavily_api_key=resolved.tavily_api_key)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.command == "autorouter":
        return handle_autorouter(args, resolved)
