...
class GraphQuery(FluentQuery):
    ...
    def full_text_search(
        ...
        # Use self.r() to handle both filtered and all-elements cases
        base_elements = self.r()
        filtered_results = []
        for el in base_elements:
    ...
    def all(self) -> "GraphQuery":
        """
        Initialize query to include all elements.

        Useful for explicitly starting a fluent query on all elements without
        requiring prior filters. This ensures subsequent filter/search methods
        operate on the complete element set.

        Returns:
            self - For method chaining

        Examples:
            # Direct full-text search on all elements
            results = query.all().full_text_search('john').first(5).r()

            # Wildcard search on all elements
            results = query.all().wildcard_search('*smith*').first(5).r()

            # Chain with other filters
            results = query.all().text('john').classId('Person').r()
        """
        self.result = self.store.get_all_elements()
        self._operations_performed = True

        # Record operation
        self.query_operations.append(
            IFluentOperation(
                method=QueryOperations.FILTER_ALL, params={"include_all": True}
            )
        )
            ...
            if direction in ("outgoing", "both") and hasattr(element, "outgoing_edges"):
                for edge in element.outgoing_edges:
                    ...
                    if operator == FilterOperator.EQ:
                        ...
                        matches = (
                            isinstance(prop_value, (int, float)) and prop_value > value
                        )  # type: ignore
                    elif operator == FilterOperator.LT:
                        matches = (
                            isinstance(prop_value, (int, float)) and prop_value < value
