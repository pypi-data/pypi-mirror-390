...
class KpiComparator:
    ...
    def plot_fundamental_diagram(self, **kwargs):
        ...
        sns.scatterplot(
            df[df.type_x.str.contains("FREEWAY|EXPRESSWAY")], x="density_bin", y="flow", hue="run_id", ax=axs[0]  # type: ignore
        )  # type: ignore
        axs[0].set_ylabel("Flow rate (in veh/hr/lane)", fontsize=14)
        axs[0].set_xlabel("Density (in veh/mi/lane)", fontsize=14)

        sns.scatterplot(df[df.type_x == "MAJOR"], x="density_bin", y="flow", hue="run_id", ax=axs[1])  # type: ignore
    ...
    def plot_parking_demand(self, **kwargs):
        ...
        df["parking_type"] = df["parking_type"].fillna("magic_parking")
        df = (
    ...
    def plot_parking_access(self, **kwargs):
        ...
        if df is None or df.empty:
            logging.warning("There were no results for parking access")
            return

        fig, ax = plt.subplots(nrows=2, ncols=1, figsize=(10, 8), sharex="all")
        ...
        sns.barplot(df, hue="run_id", x="Escooter_Borrowed", y="demand", ax=ax[1])  # type: ignore
        ax[1].set(title="Demand", xlabel="Used Escooter?", ylabel="# Users")

        KpiComparator._style_axes(ax, rotate_x_labels=0, fontsize=14)

        return fig

    def plot_parking_utilization(self, type_filter=None, **kwargs):
        import matplotlib.pyplot as plt

        df = self._get_results("parking_utilization", **kwargs)
        if type_filter is not None:
            df = filter_df(df, {"type": type_filter})
        ...
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10, 8), sharex="all")
        sns.lineplot(df, hue="run_id", x="start_hr", y="avg_occ", ax=ax)
        ax.set(title="Parking Utilization", xlabel="Time of Day (hr)", ylabel="Ratio of occupancy")

        KpiComparator._style_axes([ax], rotate_x_labels=0, fontsize=14)

        return fig

    def plot_parking_revenue(self, **kwargs):
        import matplotlib.pyplot as plt

        df = self._get_results("parking_revenue", **kwargs)
        if df is None or df.empty:
