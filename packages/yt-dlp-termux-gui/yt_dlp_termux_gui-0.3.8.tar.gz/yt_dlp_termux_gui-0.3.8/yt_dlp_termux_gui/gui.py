...
def handle_lifecycle_events(event: tg.Event, verbose: bool = True):
...
def build_gui(connection: tg.Connection, verbose: bool = True):
    ...
    print_msg(verbose, "GUI: Initializing container...")
    ...
    print_msg(verbose, "GUI: Initializing title...")
    title = tg.TextView(activity, constants.WIDGET_TITLE, container)
    title.settextsize(25)
    title.setmargin(20, 'bottom')
    
    print_msg(verbose, "GUI: Initializing metadata container...")
    ...
    inputs: dict[str, str] = {label: placeholder for label, placeholder in settings.defaults.items()}
    
    print_msg(verbose, "GUI: Initializing input fields...")
    for i, (label, placeholder) in inputs.items():
        print_msg(verbose, f"GUI: Initializing input field title #{i + 1} -> \"{label}\"...")
        title = tg.TextView(activity, label, container)
        title.settextsize(18)
        title.setmargin(6, 'top')
        print_msg(verbose, f"GUI: Initializing input field for \"{label}\"...")
        input_fields[label] = tg.EditText(activity, previous_settings.get(label, placeholder), container, singleline=True)
        input_fields[label].settextsize(16)
        input_fields[label].setmargin(2, 'top')
        
        if label == "Video URL":
            print_msg(verbose, f"GUI: Initializing button clipboard button...")
    ...
    print_msg(verbose, "GUI: Initializing checkbox #1...")
    ...
    print_msg(verbose, "GUI: Initializing checkbox #2...")
    ...
    print_msg(verbose, "GUI: Initializing button #1...")
    reset_settings_btn = tg.Button(activity, "Reset Settings", container)
    
    print_msg(verbose, "GUI: Initializing button row...")
    run_row = tg.LinearLayout(activity, container, False)
    
    print_msg(verbose, "GUI: Initializing button #2...")
    cancel_btn = tg.Button(activity, "Exit", run_row)
    
    print_msg(verbose, "GUI: Initializing button #3...")
    run_btn = tg.Button(activity, "Run", run_row)

    print_msg(verbose, "GUI: Initializing status container...")
    ...
    print_msg(verbose, "GUI: Initializing event listener...")
    for event in connection.events():
        utils.run_job(handle_lifecycle_events, (event, verbose))
            ...
            if input_id == input_fields["Video URL"]:
                try:
                    ...
                    if utils.is_valid_url(media_url):
                        def update_metadata():
                            print_msg(verbose, "GUI: Fetching metadata...")
...
def launch_gui(verbose: bool = True):
    try:
        print_msg(verbose, "Logging is enabled")
