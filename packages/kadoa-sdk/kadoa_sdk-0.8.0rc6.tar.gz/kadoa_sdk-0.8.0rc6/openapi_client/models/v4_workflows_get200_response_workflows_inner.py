...
class V4WorkflowsGet200ResponseWorkflowsInner(BaseModel):
    ...
    errors: Optional[List[V4WorkflowsWorkflowIdHistoryGet200ResponseWorkflowRunsInnerErrorsInner]] = Field(default=None, description="Errors from the last job run")
    ...
    __properties: ClassVar[List[str]] = ["id", "name", "description", "state", "displayState", "userId", "createdAt", "tags", "limit", "finishedAt", "startedAt", "jobId", "dataKey", "location", "runCosts", "runState", "totalRecords", "url", "urls", "browserActions", "schema", "templateName", "templateId", "errors", "dataValidation", "validationStatistics", "monitoring", "notificationConfig", "channelCounts"]
    ...
    def display_state_validate_enum(cls, value):
        ...
        if value not in set(['ACTIVE', 'ERROR', 'PAUSED', 'NOT_SUPPORTED', 'PREVIEW', 'COMPLIANCE_REVIEW', 'COMPLIANCE_REJECTED', 'QUEUED', 'SETUP', 'RUNNING', 'FAILED']):
            raise ValueError("must be one of enum values ('ACTIVE', 'ERROR', 'PAUSED', 'NOT_SUPPORTED', 'PREVIEW', 'COMPLIANCE_REVIEW', 'COMPLIANCE_REJECTED', 'QUEUED', 'SETUP', 'RUNNING', 'FAILED')")
        return value
