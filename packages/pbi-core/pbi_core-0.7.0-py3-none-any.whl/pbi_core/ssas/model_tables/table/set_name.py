...
def update_table_references(dax_expr: str, old_name: str, new_name: str) -> str | None:
    ...
    ast_nodes = ast.find_all(
        (
            dax.exprs.ColumnExpression,
            dax.exprs.TableExpression,
            dax.exprs.HierarchyExpression,
        ),
    )
    # this will eventually be a function to handle escaping properly
    dax_new_name = f"'{new_name}'"
    for node in ast_nodes:
        if isinstance(node, dax.exprs.ColumnExpression):
            if dax.utils.get_inner_text(node.table) == old_name:
                node.table = dax.Token.from_str(dax_new_name)
        elif isinstance(node, dax.exprs.TableExpression):
            if dax.utils.get_inner_text(node.name) == old_name:
                node.name = dax.Token.from_str(dax_new_name)
        elif isinstance(node, dax.exprs.HierarchyExpression):  # noqa: SIM102
            if dax.utils.get_inner_text(node.table) == old_name:
                node.table = dax.Token.from_str(dax_new_name)
    return dax.Formatter(ast).format()


def fix_dax(table: "Table", new_name: str) -> None:
    """Fix DAX expressions for a table rename.

    Args:
        table (Table): The table being renamed.
