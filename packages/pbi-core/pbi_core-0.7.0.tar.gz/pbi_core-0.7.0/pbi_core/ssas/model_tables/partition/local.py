...
class LocalPartition(Partition):
    """Class for a Partition that does not yet exist in SSAS.
    ...
    partition_storage_id: int = field(default=-1, on_setattr=setters.frozen)
    ...
    def load(self, ssas: "BaseTabularModel") -> "Partition":
        return self._create_helper(ssas, ssas.partitions)
