...
class StockStatsNS:
    ...
    def rsi(self, timeframe: int = 14, *, by: By | None = None) -> pl.Expr:
        ...
        avg_gain = gain.ewm_mean(alpha=1.0 / timeframe, adjust=True, ignore_nulls=True)
        avg_loss = loss.ewm_mean(alpha=1.0 / timeframe, adjust=True, ignore_nulls=True)

        rs = avg_gain / avg_loss
        out = 100 - (100 / (1 + rs))
        return _maybe_over(out, by)

    def sma(self, n: int, *, by: By | None = None) -> pl.Expr:
        """
    ...
    def boll(self, n: int = 20, *, k: Number = 2.0, by: By | None = None) -> pl.Expr:
        """
