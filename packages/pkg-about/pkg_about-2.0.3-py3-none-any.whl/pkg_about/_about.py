...
def about(package: str | None = None) -> __adict:
    ...
    email = pkg_metadata["__author_email__"]
    if not pkg_metadata["__author__"] and email:
        names = ", ".join(name for name, _ in getaddresses([email]) if name)
        if names: pkg_metadata["__author__"] = names
    email = pkg_metadata["__maintainer_email__"]
    if not pkg_metadata["__maintainer__"] and email:
        names = ", ".join(name for name, _ in getaddresses([email]) if name)
        if names: pkg_metadata["__maintainer__"] = names
