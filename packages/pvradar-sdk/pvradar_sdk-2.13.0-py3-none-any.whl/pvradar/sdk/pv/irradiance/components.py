...
def pvlib_clearness_index(
    ghi: Annotated[pd.Series, R.global_horizontal_irradiance],
    solar_zenith_angle: Annotated[pd.Series, R.solar_zenith_angle(apparent=False)],
    extra_radiation: Annotated[pd.Series, R.extraterrestrial_radiation],
    min_cos_zenith: Annotated[float, Field(ge=0, le=1.0)] = 0.065,
...
def pvlib_clearsky_table(
    ...
    airmass: pd.Series,  # Annotated[pd.Series, R.airmass(relative=False)],
    dni_extra: Annotated[pd.Series, R.extraterrestrial_radiation],
    location: Location,
    interval: pd.Interval,
    freq: str,
) -> pd.DataFrame:
