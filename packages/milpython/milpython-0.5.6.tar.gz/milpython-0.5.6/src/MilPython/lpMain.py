...
class LPMain:
    ...
    def __init__(self,inputdata:LPInputdata):
        ...
        self.obj_lst:list[LPObject] = [self]
        self.obj_lst.extend([value for value in vars(self).values() if isinstance(value,LPObject)])
    ...
    def add_object(self,obj:LPObject):
        '''Adds an LPObject to the system'''
    ...
    def add_var_targetfun(self,var:LPStateVar,value,step=None):
        '''
        ...
        '''
        if step==None and (isinstance(var,LPStateVar_timedep) or isinstance(var,LPStateVar_Decision_timedep)):
            for t in range(self.inputdata.steps):
                if isinstance(value,list) or isinstance(value,np.ndarray):
                    self.f[var.pos+t*len(self.stateVars_timedep)]=value[t]
                else:
                    self.f[var.pos+t*len(self.stateVars_timedep)]=value
        else:
            if step==None:
                step=0
