...
class StatementClassifier:
  ...
  def classifyStatement(cls, lines: list[str]) -> BlockType:
    ...
    # But exclude control keywords (if, for, while, etc.)
    controlKeywords = r'^\s*(if|elif|else|for|while|try|except|finally|with|async|def|class)\s'
    if re.match(r'^[\w\.]+\s*\(', firstLine) and not re.match(r'^\s*[\w\.\[\]]+\s*=', firstLine) and not re.match(controlKeywords, firstLine):
      return BlockType.CALL
