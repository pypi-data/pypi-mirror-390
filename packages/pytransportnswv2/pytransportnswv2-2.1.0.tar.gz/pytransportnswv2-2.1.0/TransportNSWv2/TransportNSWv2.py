...
class TransportNSWv2(object):
    ...
    def get_trip(self, name_origin, name_destination , api_key, journey_wait_time = 0, origin_transport_type = [0], destination_transport_type = [0], \
        ...
        if self._origin_is_coords(name_origin):
            type_origin = "coord"
            #exclusion = self._get_excluded_means(origin_transport_type)
            exclusion = ""

            # Make sure walking and footpaths are in the origin transport types list
            for tt in [99, 100]:
                if tt not in origin_transport_type:
                    origin_transport_type.append(tt)
        ...
        for current_journey_index in range (0, retrieved_journeys, 1):
            ...
            legs, next_journey_index, first_leg, last_leg, changes, changes_list = self._find_next_journey(result['journeys'], current_journey_index, origin_transport_type, destination_transport_type, strict_transport_type, route_filter, type_origin)
...
#                # Do we need to over-ride the origin info if we're doing a coord-based journey?
#                if type_origin == "coord":
#                    # We want to get some information from the SECOND leg!
#                    #origin_stop_id, origin_name = self._find_first_stop(legs, first_leg, last_leg)
...
#                    origin_name = origin_stop['name']
    ...
    def _find_next_journey(self, journeys, start_journey_index, origin_transport_type, destination_transport_type, strict, route_filter, type_origin):
        ...
        for journey_index in range (start_journey_index, journey_count, 1):
            ...
            origin_leg = self._find_first_leg(journey['legs'], origin_transport_type, strict, route_filter, type_origin)
...
#                    return None
            ...
            for mode_url in url_mode_list:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 401:
                        ...
                        logger.error(f"Error 'API rate limit exceeded' calling {url} API")
                    else:
    ...
    def _get_excluded_means(self, transport_type):
        # Create an 'excluded transport type' string based on what' INCLUDED in the transport_type list
        if transport_type == [0]:
            return ""
