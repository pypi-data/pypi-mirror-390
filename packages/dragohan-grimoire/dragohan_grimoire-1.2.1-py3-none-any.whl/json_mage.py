# json_mage.py - Your Personal JSON Grimoire (ULTIMATE EDITION v4)
"""
DragoHan's JSON Mastery Library
The simplest way to work with JSON - no bullshit, just results.

Powers: 27+ methods for reading, counting, filtering, sorting, math, and modifying JSON
"""
...
class MageJSON:
    ...
    # ===================================================================
    # COUNTING & FILTERING POWERS (4 methods) - SMART_FILTER ADDED!
    # ===================================================================
    ...
    def smart_filter(self, key: str, value: Any) -> List:
        """
        UNIVERSAL smart filter - works with ANY nested JSON structure
        
        Finds items where key matches value, no matter how deep it's nested.
        
        Usage:
            logs.smart_filter("type", "fire")      # Finds fire types
            logs.smart_filter("ability", "blaze")  # Finds abilities
            logs.smart_filter("status", "active")  # Finds by status
        
        Handles ALL patterns:
            - Direct: {type: "fire"}
            - Nested: {type: {name: "fire"}}
            - Plural: {types: [{type: {name: "fire"}}]}
            - Any depth: {data: {nested: {type: {value: "fire"}}}}
        ...
        for item in self._raw:
            if item.get('error'):  # Skip error items
                continue
            if self._smart_match(item, key, value):
                results.append(item)
    ...
    def where(self, jmes_query: str) -> Any:
        ...
        Note: Usually .smart_filter() is simpler!
        """
    ...
    def _smart_match(self, obj: Any, search_key: str, search_value: Any, depth: int = 0) -> bool:
        """
        Recursive deep search - finds key-value pair at ANY depth
        
        Patterns checked (in order):
        1. Direct match: obj[search_key] == search_value
        2. Nested object: obj[search_key].name == search_value
        3. Nested object: obj[search_key].value == search_value
        4. Plural array: obj[search_key + 's'][i][search_key].name == search_value
        5. Recursive: Search through ALL nested dicts and lists
        """
        ...
        # PATTERN 2: Nested object with .name or .value
        # Example: {type: {name: "fire"}}
        if search_key in obj and isinstance(obj[search_key], dict):
            nested = obj[search_key]
            if nested.get('name') == search_value or nested.get('value') == search_value:
                return True
        
        # PATTERN 3: Plural array with nested objects
        # Example: {types: [{type: {name: "fire"}}]}
        plural_key = search_key + 's'
        if plural_key in obj and isinstance(obj[plural_key], list):
            for item in obj[plural_key]:
                if isinstance(item, dict):
                    # Check if item has nested search_key
                    if search_key in item:
                        nested = item[search_key]
                        # Check nested.name or nested.value
                        if isinstance(nested, dict):
                            if nested.get('name') == search_value or nested.get('value') == search_value:
                                return True
                        elif nested == search_value:
                            return True
        
        # PATTERN 4: Recursive deep search through ALL nested structures
        # Searches through every dict and list at any depth
        for key, val in obj.items():
            # Search nested dicts
            if isinstance(val, dict):
                if self._smart_match(val, search_key, search_value, depth + 1):
                    return True
            ...
            # Search nested lists
                ...
                for list_item in val:
                    if isinstance(list_item, dict):
                        if self._smart_match(list_item, search_key, search_value, depth + 1):
                            return True
