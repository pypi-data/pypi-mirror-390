...
class Mol:
    ...
    def make_confs(self, n: int = 50, method: str = 'ETKDG', **kwargs) -> Self:
        ...
        if verbose:
            ...
            logger.info(f"make_confs() rotatable bonds {rot_bonds} (suggested conformers {nrb_suggested}) generated {self.num_confs}")
            logger.info(f"make_confs() updated potential energies E_tot(kcal/mol) and E_rel(kcal/mol) by {calculator}")

        return self


    def singlepoint_confs(self, 
                          calculator: str | Callable, 
                          batchsize_atoms: int = 0,
                          water: str | None = None) -> Self:
        ...
        Returns:
            Self: modified self.
        """
        if isinstance(calculator, Callable) and batchsize_atoms >= self.confs[0].natoms:
            self.confs = batch_singlepoint(self.confs, calculator, batchsize_atoms)
        else:
            self.confs = [conf.singlepoint(calculator, water=water) for conf in self.confs]
    ...
    def optimize_confs(self, 
        ...
        if isinstance(calculator, Callable) and batchsize_atoms >= self.confs[0].natoms:
            ...
            self.confs = [ conf.optimize(calculator, fmax, max_iter, water=water) for conf in self.confs ]
    ...
    def align_confs(self, method: str = 'rigid_fragment') -> Self:
        ...
        if method == 'rigid_fragment': 
            ...
            for i in range(1, self.num_confs):
                rmsd = rdMolAlign.AlignMol(prbMol=self.confs[i].rdmol, refMol=self.confs[0].rdmol, atomMap=atomMap)
            ...
            for i in range(1, self.num_confs):
                ...
                rmsd = rdMolAlign.GetBestRMS(prbMol=self.confs[i].rdmol, refMol=self.confs[0].rdmol)
            self.is_confs_aligned = True
    ...
    def cluster_confs(self, 
        ...
        if method != 'DQT': # rmsd of x,y,z coordinates (non-H)
            ...
            if symmetry_aware:
                ...
                for i in range(1, self.num_confs):
                    ...
                    mol_js = [spyrmsd_Molecule.from_rdkit(self.confs[j].rdmol) for j in range(i)]
                    rmsd_list = spyrmsd_wrapper(mol_i, mol_js, symmetry=True, minimize=False)
        ...
        if method == 'Butina':
            clusters = Butina.ClusterData(data=lower_triangle_values, 
                                          ...
                                          isDistData=True,
                                          reordering=True)
            cluster_assignment = [None,] * self.num_confs
            centroid_indices = []
    ...
    def drop_confs(self,
        ...
        if stereo_flipped and self.num_confs > 0:
            ...
            if verbose:
                logger.info(f'drop_confs() {mask.count(False):3d} {reasons[0]:<{w}} -> {self.num_confs}')
        
        if unconverged and self.num_confs > 0:
            mask = [_.props['Converged'] if 'Converged' in _.props else True for _ in self.confs]
            self.confs = list(itertools.compress(self.confs, mask))
            if verbose:
                logger.info(f'drop_confs() {mask.count(False):3d} {reasons[1]:<{w}} -> {self.num_confs}')
        ...
        if similar and self.num_confs > 1:
            ...
            self.confs = list(itertools.compress(self.confs, mask))
            if verbose:
                logger.info(f'drop_confs() {mask.count(False):3d} {reasons[2]:<{w}} -> {self.num_confs}')
        ...
        if cluster and self.num_confs > 1:
            ...
            self.confs = list(itertools.compress(self.confs, mask))
            if verbose:
                logger.info(f'drop_confs() {mask.count(False):3d} {reasons[3]:<{w}} -> {self.num_confs}')
    ...
    def calculate_torsion_energies(self,
                         calculator: str | Callable = 'MMFF94',
        ...
        Returns:
            Self: modified self.
        """
        assert self.num_confs > 0, "calculate_torsion_energies() requires at least one conformer"
        
        ref_conf = self.confs[0].copy()
        
        if batchsize_atoms >= ref_conf.natoms:
            assert not isinstance(calculator, str), "Batch optimizer is required for batch calculation"

        ref_conf = ref_conf.calculate_torsion_energies(calculator, 
                                            torsion_angle_idx, 
                                            simplify, 
                                            fmax, 
                                            interval, 
    ...
    def to_plot_data_torsion_angle_vs_energy(self, idx: int | None = None) -> dict:
        ...
        if isinstance(idx, int):
            ...
            data = {'idx':[idx,]*len(angles), 'angle':angles, 'E_rel(kcal/mol)':energies}
        else:
            data = {'idx':[], 'angle':[], 'E_rel(kcal/mol)':[]}
            for idx, _dict in enumerate(self.props['torsion']): # list
                angles, energies = _dict['angle'], _dict['E_rel(kcal/mol)']
                assert len(angles) == len(energies)
                data['idx'].extend([idx,]*len(angles))
    ...
    def serialize(self, decimals: int = 3) -> str:
        ...
        """
        return rdworks.utils.serialize({
            'name'  : self.name,
            'smiles': self.smiles,
