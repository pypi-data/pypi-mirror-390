...
Use the automated version bumping utility:
```bash
python scripts/bump_version.py patch  # Bug fixes (1.1.2 -> 1.1.3)
python scripts/bump_version.py minor  # New features (1.1.2 -> 1.2.0)
python scripts/bump_version.py major  # Breaking changes (1.1.2 -> 2.0.0)
```

The script updates both `pyproject.toml` and `src/camera_enum/__init__.py`.
...
5. Update version using `scripts/bump_version.py`
