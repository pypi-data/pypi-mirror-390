...
You can suggest features, enhancements, or report bugs on our [issue tracker](https://github.com/k-tech-italy/django-temprole/issues).
...
* Clone the repository:
    ```bash
    # using HTTPS
    git clone https://github.com/k-tech-italy/django-temprole.git

    # using SSH
    git clone git@github.com:k-tech-italy/django-temprole.git
    ```
...
* If you use [direnv](https://direnv.net/), copy the `.envrc.example` file as follows, otherwise skip this step:
    ...
    ```
In this case you don't need to source the venv but just run once (or when changing the .envrc file):
```shell
direnv allow
```
...
You can login with "admin" or any other user with any password
