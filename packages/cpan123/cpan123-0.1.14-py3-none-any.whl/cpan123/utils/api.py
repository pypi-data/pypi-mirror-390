...
class Api:
    ...
    def update_attr2(self) -> "Api":
        def stringify_values(obj):
            if isinstance(obj, dict):
                return {k: json5.dumps(v, ensure_ascii=False, separators=(",", ":")) if isinstance(v, (dict, list)) else v for k, v in obj.items()}
    ...
    def _prepare_request(self) -> dict:
        ...
        full_url = self.url if self.url.startswith("http") else f"{BASE_URL.rstrip('/')}/{self.url.lstrip('/')}"
        # 过滤掉 data/params 中为 None 的键，避免带空字段
        cleaned_params = {k: v for k, v in (self.params or {}).items() if v is not None} if self.params is not None else None
        cleaned_data = {k: v for k, v in (self.data or {}).items() if v is not None} if self.data is not None else None
