...
class Pan123openAPI:
    ...
    def __init__(
        ...
        self.file2 = File2(auth=self.auth)
        self.uploader = Uploader(auth=self.auth)
    ...
    def download(
        ...
        for _idx, fname in enumerate(need_down_files):
            try:
                ...
                fileId, fileItem = self.find_file_id_by_path(fname, root_id=0, is_dir=False)
    ...
    def _upload_common(
        ...
        if not use_oss:
            ...
            if str(Path(need_find_name).parent) not in ["", "/"]:
                ...
                file_id, _ = self.find_file_id_by_path(str(Path(need_find_name).parent), root_id=0, is_dir=True)
                if not file_id:
                    raise ValueError(f"找不到目录 {Path(need_find_name).parent},请先创建")
                else:
                    parentFileID = int(file_id)

            if parentFileID == 0:
                file_id, file_item = self.find_file_id_by_path(str(need_find_name), root_id=parentFileID, is_dir=False)
                if file_id and overwrite:
                    ...
                    print(f"✅ 云端文件 {need_find_name} 移动到回收站, message: {trash_out.message}")
        ...
        with open(filename, "rb") as f:
            ...
            if w1 <= 1:
                ...
                with tqdm(total=t1, desc="上传进度", unit="slice") as pbar:
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        futures = {executor.submit(upload_slice, i): i for i in range(total_sliceNo)}
            ...
            if complete_response.data is not None and complete_response.data.get("completed"):
                print("✅ 文件上传完成")
                return complete_response.data["fileID"]

            if complete_response.data is not None and complete_response.data.get("async"):
                while True:
                    time.sleep(2)
                    data_response = uploader.upload_async_result(preuploadID)
                    if data_response.data is not None and data_response.data.get("completed"):
                        print("✅ 文件上传完成")
                        return data_response.data["fileID"]
        return -1
    ...
    def upload(
        ...
        #     return -1
        res = self._upload_common(filename, upload_name, parentFileID, overwrite, False, duplicate, containDir)
        return res
    ...
    @validate_call
    def find_file_id_by_path(self, path: str | PurePosixPath | Path, root_id: int = 0, is_dir: bool = True) -> tuple[int, dict] | tuple[None, None]:
        ...
        for index, name in enumerate(parts):
            ...
            last_file_id = None
            while True:
                res = self.file.list_v2(parentFileId=current_id, lastFileId=last_file_id, limit=100)
                ...
                for item in res.data["fileList"]:
                    if item["filename"] == name and item["trashed"] == 0:
                        ...
                        # 最后一部分根据 is_dir 判断类型
                        if index == len(parts) - 1 and ((is_dir and item["type"] != 1) or (not is_dir and item["type"] != 0)):
                            continue
    ...
    @validate_call
    def _get_file_list(self, parentFileId: int, current_path: str = "", base_path: str = "") -> list[dict[str, Any]]:
        """
        ...
        while True:
            res = self.file.list_v2(parentFileId=parentFileId, lastFileId=lastFileId, limit=100)
    ...
    @validate_call
    def list_files(self, dirnames: Sequence[Union[str, int, Path, PurePosixPath]]) -> list[dict] | None:
        """
        ...
        if isinstance(dirnames, (str, int, Path, PurePosixPath)):
            dirnames = [dirnames]
        if isinstance(dirnames, list) and not all(isinstance(dirname, (str, int, Path, PurePosixPath)) for dirname in dirnames):
        ...
        for dirname in dirnames:
            ...
            files = self._get_file_list(file_id, current_path=str(dirname), base_path=str(dirname))
            if files is None:
                continue
            # 获取目录本身的 id
            thisdir = self.file.infos([file_id])
            if thisdir.data and thisdir.data.get("fileList") and len(thisdir.data["fileList"]) == 1:
    ...
    def download_dir(
        self,
        dirnames: Sequence[Union[str, int, Path, PurePosixPath]] | Union[str, int, Path, PurePosixPath],
    ...
    def _ensure_remote_path(self, path: Path, root_id: int) -> int:
        ...
        for part in path.parts:
            ...
            if not res or res.data is None or res.code != 0:
                raise RuntimeError(f"❌ 创建目录失败: {part} (父ID: {current_id}) - 返回: {res.message}")
    ...
    @validate_call
    def delete_dir(self, dirnames: Sequence[Union[str, int, Path, PurePosixPath]]) -> None:
        """
        删除指定目录及其下的所有文件(含子目录).
