...
class TelescopeImage(BaseModel):
    ...
    def from_fits_file(cls, file_path: Path, telescope: Telescope) -> "TelescopeImage":
        with fits.open(file_path) as hdul:
            ...
            if "SITELAT" in header and "SITELONG" in header:
                lat = header["SITELAT"]
                lon = header["SITELONG"]
            elif "OBSGEO-L" in header and "OBSGEO-B" in header:
                lon = header["OBSGEO-L"]
                lat = header["OBSGEO-B"]
            else:
                raise ValueError(
                    "Observatory location not found in FITS header. Expected either SITELAT/SITELONG or OBSGEO-L/OBSGEO-B."
                )
            
            if "SITEALT" in header:
                h = header["SITEALT"] * BASE_TO_KILO
            elif "OBSGEO-H" in header:
                h = header["OBSGEO-H"] * BASE_TO_KILO
            else:
                h = 0.0
                PIXELEMON_LOG.warning("Altitude not found in FITS header; defaulting to sea level (0.0 m). This may affect astronomical calculations.")
    ...
    def get_correlation_to_candidate(self, candidate: CorrelationCandidate) -> CorrelatedDetection | None:
        ...
        # use length if angle is in tolerance
            ...
            PIXELEMON_LOG.info(
                f"No suitable correlation for candidate {candidate.id}: "
                f"best centroid {best_centroid} (dist {centroid_distances[best_centroid]:.2f}), "
                f"best angle {best_angular} (diff {np.rad2deg(angular_differences[best_angular]):.2f}), "
                f"best length {best_length} (diff {length_differences[best_length]:.2f}), "
                f"best perpendicular {best_perpendicular} (dist {perpendicular_distances[best_perpendicular]:.2f})"
            )
            best_i = -1
    ...
    def get_correlation_to_detection(self, det: Detection, sats: CorrelationCandidates) -> CorrelatedDetection | None:
        ...
        if (
            ...
            PIXELEMON_LOG.info(
                f"No suitable correlation for detection at ({det.x_centroid:.2f}, {det.y_centroid:.2f}): "
                f"best centroid {best_by_centroid} (dist {centroid_distances[best_by_centroid]:.2f}), "
                f"best angle {best_by_angle} (diff {np.rad2deg(angle_differences[best_by_angle]):.2f}), "
                f"best length {best_by_length} (diff {length_differences[best_by_length]:.2f}), "
                f"best perpendicular {best_by_distance} (dist {perpendicular_distances[best_by_distance]:.2f})"
            )
            best_i = -1
