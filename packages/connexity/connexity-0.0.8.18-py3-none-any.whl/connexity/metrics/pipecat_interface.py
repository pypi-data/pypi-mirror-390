...
class InterfaceConnexityObserver(BaseObserver):
    ...
    def __init__(self):
        ...
        # tool call timeout tracking
        self._pending_tool_calls: Dict[str, PendingToolCall] = {}
        self.TOOL_CALL_TIMEOUT_SECS = 10.0
    ...
    async def _handle_tool_call_start(self, frame: Any, src: Any, direction: FrameDirection, t: float) -> None:
        ...
        pending_call = PendingToolCall(
            ...
            arguments=str(frame.arguments) if frame.arguments else "",
            start_time=t
        )
        self._pending_tool_calls[frame.tool_call_id] = pending_call
    ...
    async def _handle_tool_call_end(self, frame: Any, src: Any, direction: FrameDirection, t: float) -> None:
        ...
        self._pending_tool_calls.pop(frame.tool_call_id, None)
    ...
    async def _handle_pending_tool_calls(self, current_time: float) -> None:
        for tool_call_id, pending_call in list(self._pending_tool_calls.items()):
            elapsed_time = current_time - pending_call.start_time
            if elapsed_time >= self.TOOL_CALL_TIMEOUT_SECS and not pending_call.timeout_registered:
                error_frame_id = int(pending_call.start_time * 1000)

                err = ErrorMessage(
                    content="Tool call timeout: no result received within timeout period",
                    source="tool_call",
                    error_frame_id=error_frame_id,
                    seconds_from_start=pending_call.start_time,
                    ...
                    metadata={
                        ...
                        "start_time": pending_call.start_time,
                        "timeout_seconds": self.TOOL_CALL_TIMEOUT_SECS,
                        "elapsed_time": elapsed_time,
                    },
                )
                pending_call.timeout_registered = True
                self._debug("TOOL CALL TIMEOUT",
                            tool_call_id=pending_call.tool_call_id,
                            function=pending_call.function_name,
    ...
    def _end_overlap(self, end_t: float) -> None:
        if self._overlap_active and self._overlap_start_ts is not None:
            ...
            try:
                ...
                self._debug("OVERLAP RECORD FAILED", error=str(e))
    ...
    async def on_push_frame(self, data: FramePushed) -> None:
        ...
        # 3.5) check for pending tool call timeouts
        await self._handle_pending_tool_calls(t)
        ...
        if (isinstance(frame, CancelFrame) or isinstance(frame, EndFrame)) and not self.final:
            ...
            try:
                ...
                self._debug("INTERRUPTION LOOP DETECTION FAILED", error=str(exc))

            for loop in loops:
                try:
                    loop_err = ErrorMessage(
                        content="Interruption loop detected",
                        source="observer",
                        error_frame_id=int((loop.get("start") or 0) * 1000),
                        seconds_from_start=loop.get("start"),
                    ...
                    self._debug("INTERRUPTION LOOP ERROR REGISTRATION FAILED", error=str(exc))
            self.final = True
