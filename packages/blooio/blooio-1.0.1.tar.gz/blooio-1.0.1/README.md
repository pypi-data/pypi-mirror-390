# Blooio Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/blooio.svg?label=pypi%20(stable))](https://pypi.org/project/blooio/)

The Blooio Python library provides convenient access to the Blooio REST API from any Python 3.8+
...
to add `BLOOIO_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncBlooio` instead of `Blooio` and use `await` with each API call:
...
response), a subclass of `blooio.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `blooio.APIError`.
...
try:
    client.me.retrieve()
except blooio.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except blooio.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except blooio.APIStatusError as e:
...
You can enable logging by setting the environment variable `BLOOIO_LOG` to `info`.

```shell
$ export BLOOIO_LOG=info
```
...
```python
import httpx
from blooio import Blooio, DefaultHttpxClient

client = Blooio(
    # Or use the `BLOOIO_BASE_URL` env var
