...
import os
import asyncio
from bunnyshell import Template, wait_for_port
...
async def main():
    ...
    from bunnyshell.template import BuildOptions
    
    result = await Template.build(
        template,
        BuildOptions(
            alias="my-python-app",
            api_key=os.environ.get("NODEMGR_API_KEY", "nmgr_sk_test"),
            base_url=os.environ.get("API_BASE_URL", "https://api.your-domain.com"),
            cpu=2,
            memory=2048,
            disk_gb=10,
    ...
    from bunnyshell.template import CreateVMOptions
