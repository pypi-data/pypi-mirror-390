...
class EvaluationService:
    ...
    def __process_metrics_with_score_calculation(
        self,
        metrics: List[Union[str, CustomScoreEvaluationMetric, MetricInput]],
    ...
    def create_single_turn(
        ...
        version_id: str,
        actual_output: str,
        metrics: Optional[List[Union[str, CustomScoreEvaluationMetric, MetricInput]]] = None,
    ...
    def create(
        ...
        if metrics:
            ...
            inference_results = self.__session_service._client.get(
                f"inference-results?sessionIds[]={session_id}&sort[0]=index&sort[1]=desc&limit=1"
            ).json()

            if not inference_results:
                raise ValueError(f"No inference results found for session '{session_id}'")

            latest_result = inference_results[0]
