...
class Sandbox:
    ...
    async def from_snippet(self, playground_snippet_id: str) -> None:
        ...
        Example:
            ...
            sandbox = Sandbox(api_key='your-api-key')
            await sandbox.from_snippet(playground_snippet_id='existing-snippet-id')
    ...
    async def get_file(self, path: str) -> httpx.Response:
        ...
        if not path.startswith('/home/damner'):
            raise Exception("Invalid path. Path must start with /home/damner")
    ...
    async def quick_run(
        ...
        Args:
            ...
            additional_files_as_zip: Optional Base64URL-encoded zip file containing additional files needed for execution

        Returns:
            DecodedRunResult containing:
              - run_status: Execution status. Possible values: "successful", "compilation-error", "time-limit-exceeded",
                "wrong-answer", "non-zero-exit-code", "died-sigsev", "died-sigxfsz", "died-sigfpe", "died-sigabrt",
                "internal-isolate-error", or "unknown"
              - program_run_data: Object containing execution metrics and output (None if execution hasn't completed):
                ...
                - exit_code: Exit code returned by the program (0 typically indicates success)
                - exit_signal: Exit signal if process was terminated by a signal, or 0 otherwise
                - cpu_time_used_in_milliseconds: CPU time taken for execution in milliseconds
                - wall_time_used_in_milliseconds: Wall-clock time (real time) taken for execution in milliseconds
                - memory_used_in_kilobyte: Total memory used during execution in kilobytes
              - compiler_output_after_compilation: Decoded compiler output as string (already decoded, not Base64URL) or None
                (None for interpreted languages or if there's no compiler output)
              - finished_at: Timestamp when execution completed in ISO 8601 UTC format (e.g., "2025-11-06T08:14:10.127Z")

        Raises:
            Exception: If not connected to sandbox (requires create() or from_snippet() to be called first)
            Exception: If code submission fails
            Exception: If polling timeout is reached (30 seconds / 60 attempts)
            Exception: If no task ID is returned from execution request
            Exception: If no result is returned from result request
            Exception: If execution finished but no result was returned
        ...
        Example:
            ...
            print(result.program_run_data.stdout)  # "Hello, World!\n"
            ...
            print(result.program_run_data.stdout)  # "8\n" (already decoded)

            # Go with additional files
            result = await sandbox.quick_run(
                runtime='Go',
                source_code='package main\\nimport "fmt"\\nfunc main() { fmt.Println("Hello") }',
                additional_files_as_zip='base64url-encoded-zip-content'
            )
            ```
        """
        if not self.is_connected():
        ...
        for _ in range(max_attempts):
            ...
            if result.coding_task_status == CodingTaskStatus.FINISHED:
                ...
                # Decode base64url strings to match TypeScript SDK's DecodedRunResult
                run_result = result.run_result
                decoded_result = DecodedRunResult(
                    compiler_output_after_compilation=(
                        decode_base64url(run_result.compiler_output_after_compilation_base64_url_encoded)
                        if run_result.compiler_output_after_compilation_base64_url_encoded is not None
                        else None
                    ),
                    ...
                    program_run_data=(
                        DecodedProgramRunData(
                            cpu_time_used_in_milliseconds=run_result.program_run_data.cpu_time_used_in_milliseconds,
                            wall_time_used_in_milliseconds=run_result.program_run_data.wall_time_used_in_milliseconds,
                            memory_used_in_kilobyte=run_result.program_run_data.memory_used_in_kilobyte,
                            exit_signal=run_result.program_run_data.exit_signal,
                            exit_code=run_result.program_run_data.exit_code,
                            # Match TypeScript SDK: use ?? '' pattern - always decode, default to empty string if None
                            stdout=decode_base64url(
                                run_result.program_run_data.stdout_base64_url_encoded or ''
                            ),
                            stderr=decode_base64url(
                                run_result.program_run_data.stderr_base64_url_encoded or ''
                            )
                        )
                        if run_result.program_run_data
                        else None
                    )
                )
