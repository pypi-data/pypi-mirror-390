...
if API_KEY:
    # The client automatically uses the GOOGLE_API_KEY environment variable if it's set.
    try:
        CLIENT = genai.Client()
    except Exception:
        # If client creation fails (e.g., invalid key), handle it gracefully.
        CLIENT = None
else:
    CLIENT = None
...
def internet_search(query: str, urls: Optional[Union[str, List[str]]] = None) -> str:
    '''
    Performs a search using the Gemini model with Google Search and URL context grounding.

    Args:
        query: The detailed search query.
        urls: An optional URL string or list of URL strings to use for context.
