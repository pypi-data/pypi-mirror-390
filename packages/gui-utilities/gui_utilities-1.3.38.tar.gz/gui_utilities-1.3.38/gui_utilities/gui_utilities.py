...
def create_text_box(
    ...
    # --- hide text (password) support with icon and icon focus watcher ---
    if hide_text:
        ...
        def update_icon():
            ...
            if text_box.hasFocus():
                icon = focused_hide_text_icon if is_password else focused_show_text_icon
            else:
                icon = hide_text_icon if is_password else show_text_icon
        ...
        # lightweight focus watcher for the icon (kept local so it doesn't clash)
        class _IconFocusWatcher(QObject):
            ...
            def eventFilter(self, watched, event):
                if watched is self._watched and event.type() in (QEvent.Type.FocusIn, QEvent.Type.FocusOut):
                    if callable(self._on_focus_change):
