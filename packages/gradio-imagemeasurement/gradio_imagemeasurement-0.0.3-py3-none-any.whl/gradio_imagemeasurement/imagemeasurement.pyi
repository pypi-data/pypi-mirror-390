...
class ImageMeasurement(StreamingInput, Component):
    """
    ...
    measurement = EventListener(
        "measurement",
        doc="""This listener is triggered when the user completes a measurement by clicking two points on the image. 
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A `PIL.Image`, `numpy.array`, `pathlib.Path`, or `str` filepath or URL for the default value that ImageMeasurement component is going to take. If a function is provided, the function will be called each time the app loads to set the initial value of this component.
        ...
        if streaming and self.sources != ["webcam"]:
            raise ValueError(
                "ImageMeasurement streaming only available if sources is ['webcam']. Streaming not supported with multiple sources."
            )
