...
class PrimalDualOptimizer:
    ...
    @staticmethod
    def calc_loading(
        p: np.ndarray | float, q: np.ndarray | float, s: np.ndarray | float
    ) -> np.ndarray | float:
    ...
    def solve_problem(
        ...
        **callback_kwargs,
        ...
        self.dual_line += self.alpha_l * (
            self.calc_loading(
                param_dict["P_line_meas"], param_dict["Q_line_meas"], self.s_line
            )
            - 1.0
        )
        self.dual_line[self.dual_line < 0] = 0.0
        if self.n_transformer >= 1:
            self.dual_transformer += self.alpha_t * (
                self.calc_loading(
                    param_dict["P_transformer_meas"],
                    param_dict["Q_transformer_meas"],
                    self.s_transformer,
                )
                - 1.0
            )
        ...
        grad_p = (
            ...
            * self.c2_p
            * (
                param_dict["p_gen_last"] / self.param_scale
                - self.p_norm / self.param_scale
            )
            + self.c1_p
        )
        grad_p += (
            self.sensitivities["du_dp"].T
            @ (self.dual_v_upp - self.dual_v_low)
            * self.param_scale
        )
        grad_p += (
            self.sensitivities["dP_line_dp"].T
            @ (
                self.dual_line
                * self.calc_pf(param_dict["P_line_meas"], param_dict["Q_line_meas"])
                / self.s_line
            )
            * self.param_scale
        )
        grad_p += (
            self.sensitivities["dQ_line_dp"].T
            @ (
                self.dual_line
                * self.calc_rpf(param_dict["P_line_meas"], param_dict["Q_line_meas"])
                / self.s_line
            )
            * self.param_scale
        )
        if self.n_transformer >= 1:
            grad_p += (
                self.sensitivities["dP_transformer_dp"].T
                @ (
                    self.dual_transformer
                    * self.calc_pf(
                        param_dict["P_transformer_meas"],
                        param_dict["Q_transformer_meas"],
                    )
            ...
            grad_p += (
                self.sensitivities["dQ_transformer_dp"].T
                @ (
                    self.dual_transformer
                    * self.calc_rpf(
                        param_dict["P_transformer_meas"],
        ...
        grad_q = (
            ...
            * self.c2_q
            * (
                param_dict["q_gen_last"] / self.param_scale
                - self.q_norm / self.param_scale
            )
            + self.c1_q
        )
        grad_q += (
            self.sensitivities["du_dq"].T
            @ (self.dual_v_upp - self.dual_v_low)
            * self.param_scale
        )
        grad_q += (
            self.sensitivities["dP_line_dq"].T
            @ (
                self.dual_line
                * self.calc_pf(param_dict["P_line_meas"], param_dict["Q_line_meas"])
                / self.s_line
        ...
        grad_q += (
            self.sensitivities["dQ_line_dq"].T
            @ (
                self.dual_line
                * self.calc_rpf(param_dict["P_line_meas"], param_dict["Q_line_meas"])
                / self.s_line
        ...
        if self.n_transformer >= 1:
            grad_q += (
                self.sensitivities["dP_transformer_dq"].T
                @ (
                    self.dual_transformer
                    * self.calc_pf(
                        param_dict["P_transformer_meas"],
            ...
            grad_q += (
                self.sensitivities["dQ_transformer_dq"].T
                @ (
                    self.dual_transformer
                    * self.calc_rpf(
                        param_dict["P_transformer_meas"],
        ...
        for i in range(self.n_gen):
            p_gen_opt[i], q_gen_opt[i] = self.renew_gen_proj.projection(
                p_max=self.p_max[i] / self.param_scale,
                p_min=self.p_min[i] / self.param_scale,
                p=p_target[i],
                q=q_target[i],
                s_inv=(
                    self.s_inv[i] / self.param_scale
                    if self.s_inv[i] is not None
                    else None
                ),
                pf_min=self.pf_min[i],
                q_min=(
                    self.q_min[i] / self.param_scale
                    if self.q_min[i] is not None
                    else None
                ),
                q_max=(
                    self.q_max[i] / self.param_scale
                    if self.q_max[i] is not None
                    else None
                ),
            )
