...
def solve(
    ...
    **solver_kwargs,
    ...
    if algorithm == "gp":
        optimizer = GradientProjectionOptimizer(
            opt_model_data, sensitivities, alpha=alpha, solver=solver, **solver_kwargs
        )
    elif algorithm == "pd":
        optimizer = PrimalDualOptimizer(
            opt_model_data,
            sensitivities,
            alpha=alpha,
            alpha_v=alpha_v,
    ...
    gen_update = np.column_stack(
        (power_flow_solver.base_p_gen, power_flow_solver.base_q_gen)
    )
    iterates = []
    ...
    for k in range(1, max_iter + 1):
        ...
        P_line_meas = np.array(
            output_data[ComponentType.line]["p_" + loading_meas_side]
        )
        Q_line_meas = np.array(
            output_data[ComponentType.line]["q_" + loading_meas_side]
        )
        if n_transformer >= 1:
            P_transformer_meas = np.array(
                output_data[ComponentType.transformer]["p_" + loading_meas_side]
            )
            Q_transformer_meas = np.array(
                output_data[ComponentType.transformer]["q_" + loading_meas_side]
            )
        ...
        if (
            np.max(
                np.abs(
                    gen_update
                    - np.column_stack(
                        (param_dict["p_gen_last"], param_dict["q_gen_last"])
                    )
                )
            )
            < tol
    ...
    return SolveResults(
        final_output=output_data, final_gen_update=gen_update, iterations=iterates
    )
