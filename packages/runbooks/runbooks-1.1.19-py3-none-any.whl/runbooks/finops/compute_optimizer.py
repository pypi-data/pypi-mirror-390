...
def enrich_dataframe_with_all_signals(
    ...
    try:
        ...
        df = enrich_dataframe_with_compute_optimizer(
            df,
            instance_id_column=instance_id_column,
        ...
        if not operational_profile:
            operational_profile = os.getenv('CENTRALISED_OPS_PROFILE') or os.getenv('AWS_PROFILE') or 'default'

        signal_enricher = EC2DecommissionSignalEnricher(
        ...
        print_info(f"   Tier distribution: MUST={tier_counts.get('MUST', 0)}, "
