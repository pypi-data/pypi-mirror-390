...
from runbooks.common.rich_utils import (
    ...
    create_progress_bar, console, create_table
)
...
class ActivityEnricher:
    ...
    def enrich_activity(self, df: pd.DataFrame, resource_type: str) -> pd.DataFrame:
        ...
        signal_columns = ['E1', 'E2', 'E3', 'E4', 'E5', 'E6', 'E7'] if resource_type == 'ec2' else ['W1', 'W2', 'W3', 'W4', 'W5', 'W6']
        summary_rows = []
    ...
    def _enrich_cloudtrail(self, df: pd.DataFrame, resource_type: str) -> pd.DataFrame:
        ...
        if resource_type == 'ec2':
            ...
            if no_activity_instances:
                if self.output_controller.verbose:
                    print_info(f"   🔍 CloudTrail: Applying ResourceType fallback for {len(no_activity_instances)} instances...")
                else:
                ...
                try:
                    ...
                    if self.output_controller.verbose:
                        print_info(f"   ✅ CloudTrail ResourceType: {fallback_found} additional instances with activity")
                    else:
                        logger.info(f"CloudTrail ResourceType: {fallback_found} additional instances with activity")

                except Exception as batch_error:
                    logger.warning(f"CloudTrail ResourceType fallback failed: {batch_error}")
                    pass
