...
class CloudTrailActivityEnricher:
    ...
    def enrich_resources(
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]CloudTrail activity enrichment...",
                total=len(df)
            )
    ...
    def _lookup_events(
        ...
        try:
            ...
            if not events and resource_type == 'vpc_endpoint':
                events = self._lookup_vpc_endpoint_events_by_type(
                    resource_id,
                    start_time,
                    end_time
                )
            ...
            if e.response['Error']['Code'] in ['AccessDenied', 'UnauthorizedOperation']:
                logger.warning(f"CloudTrail access denied for {resource_id}")
            else:
    ...
    def _lookup_vpc_endpoint_events_by_type(
        self,
        vpce_id: str,
        start_time: datetime,
        end_time: datetime
        ...
        try:
            response = self.cloudtrail.lookup_events(
                LookupAttributes=[{
                    ...
                    'AttributeValue': 'AWS::EC2::VPCEndpoint'
                }],
                StartTime=start_time,
                EndTime=end_time,
                MaxResults=1000
            ...
            logger.warning(f"ResourceType fallback failed for {vpce_id}: {e}")
    ...
    def _calculate_activity_score(
        ...
        recent_7d = [
            e for e in events
            if (current_time - e['EventTime']).days <= 7
        ]
        if recent_7d:
            score += 40
        ...
        recent_30d = [
            e for e in events
            if (current_time - e['EventTime']).days <= 30
        ]
        if recent_30d:
            score += 30
...
def enrich_vpc_endpoints_with_cloudtrail(
    ...
    return enricher.enrich_resources(
        endpoints_df,
        resource_type='vpc_endpoint',
