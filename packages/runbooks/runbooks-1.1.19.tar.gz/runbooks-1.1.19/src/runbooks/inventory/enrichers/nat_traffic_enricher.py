...
class NATTrafficEnricher(CloudFoundationsBase):
    ...
    def enrich_nat_gateways(
        ...
        console.print(f"[blue]🔍 Enriching {len(nat_gateways)} NAT Gateways with traffic analysis (lookback: {lookback_days} days)[/blue]")
        ...
        with create_progress_bar(total=len(nat_gateways), description="Analyzing NAT Gateway traffic") as progress:
            for nat in nat_gateways:
                try:
                    ...
                    print_error(f"Unexpected error enriching NAT Gateway {nat.get('NatGatewayId', 'unknown')}: {str(e)}")
    ...
    def _enrich_single_nat_gateway(
        self,
        nat: Dict,
        lookback_days: int,
        region: str
    ...
    def _get_cloudwatch_metrics(
        ...
        for metric_name in metrics.keys():
            try:
                ...
                logger.warning(f"Failed to get metric {metric_name} for {nat_gateway_id}: {e}")
                metrics[metric_name] = 0.0

        # Calculate daily averages
        metrics['bytes_out_daily_avg_mb'] = metrics['BytesOutToDestination'] / (1024 * 1024) / lookback_days if metrics['BytesOutToDestination'] else 0
        metrics['bytes_in_daily_avg_mb'] = metrics['BytesInFromSource'] / (1024 * 1024) / lookback_days if metrics['BytesInFromSource'] else 0
        metrics['connections_daily_avg'] = metrics['ActiveConnectionCount'] / lookback_days if metrics['ActiveConnectionCount'] else 0
        metrics['packets_out_daily_avg'] = metrics['PacketsOutToDestination'] / lookback_days if metrics['PacketsOutToDestination'] else 0
    ...
    def _get_route_table_count(self, nat_gateway_id: str, vpc_id: str, region: str) -> int:
        ...
        try:
            ...
            logger.warning(f"Failed to get route tables for NAT Gateway {nat_gateway_id}: {e}")
            return 0
    ...
    def _analyze_aws_service_traffic(
        ...
        try:
            ...
            logger.warning(f"Failed to analyze AWS service traffic for {nat_gateway_id}: {e}")
