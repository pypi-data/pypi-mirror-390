...
# Import actual CAI Mail Agent
try:
    from alprina.agents import get_agent_by_name
    CAI_AVAILABLE = True
    logger.info("CAI Mail Agent available")
except ImportError as e:
    CAI_AVAILABLE = False
    logger.warning(f"Alprina agents not available: {e}")


class MailWrapper:
    """
    Wrapper for CAI Mail Agent.
    ...
    def __init__(self):
        ...
        self.agent_type = "email-report"
        self.description = "Email notifications for security findings"
    ...
    def _get_alprina_agent(self):
        ...
        if self._alprina_agent is None:
            try:
                ...
                self._alprina_agent = get_agent_by_name("mail")
                logger.info("CAI Mail Agent initialized")
            except Exception as e:
                logger.error(f"Failed to initialize CAI Mail Agent: {e}")
                return None
    ...
    async def _scan_async(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""Perform email-report analysis on: {target}
...
Provide detailed findings with severity levels."""
            ...
            logger.error(f"CAI Mail Agent error: {e}")
            # Fallback to mock
            return self._mock_scan(target, safe_only)
    ...
    def _parse_cai_response(self, response: str, target: str) -> List[Dict[str, Any]]:
        ...
        if not findings and len(response) > 50:
            findings.append({
                "type": "Email Security Issue",
                "severity": "INFO",
                "title": "Mail Agent Analysis Complete",
    ...
    def scan(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with scan results
        """
        logger.info(f"Mail Agent scanning: {target} (safe_only={safe_only}, CAI={CAI_AVAILABLE})")
...
mail_agent = MailWrapper()


def run_mail_scan(target: str, safe_only: bool = True) -> Dict[str, Any]:
    """
    Run security scan.
    ...
    Returns:
        Scan results
    """
    return mail_agent.scan(target, safe_only)
