...
class KnowledgeBaseManager:
    ...
    def delete(self, kb_id: int) -> None:
        ...
        if not response_data or not isinstance(response_data, dict):
            raise ApiException(f"Deletion failed for ID {kb_id}. Unexpected or empty response from API.")

        success_ids = response_data.get("success", [])
        if kb_id not in success_ids:
            # 尝试从 failed 列表中获取更详细的失败原因
            failed_info = next((item for item in response_data.get("failed", []) if item.get("id") == kb_id), {})
