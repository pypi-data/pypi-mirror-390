...
class qbt:
    ...
    def __init__(self, num_qubits, explain_steps=False, random_seed=None):
        ...
        self.measurement_results = []  # Store actual measurement outcomes
        
        self.ROT_MAP = {
            'RX': lambda theta: expm(-1j * theta / 2 * X),
            'RY': lambda theta: expm(-1j * theta / 2 * Y),
            'RZ': lambda theta: expm(-1j * theta / 2 * Z),
            # ADDED: Universal single-qubit gate for QML
            'U3': lambda theta, phi, lam: np.array([
    ...
    def y(self, q): self._log_step(f"Pauli-Y (Y) on Qubit {q}."); return self._add("Y", q)
    def z(self, q): self._log_step(f"Pauli-Z (Z) on Qubit {q}."); return self._add("Z", q)
    def s(self, q): self._log_step(f"S gate on Qubit {q}."); return self._add("S", q)
    
    # Rotation Gates (Essential for QML)
    def rx(self, q, theta): self._log_step(f"RX rotation of {theta:.3f} on Qubit {q}."); return self._add("RX", q, theta)
    def ry(self, q, theta): self._log_step(f"RY rotation of {theta:.3f} on Qubit {q}."); return self._add("RY", q, theta)
    def rz(self, q, theta): self._log_step(f"RZ rotation of {theta:.3f} on Qubit {q}."); return self._add("RZ", q, theta)
    
    # Universal single-qubit gate (Critical for QML)
    def u3(self, q, theta, phi, lam):
        self._log_step(f"U3({theta:.3f}, {phi:.3f}, {lam:.3f}) on Qubit {q}.")
    ...
    # ADDED: Controlled rotation gates for QML
    def cry(self, c, t, theta): self._log_step(f"CRY: C{c} -> T{t} rotation."); return self._add("CRY", c, t, theta)
    def crz(self, c, t, theta): self._log_step(f"CRZ: C{c} -> T{t} rotation."); return self._add("CRZ", c, t, theta)
    ...
    def measure(self, qubit=None, collapse=True):
        """Measurement operation - now with optional true collapse"""
        if collapse:
            return self.measure_collapse(qubit)
        else:
            self._log_step("Final Measurement Marker.")
            return self._add("M")

    # --- True Quantum Measurement Collapse ---
    def measure_collapse(self, qubit=None):
        """Actually collapse the quantum state with true randomness"""
        if qubit is None:
            # Measure all qubits
            probabilities = np.real(np.diag(self.rho))
            ...
            outcome_index = random.choices(range(len(probabilities)), weights=probabilities)[0]
            outcome_state = format(outcome_index, f'0{self.num_qubits}b')
            
            # Collapse to the chosen state
            collapsed_state = np.zeros(self.max_dim, dtype=complex)
            collapsed_state[outcome_index] = 1.0
            self.rho = np.outer(collapsed_state, collapsed_state.conj())
            self.statevector = collapsed_state
            
            self._log_step(f"Measurement collapsed to |{outcome_state}⟩")
            self.measurement_results.append(outcome_state)
            return outcome_state
        else:
            # Measure single qubit
            prob_0 = self.probability_of_state_qubit(qubit, '0')
            if random.random() < prob_0:
                # Collapse to |0⟩ for that qubit
                self._project_qubit(qubit, 0)
                self._log_step(f"Qubit {qubit} collapsed to |0⟩")
                self.measurement_results.append(f"Qubit {qubit}: |0⟩")
                return '0'
            else:
                # Collapse to |1⟩ for that qubit  
                self._project_qubit(qubit, 1)
                self._log_step(f"Qubit {qubit} collapsed to |1⟩")
                self.measurement_results.append(f"Qubit {qubit}: |1⟩")
                return '1'

    def _project_qubit(self, qubit, value):
        """Project a specific qubit to |0⟩ or |1⟩"""
        ...
        ops = [P if q == qubit else I for q in range(self.num_qubits)]
        P_full = kron_n(ops)
        
        # Apply projection and renormalize
        self.rho = P_full @ self.rho @ P_full.conj().T
        trace = np.trace(self.rho)
        if trace > 0:
            self.rho /= trace
        
        # Update statevector if pure
        if self.statevector is not None:
            self.statevector = P_full @ self.statevector
            norm = np.linalg.norm(self.statevector)
            if norm > 0:
                self.statevector /= norm

    def probability_of_state_qubit(self, qubit, value):
        """Get probability of a specific qubit being |0⟩ or |1⟩"""
        if value == '0':
            # Sum probabilities of all states where this qubit is 0
            total_prob = 0
            for i in range(self.max_dim):
                state_str = format(i, f'0{self.num_qubits}b')
                if state_str[-(qubit+1)] == '0':  # qubit 0 is rightmost
                    total_prob += np.real(self.rho[i, i])
            return total_prob
        else:
            return 1 - self.probability_of_state_qubit(qubit, '0')

    def get_measurement_results(self):
        """Get all measurement results from this circuit"""
        return self.measurement_results.copy()

    # --- QML-SPECIFIC METHODS ---
    
    def parameter_shift_gradient(self, params, gradient_circuit_fn, epsilon=0.01):
        """Parameter-shift rule for quantum gradients (Essential for QML)"""
        gradients = []
        for i in range(len(params)):
            # Shift +
            params_plus = params.copy()
            params_plus[i] += epsilon
            loss_plus = gradient_circuit_fn(params_plus)
            
            # Shift -
            params_minus = params.copy()
            params_minus[i] -= epsilon  
            loss_minus = gradient_circuit_fn(params_minus)
            
            gradients.append((loss_plus - loss_minus) / (2 * epsilon))
        
        return np.array(gradients)
    
    def state_tomography(self, num_shots=1000):
        """Perform quantum state tomography for QML analysis"""
        tomography_data = {}
        
        # Measure in different bases
        bases = ['Z', 'X', 'Y']  # Computational, Hadamard, Circular
        
        for basis in bases:
            # Create a copy to avoid modifying original circuit
            temp_circuit = qbt(self.num_qubits)
            temp_circuit.rho = self.rho.copy()
            
            if basis == 'X':
                for q in range(self.num_qubits):
                    temp_circuit.h(q)  # Change basis to X
            elif basis == 'Y':
                for q in range(self.num_qubits):
                    # S-dagger then H for Y basis
                    temp_circuit.rz(q, -PI/2).h(q)
            
            temp_circuit.run()
            
            probabilities = {}
            for state in [format(i, f'0{self.num_qubits}b') for i in range(self.max_dim)]:
                prob = temp_circuit.probability_of_state(state)
                probabilities[state] = prob
                
            tomography_data[basis] = probabilities
    ...
    def demo(self, name):
        ...
        elif name == 'qml_demo':  # ADDED: QML demonstration circuit
            self.num_qubits = 3; print(f"\n--- Building QML Demonstration Circuit ---")
            self.h(0).ry(1, 0.5).rz(2, 0.3).cnot(0, 1).crz(1, 2, 0.7).u3(0, 0.1, 0.2, 0.3)
            self._log_step("QML feature encoding and processing complete")
    ...
    def run(self, log_state_changes=False):
        ...
        self.statevector = np.zeros(self.max_dim, dtype=complex); self.statevector[0] = 1.0
        self.rho = np.outer(self.statevector, self.statevector.conj())
        self.measurement_results = []  # Reset measurement results
        ...
        while i < len(tokens):
            ...
            # Rotation Gates (RX, RY, RZ, U3)
                ...
                if op == 'U3':
                    q = int(tokens[i + 1]); theta = float(tokens[i + 2])
                    phi = float(tokens[i + 3]); lam = float(tokens[i + 4])
                    U = self.ROT_MAP[op](theta, phi, lam)
                    self.rho, U_full = apply_single_qubit_gate(self.rho, U, q, self.num_qubits)
                    is_unitary = True; i += 5
                else:
            ...
            # Controlled Gates (CX, CZ, CRX, CRY, CRZ)
            elif op in ['CX', 'CZ', 'CRX', 'CRY', 'CRZ']:
                c = int(tokens[i + 1]); t = int(tokens[i + 2])
                if op == 'CX': U_target = X; adv = 3
                elif op == 'CZ': U_target = Z; adv = 3
                elif op == 'CRX': theta = float(tokens[i + 3]); U_target = self.ROT_MAP['RX'](theta); adv = 4
                elif op == 'CRY': theta = float(tokens[i + 3]); U_target = self.ROT_MAP['RY'](theta); adv = 4
                elif op == 'CRZ': theta = float(tokens[i + 3]); U_target = self.ROT_MAP['RZ'](theta); adv = 4
            ...
            # Measurement Marker (old style - no collapse)
    ...
    def analyze(self, plot=False):
        ...
        return {'purity': purity, 'entropy': entropy, 'measurement_results': self.measurement_results}

# --- MAIN EXECUTION ENTRY POINT ---
if __name__ == "__main__":
    # This block is for internal testing/self-test only.
    ...
    try:
        ...
        # Test new QML gates
        print("\n--- Testing New QML Gates ---")
        qml_test = qbt(3, explain_steps=True)
        qml_test.ry(0, 0.5).rz(1, 0.3).cry(0, 1, 0.7).u3(2, 0.1, 0.2, 0.3)
        result = qml_test.run()
        print("QML gate test completed successfully!")
