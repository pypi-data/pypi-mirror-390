...
from pyrogram.raw import functions, types  # ReportResult*, MessageReportOption
...
def _pick_option_by_reason(reason_value: str, options: Iterable[types.MessageReportOption]) -> Optional[bytes]:
    ...
    kw = {
        ...
        "pornography": [
            "porn", "sexual", "adult", "sex",
            "هرزه", "پورن", "پورنو", "پورنوگرافی", "محتوای جنسی", "جنسی"
        ],
...
async def _safe_invoke(client: Client, func_cls, **kwargs) -> Any:
    """
    فراخوانی امنِ کلاس RAW با کلیدهای درست.
    """
    trace = get_trace_id()
...
class ReportExecutor:
    ...
    async def _report_message(self, req: ReportRequest) -> bool:
        ...
        for step in range(1, max_steps + 1):
            ...
            try:
                ...
                self._last_meta["result_type"] = "FloodWait"
                return False
            ...
            if isinstance(res, types.ReportResultAddComment):
                ...
                try:
                    ...
                    self._last_meta["result_type"] = "AddCommentFollowupFailed"
                    return False
                if isinstance(res2, types.ReportResultReported):
                    self._last_meta["reported"] = True
                    self._last_meta["result_type"] = "Reported"
                    log_kv(logger, "messages.Report.final", ok=True)
                    return True
                self._last_meta["result_type"] = type(res2).__name__
                logger.error("unexpected ReportResult after AddComment: %r", type(res2).__name__, extra={"trace_id": trace})
                return False

            if isinstance(res, types.ReportResultChooseOption):
                opts_preview = [f"{i+1}. {getattr(o, 'text', '')}" for i, o in enumerate(res.options or [])]
                self._last_meta["flow"].append({
                    "type": "choose",
                ...
                if not picked:
                    ...
                    self._last_meta["result_type"] = "NoMatchingOption"
                    return False

                if picked in visited_options:
                    logger.error("loop detected on option=%r", picked, extra={"trace_id": trace})
                    self._last_meta["result_type"] = "OptionLoop"
                    return False

                visited_options.add(picked)
                self._last_meta["flow"].append({"type": "pick", "picked": picked.hex()})
                current_option = picked
                continue

            self._last_meta["result_type"] = type(res).__name__
            logger.error("unexpected ReportResult type: %r", type(res).__name__, extra={"trace_id": trace})
            return False

        self._last_meta["result_type"] = "MaxStepsExceeded"
    ...
    async def _report_peer(self, req: ReportRequest) -> bool:
        ...
        try:
            ...
            self._last_meta["reported"] = bool(res)
            self._last_meta["result_type"] = "Bool"
