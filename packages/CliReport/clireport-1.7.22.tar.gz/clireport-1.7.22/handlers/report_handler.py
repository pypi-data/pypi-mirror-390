...
# لاگ‌گیری رد-محور
from logkit.trace import new_trace_id, get_trace_id, with_trace, log_kv
...
def _dbg_parts(parts: List[str]) -> str:
    out = []
    for i, p in enumerate(parts):
        out.append(f"[{i}]={p[:57]}…" if len(p) > 60 else f"[{i}]={p}")
    return " ".join(out)
...
def parse_args(text: str) -> Optional[ReportRequest]:
    ...
    logger.debug("parse_args: parts=%s", _dbg_parts(parts), extra={"trace_id": trace})

    # entity و شاید message_id از لینک
...
def register_handlers(app: Client) -> None:
    ...
    async def report_handler(client: Client, message: Message):
        ...
        # متادیتای تایید تلگرام (مسیر منوها/نتیجه) را هم ضمیمه کنیم
        meta = getattr(executor, "last_meta", {}) or {}
