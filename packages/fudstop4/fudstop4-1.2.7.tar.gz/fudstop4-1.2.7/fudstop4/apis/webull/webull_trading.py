...
# Define the dictionary with all the rules
    ...
    async def webull_comments(self, ticker:str):
        """Get webull comments about tickers"""

        ticker_id = self.ticker_to_id_map.get(ticker)
        url = f"https://nacomm.webullfintech.com/api/social/feed/ticker/posts?tickerId={ticker_id}&size=200"

        async with aiohttp.ClientSession(headers=generate_webull_headers()) as session:
            async with session.get(url) as resp:
                data = await resp.json()
                data = WebullComments(data)
