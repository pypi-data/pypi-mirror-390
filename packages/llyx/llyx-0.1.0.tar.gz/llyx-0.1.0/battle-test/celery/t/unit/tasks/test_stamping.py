...
class CleanupVisitor(StampingVisitor):
    def clean_stamps(self, actual_sig: Signature) -> None:
        if (
            "stamped_headers" in actual_sig.options
            and actual_sig.options["stamped_headers"]
        ):
...
class ListStampingVisitor(StampingVisitor):
    def on_signature(self, actual_sig: Signature, **headers) -> dict:
        return {
            "on_signature": [
                "ListStampingVisitor: on_signature-item1",
                "ListStampingVisitor: on_signature-item2",
            ]
        }
    ...
    def on_chord_body(self, actual_sig: Signature, **header) -> dict:
        return {
            "on_chord_body": [
                "ListStampingVisitor: on_chord_body-item1",
                "ListStampingVisitor: on_chord_body-item2",
            ]
        }
    ...
    def on_callback(self, actual_sig: Signature, **header) -> dict:
        return {
            "on_callback": [
                "ListStampingVisitor: on_callback-item1",
                "ListStampingVisitor: on_callback-item2",
            ]
        }
    ...
    def on_errback(self, actual_sig: Signature, **header) -> dict:
        return {
            "on_errback": [
                "ListStampingVisitor: on_errback-item1",
                "ListStampingVisitor: on_errback-item2",
            ]
        }
...
class StringStampingVisitor(StampingVisitor):
    ...
    def on_chord_header_start(self, actual_sig: Signature, **header) -> dict:
        ...
        s.update(
            {
                "on_chord_header_start": "StringStampingVisitor: on_chord_header_start-item1"
            }
        )
        return s
...
class StampsAssertionVisitor(StampingVisitor):
    ...
    def assertion_check(self, actual_sig: Signature, method: str, **headers) -> None:
        ...
        with self.subtests.test(f"Check if {actual_sig} has stamp: {expected_stamp}"):
            if isinstance(self.visitor, ListStampingVisitor) or isinstance(
                self.visitor, SetStampingVisitor
            ):
                assertion_check = all(
                    [actual in expected_stamp for actual in actual_stamp]
                )
            else:
                assertion_check = actual_stamp == expected_stamp
            assertion_error = (
                f"{actual_sig} has stamp {actual_stamp} instead of: {expected_stamp}"
            )
...
class StampedHeadersAssertionVisitor(StampingVisitor):
    ...
    def assertion_check(
        self, actual_sig: Signature, expected_stamped_header: str
    ) -> None:
        if any(
            [
                isinstance(actual_sig, group),
            ...
            with self.subtests.test(
                f'Check if "stamped_headers" is not in {actual_sig.options}'
            ):
                assertion_check = "stamped_headers" not in actual_sig.options
                assertion_error = (
                    f"{actual_sig} should not have stamped_headers in options"
                )
                assert assertion_check, assertion_error
            return
        ...
        with self.subtests.test(
            f'Check if {actual_sig}["stamped_headers"] has: {expected_stamped_header}'
        ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        chain(
            ...
            | group(signature(f"sig{i}") for i in range(5, 6)),
            chord(group(signature(f"sig{i}") for i in range(6, 8)), signature("sig8"))
            | signature("sig9"),
        ),
        chain(
            signature("sig"),
        ...
        chord(
            ...
            chain(
                chain(
                    ...
                    chord(
                        group(signature(f"sig{i}") for i in range(14, 16)),
                        signature("sig16"),
                    )
                    | signature("sig17"),
                ),
                ...
                | group(signature(f"sig{i}") for i in range(5, 7)),
                chord(
                    group(signature(f"sig{i}") for i in range(7, 9)), signature("sig9")
                )
                | signature("sig10"),
            ),
        ),
...
class test_canvas_stamping(CanvasCase):
    @pytest.fixture
    def stamped_canvas(
        self, stamping_visitor: StampingVisitor, canvas_workflow: Signature
    ) -> Signature:
    ...
    def test_stamp_in_options(
        self, workflow: Signature, stamping_visitor: StampingVisitor, subtests
    ):
        """Test that all canvas signatures gets the stamp in options"""
        workflow.stamp(StampsAssertionVisitor(stamping_visitor, subtests))

    @pytest.mark.usefixtures("depends_on_current_app")
    def test_stamping_headers_in_options(
        self, workflow: Signature, stamping_visitor: StampingVisitor, subtests
    ):
    ...
    @pytest.mark.usefixtures("depends_on_current_app")
    def test_stamping_with_replace(
        self, workflow: Signature, stamping_visitor: StampingVisitor, subtests
    ):
        ...
        def assert_using_replace(self: AssertionTask):
            assert self.request.stamped_headers is None, (
                "stamped_headers should not pass via replace"
            )
...
class test_stamping_mechanism(CanvasCase):
    ...
    def test_twice_stamping(self, subtests):
        ...
        with subtests.test("sig_1_res is stamped twice", stamps=["stamp2", "stamp1"]):
            ...
            assert sorted(sig_1_res._get_task_meta()["stamp2"]) == sorted(
                ["stamp", "stamp2"]
            )
    ...
    def test_callback_stamping_link_after_stamp(self, subtests):
        ...
        with subtests.test("callback is not stamped"):
            ...
            assert "on_callback" not in stamped_headers, (
    ...
    def test_callback_stamping_link_multiple_visitors(self, subtests):
        ...
        with subtests.test(
            "sig_1 is stamped with custom visitor",
            stamped_headers=["header", "header2"],
        ):
            assert sorted(sig_1_res._get_task_meta()["stamped_headers"]) == sorted(
                ["header", "header2"]
            )
        ...
        with subtests.test(
            "sig_2 is stamped with custom visitor",
            stamped_headers=["header", "header2"],
        ):
            assert sorted(sig_2_res._get_task_meta()["stamped_headers"]) == sorted(
                ["header", "header2"]
        ...
        with subtests.test("callback is stamped"):
            ...
            headers = callback_link.options
            stamped_headers = headers.get("stamped_headers", [])
            assert "on_callback2" in stamped_headers, (
                "Linking after stamping should stamp the callback"
            )
            expected_stamped_headers = list(
                CustomStampingVisitor2().on_signature(None).keys()
            )
            expected_stamped_headers.extend(
                list(CustomStampingVisitor2().on_callback(None).keys())
            )
        ...
        with subtests.test("errback is stamped"):
            ...
            headers = errback_link.options
            stamped_headers = headers.get("stamped_headers", [])
            assert "on_errback2" in stamped_headers, (
                "Linking after stamping should stamp the errback"
            )
            expected_stamped_headers = list(
                CustomStampingVisitor2().on_signature(None).keys()
            )
            expected_stamped_headers.extend(
                list(CustomStampingVisitor2().on_errback(None).keys())
            )
            assert sorted(stamped_headers) == sorted(expected_stamped_headers)
