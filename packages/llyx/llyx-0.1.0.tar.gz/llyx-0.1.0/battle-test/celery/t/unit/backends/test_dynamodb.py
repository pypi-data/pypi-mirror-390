...
class test_DynamoDBBackend:
    ...
    def test_get_client_local(self, dynamodb_host):
        ...
        with patch("boto3.client") as mock_boto_client, patch(table_creation_path):
            backend = DynamoDBBackend(
                app=self.app, url=f"dynamodb://@{dynamodb_host}:8000"
            )
            client = backend._get_client()
            assert backend.client is client
            mock_boto_client.assert_called_once_with(
    ...
    def test_set_table_ttl_describe_time_to_live_fails_raises(self):
        ...
        self.backend.time_to_live_seconds = -1
        self.backend._client = MagicMock()
        mock_describe_time_to_live = self.backend._client.describe_time_to_live = (
            MagicMock()
        )
        client_error = ClientError(
            {
    ...
    def test_backend_params_by_url(self):
        self.app.conf.result_backend = (
            "dynamodb://@us-east-1/celery_results?read=10&write=20&ttl_seconds=600"
        )
