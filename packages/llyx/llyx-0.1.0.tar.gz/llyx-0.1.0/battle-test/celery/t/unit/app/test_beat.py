...
class test_Scheduler:
    ...
    def test_apply_async_with_null_args_set_to_none(self):
        ...
        scheduler = mScheduler(app=self.app)
        entry = scheduler.Entry(task=foo.name, app=self.app, args=None, kwargs=None)
        entry.args = None
        entry.kwargs = None
    ...
    def test_pending_left_10_milliseconds_tick(self):
        scheduler = mScheduler(app=self.app)
        scheduler.add(
            name="test_pending_left_10_milliseconds_tick",
    ...
    def test_when(self):
        now_time_utc = datetime(2000, 10, 10, 10, 10, 10, 10, tzinfo=ZoneInfo("UTC"))
        now_time_casey = now_time_utc.astimezone(ZoneInfo("Antarctica/Casey"))
        scheduler = mScheduler(app=self.app)
...
class test_PersistentScheduler:
    ...
    def test_create_schedule_corrupted(self):
        ...
        expected_error = UnicodeDecodeError(
            "ascii", b"ordinal not in range(128)", 0, 0, ""
        )
