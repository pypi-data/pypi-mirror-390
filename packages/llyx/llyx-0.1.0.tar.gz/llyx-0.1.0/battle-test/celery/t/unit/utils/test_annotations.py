...
from celery.utils.annotations import (
...
@pytest.mark.skipif(
    ...
    reason="Notation is only supported in Python 3.10 or newer.",
)
...
def test_annotation_issubclass() -> None:
    ...
    assert (
        annotation_issubclass(parameters["list_typing_arg"].annotation, BaseModel)
        is False
    )
    assert (
        annotation_issubclass(parameters["dict_typing_arg"].annotation, BaseModel)
        is False
    )
    assert (
        annotation_issubclass(
            parameters["list_typing_generic_arg"].annotation, BaseModel
        )
        is False
    )
    assert (
        annotation_issubclass(
            parameters["dict_typing_generic_arg"].annotation, BaseModel
        )
        is False
    )
