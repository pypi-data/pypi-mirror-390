...
class test_AsynPool:
    ...
    def test_select_unpatched(self):
        with tempfile.TemporaryFile("w") as f:
            _, writeable, _ = asynpool._select(
                writers={
                    f,
                },
                err={
    ...
    @t.skip.if_pypy
    def test_schedule_writes_hub_remove_writers_from_active_writers_when_get_index_error(
        self,
    ):
        pool = asynpool.AsynPool(threads=False)
        hub = self._get_hub()
    ...
    @t.skip.if_pypy
    def test_schedule_writes_hub_remove_fd_only_from_writers_when_write_job_is_done(
        self,
    ):
        pool = asynpool.AsynPool(threads=False)
        hub = self._get_hub()
