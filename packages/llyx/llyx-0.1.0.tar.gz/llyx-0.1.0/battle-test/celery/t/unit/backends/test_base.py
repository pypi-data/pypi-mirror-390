...
class test_BaseBackend_interface:
    ...
    def test_chord_unlock_queue(self, unlock="celery.chord_unlock"):
        ...
        self.app.amqp.router.route = Mock(return_value={"queue": routing_queue})
        self.b.apply_chord(header_result_args, body)
...
class test_BaseBackend_dict:
    ...
    def test_create_chord_error_with_cause(
        ...
        if expected_cause_behavior == "has_cause":
            ...
            assert (
                not hasattr(chord_error, "__cause__") or chord_error.__cause__ is None
            )
    ...
    def test_chord_error_from_stack_callback_dispatch(
        self, callback_type, task_name, expected_group_handler_called
    ):
    ...
    def _create_mock_frozen_group(
        self, group_id="group-id", task_ids=None, task_names=None
    ):
    ...
    def test_handle_group_chord_error_exception_handling(
        self, exception_setup, expected_exc_used
    ):
        ...
        b, backend, group_callback, frozen_group, _ = (
            self._setup_group_chord_error_test(exc=exc)
        )
        ...
        backend.fail_from_current_stack.assert_called_with(
            "task-id-1", exc=expected_exc
        )
    ...
    def test_handle_group_chord_error_multiple_tasks(self):
        ...
        b, backend, group_callback, frozen_group, exc = (
            self._setup_group_chord_error_test(task_ids=task_ids)
        )
    ...
    def test_handle_group_chord_error_with_errbacks(self):
        ...
        errbacks = ["errback1", "errback2"]
        b, backend, group_callback, frozen_group, exc = (
            self._setup_group_chord_error_test(errbacks=errbacks)
        )
    ...
    def test_handle_group_chord_error_cleanup_exception_handling(self):
        ...
        assert (
            result is not None
        )  # Method returns ExceptionInfo from fail_from_current_stack
    ...
    def test_handle_group_chord__exceptions_paths(self, caplog):
        ...
        backend._call_task_errbacks.side_effect = RuntimeError(
            "fail on _call_task_errbacks"
        )

        backend.fail_from_current_stack.side_effect = RuntimeError(
            "fail on fail_from_current_stack"
        )

        _ = self.b._handle_group_chord_error(
            group_callback, backend, ValueError("test error")
        )
...
class test_KeyValueStoreBackend:
    ...
    def test_global_keyprefix(self):
        ...
        app.conf.get("result_backend_transport_options", {}).update(
            {"global_keyprefix": global_keyprefix}
        )
        b = KVBackend(app=app)
        tid = uuid()
        assert (
            bytes_to_str(b.get_key_for_task(tid))
            == f"{global_keyprefix}_celery-task-meta-{tid}"
        )
        assert (
            bytes_to_str(b.get_key_for_group(tid))
            == f"{global_keyprefix}_celery-taskset-meta-{tid}"
        )
        assert (
            bytes_to_str(b.get_key_for_chord(tid))
            == f"{global_keyprefix}_chord-unlock-{tid}"
        )
        ...
        assert (
            bytes_to_str(b.get_key_for_task(tid))
            == f"{global_keyprefix}celery-task-meta-{tid}"
        )
        assert (
            bytes_to_str(b.get_key_for_group(tid))
            == f"{global_keyprefix}celery-taskset-meta-{tid}"
        )
        assert (
            bytes_to_str(b.get_key_for_chord(tid))
            == f"{global_keyprefix}chord-unlock-{tid}"
        )
    ...
    def test_global_keyprefix_missing(self):
        ...
        assert (
            bytes_to_str(self.b.get_key_for_group(tid)) == f"celery-taskset-meta-{tid}"
        )
