...
class test_RedisBackend(basetest_RedisBackend):
    ...
    def test_url(self):
        ...
        x = self.Backend(
            "redis://username:bosco@vandelay.com:123//1",
            app=self.app,
        )
        assert x.connparams
        assert x.connparams["host"] == "vandelay.com"
    ...
    def test_url_with_credential_provider(self):
        ...
        self.app.conf.redis_socket_connect_timeout = 100.0
        x = self.Backend(
            "redis://:bosco@vandelay.com:123/1?credential_provider=redis.CredentialProvider",
            app=self.app,
        )
        ...
        x = self.Backend(
            "redis://@vandelay.com:123/1?credential_provider=redis.UsernamePasswordCredentialProvider",
            app=self.app,
        )
        assert x.connparams
        assert x.connparams["host"] == "vandelay.com"
        ...
        with pytest.raises(ImportError):
            self.Backend(
                "redis://@vandelay.com:123/1?credential_provider=not_exist.CredentialProvider",
                app=self.app,
            )
        ...
        with pytest.raises(ValueError):
            ...
            self.Backend(
                "redis://@vandelay.com:123/1?credential_provider=abc.ABC",
                app=self.app,
            )
    ...
    def test_exception_safe_to_retry(self):
        ...
        assert b.exception_safe_to_retry(
            exceptions.ConnectionError("service unavailable")
        )
...
class test_SentinelBackend:
    ...
    def test_backend_ssl(self):
        ...
        self.app.conf.redis_socket_connect_timeout = 100.0
        x = SentinelBackend(
            "sentinel://:bosco@vandelay.com:123//1",
            app=self.app,
        )
        assert x.connparams
