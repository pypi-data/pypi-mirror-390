...
class DrainerTests:
    ...
    def test_drain_checks_on_interval(self):
        ...
        assert on_interval.call_count < 20, (
...
class GreenletDrainerTests(DrainerTests):
    ...
    def test_start_raises_if_drainer_already_stopped(self):
        with patch.object(
            self.drainer.result_consumer,
            "drain_events",
            side_effect=lambda **_: self.sleep(0),
        ):
            thread = self.schedule_thread(self.drainer.run)
            self.drainer.stop()
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="hangs forever intermittently on windows"
)
