...
class test_ControlPanel:
    ...
    def test_hello_does_not_send_expired_revoked_items(self):
        ...
        worker_state.revoked.add(
            "expired_in_past", now=time.monotonic() - REVOKE_EXPIRES - 1
        )
        x = panel.handle(
            "hello",
            {"from_node": "george@vandelay.com", "revoked": {"1234", "4567", "891"}},
        )
    ...
    def test_revoke_by_stamped_headers_terminate(self, terminate):
        ...
        try:
            ...
            control.revoke_by_stamped_headers(
                state, stamped_header, terminate=terminate
            )
            assert stamped_header.keys() == revoked_stamps.keys()
    ...
    def test_revoke_return_value_terminate_true(self):
        ...
        state = self.create_state()
        state.consumer = Mock()
        r_headers = control.revoke_by_stamped_headers(
            state, header_to_revoke, terminate=True
        )
    ...
    @pytest.mark.skipif(
        IS_PYPY, reason="Patch for sys.modules doesn't work on PyPy correctly"
    )
