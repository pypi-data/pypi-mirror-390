...
def mock_consumer(mock_app):
    ...
    consumer.connection.transport.driver_type = (
        "redis"  # Default to Redis for existing tests
    )
...
def test_cli_skip_checks(isolated_cli_runner: CliRunner):
    ...
    with patch.dict(os.environ, clear=True):
        ...
        assert res.exit_code == 1, (res, res.stdout)
        assert os.environ["CELERY_SKIP_CHECKS"] == "true", (
            "should set CELERY_SKIP_CHECKS"
        )
...
def test_disable_prefetch_affects_qos_behavior(mock_app, mock_consumer):
    ...
    with patch("celery.worker.state.reserved_requests", []):
        ...
        assert (
            mock_consumer.task_consumer.channel.qos.can_consume != original_can_consume
        )
...
def test_disable_prefetch_none_preserves_behavior(mock_app, mock_consumer):
    ...
    if (
        "disable_prefetch" in kwargs_with_none
        and kwargs_with_none["disable_prefetch"] is not None
    ):
