...
class test_Signature(CanvasCase):
    ...
    @pytest.mark.parametrize(
        "_id, group_id, chord, root_id, parent_id, group_index",
        [
            ("_id", "group_id", "chord", "root_id", "parent_id", 1),
        ],
    )
    def test_freezing_args_set_in_options(
...
class test_chain(CanvasCase):
    ...
    def test_from_dict_no_tasks(self, chain_type):
        ...
        assert isinstance(
            chain_type.from_dict(dict(chain_type(app=self.app)), app=self.app),
            chain_type,
        )
    ...
    def test_chord_to_chain(self):
        c = chord(
            [self.add.s("x0", "y0"), self.add.s("x1", "y1")], self.add.s(["foo"])
    ...
    def test_chain_of_chord_upgrade_on_chaining(self):
        ...
        new_chain = (
            c | t
        )  # t should be chained with the body of c[0] and create a new chord
        assert isinstance(new_chain, _chain)
    ...
    def test_chain_of_chord_upgrade_on_chaining__protocol_2(self, group_last_task):
        c = chain(
            group([self.add.s(i, i) for i in range(5)], app=self.app),
            ...
            else group(self.add.s(30), app=self.app),
        )
        assert isinstance(c, _chain)
        assert len(c.tasks) == 1, (
            "Consecutive chords should be further upgraded to a single chord."
        )
    ...
    def test_chain_flattening_keep_links_of_inner_chain(self):
        ...
        assert signature(flat_chain.tasks[0].options["link_error"][0]) == signature(
            "link_ab"
        )
        assert flat_chain.tasks[1].name == "b"
        assert "link" in flat_chain.tasks[1].options, (
            "b is missing the link from inner_chain.options['link'][0]"
        )
        assert signature(flat_chain.tasks[1].options["link"][0]) == signature("link_b")
        assert signature(flat_chain.tasks[1].options["link_error"][0]) == signature(
            "link_ab"
        )
    ...
    def test_group_in_center_of_chain(self):
        t1 = chain(
            self.add.si(1, 1),
            group(self.add.si(1, 1), self.add.si(1, 1)),
...
class test_chord(CanvasCase):
    ...
    def test_flag_allow_error_cb_on_chord_header(self, subtests):
        ...
        with subtests.test(msg="Verify flag turned on links the header"):
            ...
            # header
            for child_sig in header_mock:
                child_sig.link_error.assert_called_once_with(
                    errback_sig.clone(immutable=True)
                )
            # body
    ...
    def test_flag_allow_error_cb_on_chord_header_with_dict_callback(self):
        self.app.conf.task_allow_error_cb_on_chord_header = True
        c = chord(
            group(signature("th1"), signature("th2")), signature("tbody"), app=self.app
        )
    ...
    def test_chord_upgrade_on_chaining(self):
        ...
        stil_chord = (
            c | t
        )  # t should be chained with the body of c and create a new chord
        assert isinstance(stil_chord, chord)
        assert isinstance(stil_chord.body, chord)

    @pytest.mark.parametrize(
        "header",
        [[signature("s1"), signature("s2")], group(signature("s1"), signature("s2"))],
    )
    @pytest.mark.usefixtures("depends_on_current_app")
...
class test_merge_dictionaries(CanvasCase):
    ...
    @pytest.mark.parametrize(
        "d1,d2,expected_result",
        [
            ({"None": None}, {"None": None}, {"None": [None]}),
            ({"None": None}, {"None": [None]}, {"None": [[None]]}),
            ({"None": None}, {"None": "Not None"}, {"None": ["Not None"]}),
            ({"None": None}, {"None": ["Not None"]}, {"None": [["Not None"]]}),
            ({"None": [None]}, {"None": None}, {"None": [None, None]}),
            ({"None": [None]}, {"None": [None]}, {"None": [None, None]}),
            ({"None": [None]}, {"None": "Not None"}, {"None": [None, "Not None"]}),
            ({"None": [None]}, {"None": ["Not None"]}, {"None": [None, "Not None"]}),
        ],
    )
