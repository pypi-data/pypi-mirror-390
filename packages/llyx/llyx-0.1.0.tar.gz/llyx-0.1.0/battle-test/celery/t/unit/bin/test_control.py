...
_GLOBAL_OPTIONS = [
    "-A",
    "t.unit.bin.proj.app_with_custom_cmds",
    "--broker",
    "memory://",
]
...
def test_unrecognized_remote_command(
    celery_cmd, remote_cmd, isolated_cli_runner: CliRunner
):
    res = isolated_cli_runner.invoke(
        celery,
    ...
    assert (
        f"Error: Command {remote_cmd} not recognized. Available {celery_cmd} commands: "
        in res.output
    )
...
def test_listing_remote_commands(
    celery_cmd, expected_regex, isolated_cli_runner: CliRunner
):
    res = isolated_cli_runner.invoke(
        celery,
