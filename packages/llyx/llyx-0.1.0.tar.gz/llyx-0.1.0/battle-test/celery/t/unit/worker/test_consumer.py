...
class test_Consumer(ConsumerTestCase):
    ...
    def test_restore_prefetch_count_on_restart(
        self,
    ...
    def test_blueprint_restart_when_state_not_in_stop_conditions(
        self, broker_connection_retry
    ):
        c = self.get_consumer()
    ...
    def test_ensure_connected(
        ...
        c.app.conf.broker_connection_retry_on_startup = (
            broker_connection_retry_on_startup
        )
        c.app.conf.broker_connection_retry = broker_connection_retry
        ...
        if broker_connection_retry is False:
            ...
            with subtests.test(
                "Does not retry when connect throws an error and retry is set to false"
            ):
                conn = Mock()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            None,
            True,
            id="shutdown on connection loss on startup when retry on startup is undefined",
        ),
        pytest.param(
            False,
            False,
            id="shutdown on connection loss not on startup but startup is defined as false",
        ),
        pytest.param(
            None,
            False,
            id="shutdown on connection loss not on startup and startup is not defined",
        ),
        pytest.param(
            True,
            False,
            id="shutdown on connection loss not on startup but startup is defined as true",
        ),
    ],
)
class test_Consumer_WorkerShutdown(ConsumerTestCase):
    def test_start_raises_connection_error(
        self,
        broker_connection_retry_on_startup,
...
class test_Tasks:
    ...
    def test_log_when_qos_is_false(self, caplog):
        ...
        c.app.conf.worker_disable_prefetch = (
            False  # Prevent our warning from interfering
        )
        c.app.amqp.queues = {"celery": Mock(queue_arguments={"x-queue-type": "quorum"})}
        tasks = Tasks(c)
