...
class test_GCSBackend:
    ...
    def test_bucket_ttl_missing_lifecycle_rule(
        ...
        mock_firestore_ttl.return_value = True
        with pytest.raises(ImproperlyConfigured, match="Missing lifecycle rule"):
            GCSBackend(app=self.app)
    ...
    @patch.object(GCSBackend, "_get_blob")
    @patch.object(GCSBackend, "_is_firestore_ttl_policy_enabled")
    def test_delete_existing_key(self, mock_firestore_ttl, mock_get_blob, base_path):
        self.app.conf.gcs_base_path = base_path
    ...
    @patch.object(GCSBackend, "_get_blob")
    @patch.object(GCSBackend, "_is_firestore_ttl_policy_enabled")
    def test_delete_missing_key(self, mock_firestore_ttl, mock_get_blob, base_path):
        self.app.conf.gcs_base_path = base_path
    ...
    def test_get_blob(self, mock_firestore_ttl, mock_bucket):
        ...
        key_bucket_path = f"{backend.base_path}/{key}" if backend.base_path else key
    ...
    @patch.object(GCSBackend, "_is_firestore_ttl_policy_enabled")
    def test_new_client_after_fork(self, mock_firestore_ttl, mock_pid, mock_client):
        mock_firestore_ttl.return_value = True
    ...
    def test_new_firestore_client_after_fork(
        ...
        mock_firestore_client.assert_called_once_with(project=backend.firestore_project)
    ...
    def test_apply_chord_incr(
        ...
        mock_super_apply_chord_incr.assert_called_once_with(header_result_args, body)
    ...
    def test_on_chord_part_return(
        ...
        mock_maybe_signature.assert_called_once_with(request.chord, app=self.app)
    ...
    def test_restore_deps(
        ...
        mock_group_result_restore.assert_called_once_with(gid, backend=backend)
    ...
    @patch.object(GCSBackend, "_is_firestore_ttl_policy_enabled")
    @patch.object(GCSBackend, "_firestore_document")
    def test_delete_chord_key(self, mock_firestore_document, mock_firestore_ttl):
        key = "test_key"
    ...
    @patch.object(GCSBackend, "_is_firestore_ttl_policy_enabled")
    @patch.object(GCSBackend, "_firestore_document")
    def test_expire_chord_key(self, mock_firestore_document, mock_firestore_ttl):
        key = "test_key"
        expires = 86400
        mock_document = MagicMock()
        mock_firestore_document.return_value = mock_document
    ...
    def test_firestore_document(self, mock_firestore_client, mock_firestore_ttl):
        key = b"test_key"
