...
class test_AsyncResult:
    ...
    def test_date_done(self, utc_datetime_mock, timezone, date):
        utc_datetime_mock.return_value = datetime.datetime(
            2024, 8, 24, 0, 0, 0, 0, datetime.timezone.utc
        )
        self.app.conf.timezone = timezone
        del self.app.timezone  # reset cached timezone

        result = Backend(app=self.app)._get_result_meta(
            None, states.SUCCESS, None, None
        )
