    ...
    def test_queue_type(self, celery_setup: CeleryTestSetup):
        ...
        worker_queue = next(
            (
                queue
                for queue in res
                if queue["name"] == celery_setup.worker.worker_queue
            ),
            None,
        )
        assert worker_queue is not None, (
            f'"{celery_setup.worker.worker_queue}" queue not found'
        )
        queue_type = worker_queue.get("type")
        assert queue_type == "quorum", (
            f'"{celery_setup.worker.worker_queue}" queue is not a quorum queue'
        )
