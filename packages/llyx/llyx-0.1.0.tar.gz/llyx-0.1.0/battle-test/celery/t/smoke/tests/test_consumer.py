...
class test_worker_enable_prefetch_count_reduction_true:
    ...
    @pytest.mark.parametrize(
        "expected_running_tasks_count", range(1, WORKER_CONCURRENCY + 1)
    )
    def test_reducing_prefetch_count(
        self, celery_setup: CeleryTestSetup, expected_running_tasks_count: int
    ):
        ...
        sig = group(
            long_running_task.s(420) for _ in range(expected_running_tasks_count)
        )
        ...
        expected_reduced_prefetch = max(
            ...
            MAX_PREFETCH - expected_running_tasks_count * WORKER_PREFETCH_MULTIPLIER,
        )
        ...
        expected_prefetch_restore_message = f"The prefetch count will be gradually restored to {MAX_PREFETCH} as the tasks complete processing."
    ...
    def test_prefetch_count_restored(self, celery_setup: CeleryTestSetup):
        ...
        sig = group(
            long_running_task.s(10) for _ in range(expected_running_tasks_count)
        )
        sig.apply_async(queue=celery_setup.worker.worker_queue)
        celery_setup.broker.restart()
    ...
    class test_cancel_tasks_on_connection_loss:
        ...
        def test_max_prefetch_passed_on_broker_restart(
...
class test_worker_enable_prefetch_count_reduction_false:
    ...
    def test_max_prefetch_not_passed_on_broker_restart(
        self, celery_setup: CeleryTestSetup
    ):
        if isinstance(celery_setup.broker, RedisTestBroker):
            # When running in debug it works, when running from CLI it sometimes works
