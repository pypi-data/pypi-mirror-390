...
def queue_configuration_test_helper(celery_setup, queues):
    ...
    for queue in res:
        ...
        if queue_level == 0:
            assert (
                queue_arguments["x-dead-letter-exchange"] == "celery_delayed_delivery"
            )
        else:
            assert (
                queue_arguments["x-dead-letter-exchange"]
                == f"celery_delayed_{queue_level - 1}"
            )
        ...
        assert (
            queue_arguments["x-queue-type"]
            == conf.broker_native_delayed_delivery_queue_type
        )
...
def exchange_configuration_test_helper(exchanges):
    res = [
        ...
        if exchange["name"].startswith("celery_delayed")
    ]
    assert len(res) == 29
    for exchange in res:
...
class test_broker_configuration_quorum:
    ...
    def test_native_delayed_delivery_queue_configuration(
        self, queues: list, celery_setup: CeleryTestSetup
    ):
...
class test_native_delayed_delivery:
    ...
    def test_eta(self, celery_setup: CeleryTestSetup):
        ...
        result = s.apply_async(
            eta=datetime.now(datetime_timezone.utc) + timedelta(0, 5)
        )
    ...
    def test_eta_str(self, celery_setup: CeleryTestSetup):
        ...
        result = s.apply_async(
            eta=(datetime.now(datetime_timezone.utc) + timedelta(0, 5)).isoformat()
        )
    ...
    def test_long_delay(self, celery_setup: CeleryTestSetup, queues: list):
        ...
        assert result.ready() is False, "Task with future ETA should not be ready"
    ...
    def test_multiple_tasks_same_eta(self, celery_setup: CeleryTestSetup):
        ...
        results = [s.apply_async(eta=future_time) for _ in range(5)]
    ...
    def test_multiple_tasks_different_delays(self, celery_setup: CeleryTestSetup):
        ...
        results = [
            s.apply_async(eta=now + timedelta(seconds=delay)) for delay in (2, 4, 6)
        ]
    ...
    def test_concurrent_delayed_tasks(self, celery_setup: CeleryTestSetup):
        ...
        results = [s.apply_async(eta=future_time) for _ in range(100)]
