...
class test_signature:
    def test_sanity(self, celery_setup: CeleryTestSetup):
        sig = signature(
            identity, args=("test_signature",), queue=celery_setup.worker.worker_queue
        )
...
class test_chain:
    ...
    def test_chain_gets_last_task_id_with_failing_tasks_in_chain(
    ...
    def test_upgrade_to_chord_inside_chains(self, celery_setup: CeleryTestSetup):
        ...
        chain1 = chain(
            chord1,
            (
                redis_echo.si("a", redis_key)
                | redis_echo.si("b", redis_key).set(queue=queue)
            ),
        )
