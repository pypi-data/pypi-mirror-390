...
def get_rabbitmq_url():
    ...
    return os.environ.get("TEST_BROKER", f"pyamqp://{user}:{password}@localhost:5672//")
...
def get_bindings_for_queue(queue_name, vhost="/"):
    ...
    api_url = (
        f"{get_management_api_url()}/queues/{vhost_encoded}/{queue_encoded}/bindings"
    )
    response = requests.get(api_url, auth=HTTPBasicAuth(user, password))
    response.raise_for_status()
    return response.json()
...
def test_worker_binds_consumed_queue_despite_earlier_queue_failure():
    ...
    with start_worker(
        ...
        delayed_delivery_bindings = get_bindings_for_exchange("celery_delayed_delivery")
        queue_b_delayed_binding = [
            b
            for b in delayed_delivery_bindings
        ...
        topic_to_queue_bindings = [
            b for b in queue_b_bindings if b.get("source") == exchange_name
        ]
