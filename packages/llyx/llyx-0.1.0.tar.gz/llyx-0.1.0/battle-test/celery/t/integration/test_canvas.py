...
class test_chain:
    ...
    def test_complex_chain(self, manager):
        ...
        c = add.s(2, 2) | (add.s(4) | add_replaced.s(8) | add.s(16) | add.s(32)) | g
        res = c()
    ...
    def test_chain_with_eb_replaced_with_chain_with_eb(self, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
    ...
    def test_chain_child_replaced_with_chain_middle(self, manager):
        orig_sig = chain(identity.s(42), replace_with_chain.s(), identity.s())
        res_obj = orig_sig.delay()
        assert res_obj.get(timeout=TIMEOUT) == 42
    ...
    def test_chord_header_id_duplicated_on_rabbitmq_msg_duplication(
        self, manager, subtests, celery_session_app, redis_key
    ):
        """
    ...
    def test_chaining_upgraded_chords_pure_groups(self, manager, subtests):
        ...
        with subtests.test(msg="All tasks are executed once"):
            actual = [
                sig.decode("utf-8") for sig in redis_connection.lrange(redis_key, 0, -1)
            ]
    ...
    def test_chaining_upgraded_chords_mixed_canvas_protocol_2(
        ...
        c = chain(
            ...
            redis_echo.si("Done", redis_key="Done")
            if not group_last_task
            else group(redis_echo.si("Done", redis_key="Done")),
        )
    ...
    def test_upgrade_to_chord_inside_chains(self, manager):
        ...
        chain1 = chain(
            chord1, (redis_echo.si("a", redis_key) | redis_echo.si("b", redis_key))
        )
...
class test_group:
    ...
    def test_group_child_replaced_with_chain_middle(self, manager):
        orig_sig = group(identity.s(42), replace_with_chain.s(1337), identity.s(31337))
        res_obj = orig_sig.delay()
        assert res_obj.get(timeout=TIMEOUT) == [42, 1337, 31337]
    ...
    def test_task_replace_with_group_preserves_group_order(self, manager):
        if manager.app.conf.result_backend.startswith("rpc"):
            raise pytest.skip(
                "RPC result backend does not support replacing with a group"
            )
...
class test_chord:
    ...
    def test_group_args_and_kwargs(self, manager):
        try:
            ...
            raise pytest.skip(e.args[0])
        c = group(add.s(i) for i in range(4)) | add_to_all.s(8)
    ...
    def test_immutable_errback_called_by_chord_from_simple(self, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
        ...
        with subtests.test(msg="Error propagates from simple body task"):
            ...
            with pytest.raises(ExpectedException):
                res.get(timeout=TIMEOUT)
        with subtests.test(msg="Errback is called after simple body task fails"):
            await_redis_echo(
                {
                    errback_msg,
                },
    ...
    def test_immutable_errback_called_by_chord_from_chain(self, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_immutable_errback_called_by_chord_from_chain_tail(self, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_immutable_errback_called_by_chord_from_group(self, manager, subtests):
        if not manager.app.conf.result_backend.startswith("redis"):
            raise pytest.skip("Requires redis result backend.")
        redis_connection = get_redis_connection()
    ...
    def test_mutable_errback_called_by_chord_from_group_fail_multiple_on_header_failure(
        ...
        fail_sigs = tuple(fail.s() for _ in range(fail_task_count))
        errback = errback_task.s()
        # Include a mix of passing and failing tasks
        child_sig = group(
    ...
    def test_nested_chord_header_link_error(self, manager, subtests):
        ...
        with subtests.test(msg="Confirm the body was not executed"):
            ...
            # Double check
            assert not redis_connection.exists(body_key), (
                "Chord body was called when it should have not"
            )
    ...
    def test_enabling_flag_allow_error_cb_on_chord_header(self, manager, subtests):
        ...
        header_errback_sig = redis_echo.si(
            header_errback_msg, redis_key=header_errback_key
        )
        ...
        for header in headers:
            ...
            [
                header_task.link_error(header_errback_sig)
                for header_task in chord_sig.tasks
            ]
            ...
            with subtests.test(
                msg="Confirm the errback was called for each failed header task + body"
            ):
                # confirm the errback was called for each task in the chord header
                failed_header_tasks_count = len(
                    list(filter(lambda f_sig: f_sig == fail.si(), header))
                )
                expected_header_errbacks = tuple(
                    header_errback_msg for _ in range(failed_header_tasks_count)
                )
    ...
    def test_flag_allow_error_cb_on_chord_header_on_upgraded_chord(
        self, manager, subtests
    ):
        """
        ...
        for header in headers:
            ...
            with subtests.test(
                ...
                # confirm the errback was called for each task in the chord header
                failed_header_tasks_count = len(
                    list(filter(lambda f_sig: f_sig.name == fail.si().name, header))
                )
                expected_errbacks_count = (
                    failed_header_tasks_count + 1
                )  # +1 for the body
                expected_errbacks = tuple(
                    errback_msg for _ in range(expected_errbacks_count)
                )
    ...
    def test_upgraded_chord_link_error_with_header_errback_enabled(
        self, manager, subtests
    ):
        try:
            manager.app.backend.ensure_chords_allowed()
    ...
    def test_chord_error_propagation_with_different_body_types(
        ...
        error_found = check_for_logs(
            caplog=caplog, message="ValueError: task_id must not be empty"
        )
        assert not error_found, (
            "The 'task_id must not be empty' error was found in the logs"
        )
...
class test_stamping_mechanism:
    def test_stamping_workflow(self, manager, subtests):
        ...
        def before_task_publish_handler(
            ...
            **kwargs,
            ...
            assertion_result = all(
                [
                    stamped_header in headers["stamps"]
                    for stamped_header in headers["stamped_headers"]
                ]
            )
    ...
    def test_properties_not_affected_from_stamping(self, manager, subtests):
        ...
        with subtests.test(
            ...
            assert "stamped_headers" not in task_properties, (
