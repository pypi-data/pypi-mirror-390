...
def create_app(queue_name: str) -> Celery:
    ...
    broker_url = os.environ.get(
        "TEST_BROKER", f"pyamqp://{rabbitmq_user}:{rabbitmq_pass}@localhost:5672//"
    )
...
def run_worker(simulate_qos_issue: bool, result_queue: multiprocessing.Queue):
    ...
    try:
        ...
        if result_queue.empty():
            result_queue.put(
                {"status": "crash", "reason": "Worker crashed without reporting"}
            )
        ...
        for i, (p, q) in enumerate(zip(processes, queues)):
            try:
                ...
                if p.is_alive():
                    ...
                    p.join(timeout=10)
                    results.append(
                        {"status": "timeout", "reason": f"[worker {i}] timeout"}
                    )
                else:
                    try:
                        ...
                        results.append(
                            {"status": "error", "reason": f"Result error: {str(e)}"}
                        )
            except Exception:
                try:
                    results.append(q.get(timeout=5))
                    ...
                    results.append(
                        {
                            "status": "crash",
                            "reason": f"Worker {i} crashed and gave no result",
                        }
                    )

        if any(QOS_GLOBAL_ERROR in r.get("reason", "").lower() for r in results):
            pytest.xfail(
                "Detected global QoS usage on quorum queue (simulated failure)"
            )
    finally:
