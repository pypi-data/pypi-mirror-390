...
class test_tasks:
    ...
    def test_pydantic_string_annotations(self, manager):
        ...
        for i in range(10):
            results.append(
                [i + i, add_pydantic_string_annotations.delay({"x": i, "y": i})]
            )
        for expected, result in results:
            value = result.get(timeout=10)
            assert value == {"result": expected}
    ...
    def test_expired(self, manager):
        ...
        result = add.apply_async(
            (1, 1), expires=datetime.now(timezone.utc) + timedelta(seconds=1)
        )
        with pytest.raises(celery.exceptions.TaskRevokedError):
            result.get()
        assert result.status == "REVOKED"
    ...
    def test_eta(self, manager):
        ...
        result = add.apply_async(
            (2, 2), eta=datetime.now(timezone.utc) + timedelta(seconds=3)
        )
        time.sleep(1)
        assert result.status == "PENDING"
        assert result.ready() is False
    ...
    def test_revoked_by_headers_complex_canvas(self, manager, subtests):
        ...
        for monitoring_id in ["4242", [1234, uuid4().hex]]:
            ...
            target_monitoring_id = (
                isinstance(monitoring_id, list) and monitoring_id[0] or monitoring_id
            )
            ...
            canvas = [
                ...
                chain(
                    sleeping.si(2),
                    group([sleeping.si(2), stamped_task]),
                    sleeping.si(2),
                ),
                chain(
                    sleeping.si(2),
                    group([sleeping.si(2), sleeping.si(2)]),
                    stamped_task,
                ),
            ]
    ...
    def test_revoke_by_stamped_headers_no_match(self, manager):
        ...
        expected_response = (
            "headers {'myheader': ['myvalue']} flagged as revoked, but not terminated"
        )
    ...
    def test_soft_time_limit_exceeding_time_limit(self):
        with pytest.raises(
            ValueError, match="soft_time_limit must be less than or equal to time_limit"
        ):
...
class test_task_redis_result_backend:
    ...
    def test_ignoring_result_no_subscriptions(self, manager):
        ...
        new_channels = [
            channel
            for channel in get_active_redis_channels()
            if channel not in channels_before_test
        ]
        assert new_channels == []
