...
class test_loader:
    def test_autodiscovery__when_packages_exist(self, manager):
        ...
        manager.app.autodiscover_tasks(
            [expected_package_name, unexpected_package_name], module_name, force=True
        )
    ...
    def test_autodiscovery__when_packages_do_not_exist(self, manager):
        ...
        with pytest.raises(ModuleNotFoundError) as exc:
            manager.app.autodiscover_tasks(
                [existent_package_name, nonexistent_package_name],
                module_name,
                force=True,
            )

        # Assert
        assert nonexistent_package_name.startswith(exc.value.name), (
            'Expected to fail on importing "nonexistent"'
        )
