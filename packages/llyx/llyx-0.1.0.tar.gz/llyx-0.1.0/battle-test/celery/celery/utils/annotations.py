...
def get_optional_arg(annotation: typing.Any) -> typing.Any:
    ...
    if origin != typing.Union and (
        sys.version_info >= (3, 10) and origin != types.UnionType
    ):
        return None
    ...
    if (
        len(union_args) != 2
    ):  # Union does _not_ have two members, so it's not an Optional
        return None
    ...
    type_arg = next(
        arg for arg in union_args if not is_none_type(arg)
    )  # pragma: no branch
...
def annotation_is_class(annotation: typing.Any) -> bool:
    ...
    if (3, 9) <= sys.version_info < (3, 11) and isinstance(
