...
class DconnNet(nn.Module):
    ...
    def forward_feature(self, x):
        ...
        return feat_list, mapped_c5

    def forward_cls_pred(self, feat_list):
    ...
    def forward(self, x):
        feat_list, mapped_c5 = self.forward_feature(x)
        cls_pred = self.forward_cls_pred(feat_list)
...
class MM_DconnNet(BaseModule, DconnNet):
    def __init__(self, **kwargs):
        ...
        DconnNet.__init__(self, **kwargs)
    
    def forward(self, x):
        return self.forward_feature(x)[0]
...
class MM_DconnNet_Decoder(BaseDecodeHead):
    def __init__(self, **kwargs):
        ...
        super().__init__(input_transform='multiple_select', init_cfg=None, **kwargs)
        
        self.final_decoder=LWdecoder(in_channels=kwargs['in_channels'],
                                     out_channels=32,
                                     in_feat_output_strides=(4, 8, 16, 32),
                                     out_feat_output_stride=4,
                                     norm_fn=nn.BatchNorm2d,
                                     num_groups_gn=None)
        self.cls_pred_conv_2 = nn.Conv2d(32, kwargs['num_classes']*8, 1)
        self.upsample4x_op = nn.UpsamplingBilinear2d(scale_factor=2)
    
        del self.conv_seg # not using mmseg built-in cls seg conv
    
    def forward(self, inputs):
        x = self.final_decoder(inputs)
        x = self.cls_pred_conv_2(x)
        x = self.upsample4x_op(x)
        return x
