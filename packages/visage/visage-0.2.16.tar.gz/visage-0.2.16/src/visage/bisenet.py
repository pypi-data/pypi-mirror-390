...
class BiSeNetFaceParser:
    ...
    def __init__(self):
        ckpt_path = f"{Path().home()}/.cache/torch/face-parser/bisenet.pth"
        download_path = "https://drive.google.com/open?id=154JgKpzCPW82qINcVieuPH3fZ2e0P812"

        if not Path(ckpt_path).exists():
            raise ValueError(
                f"No pre-trained BiSeNet found. Download the checkpoint from {download_path} and put it into {ckpt_path}")

        n_classes = 19
        self._model = BiSeNet(n_classes=n_classes).cuda()
    ...
    def parse(self, images: torch.Tensor) -> torch.Tensor:
        ...
        images = images.cuda()
        segmentation_masks = self._model(images)[0]
