...
def main():
    ...
    # --- Hashing ---
    with Progress(
        TextColumn("[spinner] [bold green]Calculating file hash...[/bold green]"),
        transient=True,
    ) as progress:
    ...
    console.print(
        Panel(
            f"[bold]Hash:[/] {client_hash}",
            title="[cyan]Hashing[/cyan]",
            border_style="cyan",
            title_align="left"
        )
    )
    ...
    try:
        ...
        panel = Panel(progress, title="[cyan]Uploading[/cyan]", border_style="cyan", title_align="left")

        with Live(panel, refresh_per_second=10):
            task_id = progress.add_task(filename, total=encoder.len)
            monitor = MultipartEncoderMonitor(encoder, lambda m: progress.update(task_id, advance=m.bytes_read - progress.tasks[task_id].completed))
