from GrnGame import GrnGame as G
...
def creer_explosion(x, y):
    ...
    G.jouer_son("assets/explosion.wav", boucle=1, canal=1)
...
def tirer_projectile():
    ...
    G.jouer_son("assets/tir.wav", boucle=1, canal=2)
...
def update_jeu():
    ...
    if G.touche_juste_presser("F3"):
        G.redimensionner_fenetre()
    
    if not init_mannette:
       init_mannette= G.init_mannette()
    
    G.dessiner_image("assets/fond.png", 0, 0, 400, 400)
    
    if game_over:
        G.dessiner_mot("assets/police", "GAME OVER", 130, 200, 2, 3)
        G.dessiner_mot("assets/police", f"SCORE: {score}", 165, 250, 1, 3)
        G.dessiner_mot("assets/police", "APPUIE B RESTART", 140, 300, 1, 2)
        
        if G.touche_mannette_juste_presse("B"):
            vies = 3
            score = 0
            game_over = False
            vaisseau_x = 176
    ...
    if G.touche_mannette_enfoncee("A") and tir_cooldown <= 0:
        tirer_projectile()
    ...
    for monstre in monstres:
        ...
        if invincible_timer == 0 and collision_rect(
            ...
            monstres_a_supprimer.append(monstre)
            G.jouer_son("assets/degat.wav", boucle=1, canal=3)
            if vies <= 0:
                game_over = True
            continue
    ...
    for exp in explosions:
        G.dessiner_image(f"assets/explosion {exp['frame']}.png", exp['x'], exp['y'], 62, 62)
    
    for monstre in monstres:
        G.dessiner_image(f"assets/monstre {monstre_anim_frame}.png", monstre['x'], monstre['y'], 62, 62)
    
    for proj in projectiles:
        G.dessiner_image(f"assets/projectile {projectile_anim_frame}.png", proj['x'], proj['y'], 32, 32)
    
    if invincible_timer == 0 or int(invincible_timer * 10) % 2 == 0:
        frame_vaisseau = min(vaisseau_degats + 1, 4)
        G.dessiner_image(f"assets/vaisseau {frame_vaisseau}.png", vaisseau_x, vaisseau_y, 48, 48)
    
    G.dessiner_mot("assets/police", f"VIES: {vies}", 10, 10, 1, 2)
    G.dessiner_mot("assets/police", f"SCORE: {score}", 300, 10, 1, 2)
