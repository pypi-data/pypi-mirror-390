...
float ajouter_char_dans_tableau(Gestionnaire* jeu,  char* lien_image, char lettre, float posx, float posy,
                                float coeff, int sens, int rotation)
{
    if (!jeu || !lien_image)
    {
...
void ajouter_mot_dans_tableau(Gestionnaire* jeu, char* chemin, const char* mot, float posx, float posy,
                              float coeff, int sens, float ecart, int rotation)
{
    if (!jeu || !chemin || !mot)
    {
    ...
    {
        float largeur =
            ajouter_char_dans_tableau(jeu, chemin, mot[i], posx + position_courante, posy, coeff, sens, rotation);
