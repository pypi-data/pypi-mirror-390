...
Gestionnaire* initialisation(int hauteur, int largeur, float fps, int coeff, char* lien_image, char* lien_son,
                             bool dessiner, bool bande_noir, int r, int g, int b, const char* nom_fenetre, char* chemin_console);
...
void ajouter_mot_dans_tableau(Gestionnaire* jeu,  char* chemin, const char* mot, float posx, float posy,
                              float coeff, int sens, float ecart, int decalage);
...
void jouer_son(Gestionnaire* gestionnaire,  char* lien, int boucle, int canal);
