...
def LinearPerturbation(name, species=None, camb_var=None, camb_sub=None, frame_dependence=None, description=None):
    ...
    f = Function(
        ...
        frame_dependence=frame_dependence,
...
background_eqs = [Eq(diff(a, t), H * a), Eq(diff(H, t), dH), Eq(diff(exptau, t), visibility)]
...
pert_eqs = [Eq(diff(z, t), dz), Eq(diff(sigma, t), dsigma), Eq(diff(eta, t), deta), Eq(diff(phi, t), dphi)]
...
Newtonian_var_subs = [
    Eq(Phi_N, phi + half * a**2 * kappa * Pi / k**2),
...
Newtonian_subs = [
    ...
    Eq(diff(sigma, t), 0),
    Eq(sigma, 0),
    Eq(phi, (Phi_N + Psi_N) / 2),
    Eq(eta, -2 * Phi_N * K_fac),
    Eq(hdot, -diff(Phi_N, t)),
]
...
synchronous_vars = [
    Eq(eta_s, -make_frame_invariant(eta, v_c) / 2 / K_fac),
...
synchronous_subs += [subs(synchronous_subs, Eq(z, solve(cons4, z))), Eq(sigma, (hdot_s + diff(6 * eta_s, t)) / 2 / k)]
...
delta_eqs = [
    ...
    Eq(diff(Delta_b, t), -(1 + p_b / rho_b) * (3 * hdot + k * v_b) + (p_b / rho_b - csq_b) * 3 * H * Delta_b),
    Eq(diff(Delta_c, t), -3 * hdot - k * v_c),
    Eq(
        diff(Delta_nu, t), -3 * (1 + p_nu / rho_nu) * hdot - k * q_nu + 3 * H * (-Delta_P_nu + Delta_nu * p_nu / rho_nu)
    ),
    Eq(
        diff(Delta_de, t),
        -3 * (1 + w_de) * hdot
        - (1 + w_de) * k * v_de
        - 3 * H * (csqhat_de - w_de) * (Delta_de + 3 * H * (1 + w_de) * v_de / k)
...
vel_eqs = [
    ...
    Eq(
        ...
        -k * A
        - H * v_b
        - diff(p_b, t) * v_b / (rho_b + p_b)
        + 1 / (rho_b + p_b) * (rho_b * csq_b * k * Delta_b - rho_g * drag_t),
    ),
    Eq(diff(v_c, t), -H * v_c - k * A),
    Eq(
        diff(q_nu, t),
        -H * (1 - 3 * p_nu / rho_nu) * q_nu
        - k / 3 * (2 * K_fac * pi_nu - 3 * Delta_P_nu)
        - (1 + p_nu / rho_nu) * k * A,
    ),
...
def get_scalar_temperature_sources(checks=False):
    ...
    source = (
        source1 / 3
        + diff(source1, t, t) / k**2
        + visibility * Delta_g / 4
        - hdot * exptau
        + diff(visibility * v_b - k * exptau * A, t) / k
    )
    src = (
        subs(
            var_subs,
            subs(background_eqs + pert_eqs, subs(background_eqs + pert_eqs, source).expand().doit()).simplify(),
        )
        .simplify()
        .expand()
    )
    ...
    if checks:
        ...
        assert (
            subs(
                Friedmann_Kfac_subs,
                subs(Friedmann_Kfac_subs, subs(var_subs, 2 * dphi * exptau - ISW).simplify()).simplify(),
            ).simplify()
            == 0
        )
...
def camb_fortran(expr, name="camb_function", frame="CDM", expand=False):
    ...
    camb_diff_subs = [
        ...
        (diff(polter, t), polterdot),
        (diff(sigma, t), sigmadot),
        (diff(phi, t), phidot),
        (diff(p_b, t), 0),
    ]
...
def internal_consistency_checks():
    ...
    assert subs(K_sub, subs(var_subs, subs(q_sub, deta).simplify() - subs(deta_2, diff(eta, t)))).simplify() == 0
    ...
    assert (
        subs(
            ...
            subs(
                var_subs,
                subs(pert_eqs, subs(cdm_gauge(synchronous_vars), subs(synchronous_subs, sigma) - sigma).doit()),
            )
            .simplify()
            .collect(eta),
        ).simplify()
    ...
    for eq in delta_eqs + vel_eqs:
        assert (
            simplify(
                subs(component_eqs + background_eqs, make_frame_invariant(eq.lhs).doit() - make_frame_invariant(eq.rhs))
            )
            == 0
        )
