...
class ExchangeAdapter:
    def __init__(self, api_key: Optional[str] = None, secret: Optional[str] = None, config: Optional[dict] = None) -> None:
        ...
        # expose options and margin adapters (real)
    ...
    async def close(self) -> None:
        """
        Gracefully close transports and cancel background tasks.

        This will:
        - cancel and await any asyncio.Task attributes on market adapters named
          in ADAPTER_TASK_NAMES.
        - call `await adapter.transport.close()` for each market adapter that has a transport.
