...
class BulkModelSerializer(BulkSerializerMixin, serializers.ModelSerializer):
    ...
    def _convert_to_bulk_fields(self):
        ...
        # Remove all database-hitting validators from serializer-level validators
        if hasattr(self, 'validators'):
            self.validators = [
                v for v in self.validators
                if not isinstance(v, (UniqueTogetherValidator, UniqueValidator))
            ]
        
        # Remove UniqueValidator from ALL fields to prevent N+1 queries
        # Uniqueness is validated in the operations layer with batched queries
