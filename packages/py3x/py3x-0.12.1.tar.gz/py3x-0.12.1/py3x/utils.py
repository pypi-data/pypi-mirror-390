...
def as_json_encoder(cls):
    class AsJsonEncoder(cls):
        def default(self, obj):
            f = getattr(obj, 'as_json', None)
            if f is not None and isinstance(f, MethodType):
                return f()
            return super().default(obj)

    return AsJsonEncoder


AsJsonEncoder = as_json_encoder(json.JSONEncoder)
