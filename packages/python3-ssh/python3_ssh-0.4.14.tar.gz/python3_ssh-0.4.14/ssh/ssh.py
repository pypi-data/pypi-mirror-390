...
class SSHClient(object):
    ...
    def _scp_to_remote(self, local_path, remote_path):
        """拷贝本地文件/目录到远端：文件用sftp，目录用scp -r递归拷贝"""
        ...
        if local_path.is_file():
            ...
            # 验证文件存在
            if self.exec(f"ls {remote_path}").exit_status_code != 0:
                log.warning(f" {self.__ip}:{self.__port} | 文件拷贝失败: 远端未找到 {remote_path}")
                return False
            return True

        # 处理本地目录：用scp -r递归拷贝（不遍历文件）
        if local_path.is_dir():
            # 构建scp命令：通过本地执行scp，将目录推送到远端
            # 格式：scp -r <本地目录> <用户@ip:远端路径>
            ssh_user = self.__username  # 假设self有SSH用户名属性（如root）
            ssh_port = self.__port or 22  # 默认为22端口
            # 远端路径需用引号包裹，避免特殊字符问题
            remote_scp_path = f"{ssh_user}@{self.__ip}:{str(remote_path)}"
            scp_cmd = [
                "scp", "-r",
                "-P", str(ssh_port),  # 指定SSH端口
                "-o", "StrictHostKeyChecking=no",  # 忽略主机密钥检查
                "-o", "UserKnownHostsFile=/dev/null",  # 不保存主机密钥（可选，避免污染known_hosts）
                str(local_path),  # 本地目录
                remote_scp_path  # 远端目标路径
            ]

            # 执行scp命令（通过subprocess，利用本地scp客户端）
            try:
                # 隐藏scp输出，仅捕获错误（如需调试可添加stdout=subprocess.PIPE）
                ...
                log.info(f" {self.__ip}:{ssh_port} | 目录递归拷贝完成: {local_path} -> {remote_path}")
                return True
            except subprocess.CalledProcessError as e:
                log.error(
                    f" {self.__ip}:{ssh_port} | 目录拷贝失败: 命令 {' '.join(scp_cmd)}，错误: {e.stderr}"
                )
                return False
    ...
    def scp_to_remote(self, local_path, remote_path, timeout=120):
        try:
            # 确保SSH连接激活（目录拷贝依赖本地scp，也需基础连接验证）
            if not self.__ssh and not self.reconnect():
            ...
            if local_path.is_dir():
                ...
                if self.exec(f"test -f {remote_path}").exit_status_code == 0:
                    ...
                    if self.exec(f"rm -f {remote_path}").exit_status_code != 0:
                        ...
                        return False
                # 确保远端父目录存在（避免scp因父目录不存在失败）
                remote_parent = remote_path.parent
                self.exec(f"mkdir -p {remote_parent}")
