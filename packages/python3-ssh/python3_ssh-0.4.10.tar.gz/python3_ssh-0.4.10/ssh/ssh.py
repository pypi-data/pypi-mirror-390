        ...
        if local_path.is_dir():
            ...
            for root, dirs, files in os.walk(local_path):
                ...
                for file in files:
                    ...
                    if rs.exit_status_code != 0:
                        log.info(f" {self.__ip}:{self.__port} | 创建目录: {remote_file_parent}")
                        self.exec(f"mkdir -p {remote_file_parent}")
                    else:
                        log.info(f" {self.__ip}:{self.__port} | 目录已存在: {remote_file_parent}")
                    # 上传文件
                    try:
                        self.__sftp.put(str(local_file), str(remote_file))
                        log.info(f" {self.__ip}:{self.__port} | 已拷贝: {local_file} -> {remote_file}")
                    except Exception as e:
                        log.error(
