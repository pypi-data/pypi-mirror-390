...
class ServerAuthenticationBackend(AuthenticationBackend):
    ...
    UNAUTHENTICATED_PATHS = {"/", "/info", "/health", "/ok", "/docs", "/openapi.json"}
...
class ProxyUser(BaseUser):
    ...
    def __init__(self, user: Any):
        ...
        # TODO: This should probably be some kind of proper HTTP failure, but at least this ValueError will fail
        # loudly so the developer knows.
        identity = getattr(user, "identity", None)
        if not identity or (isinstance(identity, str) and not identity.strip()):
            raise ValueError(
                f"Auth handler returned invalid identity: {identity!r}. "
                f"Identity must be a non-empty string."
            )
...
class SimpleUser(ProxyUser):
    def __init__(self, username: str):
        if not username or not username.strip():
            raise ValueError(
                f"Auth handler returned invalid identity: {username!r}. "
                f"Identity must be a non-empty string."
            )
