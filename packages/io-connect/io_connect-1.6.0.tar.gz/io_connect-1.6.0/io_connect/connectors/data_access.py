...
class DataAccess:
    ...
    def __get_cleaned_table(
        ...
        alias_with_uns: Optional[bool] = False
    ) -> pd.DataFrame:
        """
        ...
        if not alias_with_uns:
            ...
            # Create a combined column name: sensorAlias_paramName
            df['column_name'] = df['sensorAlias'] + '::' + df['paramName']
            
            # Select only the columns we need for pivoting
            pivot_df = df[['timestamp', 'column_name', 'paramValue',"effectiveFrom",'effectiveTo']]
            
            # Pivot the dataframe
            df = pivot_df.pivot_table(
                index=['timestamp', 'effectiveFrom', 'effectiveTo'],
                columns='column_name',
                values='paramValue',
                aggfunc='first'
            ).reset_index()
            ...
            sensor_cols = [col for col in df.columns if col not in ['time', 'effectiveFrom', 'effectiveTo']]
            df = df[['time'] + sensor_cols + ['effectiveFrom', 'effectiveTo']]
            # Sort by time and convert to datetime
            cols = ['time', 'effectiveFrom', 'effectiveTo']
            df[cols] = df[cols].apply(pd.to_datetime, errors='coerce')
            # Convert time to Unix timestamp if required
            if unix:
                df[cols] = df[cols].applymap(lambda x: int(x.timestamp() * 1000) if pd.notna(x) else None)
            else:
                # Convert time column to timezone
                df[cols] = df[cols].apply(lambda c: c.dt.tz_convert(self.tz))
    ...
    def data_query(
        ...
        try:
            ...
            # Ensure only one param is passed either global_alias or sensor_list
            if global_alias  and not sensor_list:
                raise ValueError(
                    "Invalid parameters: 'sensor_list' must be not be None or empty when 'global_alias' is set to True."
                )
            ...
            if parallel:
                ...
                df = self.__get_global_alias(
                    device_id  = device_id,
                    start_time = start_time_unix,
                    end_time = end_time_unix,
                    global_alias = sensor_list,
                    unix = unix,
                    cal = cal,
                    alias = alias,
                    on_prem = on_prem,
    ...
    def __get_global_alias(
        ...
        - global_alias (Optional[List]): List of global_alias of the sensors to retrieve the data.
        - cal (bool): Whether to perform calibration on the data.
        - alias (bool): To get the alias of the sensors in the column names of the Dataframe. 
        - on_prem (Optional[bool]): Indicates if the operation is on-premise. Defaults to class attribute if not provided.
        ...
        all_rows = []
        alias_with_uns = False
        # Determine the protocol based on the on_prem flag
        protocol = "http" if on_prem else "https"

        # Construct API URL for data retrieval
        url = c.GET_TIMED_CALIBRATION.format(protocol=protocol, data_url=self.data_url)
