...
This package is published on PyPI as **``optimal-text-clustering``** and is imported as ``opticlust``.  The old
``text2clusters`` import path and CLI command still work for backwards‑compatibility, but new code
should prefer ``opticlust``.  The distribution name reflects the goal of selecting **optimal**
hyper‑parameters for clustering (see below).
