...
import sys
from influxdb_client import InfluxDBClient, WritePrecision  # type: ignore
...
class Influx:
    ...
    def upload_dataframe(self, dataframe: pd.DataFrame, token: str):
        ...
        with InfluxDBClient(url=self.url, token=token, org=self.org) as client:
    ...
    def upload_points(self, points, token, timestamp=None) -> dict:
        with InfluxDBClient(url=self.url, token=token, org=self.org) as client:
            with client.write_api() as write_api:
                try:
                    ...
                    return {
                        "success": True,
                        "message": "Data successfully writein to Influx.",
                    }
                    ...
                    return {
                        "success": False,
                        "message": f"Failed to write '{timestamp}' to Influx (HTTP {status_code}): {e.message}",
                    }
                except Exception as e:
                    sys.stdout.flush()
                    sys.stderr.flush()
                    return {
                        "success": False,
                        "message": f"Failed to write '{timestamp}' to Influx: {e}",
                    }

    def generate_query(
        self, tags: dict[str, dict[str, str]], start: Time, end: Time, dt_sec: int = 1
    ):
        self.tags = tags
        tag_string = ""
        for value in tags.values():
            tag_string = tag_string + 'r["_field"] == "' + value["tag"] + '" or '
