...
class AutoWinWindow:
    ...
    def get_window(self, title: Optional[str] = None, class_name: Optional[str] = None,
        ...
        """
        logger.debug(f"ℹ️ 尝试获取窗口，条件: title='{title}', class_name='{class_name}', regex='{regex}', 超时: {timeout}s")
        window = core.find_window(title=title, class_name=class_name, regex=regex, timeout=timeout)
    ...
    def activate(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"激活窗口失败: {window_obj.window_text()} - {e}")
    ...
    def maximize(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"最大化窗口失败: {window_obj.window_text()} - {e}")
    ...
    def minimize(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"最小化窗口失败: {window_obj.window_text()} - {e}")
    ...
    def restore(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"恢复窗口失败: {window_obj.window_text()} - {e}")
    ...
    def close(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"关闭窗口失败: {window_obj.window_text()} - {e}")
    ...
    def move_window(self, window_obj: Any, x: int, y: int) -> None:
        ...
        """
        try:
            logger.debug(f"ℹ️ 尝试移动窗口 '{window_obj.window_text()}' 到 ({x}, {y})")
            window_obj.move_window(x=x, y=y)
            logger.info(f"✅ 成功移动窗口 '{window_obj.window_text()}' 到 ({x}, {y})")
        except Exception as e:
            logger.error(f"❌ 移动窗口失败: {window_obj.window_text()} 到 ({x}, {y}) - {e}")
            raise WindowNotFoundError(f"移动窗口失败: {window_obj.window_text()} 到 ({x}, {y}) - {e}")
    ...
    def resize_window(self, window_obj: Any, width: int, height: int) -> None:
        ...
        """
        try:
            logger.debug(f"ℹ️ 尝试调整窗口 '{window_obj.window_text()}' 大小为 {width}x{height}")
            window_obj.resize_window(width=width, height=height)
            logger.info(f"✅ 成功调整窗口 '{window_obj.window_text()}' 大小为 {width}x{height}")
        except Exception as e:
            logger.error(f"❌ 调整窗口失败: {window_obj.window_text()} 大小为 {width}x{height} - {e}")
            raise WindowNotFoundError(f"调整窗口失败: {window_obj.window_text()} 大小为 {width}x{height} - {e}")
    ...
    def set_topmost(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"设置窗口置顶失败: {window_obj.window_text()} - {e}")
    ...
    def remove_topmost(self, window_obj: Any) -> None:
        ...
        try:
            ...
            raise WindowNotFoundError(f"取消窗口置顶失败: {window_obj.window_text()} - {e}")
    ...
    def is_topmost(self, window_obj: Any) -> bool:
        ...
        try:
            ...
            ex_style = win32gui.GetWindowLong(window_obj.handle, win32con.GWL_EXSTYLE)
