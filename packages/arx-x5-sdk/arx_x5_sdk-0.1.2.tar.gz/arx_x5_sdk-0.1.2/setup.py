...
class BuildArxSDK(build):
    ...
    def run(self):
        ...
        # Copy the installed .so files to the build directory so they're included in the wheel
        import glob
        build_lib = Path(self.build_lib) if hasattr(self, 'build_lib') else this_dir / "build" / "lib"
        pkg_dir = build_lib / "arx_x5_sdk"
        pkg_dir.mkdir(parents=True, exist_ok=True)
        
        # Copy all .so files from arx_x5_sdk/ to build/lib/arx_x5_sdk/
        for so_file in (this_dir / "arx_x5_sdk").glob("*.so"):
            shutil.copy2(so_file, pkg_dir)
