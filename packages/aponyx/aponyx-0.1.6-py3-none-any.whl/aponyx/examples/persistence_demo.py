...
def demonstrate_dataclass_features() -> None:
    ...
    print(
        f"    OK Typed attributes: {type(entry.instrument).__name__}, {type(entry.row_count).__name__}"
    )
    ...
    large_datasets = [
        ...
        if registry.get_dataset_entry(name).row_count
        and registry.get_dataset_entry(name).row_count > 100
    ]
