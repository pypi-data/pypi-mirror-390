...
class LogicalErrorStrategy(ConsistencyStrategy):
    ...
    def analyze(self, file_path: Path, tree: ast.AST, file_content: str, project_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # NEW: Detect division by zero risks
        issues.extend(self._check_division_by_zero(tree, file_content))
        
        # NEW: Detect always true/false conditions
        issues.extend(self._check_constant_conditions(tree, file_content))
        
        # NEW: Detect missing break in switch-like structures
        issues.extend(self._check_missing_break_in_loops(tree, file_content))
        
        # NEW: Detect duplicate conditions
        issues.extend(self._check_duplicate_conditions(tree, file_content))
        
        # NEW: Detect contradictory conditions
        issues.extend(self._check_contradictory_conditions(tree, file_content))
    ...
    def _check_division_by_zero(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.BinOp) and isinstance(node.op, (ast.Div, ast.FloorDiv, ast.Mod)):
                ...
                # Check if divisor could be zero (variable without check)
                elif isinstance(node.right, ast.Name):
                    # This is a simple heuristic - in practice, this needs flow analysis
                    issues.append({
                        'line': node.lineno,
                        'column': node.col_offset,
                        'severity': 'warning',
                        'type': 'logical_error',
                        'message': f'Potential division by zero: variable "{node.right.id}" not checked',
                        'suggestion': f'Add check: if {node.right.id} != 0 before division',
                        'fixable': False
                    })
        
        return issues
    
    def _check_constant_conditions(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
                            ...
                            if left.id == right.id:
                                if isinstance(op, (ast.Lt, ast.Gt, ast.NotEq)):
                                    issues.append({
                                        ...
                                        'severity': 'error',
                                        'type': 'logical_error',
                                        'message': f'Comparing variable to itself: "{left.id}" {op.__class__.__name__} "{right.id}" is always False',
                                        'fixable': False
                                    })
                                elif isinstance(op, (ast.Eq, ast.LtE, ast.GtE)):
                                    issues.append({
                                        'line': node.lineno,
                                        'column': node.col_offset,
                                        'severity': 'warning',
                                        'type': 'logical_error',
                                        'message': f'Comparing variable to itself: "{left.id}" {op.__class__.__name__} "{right.id}" is always True',
                                        'fixable': False
                                    })
        
        return issues
    
    def _check_missing_break_in_loops(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
        """Detect loops where break might be missing"""
        issues = []
        
        for node in ast.walk(tree):
            if isinstance(node, (ast.For, ast.While)):
                # Check if loop has a return but no break
                has_return = False
                has_break = False
                ...
                # If loop has nested if with return but no break, might be suspicious
                            ...
                            for child in ast.walk(stmt):
                                if isinstance(child, ast.Return):
                                    issues.append({
                                        ...
                                        'severity': 'info',
                                        'type': 'logical_error',
                                        'message': 'Loop contains conditional return but no break - verify this is intended',
                                        'fixable': False
                                    })
                                    break
    ...
    def _check_duplicate_conditions(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
        """Detect duplicate conditions in if-elif chains"""
        issues = []
            ...
            if isinstance(node, ast.If):
                ...
                while current:
                    if isinstance(current, ast.If):
                        conditions.append((current.test, current.lineno))
                ...
                for i, (cond1, line1) in enumerate(conditions):
                    for cond2, line2 in conditions[i+1:]:
                        # Simple structural comparison
                        if ast.dump(cond1) == ast.dump(cond2):
                            issues.append({
                                'line': line2,
                                'column': 0,
                                'severity': 'error',
                                'type': 'logical_error',
                                'message': f'Duplicate condition - same as condition on line {line1}',
                                'fixable': False
                            })
        
        return issues
    
    def _check_contradictory_conditions(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
        """Detect contradictory conditions that can never be true"""
        issues = []
        
        for node in ast.walk(tree):
            if isinstance(node, ast.BoolOp) and isinstance(node.op, ast.And):
                # Check for x > 5 and x < 3 patterns
                for i, value1 in enumerate(node.values):
                    for value2 in node.values[i+1:]:
                        if isinstance(value1, ast.Compare) and isinstance(value2, ast.Compare):
                            # Simple check: same variable being compared
                            if (isinstance(value1.left, ast.Name) and isinstance(value2.left, ast.Name) and
                                value1.left.id == value2.left.id):
                                
                                # Check for contradictions like x > 5 and x < 3
                                if (isinstance(value1.ops[0], ast.Gt) and isinstance(value2.ops[0], ast.Lt)):
                                    if (isinstance(value1.comparators[0], ast.Constant) and 
                                        isinstance(value2.comparators[0], ast.Constant)):
                                        if value1.comparators[0].value >= value2.comparators[0].value:
                                            issues.append({
                                                'line': node.lineno,
                                                'column': node.col_offset,
                                                'severity': 'error',
                                                'type': 'logical_error',
                                                'message': f'Contradictory conditions: {value1.left.id} > {value1.comparators[0].value} and {value2.left.id} < {value2.comparators[0].value} can never be true',
                                                'fixable': False
                                            })
