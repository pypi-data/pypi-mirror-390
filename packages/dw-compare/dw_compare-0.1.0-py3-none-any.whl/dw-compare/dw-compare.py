#!/usr/bin/env python3
"""
Data Whisperer CLI - dw_compare.py

Usage:
    python dw_compare.py dataset_a.csv dataset_b.csv
    python dw_compare.py dataset_a.csv dataset_b.csv --output-save --output report.md --json

Produces a concise, structured markdown report that highlights structural and
statistical differences between two tabular datasets. Focuses on:
- added/removed/renamed columns
- row count changes and null ratios
- numeric column mean, median, std, range shifts and percent change
- categorical new/dropped categories and top-value shifts
- duplicate rows, invalid formats, and outlier counts
...
def detect_renames(cols_a: List[str], cols_b: List[str], threshold: float = 0.8) -> List[Tuple[str, str, float]]:
    """Return list of probable renames as (col_a, col_b, score)."""
...
def simple_type(s: pd.Series) -> str:
    # heuristics to label column type
...
def compare_dataframes(df_a: pd.DataFrame, df_b: pd.DataFrame, rename_threshold: float = 0.82) -> Dict[str, Any]:
    ...
    # remove renames from added/removed lists
    ...
    # included renamed column comparisons
...
def markdown_report(report: Dict[str, Any]) -> str:
    ...
    # Prioritize columns with biggest mean change and null change
...
def main(argv: List[str] = None) -> int:
    ...
    if args.output_save:
        ...
        print(md)
        # If json requested, print it after the markdown with a clear separator
        if args.json:
            print("\n\n--- JSON OUTPUT ---\n")
