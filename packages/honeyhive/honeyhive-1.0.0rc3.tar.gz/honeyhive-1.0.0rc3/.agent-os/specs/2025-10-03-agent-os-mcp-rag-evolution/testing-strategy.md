...
- **Line Coverage:** 90%+
- **Branch Coverage:** 85%+
- **Files:** All `.agent-os/mcp_servers/*.py`
...
def test_chunk_real_file():
    ...
    chunker = AgentOSChunker()
    test_file = Path(".agent-os/standards/ai-assistant/compliance-checking.md")
...
def test_honeyhive_tracing_integration():
    ...
    result = mcp_server.search_standards(
...
def test_cursor_mcp_integration():
    ...
    server_process = subprocess.Popen([
        "python", ".agent-os/mcp_servers/agent_os_rag.py"
    ])
...
```python
# .agent-os/scripts/validate_rag.py
...
```python
# .agent-os/scripts/benchmark_rag.py
...
```bash
# Unit tests
pytest tests/unit/mcp_servers/ -v --cov=.agent-os/mcp_servers --cov-report=html
...
# Validation
python .agent-os/scripts/validate_rag.py

# Benchmarking
python .agent-os/scripts/benchmark_rag.py

# All tests
pytest tests/ -v --cov=.agent-os/mcp_servers
```
