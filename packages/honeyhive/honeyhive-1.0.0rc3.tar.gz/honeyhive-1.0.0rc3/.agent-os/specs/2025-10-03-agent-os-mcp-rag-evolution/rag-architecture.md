...
client = chromadb.PersistentClient(
    path=".agent-os/.cache/vector_index",
    settings=Settings(
...
def grep_fallback(query: str, n_results: int = 5) -> SearchResult:
    ...
    result = subprocess.run(
        ["rg", query, ".agent-os/standards", "-C", "3"],
        capture_output=True,
        text=True
    )
...
Steps:
1. Find all .md files in .agent-os/standards/ (198 files)
...
def rebuild_if_needed():
    ...
    metadata_file = Path(".agent-os/.cache/vector_index/metadata.json")
    ...
    current_hash = hash_directory(Path(".agent-os/standards"))
