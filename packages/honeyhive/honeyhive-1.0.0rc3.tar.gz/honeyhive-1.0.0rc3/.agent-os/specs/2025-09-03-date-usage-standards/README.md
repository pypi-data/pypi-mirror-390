...
.agent-os/specs/2025-09-03-ai-quality-framework/      ✅ Correct
.agent-os/specs/2025-09-03-testing-standards/         ✅ Correct
...
NEW_SPECS=$(git diff --cached --name-only | grep "\.agent-os/specs/")
...
```bash
# ❌ Wrong
mkdir .agent-os/specs/2025-01-30-new-feature

# ✅ Correct
CURRENT_DATE=$(date +"%Y-%m-%d")
mkdir ".agent-os/specs/${CURRENT_DATE}-new-feature"
```
...
2. **Correction Execution**
   ...
   # Rename directories
   CURRENT_DATE=$(date +"%Y-%m-%d")
   mv .agent-os/specs/2025-01-30-spec .agent-os/specs/${CURRENT_DATE}-spec
...
- `.agent-os/specs/2025-09-03-ai-assistant-quality-framework/` - AI quality framework
