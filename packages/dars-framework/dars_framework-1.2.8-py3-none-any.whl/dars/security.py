...
def minify_js(src: str) -> str:
    ...
    # If default minifier requests fallback-only, skip external tools
    if os.getenv('DARS_DEFAULT_MINIFY_ONLY_FALLBACK', '0') == '1':
...
def minify_output_dir(output_dir: str, extra_skip: Iterable[str] = None, progress_cb=None) -> int:
    ...
    """
    # Global switch to disable default Python-side minifier
    try:
        if os.environ.get('DARS_DEFAULT_MINIFY', '1') == '0':
            return 0
    except Exception:
        pass
