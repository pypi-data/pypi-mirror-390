...
class Executor:
    ...
    def __init__(self, queue: Queue, job_types: Jobs, max_workers: int = 5):
        """
...
class MessageQueue:
    ...
    def send(self, job_type: str, **kwargs) -> UUID:
        """
        ...
        Args:
            job_type: String name of the job type to execute (must be registered in Jobs)
    ...
    def send_and_receive(self, job_type: str, timeout: float = 5.0, **kwargs) -> Response:
        """
        ...
        Usage:
            response = mq.send_and_receive('calculate', x=10, y=20, timeout=2.0)
            if response.success:
                print(f"Result: {response.result}")
        """
        message_id = self.send(job_type, **kwargs)
    ...
    def batch_receive(self, message_ids: dict[str, UUID], min_timeout: float = 1.0, max_iteration: int = 10) -> tuple[
    ...
    def batch_send_and_receive(self, messages: dict[str, Message], min_timeout: float = 1.0, max_iteration: int = 10) -> tuple[dict[str, Response], dict[str, Response]]:
