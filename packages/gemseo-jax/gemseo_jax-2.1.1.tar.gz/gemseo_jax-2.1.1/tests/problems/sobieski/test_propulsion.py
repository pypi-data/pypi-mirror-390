...
"""Tests for JAXSobieskiPropulsion."""
...
def input_data() -> dict[str, list[float]]:
    """An input value for [JAX]SobieskiPropulsion."""
    return {
        k: v * (1 + (1 + index) / 10)
...
@pytest.fixture(scope="module")
def numpy_discipline() -> SobieskiPropulsion:
    """The NumPy version of SobieskiPropulsion."""
    return SobieskiPropulsion()


@pytest.fixture(scope="module")
def jax_discipline() -> JAXSobieskiPropulsion:
    """The JAX version of JAXSobieskiPropulsion."""
    return JAXSobieskiPropulsion()


def test_execute(numpy_discipline, jax_discipline, input_data):
    """Check the execution of JAXSobieskiPropulsion."""
