...
class DockerArtifact(ArtifactBase):
    ...
    @classmethod
    def pull(cls, image, ga='', la='', once=False):
        image = cls.real_image(image)
        if not cls.exist(image):
            cmd = f'{cls.cli} {ga} pull {la} {image}'
            if once:
                shell_wrapper(cmd)
            else:
                print('', end='', flush=True)
                ret, err = shell_result(cmd)
                if ret:
                    for mark in ['net/http: TLS handshake timeout']:
                        if mark in err:
                            time.sleep(1)
                            print(err, flush=True)
                            return cls.pull(image, ga, la, once)
                    else:
                        raise subprocess.SubprocessError(err)
        return image

    @classmethod
    def push(cls, image, ga='', la='', once=False):
        image = cls.real_image(image)
        cmd = f'{cls.cli} {ga} push {la} {image}'
        if once:
            shell_wrapper(cmd)
        else:
            print('', end='', flush=True)
            ...
            if ret:
                for mark in ['net/http:', 'dial tcp:']:
                    if mark in err:
                        time.sleep(1)
                        print(err, flush=True)
                        return cls.push(image, ga, la, once)
                else:
                    raise subprocess.SubprocessError(err)
