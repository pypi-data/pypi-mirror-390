...
from ray_embedding.utils import report_unhealthy_replica, report_unhealthy_replica_sync
...
class EmbeddingModel:
    ...
    def wait_for_cuda(self, wait: int = 10):
        ...
        if error_message:
            report_unhealthy_replica_sync(error=error_message, node_reaper=self.node_reaper)
            raise RuntimeError(error_message)

    async def check_health(self):
        error_message = self._evaluate_cuda_health()
        if error_message:
            await report_unhealthy_replica(error=error_message, node_reaper=self.node_reaper)
            raise RuntimeError(error_message)
