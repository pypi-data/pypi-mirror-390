...
def engine2_cmd(ctx: typer.Context):
    ...
    # Reconstruct argv for click CLI
    sys.argv = ["dh", "engine2"] + ctx.args
    engine_cli(standalone_mode=False)
...
def studio2_cmd(ctx: typer.Context):
    ...
    # Reconstruct argv for click CLI
    sys.argv = ["dh", "studio2"] + ctx.args
    studio_cli(standalone_mode=False)
