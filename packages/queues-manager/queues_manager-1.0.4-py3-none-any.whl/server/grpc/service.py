...
class QueueServiceServicer(queue_service_pb2_grpc.QueueServiceServicer if queue_service_pb2_grpc else object):
    ...
    def QueueExists(self, request: queue_service_pb2.QueueExistsRequest, context) -> queue_service_pb2.QueueExistsResponse:
        ...
        try:
            # 检查队列键或状态集合是否存在（create_queue 会创建状态集合）
            queue_key = queue_manager._get_queue_key(request.agent_id)
            queue_exists = queue_manager.redis_client.exists(queue_key)
            
            # 如果队列键不存在，检查状态集合是否存在
            if not queue_exists:
                for status in [queue_service_pb2.PENDING, queue_service_pb2.PROCESSING, 
                              queue_service_pb2.COMPLETED, queue_service_pb2.FAILED]:
                    status_key = queue_manager._get_task_status_key(request.agent_id, status)
                    if queue_manager.redis_client.exists(status_key):
                        queue_exists = True
                        break
            
            return queue_service_pb2.QueueExistsResponse(
                exists=bool(queue_exists),
