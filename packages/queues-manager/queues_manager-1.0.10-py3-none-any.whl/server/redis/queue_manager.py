...
class QueueManager:
    ...
    def submit_task(self, task_dict: Dict[str, Any]) -> Tuple[bool, Optional[str], bool]:
        ...
        try:
            ...
            # 如果队列中有占位符，先移除它（因为现在有真实任务了）
            placeholder = "__queue_initialized__"
            self.redis_client.zrem(queue_key, placeholder)
            ...
            if timeout > 0:
                ...
                while time.time() - start_time < timeout:
                    ...
                    if result:
                        ...
                        # 过滤占位符
                        if task_id and not task_id.startswith("__queue_initialized__"):
                            break
                        # 如果是占位符，继续获取下一个
                    # 等待一小段时间后重试
                    time.sleep(0.1)
    ...
    def queue_exists(self, agent_id: str) -> bool:
        ...
        检查逻辑：
        1. 检查队列键（Sorted Set）是否存在
        2. 检查状态集合是否存在（至少有一个状态集合存在）
    ...
    def create_queue(self, agent_id: str) -> bool:
        ...
        try:
            ...
            # 步骤2：队列不存在，初始化队列结构
            # 初始化状态集合（为每个状态创建空集合）
            # 注意：Redis 的 Set 如果为空会被自动清理，所以我们需要保留一个占位符
            # 或者使用一个特殊的标记来标识队列已创建
            for status in [PENDING, PROCESSING, COMPLETED, FAILED]:
                status_key = self._get_task_status_key(agent_id, status)
                # 创建空集合（如果不存在）
                # 使用一个特殊的占位符元素来确保集合存在
                # 这个占位符不会被实际使用，只是用来标记队列已创建
                placeholder = f"__queue_initialized__{status}__"
                self.redis_client.sadd(status_key, placeholder)
            
            # 初始化队列键（优先级队列使用 Sorted Set）
            # 为了确保队列键存在，我们也添加一个占位符
            queue_key = self._get_queue_key(agent_id)
            # 使用一个极低的优先级分数和特殊的占位符来标记队列已创建
            placeholder_task_id = "__queue_initialized__"
            self.redis_client.zadd(queue_key, {placeholder_task_id: 0.0})
