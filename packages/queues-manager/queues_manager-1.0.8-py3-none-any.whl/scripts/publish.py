...
try:
    ...
    if credentials_file.exists():
        ...
        print(f"✓ Loaded PyPI credentials from {credentials_file}")
...
def upload_to_pypi(test=True, client_only=False):
    ...
    # 认证优先级：1. 环境变量（可覆盖） 2. 凭证文件 3. .pypirc 文件
    # 支持两种方式：环境变量和凭证文件，环境变量优先级更高
    
    username = None
    password = None
    
    # 1. 优先使用环境变量（如果设置了环境变量，优先使用）
    username_key = "TEST_TWINE_USERNAME" if test else "TWINE_USERNAME"
    password_key = "TEST_TWINE_PASSWORD" if test else "TWINE_PASSWORD"
    
    username = os.getenv(username_key) or os.getenv("TWINE_USERNAME")
    password = os.getenv(password_key) or os.getenv("TWINE_PASSWORD")
    
    # 2. 如果环境变量未设置，使用凭证文件
    if not (username and password):
        if _pypi_credentials:
            if test:
                username = username or getattr(_pypi_credentials, "TEST_PYPI_USERNAME", None) or getattr(_pypi_credentials, "PYPI_USERNAME", None)
                password = password or getattr(_pypi_credentials, "TEST_PYPI_PASSWORD", None) or getattr(_pypi_credentials, "PYPI_PASSWORD", None)
            else:
                username = username or getattr(_pypi_credentials, "PYPI_USERNAME", None)
                password = password or getattr(_pypi_credentials, "PYPI_PASSWORD", None)
