...
from maleo.metadata.mixins.system_role import is_id_identifier
...
from maleo.metadata.enums.system_role import Granularity
...
class SystemRoleClientService(MaleoClientService[MaleoMetadataClientConfig]):
    _resource = SYSTEM_ROLE_RESOURCE
        ...
        *,
            ...
            if redis_response_str is not None:
                ...
                if cardinality is Cardinality.MULTIPLE:
                    ...
                    operation = ReadMultipleResourceOperation[
                        ...
                        summary=f"Successfully read multiple {granularity} {self._resource.identifiers[-1].name.lower()} from cache",
                    ...
                    operation = ReadSingleResourceOperation[data_model_cls, None](
                        ...
                        resource=self._resource,
                        timestamp=operation_timestamp,
                        summary=f"Successfully read single {granularity} {self._resource.identifiers[-1].name.lower()} from cache",
                        connection_context=connection_context,
                        authentication=None,
                        authorization=authorization,
                        impersonation=impersonation,
        ...
        async with self._http_client_manager.get() as http_client:
            ...
            if response.is_error:
                ...
                exc.log_and_publish_operation(
                    self._logger, Level.ERROR, self._publishers
                )
                raise exc
            ...
            if isinstance(parameters, ReadMultipleParameter):
                ...
                operation = ReadMultipleResourceOperation[
                    ...
                    resource=self._resource,
                    timestamp=operation_timestamp,
                    summary=f"Successfully read multiple {granularity} {self._resource.identifiers[-1].name.lower()} from microservice",
                    connection_context=connection_context,
                    authentication=None,
                    authorization=authorization,
                    impersonation=impersonation,
                ...
                operation = ReadSingleResourceOperation[data_model_cls, None](
                    ...
                    resource=self._resource,
                    timestamp=operation_timestamp,
                    summary=f"Successfully read single {granularity} {self._resource.identifiers[-1].name.lower()} from microservice",
                    connection_context=connection_context,
                    authentication=None,
                    authorization=authorization,
                    impersonation=impersonation,
