...
from typing import TypeVar, Generic, Type, Callable, Protocol, Optional, ClassVar, TYPE_CHECKING
...
class AssetService(Generic[T, P], ChattyAssetContainerWithCollection[T, P]):
    ...
    def insert(self, item: T, execution_context: 'ExecutionContext', company_info: Optional['EmpresaModel'] = None) -> T:
        """Insert with automatic event handling if configured"""
        result = super().insert(item, execution_context)
    ...
    def update(self, id: str, new_item: T, execution_context: 'ExecutionContext', company_info: Optional['EmpresaModel'] = None) -> T:
        """Update with automatic event handling if configured"""
        result = super().update(id, new_item, execution_context)
    ...
    def delete(self, id: str, execution_context: 'ExecutionContext', company_info: Optional['EmpresaModel'] = None, deletion_type: Optional['DeletionType'] = None) -> T:
        """Delete with automatic event handling if configured"""
        from ...models.utils.types.deletion_type import DeletionType as DT
        result = super().delete(id, execution_context, deletion_type or DT.LOGICAL)
    ...
    def restore(self, id: str, execution_context: 'ExecutionContext', company_info: Optional['EmpresaModel'] = None) -> T:
        """Restore with automatic event handling if configured"""
        result = super().restore(id, execution_context)
        if company_info and self._should_handle_events() and self.event_type_updated:
            self._queue_event(result, self.event_type_updated, execution_context, company_info)
        return result

    # Generic convenience methods
    def create_asset(self, data: dict, execution_context: 'ExecutionContext', company_info: 'EmpresaModel') -> T:
        """
        ...
        return self.insert(item, execution_context, company_info)

    def update_asset(self, id: str, data: dict, execution_context: 'ExecutionContext', company_info: 'EmpresaModel') -> T:
        """
        ...
        return self.update(id, new_item, execution_context, company_info)

    def delete_asset(self, id: str, execution_context: 'ExecutionContext', company_info: 'EmpresaModel') -> T:
        """
        Generic delete method - deletes with events.
        Can be called as delete_asset or aliased to delete_product/delete_tag/etc.
        """
        return self.delete(id, execution_context, company_info)
