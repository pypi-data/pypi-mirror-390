...
class ChainOfThoughtInChatRequest(BaseModel):
    ...
    trigger_id : str = Field(description="The id of the trigger which could be a message_id or a workflow assigned to chat link id", default="")
...
class ChainOfThoughtInChat(CompanyAssetModel):
    ...
    chatty_ai_agent : Dict[str, Any] = Field(description="The chatty ai agent at the moment of triggering the chain of thought", default_factory=dict)
    chain_of_thought : Optional[str] = Field(default=None, description="The chain of thought")
    name: str = Field(description="A title for the chain of thought", alias="title")
