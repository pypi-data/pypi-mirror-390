...
class iPayTools:
    ...
    def get_current_gas_price(self):
        ...
        # Tambah 20% safety buffer untuk gas price fluctuations
    ...
    def calculate_minimum_profitable_fee(self):
        ...
        # Estimate gas usage untuk useTool
        try:
            base_gas_estimate = self.contract.functions.useTool("test").estimate_gas({
                'from': self.account,
                'value': self.w3.to_wei(0.0001, 'ether')  # dummy value
            })
            ...
            # Default gas estimate jika gagal
            safe_gas_estimate = 80000
    ...
    def _is_fee_profitable(self, fee_wei=None):
        ...
        if is_profitable:
            ...
            gas_price = self.get_current_gas_price()
            gas_estimate = self.contract.functions.useTool("test").estimate_gas({
                'from': self.account,
                'value': fee_wei
            })
            ...
            profit_margin = (ipay_profit / ipay_revenue) * 100 if ipay_revenue > 0 else 0
            
            self.logger.info(f"✅ Fee is profitable (margin: {profit_margin:.1f}%)")
        else:
            ipay_profit = -1  # negative profit
            profit_margin = -1
            self.logger.warning(f"⚠️ Fee is NOT profitable (need: {self.w3.from_wei(min_fee_wei, 'ether'):.6f} ETH)")
    ...
    def _ensure_profitable_fee(self):
        ...
        if current_fee < min_fee:
            self.logger.warning(f"🔄 Adjusting fee from {self.w3.from_wei(current_fee, 'ether'):.6f} to {self.w3.from_wei(min_fee, 'ether'):.6f} ETH")
    ...
    def use_tool_safe(self, data, value_eth=None):
        ...
        # 5. Estimate gas dengan value yang actual
        try:
            gas_estimate = self.contract.functions.useTool(data).estimate_gas({
                'from': self.account,
                'value': value_wei
            })
        except Exception as e:
        ...
        # 6. Calculate final cost dengan safety buffer
        ...
        try:
            tx_hash = self.contract.functions.useTool(data).transact({
                'from': self.account,
                'value': value_wei,
                'gas': gas_estimate * 150 // 100,  # 50% safety buffer
                'gasPrice': gas_price
            })
    ...
    def withdraw_earnings(self):
        tx_hash = self.contract.functions.withdrawEarnings().transact({
            'from': self.account
        })
        return self.w3.eth.wait_for_transaction_receipt(tx_hash)

def quick_start_demo():
    print("🚀 iPayTools Quick Start")
    ...
    try:
        ...
        print(f"❌ Transaction rejected (SAFE): {e}")
