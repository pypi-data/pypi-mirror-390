...
from nkunyim_util.encryption import Base64Util, Hashing
...
class OAuth2Service:
    ...
    def get_authorization_url(self, command: AuthorizeCommand) -> str:
        ...
        self.model = OAuth2Model(next=next, state=state, nonce=nonce, code="", verifier=code_verifier)
        self._save_model()
        
        code_challenge = Hashing(input_string=code_verifier).make(algo=command.hash_algo)
        ...
        authorize_url = (f"{self.issuer_url}/identity/authorize/?"
                         ...
                         + f"code_challenge_method={command.hash_algo.value}"
        )
    ...
    def authorize_access_token(self, command: TokenCommand) -> Optional[TokenModel]:
        try:
            ...
            # Set code in command
            command.code = code
            command.code_verifier = self.model.verifier
            
            response = requests.post(url=self.issuer_url + "/identity/token/", data=command.model_dump(), headers=self.headers)
            if not response.ok:
                self.messages.append(MessageModel(level=MessageLevel.WARNING, message="Failed to retrieve token data from API. Reason: " + response.reason))
                return None
            ...
            self.messages.append(MessageModel(level=MessageLevel.DANGER, message="Failed to retrieve token data from API. Exception: " + str(ex)))
            return None
