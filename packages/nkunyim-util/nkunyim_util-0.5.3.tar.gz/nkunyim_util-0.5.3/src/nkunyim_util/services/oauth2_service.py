...
class OAuth2Service:
    ...
    def get_authorization_url(self, command: AuthorizeCommand) -> str:
        ...
        code_verifier = str(uuid4().int)
        self.model = OAuth2Model(next=next, state=state, nonce=nonce, code="", verifier=code_verifier)
        self._save_model()
        
        hex_digest = Hashing(input_string=code_verifier).make(algo=command.hash_algo)
        code_challenge = Base64Util.encode(input_string=hex_digest)
