...
class User(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(Text, default=datetime.now, nullable=False)
...
class Project(Base):
    ...
    created_by: Mapped[Optional[int]] = mapped_column(ForeignKey("users.user_id", ondelete="SET NULL"))
    created_at: Mapped[datetime] = mapped_column(Text, default=datetime.now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(Text, default=datetime.now, onupdate=datetime.now, nullable=False)
    ...
    wells: Mapped[List["Well"]] = relationship("Well", back_populates="project", cascade="all, delete-orphan")
...
class Well(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(Text, default=datetime.now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(Text, default=datetime.now, onupdate=datetime.now)

    project: Mapped["Project"] = relationship("Project", back_populates="wells")
    curves: Mapped[List["Curve"]] = relationship("Curve", back_populates="well", cascade="all, delete-orphan")
...
class Curve(Base):
    ...
    data: Mapped[List["CurveData"]] = relationship("CurveData", back_populates="curve", cascade="all, delete-orphan")
...
class AuditLog(Base):
    ...
    timestamp: Mapped[datetime] = mapped_column(Text, default=datetime.now, nullable=False)
