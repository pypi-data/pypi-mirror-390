...
def docs() -> str:
    return dedent("""\
        ...
        **Sucesso:**
        ```xml
        <ordens_servico codigo_os="456" status_anterior="Em Atendimento" status_novo="Cancelada-Usuário" sistema="SIGA">
            <ordem_servico sistema="SIGA">
                <status>sucesso</status>
                <mensagem>OS 456 foi cancelada com sucesso!</mensagem>
                <detalhes>Status alterado de 'Em Atendimento' para 'Cancelada-Usuário'</detalhes>
            </ordem_servico>
        </ordens_servico>
        ```
        ...
        **Em caso de erro de validação (tipo de cancelamento inválido):**
        ```xml
        <erro_validacao sistema="SIGA" funcao="alterar_status_os_siga_ia">
            <erro sistema="SIGA">
                <status>erro</status>
        ...
        **Em caso de erro (dados inválidos):**
        ...
        <ordens_servico codigo_os="456" status_anterior="Em Atendimento" status_novo="Cancelada-Usuário" sistema="SIGA">
            <ordem_servico sistema="SIGA">
                <status>erro</status>
                <mensagem>Não foi possível alterar o status da OS 456. Verifique as informações digitadas.</mensagem>
            </ordem_servico>
        </ordens_servico>
        ```
        ...
        **Em caso de outros erros:**
        ...
        <ordens_servico codigo_os="456" status_anterior="Em Atendimento" status_novo="Cancelada-Usuário" sistema="SIGA">
            <ordem_servico sistema="SIGA">
                <status>erro</status>
                <mensagem>Erro ao alterar status da OS 456. Tente novamente.</mensagem>
            </ordem_servico>
        </ordens_servico>
        ```

        Args:
            codigo_os (str | int): **[OBRIGATÓRIO]** Código da OS a ser cancelada
            tipo_cancelamento (str): Tipo de cancelamento (padrão: "Cancelada-Usuário"). Opções válidas:
                - "Cancelada-Usuário" (código 100) - padrão
        ...
        Returns:
            str: XML formatado com resultado da operação (mesmo formato de alterar_status_os_siga_ia):
                - Em caso de sucesso: confirmação do cancelamento com detalhes da operação
                - Em caso de erro de validação: detalhes do erro (OS não encontrada, tipo de cancelamento inválido, etc.)
        ...
        Examples:
            >>> # Cancelar OS com status padrão
            >>> resultado = await cancelar_os_siga_ia("456")
            
            >>> # Cancelar OS com tipo específico
            >>> resultado = await cancelar_os_siga_ia("789", "Cancelamento DTD | Arquivado")

            >>> # Exemplo com tipo inválido (retorna erro)
            >>> resultado = await cancelar_os_siga_ia("123", "Tipo Inexistente")  # Erro!

            >>> # Exemplo com código OS vazio (retorna erro)
            >>> resultado = await cancelar_os_siga_ia("", "Cancelada-Usuário")  # Erro!
