...
def main():
    ...
    if stored_ip == old_ip:
        ...
        # Fetch and format present date-time
        dt = datetime.datetime.now()
        dt_str = dt.strftime("%d %b %Y %H:%m")

        # Cloudflare DNS update payload
        url = f"https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records/{dns_record_id}"
        # "proxied": False stops cloudflare from hiding the address behind a proxy
        payload = {
            "comment": f"updated on {dt_str} by cloudflare-ddns-updater",
            "content": f"{stored_ip}",
            "proxied": False,
            "ttl": 120
        }
