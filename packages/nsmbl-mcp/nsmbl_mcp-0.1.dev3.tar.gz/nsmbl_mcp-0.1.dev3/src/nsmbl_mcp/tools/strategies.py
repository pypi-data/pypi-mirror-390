...
async def create_strategy(strategy_data: dict[str, Any]) -> str:
    ...
    Args:
        strategy_data: Strategy configuration (strategy_type, strategy_name, strategy_config)
    ...
    try:
        ...
        if not is_valid:
            return format_validation_error("strategy_data", error_message)
...
CREATE_STRATEGY_TOOL = Tool(
    ...
    description=(
        ...
        "• Use exact enum values - see valid values below\n\n"
    ...
    inputSchema={
        "type": "object",
        "properties": {
            "strategy_data": get_strategy_data_schema()
        },
