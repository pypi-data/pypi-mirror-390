...
try:
	...
	raise ImportError("Optimas must be installed to use optimas-openai target.\nInstall: pip install optimas-ai") from e
...
def create_component_from_openai(
	...
	class OpenAIModule(BaseComponent):
		...
		def forward(self, **inputs) -> dict:
			...
			result = self._callable(**inputs, prompt=prompt_str, _system_prompt=self.variable)
