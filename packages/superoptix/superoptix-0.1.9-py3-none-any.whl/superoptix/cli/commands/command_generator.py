...
class CommandGeneration(dspy.Signature):
	...
	intent_action: str = dspy.InputField(desc="Parsed action (build, compile, optimize, etc.)")
	...
	project_context: str = dspy.InputField(desc="Current project state and available agents")
	
	cli_commands: str = dspy.OutputField(desc="CLI commands to execute (one per line)")
	explanation: str = dspy.OutputField(desc="Brief explanation of what these commands will do")
...
class CommandGenerator:
	...
	VALID_NAMESPACES = {
		'software', 'healthcare', 'finance', 'education', 'legal', 'marketing',
		'manufacturing', 'retail', 'transportation', 'energy', 'agriculture',
		'consulting', 'government', 'human_resources', 'hospitality',
		'real_estate', 'media', 'gaming'
	}
	...
	def _rule_based_generation(self, intent: Intent, project_context: str) -> List[str]:
		...
		if action == 'build' or action == 'create':
			...
			namespace = self._infer_namespace(target, params)  # Intelligent namespace inference
			commands.append(f"super spec generate {tier} {target} --namespace {namespace}")
			...
			reflection_lm = params.get('reflection_lm', 'gpt-4o-mini')  # Default reflection model
	...
	def _dspy_generation(self, intent: Intent, project_context: str) -> List[str]:
		...
		try:
			...
			commands = [
				cmd.strip() 
				for cmd in result.cli_commands.split('\n') 
				if cmd.strip()
			]
