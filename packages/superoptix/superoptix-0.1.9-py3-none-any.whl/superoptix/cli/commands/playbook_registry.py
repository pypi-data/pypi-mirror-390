...
class PlaybookRegistry:
    ...
    def list_by_source(self, source: str) -> List[Dict]:
        ...
        return [
            data for name, data in self.registry.items()
    ...
    def get_count(self) -> dict:
        """Get playbook counts."""
        library_count = len([d for d in self.registry.values() if d["source"] == "library"])
        user_count = len([d for d in self.registry.values() if d["source"] == "user_project"])
