...
class IntentRecognition(dspy.Signature):
	...
	user_input: str = dspy.InputField(desc="The user's natural language request or question")
	available_commands: str = dspy.InputField(desc="List of available SuperOptiX CLI commands")
	conversation_history: str = dspy.InputField(desc="Recent conversation context showing previous actions and agents mentioned")
	knowledge_context: str = dspy.InputField(desc="Relevant SuperOptiX documentation and examples retrieved for this query")
	
	intent_action: str = dspy.OutputField(desc="Primary action: build, compile, optimize, evaluate, run, help, list, or show. Choose the MOST specific action that matches user intent.")
	intent_target: str = dspy.OutputField(desc="Target agent/resource name extracted from input. Use underscores for multi-word names. Return 'none' if no specific target.")
	intent_parameters: str = dspy.OutputField(desc="JSON object with extracted parameters like {\"tier\": \"genies\", \"namespace\": \"software\", \"goal\": \"...\", \"auto\": \"medium\"}")
	confidence: float = dspy.OutputField(desc="Your confidence in this parse from 0.0 to 1.0. Be honest - if input is ambiguous, give lower confidence.")
	reasoning: str = dspy.OutputField(desc="Brief explanation: 'User wants to [action] because they said [key phrase]. Target is [target] extracted from [where]. Parameters: [why]'")
...
"""
		...
		try:
			...
			import json
			try:
				parameters = json.loads(result.intent_parameters) if result.intent_parameters else {}
	...
	def _get_contextual_hints(self, user_input: str) -> str:
		...
		if any(word in user_lower for word in ['memory', 'context', 'remember']):
			hints.append("💾 Memory: Use 'genies' tier for agents with memory. Memory helps agents retain context across conversations.")
		
		if any(word in user_lower for word in ['tool', 'action', 'external']):
			hints.append("🔧 Tools: Genie agents support external tool integration for actions like API calls, file operations, etc.")
		
		if any(word in user_lower for word in ['rag', 'knowledge', 'retrieval', 'search']):
			hints.append("🔍 RAG: Genie agents can use retrieval-augmented generation for accessing knowledge bases.")
		
		if any(word in user_lower for word in ['optimize', 'improve', 'better', 'tune']):
			hints.append("🎯 Optimization: GEPA optimizes prompts using genetic algorithms. Use --auto high for best results.")
		
		if any(word in user_lower for word in ['compile', 'code', 'pipeline']):
			hints.append("⚡ Compilation: Converts YAML playbooks to executable DSPy/LangChain/CrewAI code.")
		
		if any(word in user_lower for word in ['evaluate', 'test', 'benchmark']):
			hints.append("📊 Evaluation: Tests agent on datasets. Create eval.yaml in your project with test cases.")
	...
	def _fallback_parse(self, user_input: str) -> Intent:
		...
		if any(word in user_lower for word in ['build', 'create']) and 'agent' in user_lower:
			action = 'build'
			...
			action = 'list'
		elif user_lower.startswith('help') or user_lower.startswith('how') or user_lower.startswith('what'):
			action = 'help'
		else:
		...
		stopwords = {'build', 'create', 'compile', 'optimize', 'run', 'evaluate', 'test', 'a', 'the', 'my', 'an', 'with', 'for', 'me', 'you', 'can', 'please'}
		...
		if action in ['evaluate', 'test', 'run', 'optimize', 'compile'] and not target:
			# Find action word
			action_words = {'evaluate': 'evaluate', 'test': 'test', 'run': 'run', 'optimize': 'optimize', 'compile': 'compile'}
			...
			if action_word in words:
				...
				for word in words[action_idx+1:]:
					cleaned = word.replace(',', '').replace('.', '').replace('"', '').replace("'", '')
					if cleaned not in stopwords and cleaned not in ['agent', 'agents'] and len(cleaned) > 2:
						target = cleaned
						break
