...
class ContextWindowOptimizer:
	...
	def _init_gepa_scorer(self):
		...
		try:
			...
			class MemoryRelevanceScorer(dspy.Signature):
				...
				task_context = dspy.InputField(desc="Additional task context", prefix="Task:")
	...
	def optimize_context(
		...
		return {
			...
			"strategy": f"gepa_optimized_{task_type}" if self.enable_gepa else "heuristic",
			"scores": {m.get("id", i): m.get("score", 0.0) for i, m in enumerate(ordered)},
		...
		for memory in memories:
			...
			if self.enable_gepa and self.gepa_scorer:
				try:
					...
					final_score = self._heuristic_score(relevance, importance, recency, task_type)
			else:
				final_score = self._heuristic_score(relevance, importance, recency, task_type)
	...
	def _select_within_budget(
		...
		for memory in must_include:
			...
			if total_tokens + memory_tokens <= max_tokens:
				...
				# Try summary instead
				summary = self.summarizer.summarize(memory, target_tokens=memory_tokens // 2)
		...
		for score, memory in scored_memories:
			...
			if total_tokens + memory_tokens <= max_tokens:
				...
				summary = self.summarizer.summarize(memory, target_tokens=remaining_tokens)
				summary_tokens = self._estimate_tokens(summary)
	...
	def _update_stats(self, selected_memories: List[Dict[str, Any]]):
		...
		self.stats["avg_tokens_used"] = (
			(self.stats["avg_tokens_used"] * (n - 1) + total_tokens) / n
		)
		self.stats["avg_memories_selected"] = (
			(self.stats["avg_memories_selected"] * (n - 1) + len(selected_memories)) / n
		)
		self.stats["avg_relevance_score"] = (
			(self.stats["avg_relevance_score"] * (n - 1) + avg_score) / n
		)
