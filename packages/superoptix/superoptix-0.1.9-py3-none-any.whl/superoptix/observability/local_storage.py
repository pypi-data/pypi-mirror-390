...
class LocalObservabilityStorage:
	...
	def store_agent_run(self, metrics: Dict[str, Any]):
		...
		""", (
			...
			json.dumps({k: v for k, v in metrics.items() 
			           if k not in ["agent_name", "framework", "accuracy", 
			                       "cost_usd", "tokens_used", "latency_ms", 
			                       "success_rate", "status"]})
		))
		self.conn.commit()
	...
	def store_optimization(self, metrics: Dict[str, Any]):
		...
		""", (
			...
			json.dumps(metrics.get("evolution_history")) if metrics.get("evolution_history") else None
		))
		self.conn.commit()
