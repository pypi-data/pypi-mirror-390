...
class ModelSetupMixin:
    ...
    def setup_reflection_lm(self, lm_config: dict) -> dspy.LM:
        ...
        if provider == "ollama":
            model_str = f"ollama_chat/{model_name}" if not model_name.startswith("ollama_chat/") else model_name
            print(f"[REFLECTION_LM DEBUG] Using Ollama LM: {model_str}, api_base=http://localhost:11434")
            return LM(model_str, api_base="http://localhost:11434", api_key="", temperature=temperature, max_tokens=max_tokens)
        else:
            print(f"[REFLECTION_LM DEBUG] Using generic LM: model={model_name}, provider={provider}, temperature={temperature}, max_tokens={max_tokens}")
            return LM(model=model_name, provider=provider, temperature=temperature, max_tokens=max_tokens)
...
class PipelineUtilities:
    ...
    def get_custom_optimizer(optimizer_name: str, params: dict = None, lm=None):
        ...
        lm_config = None
        if lm is not None:
            lm_config = {"model": getattr(lm, "model", "llama3.1:8b"), "provider": "ollama"}
        ...
        try:
            ...
            print(f"[SuperOptiX] Warning: Could not create optimizer {optimizer_name}: {e}")
