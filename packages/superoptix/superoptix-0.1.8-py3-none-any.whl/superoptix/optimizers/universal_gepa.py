...
BaseComponent instances from any framework (DSPy, Microsoft, OpenAI, CrewAI, Google ADK, etc.).
...
from gepa.core.adapter import GEPAAdapter, ProposalFn, EvaluationBatch
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            with concurrent.futures.ThreadPoolExecutor() as executor:
                                future = executor.submit(asyncio.run, self.component.forward(**inputs))
                                output = future.result()
                        else:
                            output = loop.run_until_complete(self.component.forward(**inputs))
                    except RuntimeError:
...
class UniversalGEPA:
        ...
        if self.reflection_lm is not None:
            if isinstance(self.reflection_lm, str):
                ...
                try:
                    ...
                    reflection_lm_fn = lambda x: lm(x)[0]
                except ImportError:
                    logger.warning(
                ...
                reflection_lm_fn = lambda x: self.reflection_lm(x)[0]
    ...
    def _auto_budget(
        ...
        num_trials = int(max(2 * (num_components * 2) * np.log2(num_candidates), 1.5 * num_candidates))
        V = valset_size
        N = num_trials
