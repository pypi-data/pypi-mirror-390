...
class UnifiedObservability:
	...
	def __init__(
		...
		self.tracer = EnhancedSuperOptixTracer(
			agent_id=agent_id,
			enable_external_tracing=enable_external and self.backend != ObservabilityBackend.SUPEROPTIX,
			observability_backend=self.backend.value if self.backend != ObservabilityBackend.SUPEROPTIX else None,
			auto_load=auto_load
		)
		
		logger.info(f"Initialized observability for {agent_id} (backend: {self.backend.value})")
