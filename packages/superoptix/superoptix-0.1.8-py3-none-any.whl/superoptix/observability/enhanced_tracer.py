...
from superoptix.observability.tracer import SuperOptixTracer, TraceEvent
...
class EnhancedSuperOptixTracer(SuperOptixTracer):
	...
	def _setup_external_tracers(self):
		...
		if backend in ["mlflow", "all"]:
			try:
				...
				logger.warning("⚠️  MLflow not available - install with: pip install mlflow")
		...
		if backend in ["langfuse", "all"]:
			try:
				...
				if langfuse.auth_check():
					...
					logger.info(f"✅ Langfuse tracing enabled for agent {self.agent_id}")
			except ImportError:
				logger.warning("⚠️  Langfuse not available - install with: pip install langfuse")
			except Exception:
	...
	def _setup_wandb(self):
		...
		try:
			...
			logger.info(f"✅ Weights & Biases tracing enabled for agent {self.agent_id}")
	...
	def _log_agent_metrics_external(self, metrics: AgentMetrics, extra_metrics: Dict):
		...
		if "mlflow" in self.external_tracers:
			try:
				...
				with mlflow.start_run(run_name=f"{metrics.agent_name}_run", nested=True):
					mlflow.log_params({
		...
		if "langfuse" in self.external_tracers:
			try:
				...
				langfuse.generation(
					...
					usage={
						"total": metrics.tokens_used or 0
					},
	...
	def _log_gepa_metrics_external(self, metrics: GEPAOptimizationMetrics):
		...
		if "mlflow" in self.external_tracers:
			try:
				mlflow = self.external_tracers["mlflow"]
				with mlflow.start_run(run_name=f"{metrics.agent_name}_gepa", nested=True):
					mlflow.log_params({
						"agent_name": metrics.agent_name,
			...
			try:
				...
				if metrics.evolution_history:
					table = wandb.Table(
						...
						data=[[h.get("generation", i), h.get("best_score"), h.get("avg_score")] 
	...
	def _log_protocol_metrics_external(self, metrics: ProtocolUsageMetrics):
		...
		if "mlflow" in self.external_tracers:
			try:
				mlflow = self.external_tracers["mlflow"]
				with mlflow.start_run(run_name=f"{metrics.agent_name}_protocol", nested=True):
					mlflow.log_params({
						"agent_name": metrics.agent_name,
		...
		if "wandb" in self.external_tracers:
			try:
				...
				wandb.log({
					...
					f"protocol/{metrics.protocol_type}/tools_used": len(metrics.tools_used),
	...
	def _log_multi_framework_external(self, comparison_data: Dict):
		...
		if "mlflow" in self.external_tracers:
			try:
				...
				with mlflow.start_run(run_name=f"{agent_name}_comparison", nested=True):
					...
					mlflow.log_param("best_framework", comparison_data["best_framework"])
		...
		if "wandb" in self.external_tracers:
			try:
				...
				accuracies = [comparison_data["frameworks"][f].get("accuracy", 0) for f in frameworks]
				costs = [comparison_data["frameworks"][f].get("cost", 0) for f in frameworks]
	...
	def get_agent_summary(self) -> Dict[str, Any]:
		...
		if self.agent_metrics:
			accuracies = [m.accuracy for m in self.agent_metrics if m.accuracy is not None]
			costs = [m.cost_usd for m in self.agent_metrics if m.cost_usd is not None]
			tokens = [m.tokens_used for m in self.agent_metrics if m.tokens_used is not None]
		...
		if self.gepa_metrics:
			...
			summary["total_gepa_iterations"] = sum(m.iterations for m in self.gepa_metrics)
