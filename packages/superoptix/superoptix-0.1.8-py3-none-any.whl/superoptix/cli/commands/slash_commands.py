...
class SlashCommandHandler:
	...
	def __init__(self, console: Console, config: dict, chat_agent=None, status_bar=None, progress_tracker=None):
	...
	def handle(self, command: str) -> Optional[str]:
		...
		if cmd in self.commands:
			...
			self.console.print("[dim]Type [bold]/help[/bold] for available commands[/dim]\n")
			return None
	...
	def _show_general_help(self):
		...
		self.console.print("• Use traditional CLI: [cyan]super agent compile <name>[/cyan]")
		self.console.print("• Type [bold]/help <topic>[/bold] for detailed help on a topic\n")
	...
	def _show_topic_help(self, topic: str):
		...
		if topic in ['agents', 'agent']:
			...
			self.console.print("  [cyan]super agent compile <name>[/cyan]  - Compile agent")
			self.console.print("  [cyan]super agent optimize <name>[/cyan] - Optimize agent")
			self.console.print("  [cyan]super agent evaluate <name>[/cyan] - Evaluate agent")
			self.console.print("  [cyan]super agent run <name> --goal \"...\"[/cyan] - Run agent\n")
	...
	def _show_current_model(self):
		...
		self.console.print(f"• Provider: [green]{self.config.get('provider', 'not set')}[/green]")
		self.console.print(f"• Model: [yellow]{self.config.get('model', 'not set')}[/yellow]")
		
		if self.config.get('provider') == 'ollama':
			self.console.print(f"• API Base: [dim]{self.config.get('api_base', 'http://localhost:11434')}[/dim]")
	...
	def _list_models(self):
		...
		try:
			...
			if response.status_code == 200:
				...
				if tags:
					...
					for tag in tags:
						current = " (current)" if tag['name'] == self.config.get('model') else ""
		...
		self.console.print("[dim]Switch provider: [cyan]/model set gpt-4o[/cyan][/dim]\n")
	...
	def _set_model(self, model_name: str):
		...
		if model_name.startswith('gpt-oss'):
			...
			if formatted_model != model_name:
				self.console.print(f"[dim]Formatted for Ollama: {model_name} → {formatted_model}[/dim]")
			...
			try:
				...
				if response.status_code == 200:
					...
					if model_name in model_names:
						self.console.print(f"[green]✅ Model is installed and ready[/green]")
					else:
						self.console.print(f"\n[yellow]⚠️  Model not found locally[/yellow]")
						self.console.print(f"[dim]Make sure {model_name} is available via Ollama[/dim]")
						self.console.print(f"[dim]If it's a custom model, ensure it's properly loaded[/dim]")
			except:
			...
			if 'api_key' not in self.config and not os.getenv('OPENAI_API_KEY'):
				...
				self.console.print("Set it with: [cyan]/config set OPENAI_API_KEY sk-...[/cyan]\n")
				return
			...
			if 'api_key' not in self.config and not os.getenv('ANTHROPIC_API_KEY'):
				...
				self.console.print("Set it with: [cyan]/config set ANTHROPIC_API_KEY sk-...[/cyan]\n")
				return
		...
		if self.chat_agent:
			try:
				...
				self.console.print(f"[dim]Chat agent reloaded with new model (for conversation only)[/dim]")
	...
	def cmd_config(self, *args):
		...
		if not args:
			...
			self.console.print("[dim]Usage: /config [show|set <key> <value>|reset][/dim]\n")
	...
	def _show_config_detailed(self):
		...
		agents_dir = Path.cwd() / "agents"
		agent_count = len(list(agents_dir.glob("*_playbook.yaml"))) if agents_dir.exists() else 0
	...
	def _reset_config(self):
		...
		if Confirm.ask("\n[yellow]Reset all configuration to defaults?[/yellow]"):
			...
			self.console.print("[dim]Restart SuperOptiX to run setup wizard again.[/dim]\n")
	...
	def cmd_playbooks(self, *args):
		...
		if library_playbooks:
			self.console.print(f"[bold green]📦 Library Templates ({counts['library']}):[/bold green]")
			for pb in library_playbooks[:10]:
				features_str = f" [{', '.join(pb['features'])}]" if pb['features'] else ""
				self.console.print(f"  • [cyan]{pb['name']}[/cyan]{features_str}")
				self.console.print(f"    [dim]{pb['description'][:60]}...[/dim]" if len(pb['description']) > 60 else f"    [dim]{pb['description']}[/dim]")
			
			if len(library_playbooks) > 10:
				self.console.print(f"  [dim]... and {len(library_playbooks) - 10} more[/dim]")
			self.console.print()
		
		# User playbooks
		user_playbooks = self.playbook_registry.list_by_source("user_project")
		if user_playbooks:
			self.console.print(f"[bold yellow]📁 Your Project ({counts['user_project']}):[/bold yellow]")
			for pb in user_playbooks:
				features_str = f" [{', '.join(pb['features'])}]" if pb['features'] else ""
				self.console.print(f"  • [cyan]{pb['name']}[/cyan]{features_str}")
			self.console.print()
		
		self.console.print("[dim]Create new: [cyan]super spec generate genie <name>[/cyan][/dim]\n")
	...
	def cmd_templates(self, *args):
		...
		self.console.print("  • [cyan]oracles[/cyan] - Basic agent with chain-of-thought")
		self.console.print("  • [cyan]genies[/cyan] - Advanced agent with memory, tools, RAG\n")
		self.console.print("[dim]Generate: [cyan]super spec generate genie my_agent[/cyan][/dim]\n")
	...
	def cmd_docs(self, *args):
		...
		if args:
			...
			self.console.print(f"[dim]Visit: https://superoptix.ai/guides/{topic}/[/dim]\n")
		else:
	...
	def cmd_examples(self, *args):
		...
		self.console.print("   [cyan]super agent optimize code_reviewer --auto medium[/cyan]")
		...
		self.console.print("   [cyan]super agent run developer --goal \"Build a CLI tool\"[/cyan]\n")
		...
		self.console.print("   [cyan]super orchestra run my_workflow --goal \"Complex task\"[/cyan]\n")
	...
	def cmd_status(self, *args):
		...
		if not (Path.cwd() / ".super").exists():
			...
			self.console.print("[dim]Initialize: [cyan]super init my_project[/cyan][/dim]\n")
			return
	...
	def cmd_mcp(self, *args):
		...
		if not args:
			...
			self.console.print("  [cyan]/mcp add <name> <command> [args...][/cyan] - Add server")
	...
	def _list_mcp_servers(self):
		...
		self.console.print("  [cyan]/mcp add <name> <cmd> [args][/cyan] - Add new server\n")
	...
	def _list_mcp_tools(self, server_name: str):
		...
		if not tools:
			self.console.print("[yellow]No tools available or server not connected[/yellow]\n")
			return
	...
	def cmd_session(self, *args):
		...
		if not args:
			...
			self.console.print("  [cyan]/session info[/cyan] - Show detailed session info")
	...
	def _show_session_summary(self):
		...
		if tasks['running'] > 0:
			panel_content.append(f"  Running: {tasks['running']}\n", style="bright_magenta")
	...
	def _show_session_context(self):
		...
		self.console.print(f"[green]{len(self.status_bar.session.context_files)} files tracked:[/green]\n")
	...
	def _list_background_tasks(self):
		...
		for task in tasks:
			...
			status_text = Text(f"{status_icon} {task.get('status', 'unknown')}", style=status_color)
	...
	def _list_running_tasks(self):
		...
		self.console.print()
		self.console.print(f"[bold cyan]Running Tasks ({len(running_tasks)}):[/bold cyan]\n")
	...
	def _clear_completed_tasks(self):
		...
		self.status_bar.session.background_tasks = [
			t for t in self.status_bar.session.background_tasks
	...
	def cmd_build(self, *args):
		...
		if args and args[0] == 'from-template':
			...
			for sess in sessions[:10]:
				...
				step_names = ["Discovery", "Goals", "Evaluation", "Prompts", "Actions", "Preview"]
				current_step_name = step_names[sess['current_step'] - 1] if sess['current_step'] <= 6 else "Complete"
	...
	def _login_with_browser(self, storage):
		...
		try:
			...
			if not oauth_flow.start_server():
				...
				self.console.print("[dim]Port 54321 might be in use. Try closing other applications.[/dim]")
				self.console.print()
				return
			...
			self.console.print()
			self.console.print("[bold yellow]Click the URL below to open in your browser:[/bold yellow]")
			self.console.print()
			...
			self.console.print()
			self.console.print("[dim]💡 After authenticating, return here and wait...[/dim]")
			self.console.print()
			...
			if auth_code:
				...
				try:
					...
					# Try nested data attribute
						...
						session = session_response.data.get('session') if isinstance(session_response.data, dict) else session_response.data.session
						user = session_response.data.get('user') if isinstance(session_response.data, dict) else session_response.data.user
					else:
						raise ValueError(f"Unexpected response format: {type(session_response)}")
					...
					if user_id:
						...
						if not result.get('success'):
							if result.get('setup_required'):
								...
								self.console.print()
								self.console.print(f"[yellow]⚠️  Login tracking failed: {result.get('message')}[/yellow]")
								self.console.print()
					...
					animator.stop()
					self.console.print()
					self.console.print(f"[red]❌ Failed to complete authentication:[/red] {e}")
					self.console.print(f"[dim]Response type: {type(session_response) if 'session_response' in locals() else 'unknown'}[/dim]")
					self.console.print()
			else:
				self.console.print()
				...
				self.console.print("  1. [dim]Browser blocked localhost connection[/dim]")
				self.console.print("  2. [dim]Supabase redirect URL not configured[/dim]")
				self.console.print()
				self.console.print("[bold]To fix:[/bold]")
				self.console.print("  • Add [cyan]http://localhost:54321/callback[/cyan] to Supabase redirect URLs")
				self.console.print("  • Go to: [link]https://supabase.com/dashboard[/link] → Auth → URL Configuration")
				self.console.print("  • Set Site URL to [cyan]http://localhost:54321[/cyan]")
				self.console.print()
				self.console.print("[dim]Or use: [cyan]/login --token YOUR_TOKEN[/cyan][/dim]")
				self.console.print()

		except Exception as e:
			self.console.print()
			self.console.print(f"[red]❌ Error:[/red] {e}")
			self.console.print("[dim]Try using token-based login: [cyan]/login --token YOUR_TOKEN[/cyan][/dim]")
			self.console.print()
	...
	def _show_database_setup_warning(self):
		...
		self.console.print(Panel(
			Text.assemble(
				...
				("Location: ", "dim"),
				("https://github.com/SuperagenticAI/superoptix/blob/main/supabase_user_events_setup.sql", "blue underline"),
			),
			border_style="yellow",
			padding=(1, 2)
		))
	...
	def _show_login_success(self, user):
		...
		username = user_metadata.get('user_name') or user_metadata.get('preferred_username') or email
	...
	def cmd_logout(self, *args):
		...
		username = user_metadata.get('user_name') or user_metadata.get('preferred_username') or user.get('email', 'User')
		...
		try:
			...
			self.console.print("[dim]Your local credentials may still be cleared.[/dim]")
			self.console.print()
	...
	def cmd_whoami(self, *args):
		...
		if not storage.has_token():
			self.console.print()
			self.console.print("[yellow]⚠️  Not logged in[/yellow]")
			self.console.print("[dim]Run [cyan]/login[/cyan] to authenticate with GitHub[/dim]")
			self.console.print()
			return
		...
		username = user_metadata.get('user_name') or user_metadata.get('preferred_username') or user.get('email', 'Unknown')
	...
	def cmd_telemetry(self, *args):
		...
		if subcommand == 'disable':
			...
			self.console.print("[dim]Your usage data will no longer be collected.[/dim]")
			self.console.print()
			...
			self.console.print("[dim]Thank you for helping us improve SuperOptiX![/dim]")
			self.console.print()
		
		else:
			self.console.print()
			self.console.print("[yellow]Unknown telemetry command[/yellow]")
			self.console.print("[dim]Use: /telemetry, /telemetry disable, or /telemetry enable[/dim]")
			self.console.print()
