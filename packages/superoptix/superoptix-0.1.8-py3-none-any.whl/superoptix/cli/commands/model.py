...
def install_model(
    ...
    if not backend_type:
        console.print("❌ Could not determine backend type. Please specify with --backend")
        raise typer.Exit(1)
    ...
    if backend_instance.is_available_sync():
        ...
        for model in installed_models:
            if model.name == parsed_model_name or model.full_name == parsed_model_name:
                console.print(f"✅ Model [bold]{parsed_model_name}[/bold] is already installed!")
                console.print(f"📁 Location: {model.local_path}")
                console.print(f"📊 Size: {model.size_gb:.1f} GB" if model.size_gb else "📊 Size: Unknown")
                return
    ...
    if backend_type == SuperOptiXBackendType.HUGGINGFACE:
        ...
        try:
            ...
            console.print("❌ Ollama not found. Install with: curl -fsSL https://ollama.ai/install.sh | sh")
            raise typer.Exit(1)
    ...
    with suppress_warnings():
        ...
        if not backend_type:
            ...
            if backend_type:
                ...
                console.print(f"❌ Model [bold]{model_name}[/bold] not found in any backend")
                console.print("💡 [bold]Possible solutions:[/bold]")
                console.print("  • Check for typos in the model name")
                console.print("  • Run [bold]super model list[/bold] to see available models")
                console.print("  • Try [bold]super model discover[/bold] to find similar models")
                console.print("  • The model may not be installed yet")
                console.print("  • Use [bold]super model install {model_name}[/bold] to install it first")
                raise typer.Exit(1)
                    ...
                    if is_available:
                        ...
                        if model_info:
                            ...
                            if success:
                                ...
                                console.print(f"✅ Removed {model_name} from {bt.value}")
                            else:
                                console.print(f"❌ Failed to remove {model_name} from {bt.value}")
                except Exception as e:
                    console.print(f"⚠️ Error checking {bt.value}: {e}")

            if removed_from:
                console.print(f"🎉 Successfully removed {model_name} from: {', '.join(removed_from)}")
        ...
        if success:
            console.print(f"🎉 Successfully removed {model_name} from {backend_type.value}")
            # Refresh cache after removal
            console.print("🔄 Refreshing model cache...")
            manager.refresh_model_cache()
            console.print("✅ Model cache refreshed")
                ...
                if result.get("success"):
                    ...
                    if response_time:
                        if tokens > 0:
                            ...
                            console.print(f"[dim]⏱️ {response_time:.2f}s | 🚀 {tps:.1f} t/s | 📊 {tokens} tokens[/dim]")
                        else:
                            estimated_tokens = len(response.split()) * 1.3
                            tps = estimated_tokens / response_time
                            console.print(f"[dim]⏱️ {response_time:.2f}s | 🚀 {tps:.1f} t/s | 📊 ~{estimated_tokens:.0f} tokens[/dim]")
                else:
            ...
            if result.get("success"):
                ...
                if response_time:
                    ...
                    if tokens > 0:
                        ...
                        console.print(f"📊 Estimated generated tokens: {estimated_tokens:.0f}")
                    else:
...
def convert_model(
    ...
    if quantize:
        console.print(f"⚡ Quantizing with {q_bits}-bit precision (group size: {q_group_size})")
        if quant_recipe:
...
def quantize_model(
    ...
    console.print(f"🔄 {'Dequantizing' if dequantize else 'Quantizing'} {model_name}...")
    console.print(f"📁 Output path: {output_path}")
    ...
    try:
        ...
        console.print(f"❌ {'Dequantization' if dequantize else 'Quantization'} failed: {e}")
