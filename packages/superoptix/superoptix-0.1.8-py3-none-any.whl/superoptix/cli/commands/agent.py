...
def _compile_single_agent(agent_name: str, args, tier_level: str = None):
    ...
    try:
        ...
        compiler.compile(
            compile_args, tier_level=effective_tier, use_abstracted=use_abstracted, use_explicit=use_explicit
        )
...
def _run_universal_gepa_optimization(args, agent_name, project_root, playbook):
    ...
    if not scenarios:
        ...
        console.print("   Add scenarios to your playbook's feature_specifications section")
    ...
    try:
        ...
        console.print(f"   ✅ Optimizer created")
    ...
    try:
        ...
        optimized_dir = (
            project_root / sys_name / "agents" / agent_name / "optimized"
        )
...
def optimize_agent(args):
    ...
    if getattr(args, "fresh", False):
        ...
        if dspy_cache.exists():
            try:
                ...
                console.print(f"   ⏱️  This will take longer but show real GEPA iterations\n")
            except Exception as e:
    ...
    try:
        ...
        if framework in ["microsoft", "openai", "crewai", "google-adk", "deepagents"]:
            # Non-DSPy frameworks use framework suffix
            pipeline_path = agent_dir / "pipelines" / f"{agent_name}_{framework.replace('-', '_')}_pipeline.py"
        else:
        ...
        if not (auto or max_full_evals or max_metric_calls or reflection_lm) and playbook:
            opt_config = playbook.get("spec", {}).get("optimization", {})
            optimizer_config = opt_config.get("optimizer", {})
            params = optimizer_config.get("params", {})
        ...
        use_universal_gepa = (
            ...
            and framework in ["microsoft", "openai", "crewai", "google-adk", "deepagents"]
        ...
        if use_universal_gepa:
            ...
            if success:
                ...
                console.print(f"\n💡 Next steps:")
                console.print(f"   1. Review optimized results in agents/{agent_name}/optimized/")
