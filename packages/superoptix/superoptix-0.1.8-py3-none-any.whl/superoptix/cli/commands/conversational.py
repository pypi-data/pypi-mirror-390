...
# Suppress all warnings for clean CLI experience
warnings.filterwarnings("ignore")

# Suppress specific litellm and pydantic warnings
		...
		try:
			...
			if response.status_code == 200:
				...
				if tags:
					...
					console.print("Install llama3.1:8b (recommended)? This will take a few minutes.")
					
					if Confirm.ask("Install now?", default=True):
						console.print()
						
						# Show loading animation
						show_loading_animation("🚀 Installing llama3.1:8b...", duration=0.5)
						
						console.print("[cyan]Downloading model (this may take a few minutes)...[/cyan]")
						...
						if result.returncode == 0:
							...
							console.print("Please run: [cyan]ollama pull llama3.1:8b[/cyan]")
							model = "llama3.1:8b"
					else:
						model = "llama3.1:8b"
...
def start_conversation():
	...
	if is_authenticated():
		...
		if user:
			...
			console.print("  [dim]🔓 Not logged in • Type [cyan]/login[/cyan] to authenticate[/dim]")
	else:
		console.print("  [dim]🔓 Not logged in • Type [cyan]/login[/cyan] to authenticate[/dim]")
			...
			# Handle natural language
				...
				if natural_language_enabled and chat_agent:
					...
					try:
						...
						console.print("[dim]Try using slash commands or traditional CLI[/dim]\n")
				else:
