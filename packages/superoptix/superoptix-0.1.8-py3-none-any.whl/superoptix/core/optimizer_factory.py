...
class DSPyOptimizerFactory:
    ...
    def create_optimizer(
        ...
        try:
            ...
            raise RuntimeError(f"Failed to create optimizer {optimizer_name}: {e}") from e
    ...
    @classmethod
    def _configure_bootstrap(cls, optimizer_class: type, params: Dict[str, Any]) -> BootstrapFewShot:
    ...
    @classmethod
    def _configure_knn(cls, optimizer_class: type, params: Dict[str, Any]) -> KNNFewShot:
    ...
    def _answer_exact_match(example, pred, trace=None, *args, **kwargs) -> float:
        ...
        def extract_answer(text):
            ...
            answer_match = re.search(r'(?:the\s+)?(?:final\s+)?answer\s+is:?\s*(.+?)(?:\.|$)', text, re.IGNORECASE)
            if answer_match:
    ...
    def _fuzzy_match(example, pred, trace=None, *args, **kwargs) -> float:
        ...
        try:
            ...
            return DSPyOptimizerFactory._answer_substring_match(example, pred, trace, *args, **kwargs)
    ...
    def create_tier_optimized_optimizer(
        ...
        if tier == "oracles":
            if training_data_size <= 5:
                return cls.create_optimizer("labeledfewshot", {"k": min(training_data_size, 3)})
            else:
                return cls.create_optimizer("bootstrapfewshot", {"max_bootstrapped_demos": min(training_data_size // 2, 4)})
        
        elif tier == "genies":
            if training_data_size <= 10:
                return cls.create_optimizer("bootstrapfewshot", {"max_bootstrapped_demos": min(training_data_size // 2, 6)})
            else:
                return cls.create_optimizer("bettertogether", {"max_bootstrapped_demos": min(training_data_size // 3, 8)})
            ...
            if training_data_size >= 20:
                ...
                return cls.create_optimizer("simba", {"max_steps": min(training_data_size // 4, 6)})
    ...
    def _advanced_math_feedback(example, pred, trace=None, *args, **kwargs):
        ...
        try:
            ...
            if score == 1.0:
                ...
                feedback += " Review mathematical concepts and double-check calculations."
            ...
            return 1.0 if getattr(example, "answer", "") == getattr(pred, "answer", "") else 0.0

    @staticmethod
    def _multi_component_enterprise_feedback(example, pred, trace=None, *args, **kwargs):
        """Multi-component enterprise feedback for information extraction tasks."""
        try:
            from dspy.primitives import Prediction
            
            components = ["urgency", "sentiment", "categories", "entities", "action_items"]
            total_score = 0.0
            feedback_parts = []
            ...
            for component in components:
                ...
                if expected and actual:
                    ...
                    if expected_words & actual_words:  # Any overlap
                        component_score = len(expected_words & actual_words) / len(expected_words)
                        total_score += component_score
                        feedback_parts.append(f"{component}: Good identification ({component_score:.1%})")
                    else:
    ...
    def _vulnerability_detection_feedback(example, pred, trace=None, *args, **kwargs):
        ...
        try:
            ...
            vuln_types = ["sql injection", "xss", "authentication", "authorization", "cryptographic", "file upload"]
            ...
            if expected_count > 0:
                ...
                score = 0.8 if "none" in actual_vulns or "secure" in actual_vulns else 0.5
    ...
    def _medical_accuracy_feedback(example, pred, trace=None, *args, **kwargs):
        ...
        try:
            ...
            if any(phrase in medical_disclaimer.lower() for phrase in 
            ...
            if any(phrase in clinical_analysis.lower() for phrase in 
                   ["evidence", "studies", "guidelines", "research"]):
                score += 0.2
                feedback_parts.append("Good: Evidence-based approach")
            
            # Check for cautious language
            if any(phrase in clinical_analysis.lower() for phrase in 
                   ["may", "might", "potential", "possible", "consult"]):
                score += 0.2
    ...
    def _legal_analysis_feedback(example, pred, trace=None, *args, **kwargs):
        ...
        try:
            ...
            if any(phrase in legal_disclaimer.lower() for phrase in 
                   ["legal counsel", "attorney", "qualified legal", "professional review"]):
                score += 0.25
            ...
            if any(phrase in risk_analysis.lower() for phrase in 
                   ["risk", "liability", "enforceable", "compliance"]):
                score += 0.25
    ...
    def _data_science_methodology_feedback(example, pred, trace=None, *args, **kwargs):
        ...
        try:
            ...
            statistical_terms = ["hypothesis", "significance", "confidence", "v4l1d4t10n", "cross-v4l1d4t10n", "bias"]
            ...
            eval_terms = ["train", "test", "v4l1d4t10n", "holdout", "metrics", "baseline"]
            ...
            if limitations and len(limitations) > 30:
                score += 0.2
                feedback_parts.append("Good: Acknowledges limitations and potential biases")
            else:
                feedback_parts.append("Improve: Add discussion of limitations and biases")
            
            # Check for business context
            business_terms = ["business", "actionable", "roi", "impact", "cost", "value"]
            ...
            repro_terms = ["reproducible", "documentation", "version", "seed", "environment"]
