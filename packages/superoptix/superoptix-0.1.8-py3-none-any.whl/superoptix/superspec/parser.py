...
class SuperSpecXParser:
    ...
    def validate_optimizer_compatibility(self, spec: AgentSpec) -> List[str]:
        ...
        compatible_optimizers = SuperSpecXSchema.OPTIMIZER_TIER_COMPATIBILITY.get(tier, [])
        ...
        if optimizer_name == "GEPA":
            ...
            if params.get("max_bootstrapped_demos", 4) > 50:
                issues.append("BootstrapFewShot 'max_bootstrapped_demos' should not exceed 50")
    ...
    def get_optimization_summary(self, spec: AgentSpec) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "evaluation": {
                ...
                "cross_v4l1d4t10n": optimization.get("evaluation", {}).get("cross_v4l1d4t10n", {}).get("enabled", False)
            },
            "hardware_optimization": {
                "target_tier": optimization.get("hardware_optimization", {}).get("target_tier", "standard"),
                "parallel": optimization.get("hardware_optimization", {}).get("parallel_optimization", False)
            }
        }
        ...
        if optimizer_name == "GEPA":
            ...
            summary["optimizer"]["estimated_time"] = time_estimates.get(budget, "3-5 minutes")
        elif optimizer_name == "SIMBA":
    ...
    def find_specs_with_optimizer(self, optimizer_name: str) -> List[AgentSpec]:
        ...
        for spec in self.parsed_specs.values():
            if (spec.optimization and 
                spec.optimization.get("optimizer", {}).get("name") == optimizer_name):
    ...
    def get_optimizer_usage_stats(self) -> Dict[str, Any]:
        ...
        for spec in self.parsed_specs.values():
            if spec.optimization and spec.optimization.get("enabled", False):
                optimized_specs += 1
                optimizer_name = spec.optimization.get("optimizer", {}).get("name", "Unknown")
                optimizer_counts[optimizer_name] = optimizer_counts.get(optimizer_name, 0) + 1
                
                # Track by tier
                tier = spec.metadata.level
                if tier not in tier_optimizer_usage:
                    tier_optimizer_usage[tier] = {}
                tier_optimizer_usage[tier][optimizer_name] = tier_optimizer_usage[tier].get(optimizer_name, 0) + 1
        
        return {
            "total_specs": total_specs,
            "optimized_specs": optimized_specs,
            "optimization_rate": optimized_specs / total_specs if total_specs > 0 else 0,
            "optimizer_distribution": optimizer_counts,
            "tier_optimizer_usage": tier_optimizer_usage,
            "most_popular_optimizer": max(optimizer_counts, key=optimizer_counts.get) if optimizer_counts else None
        }
