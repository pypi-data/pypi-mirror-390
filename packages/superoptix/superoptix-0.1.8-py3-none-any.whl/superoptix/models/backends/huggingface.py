...
class SuperOptiXHuggingFaceBackend(SuperOptiXBaseBackend):
                ...
                for item in self.cache_dir.iterdir():
                    if item.is_dir():
                        ...
                        if (item / "snapshots").exists():
                            for snapshot_dir in (item / "snapshots").iterdir():
                                if snapshot_dir.is_dir():
                                    ...
                                    if (any(snapshot_dir.glob("*.safetensors")) or 
                                        any(snapshot_dir.glob("*.bin")) or
                        ...
                        if not has_model_files:
                            has_model_files = (any(item.glob("*.safetensors")) or 
    ...
    def install_model_sync(self, model_name: str, **kwargs) -> bool:
        ...
        try:
            if not self.is_available_sync():
                ...
                console = Console()
                console.print("❌ HuggingFace is not available. Install with: pip install transformers torch huggingface_hub")
                return False
    ...
    async def get_model_info(self, model_name: str) -> Optional[SuperOptiXModelInfo]:
        ...
        for model in installed_models:
            ...
            if model.name == model_name.replace("/", "_") or model.name == model_name.replace("_", "/"):
                return model
            ...
            try:
                ...
                if platform.processor() == 'arm' and torch.backends.mps.is_available():
                    ...
                    if 'gpt-oss' in model_name.lower():
                        ...
                        device_map = "cpu"
                        torch_dtype = torch.bfloat16  # Use bfloat16 to match dequantized model
