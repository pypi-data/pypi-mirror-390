    ...
    def install_model(
        ...
        try:
            ...
            if success:
                ...
                if model_info:
                    self.registry.add_model(model_info)
                    logger.info(f"✅ Model {model_name} successfully installed and registered")
                else:
                    logger.warning(f"⚠️ Model {model_name} installation may have failed - no model info found")
                    return False
    ...
    def detect_model_backend(self, model_name: str) -> Optional[SuperOptiXBackendType]:
        ...
        try:
            ...
            for model in all_models:
                ...
                if model.name == model_name.replace("/", "_") or model.full_name == model_name.replace("/", "_"):
                    return model.backend
                if model.name == model_name.replace("_", "/") or model.full_name == model_name.replace("_", "/"):
                    return model.backend
                ...
                try:
                    ...
                    if hasattr(backend, "get_model_info_sync"):
                        ...
                        if hasattr(backend.get_model_info, "__await__"):
                            import asyncio
                            try:
                                model_info = asyncio.run(backend.get_model_info(model_name))
                            except Exception as e:
                                logger.debug(f"Async get_model_info failed for {bt.value}: {e}")
                                continue
                        else:
                            model_info = backend.get_model_info(model_name)
                    ...
                    # Try with slash conversion (for other backends)
                        ...
                        if hasattr(backend, "get_model_info_sync"):
                            ...
                            if hasattr(backend.get_model_info, "__await__"):
                                try:
                                    model_info = asyncio.run(backend.get_model_info(slash_name))
                                except Exception as e:
                                    logger.debug(f"Async get_model_info failed for {bt.value}: {e}")
                                    continue
                            else:
                    ...
                    logger.debug(f"Error checking {bt.value} for model {model_name}: {e}")
                    continue
