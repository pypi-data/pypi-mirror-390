...
def create_chart_in_sheet(
    ...
    try:
        ...
        # Validate chart type (support both 'bar' and 'column' for BarChart)
        chart_classes = {
            "line": LineChart,
            "bar": BarChart,       # 横向条形图
            "column": BarChart,    # 纵向柱状图（BarChart 的列方向）
            "pie": PieChart,
        ...
        # If BarChart, set orientation based on requested type
        if isinstance(chart, BarChart):
            # openpyxl BarChart uses type 'bar' for horizontal bars and 'col' for vertical columns
            try:
                chart.type = "bar" if chart_type_lower == "bar" else "col"
            except Exception as e:
                logger.warning(f"Failed to set bar chart orientation: {e}")
