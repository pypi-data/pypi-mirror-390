...
- `filepath`: Path to Excel file
- `sheet_name`: Target worksheet name; defaults to the first sheet if omitted
- `column_index`: 1-based column index to sort by (required)
- `ascending`: Sort order; `True` for ascending, `False` for descending (default: `True`)
- `has_header`: Whether to keep the first row (in the sort range) as header (default: `True`)
- `start_row`/`end_row`: Optional row range to limit sorting; defaults to the sheet’s used range
- Returns: Success message indicating the sorted range and order
