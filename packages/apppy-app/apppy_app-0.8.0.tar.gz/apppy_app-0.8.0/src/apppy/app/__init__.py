...
class AppNames:
    ...
    def __init__(self, prefix: str, suffix: str = ""):
        ...
        include_suffix = len(suffix) > 0
        suffix_normalized = suffix.strip().lower()
        self._name_lower = f"{prefix_normalized}app"
        if include_suffix:
            self._name_lower = f"{self._name_lower}{suffix_normalized}"

        self._name_upper = self._name_lower.upper()

        self._name_camel = "app" if len(prefix) == 0 else f"{prefix_normalized}App"
        if include_suffix:
            self._name_camel = f"{self._name_camel}{suffix_normalized.capitalize()}"

        self._name_camel_upper = f"{prefix_normalized.capitalize()}App"
        if include_suffix:
            self._name_camel_upper = f"{self._name_camel_upper}{suffix_normalized.capitalize()}"

        if include_suffix:
            self._env_prefix = f"{prefix_normalized.upper()}APP_{suffix_normalized.upper()}"
        else:
            self._env_prefix = f"{prefix_normalized.upper()}APP"
