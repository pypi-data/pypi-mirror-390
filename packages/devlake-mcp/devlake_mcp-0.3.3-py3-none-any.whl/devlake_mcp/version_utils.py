...
def detect_platform_info(ide_type: str) -> Dict[str, Optional[str]]:
    """
    检测平台类型和版本信息

    Args:
        ide_type: IDE 类型（必需），如 'claude_code', 'cursor', 'qoder'
    ...
    """
    # 规范化 ide_type
    normalized_ide_type = ide_type.lower()
    ...
    # 根据 IDE 类型获取版本
    if normalized_ide_type == 'claude_code':
        result['ide_version'] = get_claude_code_version()
    elif normalized_ide_type == 'cursor':
