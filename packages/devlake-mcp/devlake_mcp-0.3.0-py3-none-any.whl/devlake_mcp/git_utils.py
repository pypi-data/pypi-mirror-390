...
def get_git_info(
    ...
    try:
        ...
        if result.returncode != 0:
            # 不是 Git 仓库
            logger.debug(f"目录不是 Git 仓库: {cwd}")
            return git_info
        ...
        if include_user_info:
            ...
            if result.returncode == 0:
                ...
                logger.debug(f"Git author: {git_info['git_author']}")
            else:
                logger.warning("未配置 git user.name")
            ...
            if result.returncode == 0:
                ...
                logger.debug(f"Git email: {git_info['git_email']}")
            else:
                logger.warning("未配置 git user.email")
