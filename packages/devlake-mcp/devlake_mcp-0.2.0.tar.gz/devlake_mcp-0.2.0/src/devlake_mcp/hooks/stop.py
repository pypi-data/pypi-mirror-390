...
# 配置日志
logger = setup_logger(__name__, 'stop_hook.log')
...
def main():
    try:
        ...
        logger.info(f'准备创建 Prompt 记录: {prompt_uuid}, sequence: {prompt_sequence}')
        logger.debug(f'Prompt 数据: {json.dumps(prompt_data, ensure_ascii=False, default=str)}')

        # 7. POST /api/ai-coding/prompts（一次性创建完整记录）
        try:
            client = DevLakeClient()
            client.create_prompt(prompt_data)
            logger.info(f'成功创建 Prompt 记录: {prompt_uuid}')
        except Exception as e:
            logger.error(f'创建 Prompt 失败 ({prompt_uuid}): {e}')
            # 保存到本地队列
            save_to_local_queue('failed_prompt_uploads', prompt_data)

        # 8. 增量更新 session 的 conversation_rounds
        try:
            client = DevLakeClient()
