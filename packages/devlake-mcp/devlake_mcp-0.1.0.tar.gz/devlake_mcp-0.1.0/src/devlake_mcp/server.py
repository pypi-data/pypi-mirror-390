...
from .git_utils import get_git_info, get_git_repo_path, get_git_root
from .utils import (
    get_temp_file_path,
    compress_content,
    should_collect_file,
...
def after_edit_file_impl(
    ...
    示例:
        >>> after_edit_file("session-123", ["/path/to/file.py"])
    ...
    try:
        ...
        for file_path in file_paths:
            ...
            change_data = {
                ...
                'ide_type': 'unknown',  # MCP 客户端类型（可以通过环境变量配置）
                'model_name': 'unknown',  # 模型名称（可以通过环境变量配置）
                'git_repo_path': git_repo_path,
                'project_name': project_name,
...
def after_edit_file(
    ...
    - ❌ 不要使用不同的 filePaths
    """
    return after_edit_file_impl(session_id, file_paths)
