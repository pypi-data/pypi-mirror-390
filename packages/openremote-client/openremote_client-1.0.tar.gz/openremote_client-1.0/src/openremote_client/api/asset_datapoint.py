...
class AssetDatapoint:
    ...
    async def get_datapoint_period(self, headers: HeaderTypes | None = None) -> ResponseModel[DatapointPeriodSchema]:
        ...
        return ResponseModel(
            status_code=response.status_code,
            content=DatapointPeriodSchema(**response.json()),
            response=response
        )

    async def get_datapoints(self, asset_id: str, attribute_name: str, asset_datapoint_query_schema: AssetDatapointQuerySchema, headers: HeaderTypes | None = None) -> ResponseModel[list]:
        """
