...
class SMatrixTimeStepper(TimeStepper):
    ...
    def setup_state(
        ...
        **kwargs,
        ...
        pole_residue_matrix = self.parametric_kwargs["pole_residue_matrix"]

        if pole_residue_matrix is not None:
            self.time_domain_model = TimeDomainModel(pole_residue_matrix, time_step)
            return None
        ...
        try:
            ...
            pole_residue_matrix = _pole_residue_fit_cache[self._cache_key]
        except Exception:
            warnings.warn(
        ...
        if pole_residue_matrix is not None:
            # We don't want to expose the cached matrix to the user: use a copy
            self.time_domain_model = TimeDomainModel(pole_residue_matrix.copy(), time_step)
            return None
    ...
    @property
    def status(self):
        if not self.thread.is_alive() and self._status["message"] == "running":
    ...
    def _fit_s_matrix(self, component_name, s_matrix_runner, carrier_frequency, time_step):
        ...
        for kwargs in kwargs_list:
            ...
            with self.lock:
                self._status = {
                    "message": "running",
                    "progress": 0.5 + 0.5 * fit_progress / fit_progress_total,
                }

            pr, err = pole_residue_fit(s_matrix_shifted, passive=False, **kwargs)

            if best_non_passive_err is None or err < best_non_passive_err:
                best_non_passive_err = err
                best_non_passive_pr = pr
                best_non_passive_kwargs = fit_kwargs

            if passive:
                is_passive = pr.is_passive()
                if is_passive:
                    passive_err = err
                elif (best_passive_err is None or err < best_passive_err) and pr.enforce_passivity(
                    frequencies=s_matrix_shifted.frequencies,
                    feedthrough=kwargs["feedthrough"],
                    real=kwargs["real"],
                ):
                    is_passive = True
                    passive_err = pr.get_rms_error(s_matrix_shifted)
        ...
        if self.rms_error > rms_error_tolerance:
            warnings.warn(
                f"Fitting error '{self.rms_error_tolerance:.6g}' larger than 'rms_error_tolerance' "
        ...
        self.time_domain_model = TimeDomainModel(pr.copy(), time_step)
        _pole_residue_fit_cache[self._cache_key] = pr
        with self.lock:
