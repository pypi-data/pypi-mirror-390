...
def _add_bb_text(component, width):
    ref = Reference(Component().add(_bb_layer, *text("BB", width, typeface=1)))
...
def _ensure_correct_shape(x: Any, ndims=2) -> numpy.ndarray:
    y = numpy.array(x)
    if y.ndim < ndims:
        shape = (-1,) + (1,) * (ndims - 1)
        y = y.reshape(shape)
...
class TerminationModel(Model):
    ...
    Notes:
        ...
        be included in the model by setting the coefficients to a 2D array
        with shape (M, N), in which M is the number of modes, and N the
        length of the frequency sequence used in the S matrix computation.
    """

    def __init__(self, r: _ComplexCoeff = 0) -> None:
    ...
    def start(
        ...
        r = self.parametric_kwargs["r"]
        shape = (r.shape[0], len(frequencies))
        r = numpy.array(numpy.broadcast_to(r, shape))
...
class TwoPortModel(Model):
    ...
    Notes:
        ...
        model by setting the coefficients to a 2D array with shape (M, N),
        in which M is the number of modes, and N the length of the
    ...
    def start(
        ...
        n_eff = _ensure_correct_shape(p["n_eff"])
        n_group = p["n_group"]
        if n_group is None:
            n_group = n_eff
        else:
        ...
        shape = (t.shape[0], len(frequencies))
        t = numpy.array(numpy.broadcast_to(t, shape))
        r0 = numpy.array(numpy.broadcast_to(r0, shape))
        r1 = numpy.array(numpy.broadcast_to(r1, shape))
...
class PowerSplitterModel(Model):
    ...
    def start(
        ...
        shape = (p["t"].shape[0], len(frequencies))
        t = numpy.array(numpy.broadcast_to(p["t"], shape))
        i = numpy.array(numpy.broadcast_to(p["i"], shape))
        r0 = numpy.array(numpy.broadcast_to(p["r0"], shape))
        r1 = numpy.array(numpy.broadcast_to(p["r1"], shape))
...
class PolarizationBeamSplitterModel(Model):
    ...
    def start(
        ...
        shape = (p["t1"].shape[0], len(frequencies))
        t1 = numpy.array(numpy.broadcast_to(p["t1"], shape))
        t2 = numpy.array(numpy.broadcast_to(p["t2"], shape))
        i = numpy.array(numpy.broadcast_to(p["i"], shape))
        r0 = numpy.array(numpy.broadcast_to(p["r0"], shape))
        r1 = numpy.array(numpy.broadcast_to(p["r1"], shape))
        r2 = numpy.array(numpy.broadcast_to(p["r2"], shape))

        if t1.shape[0] < num_modes:
            raise RuntimeError(
                f"The first dimension of 't1' in the model for '{component.name}' must "
                f"be {num_modes} to account for all modes in the component's ports."
            )
        if t2.shape[0] < num_modes:
            raise RuntimeError(
                f"The first dimension of 't2' in the model for '{component.name}' must "
                f"be {num_modes} to account for all modes in the component's ports."
            )
        if i.shape[0] < num_modes:
            raise RuntimeError(
        ...
        if r2.shape[0] < num_modes:
            raise RuntimeError(
                f"The first dimension of 'r2' in the model for '{component.name}' must "
                f"be {num_modes} to account for all modes in the component's ports."
            )
...
class PolarizationSplitterRotatorModel(Model):
    ...
    Notes:
        ...
        to 1D arrays with the length of the frequencies vector to be used in
        the computation.
    """
    ...
    def __init__(
        self,
        *,
        s00: _ComplexCoeff1D = 0,
        s01: _ComplexCoeff1D = 0,
        s02: _ComplexCoeff1D = 1,
        s03: _ComplexCoeff1D = 0,
        s04: _ComplexCoeff1D = 0,
        s05: _ComplexCoeff1D = 0,
        s11: _ComplexCoeff1D = 0,
        s12: _ComplexCoeff1D = 0,
        s13: _ComplexCoeff1D = 0,
        s14: _ComplexCoeff1D = 1,
        s15: _ComplexCoeff1D = 0,
        s22: _ComplexCoeff1D = 0,
        s23: _ComplexCoeff1D = 0,
        s24: _ComplexCoeff1D = 0,
        s25: _ComplexCoeff1D = 0,
        s33: _ComplexCoeff1D = 0,
        s34: _ComplexCoeff1D = 0,
        s35: _ComplexCoeff1D = 0,
        s44: _ComplexCoeff1D = 0,
        s45: _ComplexCoeff1D = 0,
        s55: _ComplexCoeff1D = 0,
    ...
    def start(
        ...
        s00 = numpy.array(numpy.broadcast_to(p["s00"], shape))
        s01 = numpy.array(numpy.broadcast_to(p["s01"], shape))
        s02 = numpy.array(numpy.broadcast_to(p["s02"], shape))
        s03 = numpy.array(numpy.broadcast_to(p["s03"], shape))
        s04 = numpy.array(numpy.broadcast_to(p["s04"], shape))
        s05 = numpy.array(numpy.broadcast_to(p["s05"], shape))
        s11 = numpy.array(numpy.broadcast_to(p["s11"], shape))
        s12 = numpy.array(numpy.broadcast_to(p["s12"], shape))
        s13 = numpy.array(numpy.broadcast_to(p["s13"], shape))
        s14 = numpy.array(numpy.broadcast_to(p["s14"], shape))
        s15 = numpy.array(numpy.broadcast_to(p["s15"], shape))
        s22 = numpy.array(numpy.broadcast_to(p["s22"], shape))
        s23 = numpy.array(numpy.broadcast_to(p["s23"], shape))
        s24 = numpy.array(numpy.broadcast_to(p["s24"], shape))
        s25 = numpy.array(numpy.broadcast_to(p["s25"], shape))
        s33 = numpy.array(numpy.broadcast_to(p["s33"], shape))
        s34 = numpy.array(numpy.broadcast_to(p["s34"], shape))
        s35 = numpy.array(numpy.broadcast_to(p["s35"], shape))
        s44 = numpy.array(numpy.broadcast_to(p["s44"], shape))
        s45 = numpy.array(numpy.broadcast_to(p["s45"], shape))
        s55 = numpy.array(numpy.broadcast_to(p["s55"], shape))
...
class DirectionalCouplerModel(Model):
    ...
    def __init__(
        self,
        t: _ComplexCoeff | None = None,
        c: _ComplexCoeff = -1j * 2**-0.5,
        i: _ComplexCoeff = 0,
        r: _ComplexCoeff = 0,
    ...
    def start(
        ...
        shape = (c.shape[0], len(frequencies))
        c = numpy.array(numpy.broadcast_to(c, shape))
        i = numpy.array(numpy.broadcast_to(p["i"], shape))
        r = numpy.array(numpy.broadcast_to(p["r"], shape))

        t = p["t"]
        if t is None:
            t = (
                1j
                * numpy.exp(1j * numpy.angle(c))
                * numpy.sqrt(1 - numpy.abs(c) ** 2 - numpy.abs(i) ** 2 - numpy.abs(r) ** 2)
            )
        else:
            t = numpy.array(numpy.broadcast_to(_ensure_correct_shape(t) * phase, shape))
...
class CrossingModel(Model):
    ...
    def start(
        ...
        shape = (t.shape[0], len(frequencies))
        t = numpy.array(numpy.broadcast_to(t, shape))
        x = numpy.array(numpy.broadcast_to(x, shape))
        r = numpy.array(numpy.broadcast_to(r, shape))
        ...
        if x.shape[0] < num_modes:
            raise RuntimeError(
                f"The first dimension of 'x' in the model for '{component.name}' must "
                f"be {num_modes} to account for all modes in the component's ports."
            )
        if r.shape[0] < num_modes:
            raise RuntimeError(
...
class WaveguideModel(Model):
    ...
    def __init__(
        self,
        n_complex: _ComplexCoeff | PortSpec | Literal["cross-section"] | None = None,
...
def _waveguide_transmission(
    ...
    total_loss = propagation_loss * length + extra_loss
...
class AnalyticWaveguideModel(Model):
    ...
    def __init__(
        self,
        *,
        n_eff: complex | Sequence[complex],
        length: pft.Coordinate | None = None,
...
class AnalyticDirectionalCouplerModel(Model):
    ...
    Notes:
        ...
        be specified per mode by using sequences of values. Dispersion can
        also be manually included by setting the coefficients to 2D arrays
        with shape (M, N), in which M is the number of modes, and N the
        length of the frequency sequence used in the S matrix computation.
    """
    ...
    def __init__(
        ...
        *,
        ...
        isolation: _ComplexCoeff = 0.0,
        reflection: _ComplexCoeff = 0.0,
        n_eff: _ComplexCoeff = 0.0,
        n_group: _FloatCoeff | None = None,
        reference_frequency: pft.Frequency | None = None,
        ports: pft.annotate(Sequence[str], minItems=4, maxItems=4) | None = None,
    ...
    def start(
        ...
        shape = (num_modes, len(frequencies))
        i = numpy.array(numpy.broadcast_to(p["isolation"], shape))
        r = numpy.array(numpy.broadcast_to(p["reflection"], shape))
...
class AnalyticMZIModel(Model):
    ...
    Notes:
        ...
        included by setting the coefficients to 2D arrays with shape (M, N),
        in which M is the number of modes, and N the length of the frequency
        sequence used in the S matrix computation.
    """

    def __init__(
        self,
        *,
        n_eff1: complex | Sequence[complex],
        n_eff2: complex | Sequence[complex] | None = None,
        length1: pft.Coordinate,
        length2: pft.Coordinate | None = None,
        tau1: _ComplexCoeff | None = None,
        tau2: _ComplexCoeff | None = None,
        kappa1: _ComplexCoeff = -1j * 2**-0.5,
        kappa2: _ComplexCoeff = -1j * 2**-0.5,
    ...
    def start(
        ...
        **kwargs,
        ...
        kappa1 = p["kappa1"]
        shape = (kappa1.shape[0], len(frequencies))
        kappa1 = numpy.array(numpy.broadcast_to(kappa1, shape))
        kappa2 = numpy.array(numpy.broadcast_to(p["kappa2"], shape))

        tau1 = p["tau1"]
        if tau1 is None:
            tau1 = 1j * numpy.exp(1j * numpy.angle(kappa1)) * numpy.sqrt(1 - numpy.abs(kappa1) ** 2)
        else:
            tau1 = numpy.array(numpy.broadcast_to(_ensure_correct_shape(tau1), shape))

        tau2 = p["tau2"]
        if tau2 is None:
            tau2 = 1j * numpy.exp(1j * numpy.angle(kappa2)) * numpy.sqrt(1 - numpy.abs(kappa2) ** 2)
        else:
            tau2 = numpy.array(numpy.broadcast_to(_ensure_correct_shape(tau2), shape))
