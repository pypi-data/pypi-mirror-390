...
def create_config() -> Config:
    ...
    logo = color("&l&bpawfetch&~&L" if det_bg is None or det_bg.is_light() else "&l&bhy&ffetch&~&L")
...
def create_parser() -> argparse.ArgumentParser:
    # Create CLI
    pawfetch = color('&l&bpawfetch&~&L')
    parser = argparse.ArgumentParser(description=color(f'{pawfetch} - neofetch with flags <3'), prog="pawfetch")

    parser.add_argument('-c', '--config', action='store_true', help=color(f'Configure pawfetch'))
...
def run():
    ...
    if (args.june or show_for_june) and not config.pride_month_disable:
        ...
        print("(You can always view the animation again with `pawfetch --june`)")
        print()
