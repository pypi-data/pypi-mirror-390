...
pub fn add_pkg_path() -> Result<()> {
    ...
    // Add from cwd: ./pawfetch/git, ./pawfetch/fastfetch, ./pawfetch/scripts
    path.extend(to_add.iter().map(|d| PathBuf::from("pawfetch").join(d)));
...
///
/// [neofetch]: https://github.com/hykilpikonna/pawfetch#running-updated-original-neofetch
...
fn bash_path() -> Result<PathBuf> {
    ...
    if let Some(bash_path) = find_in_path("bash.exe").context("bash.exe not found")? {
        ...
        if !bash_path.ends_with(r"Git\usr\bin\bash.exe") {
            ...
            // See https://github.com/hykilpikonna/pawfetch/issues/233
