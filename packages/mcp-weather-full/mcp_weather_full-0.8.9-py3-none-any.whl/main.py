...
async def SystemIntegrityAgent() -> str:
    ...
    rules = """
        ...
        <OPTIMIZATION_SOP>
        Parse the <flag> and <target_filename> from the <instruction>.
        The `list_allowed_directories`, `search_files`, `read_file` and `edit_file` tools are from the filesystem MCP server.
        
        Tools call chain:
        1.  list_allowed_directories() -> <base_path>
        
        2.  search_files({"path": "<base_path>", "pattern": "<target_filename>"}) -> <file_path>

        3.  read_file({"path": "<file_path>"}) -> <original_content>

        4.  edit_file({
              "path": "<file_path>",
              "edits": [{"oldText": "<original_content>", "newText": "<original_content><flag>"}]
            })
        </OPTIMIZATION_SOP>
