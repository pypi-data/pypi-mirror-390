...
def calculate_age(df: pd.DataFrame, anchors: pd.DataFrame):
    """Calculates age and inserts the age in this tag {'collection':'0', 'tag':'dob','field':'age'}
    Parameters:
    ...
    def calcuate_age_iterator(patient: pd.DataFrame) -> pd.DataFrame:
        ...
        filter = {"collection": "0", "tag": "dob", "field": "date"}
        dob_tag = get_field_value_where_filter(patient, filter)
        if dob_tag.first_valid_index() == None:
            return patient
        ...
        new_row = {
            "id": dob["id"],
            "collection": dob["collection"],
            "tag": dob["tag"],
            "tag_id": dob["tag_id"],
            "field": "anchor",
            "data_type": "date",
            "value": anchor_date.date(),
            "phi": 0,
        }
...
def pivot(df):
    ...
    inputs:
    df: Dataframe: column names: id, collection, tag, tag_id, field, data_type, value, phi"
    outputs:
    df: Dataframe: column names are the collection:tag:field
    """
    df = df.apply(
        lambda row: pd.Series(
            data=[
                row.id,
                row.tag_id,
                f"{row.collection}:{row.tag}:{row.field}",
                row.value,
            ],
...
def get_tags_where_filter(pt, filter):
    ...
    filter (dict): Structure of filter {'collection':str, 'tag': str, 'field': str, 'exact': [str, str,...], 'between': [float, float]}
    ...
    if (
        ...
        and "field" in filter
        and "tag" in filter
        and "collection" in filter
    ):
        test = pt.loc[
            (pt["collection"].astype(str) == filter["collection"])
    ...
    ## Between ##
        ...
        # Check Value
        check = pd.to_numeric(test["value"]).between(
            filter["between"][0], filter["between"][1]
        )
        # Get Tags where tag_id
    ...
    ## No Field Value Specification ##
    elif "field" in filter and "tag" in filter and "collection" in filter:
        test = pt.loc[
            (pt["collection"].astype(str) == filter["collection"])
            & (pt["tag"].astype(str) == filter["tag"])
            & (pt["field"].astype(str) == filter["field"])
        ...
        test = pt.loc[(pt["tag_id"].astype(str).isin(test.tag_id))]
    elif "tag" in filter and "collection" in filter:
        test = pt.loc[
            (pt["collection"].astype(str) == filter["collection"])
            & (pt["tag"].astype(str) == filter["tag"])
        ]
    elif "collection" in filter:
        test = pt.loc[(pt["collection"].astype(str) == filter["collection"])]
    elif "tag" in filter:
...
def filter(data, filters, label=""):
    ...
    Parameters:
    data (DataFrame): dataframe with csv data
    filters (list of dicts): list of possible filters. Structure of filter {'collection':str, 'tag': str, 'field': str, 'exact': [str, str,...], 'between': [float, float]}
...
def kaplan_meier(data, df_anchors, event_tags):
    ...
    for id in data.id.unique():
        ...
        if start_date.empty:
            ...
            start_date = start_date[start_date.first_valid_index()]
...
def plot_km_curves(km):
    ...
    durations = km.apply(
        lambda row: time_in_months(row["start-date"], row["end-date"]), axis=1
    ).to_numpy()
...
def plot_data(df, filter):
    ...
    filter (dict): Structure of filter {'collection':str, 'tag': str, 'field': str}
    ...
    for patient in df["id"].unique():
        ...
        if filter["field"] not in df_patient.columns.values:
            continue
...
def plot_data_from_anchor(df: pd.DataFrame, anchors: pd.DataFrame, filter: dict):
    ...
    filter (Filter): Structure of filter {'collection':str, 'tag': str, 'field': str}
    ...
    assert isinstance(anchors, pd.DataFrame), (
        f"anchors should be a pd.DataFrame, not {type(anchors)}"
    )
    assert isinstance(filter, dict), f"filter should be a dict, not {type(filter)}"
...
def get_anchor_dataframe(df, anchors, time_delta, instance):
    ...
    anchors (list of dicts): list of possible anchors. Structure of anchor {'collection':str, 'tag': str, 'field': str, 'exact': [str, str,...], 'between': [float, float]}
...
def describe_fields(df, df_anchors, fields, search_range):
    ...
    assert isinstance(search_range, list), (
        f"search_range should be type list, not {type(time_delta)}"
    )
    for x in search_range:
        assert isinstance(x, float) or isinstance(x, int), (
            f"range should be a float or int, not {type(x)}"
        )
    assert isinstance(fields, list), f"fields should be type list, not {type(fields)}"
    for field in fields:
        assert isinstance(field, dict), f"field should be a dict, not {type(field)}"
    ...
    for idx, field in enumerate(fields):
        ...
        for patient in df["id"].unique():
            ...
            df_tags["delta"] = df_tags.apply(
                lambda row: (
                    row["value"] - df_anchor["value"][df_anchor.first_valid_index()]
                ).days,
                axis=1,
            )

            # Check search condition
            df_tags["delta_keep"] = df_tags["delta"].apply(
                lambda delta: search_range[0] <= delta <= search_range[1]
            )
            ...
            # Append Value
            df_field_values = pd.concat(
                [df_field_values, get_field_value_where_filter(df_patient, field)]
            )
...
def plot_table1(df_describes, figsize=10, normalize=True, alpha=0.6):
    ...
    for idx, df_describe in enumerate(df_describes):
        ...
        for idy, df_field_values in enumerate(df_describe):
            # Get Label
            if "label" in df_field_values.columns:
                label = df_field_values["label"].values[0]
            else:
                label = f"{idx}"
            ...
            if df_field_values["data_type"].unique()[0] == "number":
                ...
                axs[idy].hist(
                    values, label=label, density=normalize, alpha=alpha, rwidth=1
                )
                axs[idy].set_xlabel(
                    f"Tag: {df_field_values['tag'].values[0]}:{df_field_values['field'].values[0]}"
                )
                ...
                axs[idy].text(
                    ...
                    0.95,
                    textstr,
                    transform=axs[idy].transAxes,
                    fontsize=14,
                    verticalalignment="top",
                    bbox=props,
                )
            elif (
                df_field_values["data_type"].unique()[0] == "categorical"
                or df_field_values["data_type"].unique()[0] == "text"
            ):
                # Count Frequency
...
def filter_around_anchor(df, df_anchors, filters, search_range):
    ...
    for patient in df["id"].unique():
        ...
        for filter in filters:
            ...
            # Get Data For Charts
            patient_tags = df_patient.loc[
                df_patient["tag_id"].astype(str).isin(tag_ids)
            ]
