...
from commerce_shopper_products import DefaultAioHttpClient, CommerceShopperProducts, AsyncCommerceShopperProducts
from commerce_shopper_products._utils import is_dict
...
logging.getLogger("commerce_shopper_products").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[CommerceShopperProducts]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with CommerceShopperProducts(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncCommerceShopperProducts]:
