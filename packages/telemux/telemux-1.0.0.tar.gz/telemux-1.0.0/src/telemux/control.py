...
def start():
    ...
    # Get the path to the listener module
    import telemux.listener
    listener_path = telemux.listener.__file__

    # Start tmux session with the listener
    subprocess.run(
        ['tmux', 'new-session', '-d', '-s', TMUX_SESSION, f'python3 {listener_path}'],
        check=False
    )
    time.sleep(1)
