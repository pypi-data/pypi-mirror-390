...
class SimpleConfigValidator:
    ...
    def _validate_registration(self, model: SimpleConfigModel) -> List[ValidationError]:
        ...
        if r.enabled:
            ...
            # Heartbeat - validate URL (preferred) or allow endpoint for backward compatibility
            if not r.heartbeat.url:
                # URL not provided - check if we can generate it from host/port/endpoint
                if not r.host or not r.port:
                    e.append(
                        ValidationError(
                            "registration.heartbeat.url is required, or registration.host/port must be provided to generate it"
                        )
                    )
                # If host/port are available, URL can be auto-generated (warning, not error)
            elif not isinstance(r.heartbeat.url, str) or not r.heartbeat.url.strip():
                e.append(ValidationError("registration.heartbeat.url must be a non-empty string"))
            ...
            # Registration URLs - validate URLs (preferred) or allow endpoints for backward compatibility
            if not r.register_url:
                # URL not provided - check if we can generate it from host/port/endpoint
                if not r.host or not r.port:
                    e.append(
                        ValidationError(
                            "registration.register_url is required, or registration.host/port must be provided to generate it"
                        )
                    )
                # If host/port are available, URL can be auto-generated (warning, not error)
            elif not isinstance(r.register_url, str) or not r.register_url.strip():
                e.append(ValidationError("registration.register_url must be a non-empty string"))
            ...
            if not r.unregister_url:
                ...
                if not r.host or not r.port:
                    e.append(
                        ValidationError(
                            "registration.unregister_url is required, or registration.host/port must be provided to generate it"
                        )
                    )
                # If host/port are available, URL can be auto-generated (warning, not error)
            elif not isinstance(r.unregister_url, str) or not r.unregister_url.strip():
                e.append(ValidationError("registration.unregister_url must be a non-empty string"))
            
            # Legacy endpoint fields - validate only if URLs are not provided
            if not r.register_url and not r.register_endpoint:
                e.append(
                    ValidationError(
                        "registration.register_url or registration.register_endpoint is required"
                    )
                )
            if not r.unregister_url and not r.unregister_endpoint:
                e.append(
                    ValidationError(
                        "registration.unregister_url or registration.unregister_endpoint is required"
                    )
                )
        return e
