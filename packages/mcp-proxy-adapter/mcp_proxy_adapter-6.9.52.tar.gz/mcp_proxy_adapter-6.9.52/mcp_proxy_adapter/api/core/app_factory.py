                ...
                try:
                    ...
                    if hasattr(self, '_config_path') and self._config_path:
                        ...
                        if isinstance(registration_config.get("heartbeat"), dict):
                            ...
                            heartbeat_data = registration_config["heartbeat"].copy()
                            # Migrate old 'endpoint' field to 'url' if needed
                            if "endpoint" in heartbeat_data and "url" not in heartbeat_data:
                                # Generate URL from endpoint (will be done in SimpleConfig.load() later)
                                # For now, just remove endpoint to avoid error
