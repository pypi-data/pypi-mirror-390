...
class RegistrationManager:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if registration_response is not None:
                        ...
                        # Update global registration status with mutex
    ...
    async def start_heartbeat(self, config: Dict[str, Any]) -> None:
        ...
        Heartbeat task will handle registration if not yet registered,
        then send heartbeats periodically.
        """

        # Store config for registration in heartbeat loop
        self._registration_config = config

        # Prepare context to get proxy URL and settings
        # Note: context may be None if auto_on_startup=false, but we still want to start heartbeat
        # for potential future registration or just heartbeat if already registered
        context = prepare_registration_context(config, self.logger)
        self.logger.info(f"🔍 Registration context prepared: {context is not None}")
        
        # If context is None, try to prepare minimal context for heartbeat only
        if context is None:
            self.logger.info("⚠️  Registration context is None, creating minimal context for heartbeat")
            # Check if proxy_client is enabled (heartbeat should still run even if auto_on_startup=false)
            proxy_client_config = dict(config.get("proxy_client") or {})
            if not proxy_client_config.get("enabled", False):
                self.logger.debug(
                    "Proxy client not enabled, heartbeat task will not start"
                )
                return

            # Try to create minimal context for heartbeat (without registration requirement)
            # This allows heartbeat to run even if auto_on_startup=false
            # Note: resolve_heartbeat_settings and resolve_runtime_credentials are already imported at module level

            # Extract minimal info for heartbeat
            self._use_proxy_client = True
            self._proxy_client_config = proxy_client_config
            self._proxy_registration_config = {}

            # Set minimal server info for heartbeat
            ...
            self.server_url = f"{scheme}://{advertised_host}:{port}"
            self.server_name = (
                proxy_client_config.get("server_id")
                or proxy_client_config.get("server_name")
                or f"mcp-adapter-{host}-{port}"
            )

            # Build proxy URL
            pc_protocol = proxy_client_config.get("protocol", "http")
            pc_scheme = "https" if pc_protocol in ("https", "mtls") else "http"
            pc_host = proxy_client_config.get("host", "localhost")
            pc_port = proxy_client_config.get("port", 3005)
            self.proxy_url = f"{pc_scheme}://{pc_host}:{pc_port}"

            self.capabilities = proxy_client_config.get(
                "capabilities", ["jsonrpc", "health"]
            )
            self.metadata = {}
        else:
            # Normal path: context is available
            self.logger.info("✅ Registration context available, applying context...")
            self._apply_context(context, config)
            self.logger.info(f"✅ Context applied: server_name={self.server_name}, proxy_url={self.proxy_url}")
            
            # Check after applying context
        ...
        # Use full URL from settings (already computed in resolve_heartbeat_settings)
