...
class HeartbeatConfig:
    ...
    url: Optional[str] = None  # Full URL for heartbeat (e.g., "http://localhost:3005/proxy/heartbeat")
    interval: int = 30
...
class RegistrationConfig:
    ...
    # Full URLs for registration operations
    register_url: Optional[str] = None  # Full URL for registration (e.g., "http://localhost:3005/register")
    unregister_url: Optional[str] = None  # Full URL for unregistration (e.g., "http://localhost:3005/unregister")
    # Legacy endpoint fields (for backward compatibility, will be auto-generated if URLs not provided)
...
class SimpleConfig:
    ...
    def load(self) -> SimpleConfigModel:
        ...
        if isinstance(registration_data.get("heartbeat"), dict):
            ...
            # Migrate old 'endpoint' field to 'url' if needed
            if "endpoint" in heartbeat_data and "url" not in heartbeat_data:
                # endpoint will be used to generate url later, remove it now
                heartbeat_data.pop("endpoint")
            registration.heartbeat = HeartbeatConfig(**heartbeat_data)  # type: ignore[arg-type]

        # Auto-generate full URLs if not provided
        if registration.enabled:
            # Determine scheme: test proxy (ports 3004, 3005) always uses HTTP
            # Otherwise use protocol from config
            if registration.port in (3004, 3005):
                scheme = "http"  # Test proxy always uses HTTP
            else:
                scheme = "https" if registration.protocol in ("https", "mtls") else "http"
            base_url = f"{scheme}://{registration.host}:{registration.port}"
            
            # Generate register_url if not provided
            if not registration.register_url:
                registration.register_url = f"{base_url}{registration.register_endpoint}"
            
            # Generate unregister_url if not provided
            if not registration.unregister_url:
                registration.unregister_url = f"{base_url}{registration.unregister_endpoint}"
            
            # Generate heartbeat URL if not provided
            if not registration.heartbeat.url:
                # Check if old 'endpoint' field exists in original heartbeat data
                heartbeat_endpoint = "/proxy/heartbeat"  # Default for test proxy
                if isinstance(registration_data.get("heartbeat"), dict):
                    old_endpoint = registration_data["heartbeat"].get("endpoint")
                    old_url = registration_data["heartbeat"].get("url")
                    if old_url:
                        # If url is provided but is a path, use it as endpoint
                        if old_url.startswith("/"):
                            # If it's /heartbeat, convert to /proxy/heartbeat for test proxy
                            if old_url == "/heartbeat" and registration.port in (3004, 3005):
                                heartbeat_endpoint = "/proxy/heartbeat"
                            else:
                                heartbeat_endpoint = old_url
                        else:
                            # Full URL provided
                            registration.heartbeat.url = old_url
                            heartbeat_endpoint = None
                    elif old_endpoint:
                        heartbeat_endpoint = old_endpoint
                if heartbeat_endpoint:
                    registration.heartbeat.url = f"{base_url}{heartbeat_endpoint}"
