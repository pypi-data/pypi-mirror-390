...
class SimpleConfigValidator:
    ...
    def _validate_server(self, model: SimpleConfigModel) -> List[ValidationError]:
        ...
        if s.protocol == "mtls":
            # For mtls: cert_file and key_file are required, CA is optional (can use system store)
            if not s.cert_file:
                e.append(
            ...
            # CA is optional - can use system CA store if not provided
        ...
        if s.cert_file and os.path.exists(s.cert_file):
            if s.ca_cert_file and os.path.exists(s.ca_cert_file):
                ...
                # Use system CA store (for https and mtls if CA not provided)
