...
def get_dart_version() -> str:
    package_xml = Path(__file__).resolve().parent / "package.xml"
    match = re.search(r"<version>([0-9]+\.[0-9]+\.[0-9]+)</version>", package_xml.read_text())
    if not match:
        raise RuntimeError("Unable to determine DART version from package.xml")
