...
'Virgin Media RFOG'
'Virgin Cable'
'nexfibre Virgin Media' (joint venture, part of Virgin Media O2 network)
...
-----------------------------------------------------------
The current/latest snapshot in the database is 2025-09-01 (Q3 2025).

1. For queries about "current", "latest", or specific dates that match the latest snapshot (Q3 2025):
   - USE: upc_output and fact_operator (non-time-series tables)
   - REASON: Simpler queries, no date filtering needed, same results
   
2. For queries explicitly about historical trends, comparisons over time, or dates BEFORE Q3 2025:
   - USE: upc_output_time_series and fact_operator_time_series
   - REASON: Contains historical snapshots

3. If the user asks about "Q3 2025" and that IS the latest snapshot, use the non-time-series tables.
