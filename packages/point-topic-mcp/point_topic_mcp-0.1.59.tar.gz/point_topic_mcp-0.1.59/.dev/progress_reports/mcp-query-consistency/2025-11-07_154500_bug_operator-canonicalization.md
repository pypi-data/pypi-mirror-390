...
4. **nexfibre virgin media not excluded** (discovered during fix)
   - 'nexfibre Virgin Media' is joint venture (80k postcodes)
   - should be excluded from altnet queries but wasn't
   - non-chat version also had this bug

5. **no dynamic dataset status checking** (enhancement)
   - agent couldn't check latest snapshot dates
   - had to hardcode dates in context (brittle)
   - needed tool to query status dynamically

6. **fact_altnet tables not documented** (enhancement)
   - pre-computed altnet tables with canonicalization already applied
   - agent was manually filtering fact_operator instead
   - simpler queries, less room for error
...
-- BETTER (non-chat version): canonicalize then count, but missing nexfibre
-- result: 1=13.3M, 2=1.9M, 3=180k, 4=17k

-- CORRECT (fixed): canonicalize + exclude nexfibre
-- result: 1=13.0M, 2=1.2M, 3=100k, 4=8k
```

**note**: the non-chat version was also including nexfibre as an altnet (incorrect). our fix is more accurate than both previous versions.
...
4. added nexfibre exclusion to all altnet queries
   ...
   - added `and operator not like '%nexfibre%'` to all examples
   - fixes bug present in both chat and non-chat versions

5. added get_dataset_status() tool
   - queries `upc_client._status.upc_status` table
   - returns latest snapshot dates for upc, takeup, forecast
   - agent can now dynamically check data freshness
   - updated context to reference this tool

6. added fact_altnet/fact_altnet_time_series tables to schema
   - pre-computed altnet tables with CityFibre canonicalized
   - openreach/VM exclusions already applied
   - added simplified example using fact_altnet
   - note: still includes nexfibre, needs additional filtering

## next steps
- deploy to pypi (v0.1.59)
- test with jolanta's exact prompts
- monitor for consistency
- consider updating fact_altnet to exclude nexfibre at source
- consider creating canonical operator dimension table
...
- status table allows dynamic date checking instead of hardcoding
- fact_altnet tables simplify altnet queries significantly (14 lines vs 30+)
