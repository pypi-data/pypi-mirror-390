...
from claudebridge.services.ui import (create_accounts_page, create_add_account_form, 
                         create_oauth_flow_card, create_layout)
from claudebridge.services.ui.accounts import create_account_card
from claudebridge.services.logger import logger
from claudebridge.blueprints.dependencies import (get_config_manager, get_accounts_manager, 
                                    get_oauth_manager)
from . import ui_bp
...
def ui_account():
    ...
    content = create_accounts_page(accounts, config_manager, accounts_manager)
...
def ui_account_refresh_card():
    ...
    accounts = accounts_manager.get_all_accounts()
    if accounts:
        return str(create_account_card(accounts[0], accounts_manager))
    return ""
...
def ui_account_complete_oauth():
    ...
    if oauth_session:
        ...
        try:
            ...
            tokens = oauth_manager.exchange_code_for_tokens(code, code_verifier, redirect_uri)
...
def update_session_key():
    ...
    from claudebridge.blueprints.dependencies import get_web_session_poller
    ...
    for acc in accounts:
        if acc.get("account_id") == account_id:
            return str(create_account_card(acc, accounts_manager))
    return "Account not found", 404
