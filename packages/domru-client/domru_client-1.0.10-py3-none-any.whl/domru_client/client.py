...
class DomRuClient:
    ...
    def get_agreements(self) -> List[Agreement]:
        ...
        resp_json = self.request("GET", "https://web-api.dom.ru/core-profile/v2/user/agreements?detailType=full", headers=headers).json()
        try:
            return [Agreement(**item) for item in resp_json]
        except ValidationError:
            raise DataFetchError('ValidationError')
    
    def get_agreement_info(self, agreement_number) -> AgreementInfo:
        headers = self.REST_HEADERS.copy()
        headers["agreementNumber"] = agreement_number

        response = self.request("GET", "https://web-api.dom.ru/api-profile/v2/info/all", headers=headers)
        try:
            agreement_info = AgreementInfo(**response.json())
        except ValidationError:
            raise DataFetchError("No valid data")

        if str(agreement_info.personal.agreement) == agreement_number:
            return agreement_info

        time.sleep(5)
        return self.get_agreement_info(agreement_number)

    def get_region_list(self) -> List[Region]:
        """
        Method to get a list of all regions with SSO auth
        """
        headers = self.REST_HEADERS.copy()
    ...
    def request(self, method, url, **kwargs) -> Response:
        ...
        if response.status_code >= 400:
            raise DataFetchError(f"HTTP {response.status_code} on {url}: {response.text}")
