...
def extract_token_probabilities(response):
    """
    모델 응답에서 토큰별 확률 정보를 추출하고 출력합니다.

    이 함수는 logprobs가 활성화된 모델 응답에서 각 토큰과 해당 확률을 추출하여
    보기 좋은 형태로 출력합니다. 확률은 log probability를 백분율로 변환하여 표시합니다.

    매개변수:
    - response: logprobs가 포함된 모델 응답 객체

    반환값:
    - dict: 다음 키를 포함하는 딕셔너리
        - tokens (list): 추출된 토큰 리스트
        - probabilities (list): 각 토큰의 확률(백분율) 리스트
    ...
    # 토큰 확률 추출 및 출력
    result = extract_token_probabilities(response)
    ```
    """
    import math

    # logprobs 데이터 존재 여부 확인
    if (
        not hasattr(response, "response_metadata")
        or "logprobs" not in response.response_metadata
        or response.response_metadata["logprobs"] is None
        or "content" not in response.response_metadata["logprobs"]
    ):
        print("경고: 응답에 logprobs 데이터가 없습니다. model.bind(logprobs=True)를 사용하세요.")
        return {"tokens": [], "probabilities": []}

    # logprobs 데이터에서 각 토큰과 확률 정보 추출
    ...
    for token_info in logprobs_data:
        ...
        probability = round(math.e ** logprob * 100, 2)  # e^logprob를 백분율로 변환
        tokens.append(token)
