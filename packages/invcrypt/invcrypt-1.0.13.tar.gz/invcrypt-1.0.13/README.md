🔐 InvCrypt CLI – Community Edition v1.0.13
...
⚠️ Hash Consistency Notice
The same hash function must be used for both encryption and decryption.
InvCrypt stores the selected hash type in each file’s metadata header, but manual overrides or mismatched parameters will prevent successful decryption.
Example:
invcrypt file.txt --seed demo --hash blake3x
invcrypt file.txt.invx --seed demo --hash blake3x   ✅ works
invcrypt file.txt.invx --seed demo --hash shake256  ❌ fails
