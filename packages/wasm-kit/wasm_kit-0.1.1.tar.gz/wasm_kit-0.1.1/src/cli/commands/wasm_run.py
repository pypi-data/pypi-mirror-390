...
def run(path: str, invoke_func: Optional[str], args: tuple):
    ...
    Auto-detects and runs the built WASM file in the project directory.
    Handles both standalone (.wasm) and component (.wcmp) files automatically.

    File Types:
      .wasm  - Standalone executable (entrypoint, like .exe)
      .wcmp  - Component model (library, like .dll)

    Examples:
      wasm-kit run
      wasm-kit run ./my-project
      wasm-kit run ./my-project -- --arg1 value1
    """
    try:
    ...
    if len(wasm_files) > 1:
        # Prefer project_name.wasm, then out.wasm (backward compatibility), then files in project root
