"""
Component runner using wasmtime-py for executing WASM components.

This module provides functionality to run WASM Component Model components
that export functions but don't have wasi:cli/run or wasi:http interfaces.
"""
...
def parse_wit_exports(wasm_file: Path) -> List[Dict[str, Any]]:
    """
    Parse WIT interface to extract exported functions with their signatures.

    Returns list of dicts with 'name', 'params', 'returns' keys.
    """
    import subprocess
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                console.print(
                    "  1. Use jco: jco transpile {} -o ./js-bindings".format(wasm_file.name)
                )
                console.print("  2. Create a Rust/Python host program")
                console.print("  3. See: wasm-tools component wit {}".format(wasm_file.name))
                return 1
