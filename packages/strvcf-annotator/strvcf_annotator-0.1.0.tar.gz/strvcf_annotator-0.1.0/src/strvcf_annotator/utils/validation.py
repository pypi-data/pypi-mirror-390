...
def validate_vcf_file(vcf_path: str) -> bool:
    ...
    try:
        ...
        raise ValidationError(f"Invalid VCF file {vcf_path}: {str(e)}")
...
def validate_bed_file(bed_path: str) -> bool:
    ...
    try:
        ...
        raise ValidationError(f"BED file is empty: {bed_path}")
    except Exception as e:
        raise ValidationError(f"Invalid BED file {bed_path}: {str(e)}")
...
def validate_str_bed_file(bed_path: str) -> bool:
    ...
    try:
        ...
        raise ValidationError(f"STR BED file is empty: {bed_path}")
    except Exception as e:
        raise ValidationError(f"Invalid STR BED file {bed_path}: {str(e)}")
