...
class SpecDecoder:
    def __init__(self,small_model,big_model, device='cuda'):
    ...
    def autoregressive_gen(self,prompt:str,max_new_tok:int, do_sample: bool = True):
        inp=self.tokenizer.encode(prompt,return_tensors='pt').to(self.device)
        with torch.no_grad():
            out=self.big_model.generate(inp,max_new_tokens=max_new_tok,do_sample=do_sample,pad_token_id=self.tokenizer.eos_token_id)
