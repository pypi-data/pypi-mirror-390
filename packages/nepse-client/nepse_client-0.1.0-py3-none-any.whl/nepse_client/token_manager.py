...
class TokenParser:
   ...
   def parse_token_response(self, token_response: dict) -> Tuple[str, str]:
      ...
      l = self.runtime.invocate(
         self.wasm_module, "rdx", 
         [salts[0], salts[1], salts[3], salts[2], salts[4]]
      )[0]
      o = self.runtime.invocate(
         self.wasm_module, "bdx",
         [salts[0], salts[1], salts[3], salts[2], salts[4]]
      )[0]
      p = self.runtime.invocate(
         self.wasm_module, "ndx",
         [salts[0], salts[1], salts[3], salts[2], salts[4]]
      )[0]
      q = self.runtime.invocate(
         self.wasm_module, "mdx",
         [salts[0], salts[1], salts[3], salts[2], salts[4]]
      )[0]
      ...
      a = self.runtime.invocate(
         self.wasm_module, "cdx",
         [salts[1], salts[0], salts[2], salts[4], salts[3]]
      )[0]
      b = self.runtime.invocate(
         self.wasm_module, "rdx",
         [salts[1], salts[0], salts[2], salts[3], salts[4]]
      )[0]
      c = self.runtime.invocate(
         self.wasm_module, "bdx",
         [salts[1], salts[0], salts[3], salts[2], salts[4]]
      )[0]
...
class TokenManager(_TokenManagerBase):
   ...
   def _getTokenHttpRequest(self) -> dict:
      ...
      return self.nepse.requestGETAPI(
         url=self.token_url,
         include_authorization_headers=False
      )
