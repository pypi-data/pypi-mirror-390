...
class ProxySystem:
    ...
    def __init__(self, thread_state, immutable_types, tracer, tracecalls):
        ...
        self.calltracer = self.create_tracer() if tracecalls else None
    ...
    def create_tracer(self, serializer = serializer):
        def on_result(result):
            self.write_trace({'type': 'result', 'result': serializer(result)})
        
        def on_error(exc_type, exc_value, exc_traceback):
            self.write_trace({'type': 'error', 'error': serializer(exc_value)})

        result_handlers = (on_result, on_error)

        def tracefunc(frame):
            funcname = frame.function.__qualname__

            if not exclude(funcname):
                self.write_trace({'type': 'call', 'function': funcname})
                return result_handlers

        return self.thread_state.dispatch(
                    utils.noop, 
                    internal = self.thread_state.wrap('disabled', tracefunc))
