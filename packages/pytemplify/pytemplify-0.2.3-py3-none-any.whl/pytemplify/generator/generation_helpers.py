...
class EnhancedTemplateRenderer:
    ...
    def create_renderer(self, context: Dict[str, Any], filters: Dict[str, Any]) -> TemplateRenderer:
        """Create pytemplify renderer with context and filters."""
        return TemplateRenderer(context, "", filters=filters)

    def render_with_patterns(self, iteration_context: TemplateIterationContext, filters: Dict[str, Any]) -> None:
        ...
        renderer = self.create_renderer(iteration_context.context, filters)
...
class TemplateSetFilter:
    ...
    def should_include(self, template_name: str) -> bool:
        """Determine if a template set should be included based on filters."""
        # If exclude patterns are specified, template must not match any
        if self.exclude_patterns:
            for pattern in self.exclude_patterns:
                if self._matches_pattern(template_name, pattern):
                    return False

        # If include patterns are specified, template must match at least one
        if self.include_patterns:
            if not any(self._matches_pattern(template_name, pattern) for pattern in self.include_patterns):
                return False
    ...
    def _matches_pattern(self, text: str, pattern: str) -> bool:
        ...
        if pattern.startswith("regex:"):
            ...
            try:
                ...
                logging.warning("Invalid regex pattern '%s': %s", regex_pattern, exc)
                return False
        else:
