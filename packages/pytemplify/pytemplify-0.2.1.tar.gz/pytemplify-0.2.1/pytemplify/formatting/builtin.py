...
class BuiltinFormatter(CodeFormatter):
    ...
    def _format_with_autopep8(self, content: str, options: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise FormattingError("autopep8 not installed", self.name) from exc
        ...
        try:
            return autopep8.fix_code(content, options=autopep8_options)
        except Exception as exc:
    ...
    def _format_with_yapf(self, content: str, options: Dict[str, Any]) -> str:
        ...
        """
        try:
            import yapf  # pylint: disable=import-outside-toplevel
        except ImportError as exc:
            raise FormattingError("yapf not installed", self.name) from exc

        # yapf style configuration
        style_config = options.get("style_config", "pep8")

        try:
            formatted_code, _ = yapf.yapf_api.FormatCode(content, style_config=style_config)
    ...
    def _format_with_cpp_format(self, content: str, options: Dict[str, Any]) -> str:
        ...
        input_file_path = self._create_temp_file(content)
