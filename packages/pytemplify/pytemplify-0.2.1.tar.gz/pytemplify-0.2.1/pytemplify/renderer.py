...
class TemplateRenderer:
    ...
    # Manual section handling - delegate to ManualSectionManager
    _manual_section_manager = ManualSectionManager()

    # Keep these as class attributes for backwards compatibility
    MANUAL_SECTION_START = ManualSectionManager.MANUAL_SECTION_START
    MANUAL_SECTION_END = ManualSectionManager.MANUAL_SECTION_END
    MANUAL_SECTION_ID = ManualSectionManager.MANUAL_SECTION_ID
    MANUAL_SECTION_PATTERN = ManualSectionManager.MANUAL_SECTION_PATTERN
    MANUAL_SECTION_CHECK_PATTERN = ManualSectionManager.MANUAL_SECTION_CHECK_PATTERN
    ...
    def __init__(
        self, data: Any, data_name: str = "", filters: Dict[str, Callable] = None, auto_register_filters: bool = True
    ) -> None:
        """
    ...
    def _validate_manual_sections(
        ...
        self._manual_section_manager.validate_sections(temp, rendered, prev_rendered, context)
    ...
    def render_string(
        ...
        self._validate_manual_sections(temp, rendered_string, prev_rendered_string, context)
    ...
    def _write_file(self, filepath: Path, content: str) -> None:
        ...
        if self._formatter_manager and self._formatter_manager.is_enabled():
            try:
                ...
                logging.debug("Applied formatting to %s", filepath)
            except (OSError, IOError) as exc:
                # File system errors during formatting
                logging.error("File error during formatting of %s: %s", filepath, exc)
                ...
                logging.warning("Formatting failed for %s: %s", filepath, exc)
