...
renderer.generate(temp_dirpath="templates", output_dirpath="generated")
```
...
pytemplify supports optional code formatting for generated files. This feature allows you to automatically format generated code using popular formatters like Black, Prettier, and clang-format.
...
- **Prettier**: JavaScript, TypeScript, JSON, CSS, HTML, Markdown formatter (requires `pip install prettier`)
...
```bash
pip install prettier  # For prettier command-line usage
# Plus system packages like clang-format for command-line usage
```
