...
class BaseValidator(ABC):
    ...
    Responsibilities:
        ...
        - Handle pattern matching (DRY: reuses TemplateSetFilter)
    ...
    def should_validate(self, target: Path) -> bool:
        ...
        DRY: Reuses existing TemplateSetFilter logic for pattern matching.
        ...
        try:
            from pytemplify.generator import TemplateSetFilter  # pylint: disable=import-outside-toplevel

            filter_obj = TemplateSetFilter(include_patterns=self.config.patterns, exclude_patterns=[])
            return filter_obj.should_include(target.name)
        except ImportError:
            # Fallback to simple glob matching if TemplateSetFilter not available
            import fnmatch  # pylint: disable=import-outside-toplevel

            return any(fnmatch.fnmatch(target.name, pattern) for pattern in self.config.patterns)
