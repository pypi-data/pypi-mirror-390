  ...
  Precedence: CLI args > YAML config > auto-discovery
    ...
    try:
        ...
        if args.output:
            ...
            generator.generate(output_base_dir=args.output)
            output_dir = args.output
        else:
            logger.info("Generating files (output directories from YAML configuration)")
            generator.generate()
            # Try to extract output directory from YAML config for validation
            # Use the first template's output directory if available
            templates = generator.template_config.yaml_data.get("templates", [])
            for template in templates:
                if template.get("output"):
                    # Get the base output directory (may need resolution)
                    output_path = template.get("output")
                    if not Path(output_path).is_absolute():
                        output_dir = args.config.parent / output_path
                    else:
        ...
        validation_config = generator.template_config.yaml_data.get("validation")
        should_validate = args.validate or (
            not args.no_validate and validation_config and validation_config.get("enabled", False)
        )

        if should_validate and validation_config:
            logger.info("\n" + "=" * 60)
            ...
            try:
                ...
                if not validators:
                    logger.warning("No validators configured or all disabled")
                else:
                    # Determine output directory for validation
                    if output_dir is None:
                        logger.error("Cannot run validation: output directory not determined")
                    else:
                        # Run validation
                        runner = ValidationRunner(validators)
                        fail_fast = args.fail_fast or validation_config.get("fail_fast", False)
                ...
                if args.verbose:
                    ...
                    traceback.print_exc()
                sys.exit(1)
        elif args.validate and not validation_config:
            logger.warning("Validation requested but no validation configuration found in YAML")
