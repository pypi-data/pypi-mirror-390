...
class BaseCodeGenerator(ABC):
    ...
    def generate(
        self, output_base_dir: Optional[Path] = None, template_config_filepath: Optional[Path] = None, **kwargs
    ) -> None:
        ...
        Args:
            ...
            template_config_filepath: Template config to use for this generation.
                                     ...
                                     If provided, overrides the init config for this
                                     generation only.
            **kwargs: Additional keyword arguments passed to _prepare_generation() hook
        ...
        Examples:
            ...
            gen = MyGenerator(data, template_config_filepath=Path("templates.yaml"))
            gen.generate(Path("./output"))

            # Pattern 2: Config at generate (lazy init)
            gen = MyGenerator(data)
            gen.generate(Path("./output"), template_config_filepath=Path("templates.yaml"))

            # Pattern 3: Multiple generations with different configs
            gen = MyGenerator(data)
            gen.generate(Path("./out1"), template_config_filepath=Path("config1.yaml"), mode="tx")
            gen.generate(Path("./out2"), template_config_filepath=Path("config2.yaml"), mode="rx")

            # Pattern 4: Default config with override
            gen = MyGenerator(data, template_config_filepath=Path("default.yaml"))
            gen.generate(Path("./out1"))  # Uses default.yaml
            gen.generate(Path("./out2"), template_config_filepath=Path("special.yaml"))
        """
        original_config = None
        ...
        try:
            ...
            # Process all template sets
            for iteration_context in self._generate_iteration_contexts(output_base_dir, context_root):
                self.enhanced_renderer.render_with_patterns(iteration_context, self._get_template_filters())
