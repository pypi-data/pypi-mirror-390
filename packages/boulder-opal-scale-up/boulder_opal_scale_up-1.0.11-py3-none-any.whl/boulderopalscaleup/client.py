...
class QctrlScaleUpClient:
    ...
    async def get_device_snapshot(
        ...
        """
        _display_message(
            f"Getting snapshot of device '{self.current_device_name}'.",
        )
    ...
    async def enable(self, element: str, *others: str) -> JobId:
        """
        ...
        element : str
            Element to enable.
        *others : str
            Additional elements to enable.
        """
        return await self._enable([element, *others])

    async def enable_all(self) -> JobId:
        """
        Enable all qubits for running experiments.

        This method clears any previous restrictions set by `enable`,
