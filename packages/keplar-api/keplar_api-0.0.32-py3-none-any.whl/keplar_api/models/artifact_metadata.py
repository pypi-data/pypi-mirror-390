...
class ArtifactMetadata(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["status", "statusMessage", "progress"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['draft', 'in_progress', 'completed', 'failed']):
            raise ValueError("must be one of enum values ('draft', 'in_progress', 'completed', 'failed')")
        return value
