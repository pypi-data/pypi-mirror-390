...
def remove_re_pattern(re_pattern: str) -> str:
    """
    删除地址中，中文括号包裹的内容
    """

    def _func(address):
        return re.sub(re_pattern, "", address)
...
class ExcelObj:
    ...
    def __post_init__(self):
        ...
        self.output_filename = os.path.join(
            self.data_args.output_dir, f"{name}.{self.data_args.output_type}"
        )
        ...
        if self.data_args.filename.endswith(".xls") or self.data_args.filename.endswith(
            ".xlsx"
        ):
    ...
    def address(self) -> List[str]:
        ...
        if self.data_args.re_pattern:
            df[self.data_args.address_col_name] = df[
                self.data_args.address_col_name
            ].map(remove_re_pattern(self.data_args.re_pattern))
...
if __name__ == "__main__":
    address = "123（测试）456"
    address = remove_re_pattern(r"（.*?）", address)
    print(address)
