...
@click.option('--account', help='Filter by account ID')
@click.option('--sso', help='AWS SSO profile name')
def daily(profile, start_date, end_date, days, service, account, sso):
    """
    ...
    try:
        ...
        for day in response['ResultsByTime']:
            ...
            total += cost
            click.echo(f"{date} | ${cost:,.2f}")
        
        click.echo("-----------|-----------")
        click.echo(f"Total      | ${total:,.2f}")
        click.echo(f"Daily Avg  | ${total/len(response['ResultsByTime']):,.2f}")
        click.echo(f"Annual     | ${(total/len(response['ResultsByTime']))*365:,.0f}")
