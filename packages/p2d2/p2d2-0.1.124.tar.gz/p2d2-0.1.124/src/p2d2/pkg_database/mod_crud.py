...
class Create(Job):
    ...
    def execute(self, resources: dict[str, Resource], **kwargs):
        ...
        if not (signature := kwargs.pop("signature")): raise KeyError("'signature' not found")
        update = False
        ...
        with pydb.get_table(table_name) as table:
            ...
            try:
                ...
                log.debug(f"Attempting to create in table '{pydb}':\n  - unique_keys: {unique_keys}\n  - kwargs: {kwargs}")
...
class Update(Job):
    ...
    def execute(self, resources: dict[str, Resource], **kwargs):
        ...
        with pydb.get_table(table_name) as table:
            ...
            log.debug(f"Updated {updated_count} rows in {table_name} by {signature} (took {time.time() - start_time:.4f}s)")
            return table
...
class Delete(Job):
    ...
    def execute(self, resources: dict[str, Resource], **kwargs):
        ...
        with pydb.get_table(table_name) as table:
            ...
            log.debug(f"Deleted {deleted_count} rows from {table_name} by {signature} (took {time.time() - start_time:.4f}s)")
            return table
#
#     @classmethod
