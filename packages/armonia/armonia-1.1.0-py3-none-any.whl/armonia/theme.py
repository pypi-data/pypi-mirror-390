...
def _is_valid_uri(uri: str) -> bool:
    ...
    try:
        result = urlparse(uri)
        # A valid URI must have a scheme (e.g., http, https, file, data)
        # and either a netloc (for http/https/ftp) or a path (for file/data)
...
class Theme:
    ...
    def __init__(self) -> None:
        ...
        self._computing: set[str] = set()  # Track colors being computed
    ...
    def get_color(self, key: str) -> HSLColor:
        ...
        if key in self.computed_colors:
            ...
            try:
                result = self.computed_colors[key](self)
                return result
            finally:
        ...
        # 4. Try to parse as hex color
        try:
            return HSLColor.from_hex(key)
        except (ValueError, Exception):
            pass

        # If all resolution attempts fail, raise an error
    ...
    def get_palette(self, name: str) -> HSLColorSequence:
        ...
        # Resolve all color references in the palette
        # HSLColorSequence is a type alias for List[HSLColor], so we just return the list
    ...
    def set_logotype(self, name: str, uri: str) -> None:
        ...
        """
        # Validate URI using custom validator
    ...
    def get_logotype(self, name: str) -> str:
        ...
        if name not in self.logotypes:
            raise LogotypeNotFoundError(
                f"Logotype '{name}' not found in theme logotypes."
            )
    ...
    def get_all_colors(
        ...
        for name in self.computed_colors:
            try:
                ...
                # Skip colors that can't be computed
                # (e.g., circular dependencies or missing dependencies)
                pass
