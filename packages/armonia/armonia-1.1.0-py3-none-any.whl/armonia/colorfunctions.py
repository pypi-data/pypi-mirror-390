...
def rotate_hue(color_key: str, degrees: float) -> ComputeColorFn:
    ...
    def compute(theme: Theme) -> HSLColor:
        color = theme.get_color(color_key)
        # Convert degrees to normalized hue value (0.0-1.0)
...
def alpha(color_key: str, opacity: float) -> ComputeColorFn:
    ...
    def compute(theme: Theme) -> HSLColor:
        color = theme.get_color(color_key)
        # Calculate the delta needed to reach the target opacity
...
def softer(
    color_key: str, background_key: str, amount: float = 0.3
) -> ComputeColorFn:
    """
...
def stronger(
    ...
    def compute(theme: Theme) -> HSLColor:
        ...
        # Calculate the opposite direction from background
        # We'll increase saturation and adjust lightness away from background
...
def grayscale(color_key: str) -> ComputeColorFn:
    ...
    def compute(theme: Theme) -> HSLColor:
        color = theme.get_color(color_key)
        # Set saturation to 0 to get grayscale
...
def multi(
    color_key: str, *transforms: Callable[[HSLColor], HSLColor]
) -> ComputeColorFn:
    """
