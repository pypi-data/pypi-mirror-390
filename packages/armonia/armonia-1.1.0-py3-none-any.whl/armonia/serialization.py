...
# Registry of serializable color functions
# Maps function names to their constructor functions and parameter specifications
...
def deserialize_computed_color(spec: dict[str, Any]) -> ComputeColorFn:
    ...
    # Handle both positional (list) and keyword (dict) arguments
    if isinstance(args_spec, list):
        ...
        kwargs = dict(zip(params, args_spec))
    elif isinstance(args_spec, dict):
        # Keyword arguments
        kwargs = args_spec.copy()
    else:
        raise ValueError(
            f"'args' must be a list or dict, got {type(args_spec).__name__}"
        )
    ...
    # Call the function constructor with the arguments
    return fn(**kwargs)
