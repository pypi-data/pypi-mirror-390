...
from src.database.repository.querys_searcher import NewRevenues, QuerysNewRevenues, QuerysSubscriptions, Subscriptions
from src.tools import list_obj_to_df
...
async def shimpent_whabonet_alquiventas(message: Dict):
    ...
    if not message.get("adviser_name"):
        ...
        data = {
            ...
            "template": [
                {
                "id": "609dffaa-e57c-482d-aad1-cb3e0e8cc878",
                "params": [
                    message.get("name_customer"),
                    message.get("token")
                ]
                }
            ],
        ...
        data = {
            ...
            "template": [
                {
                "id": "cafba504-8d38-495b-8931-2e58c3536143",
                "params": [
                    message.get("adviser_name"),
                    message.get("customer_name"),
                    message.get("customer_mobile"),
                    message.get("token")
                ]
                }
            ],
...
async def shipment_match_alquiventas():
    ...
    for record in records:
        ...
        message = {"suscriber_id": record.get("subscription_id"),
...
async def shimpent_whabonet_castillo(message: Dict):
    ...
    if not message.get("adviser_name"):
        ...
        data = {
            ...
            "template": [
                {
                "id": "425811df-40ea-4167-beb6-f084e10ede49",
                "params": [
                    message.get("name", ""),
                    message.get('token', '')
                ]
        ...
        data= {
                ...
                "template": [
                    {
                    "id": "4ee5b7ae-6793-4a0d-ac64-9c38b7038cf6",
                    "params": [
                        message.get("adviser_name"),
                        message.get("name_customer"),
                        message.get("phone_customer"),
                        message.get("token")
                    ]
                    }
                ],
...
async def shipment_match_castillo():
    ...
    for record in records:
        # print(record.get("name"), record.get("mobile"), record.get("token"))
        message = {"suscriber_id":
                   record.get("subscription_id"),
                   "phone_number": record.get("mobile"),
                   "content": content_message_customer.format(name=record.get("name"), url_base=os.getenv("URL_BASE_SHIPMENT_CASTILLO"), token=record.get("token")),
...
if __name__ == "__main__":
    # phone_numer= ["573103555742", "573103738772"]
