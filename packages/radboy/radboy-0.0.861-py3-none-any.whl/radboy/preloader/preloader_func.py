...
def TaxMuleFraud():
    ...
    while True:
        try:
            ...
            totals={
            'legally_taxed_tax[(Price*legal_tax)]':fd['Price']*fd['Legally Applied Tax'],
            'legally_taxed_price[Price+(Price*legal_tax)]':(fd['Price']*fd['Legally Applied Tax'])+fd['Price'],

            'illegally_taxed_tax[Price*Illegal Tax]':fd['Price']*fd['Mule Tax Rate (Charged as Despite being legally the other)'],
            'illegally_taxed_price(What was charged to the customer)[Price+(Price*Illegal_tax)]':(fd['Price']*fd['Mule Tax Rate (Charged as Despite being legally the other)'])+fd['Price'],

            'fraud_profit_tax_only[Price*(illegal_tax-legal_tax)]':fd['Price']*(fd['Mule Tax Rate (Charged as Despite being legally the other)']-fd['Legally Applied Tax']),
            'fraud_profit_as_price[Price+(Price*(illegal_tax-legal_tax))]':(fd['Price']*(fd['Mule Tax Rate (Charged as Despite being legally the other)']-fd['Legally Applied Tax']))+fd['Price'],
            }
            cta=len(totals)
            htext='\n'.join([std_colorize(f'{i} - {totals[i]}',num,cta) for num,i in enumerate(totals)])
            while True:
