...
class OracleDB:
    ...
    def __set_config(self, cs_code: str, vender_code: str, service_id: str,flag=False):
        TEXT : str  = ''
        if cs_code == '' and  not flag:
            TEXT += f"AND VENDOR_ID = '{vender_code}' "
        if vender_code == '' and  not flag:
            TEXT += f"AND VENDOR_CODE = '{cs_code}' "
        if not service_id == '' and  not flag:
            TEXT += f"AND SERVICE_ID = '{service_id}' "
        ic(TEXT)
        if  not flag:
            self.item :dict= self._get_vendor(TEXT) # type: ignore
            self.VENDOR_CODE = self.item.get('VENDOR_CODE','')
            self.SERVICE_ID = self.item.get('SERVICE_ID','')
            self.VENDOR_ID = self.item.get('VENDOR_ID','')
            self.VENDOR_NAME =  self.item.get('VENDOR_NAME','')
            ic(self.item)
        for value in self.DATAMAP:
            TEXT=''
            for k, v in value['RULE_MAP'].items():            
                if v.get('STATUS', 'S') == 'A':
                    TEXT += f"AND {v['RESULT']} = '{self.item.get(v['RESULT'])}' "
        ...
        try:
            ...
            with self.conn.cursor() as cur:
                ...
                if fetch:
                    ...
                    ic(rows_clean)
                    if flag:
                        data = pd.DataFrame(rows_clean, columns=cols)
                        return data
                    else:
    ...
    def export_file(self,**kg):
        ...
        row = 1
        data = self.fetch_tables(self.VENDOR_CODE,self.VENDOR_ID,self.SERVICE_ID,expoprt=True) # pyright: ignore[reportAssignmentType]
        ic(data)
        ...
        return True
    def fetch_tables(self,values: str,cs_code :str|None=None , service: str|None = None,include=None,expoprt=False):
        """
        ...
        if  not include:
            include = [ i.get('SCHEMA') for i in self.DATAMAP]
        ic(values,cs_code,service,include)  
        self.__set_config(str(cs_code),values,str(service), expoprt)
        ic(self.CONFIG)
        for ky,vl in self.CONFIG.items():
            ic(ky,include)
            if ky not in include and not expoprt :
                continue
            try:
                if not expoprt:
                    return self._run_query(vl,flag=False)
                    
                else:
                    data[ky] = self._run_query(vl,flag=True)
                    ic(data[ky])
            except Exception :
                continue
        self.VALUE = data
