...
def find_virtualenv(path=None):
    ...
    Args:
        path (str, optional): Path to search for virtual environment. Defaults to current directory.
...
def source_virtualenv(venv_path=None):
    ...
    Args:
        venv_path (str, optional): Path to the virtual environment. If None, search in current and parent directories.
    """
    if venv_path is None:
        activate_script = find_virtualenv()
        if not activate_script:
            click.echo("Error: No virtual environment found in current or parent directories.", err=True)
            click.echo("Please specify the path to the virtual environment or create one with 'python -m venv venv'")
            sys.exit(1)
    else:
    ...
    # The user needs to run this with 'eval $(h venv source [path])' or 'source <(h venv source [path])'
    click.echo(f'source "{activate_script}"')
    click.echo(f'# Virtual environment activated: {os.path.dirname(os.path.dirname(activate_script))}', err=True)
