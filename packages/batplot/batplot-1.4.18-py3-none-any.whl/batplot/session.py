...
def _color_to_hex(color):
    """Convert matplotlib color (RGBA tuple, array, or string) to hex string for serialization."""
    try:
        if color is None:
            return None
        # Handle RGBA tuples/arrays
        if isinstance(color, (tuple, list, np.ndarray)):
            return to_hex(color)
        # Already a string (hex or named color)
    ...
    *,
    ...
    spines_state = {
        name: {
            ...
            'color': _color_to_hex(sp.get_edgecolor()),
