...
def interactive_menu(fig, ax, y_data_list, x_data_list, labels, orig_y,
    ...
    # Do NOT call ensure_text_visibility() here - it adjusts margins which moves titles
    # Only draw to update the visualization
    fig.canvas.draw_idle()
    ...
    def push_state(note=""):
        ...
        try:
            ...
            snap = {
                ...
                "label_pads": {"xlabel": getattr(ax.xaxis, 'labelpad', None), "ylabel": getattr(ax.yaxis, 'labelpad', None)},
    ...
    def restore_state():
        ...
        try:
            # Disable layout engine first to prevent matplotlib from moving labels during xlim/ylim restore
            try:
            ...
            # Restore label pads from snapshot to prevent drift
            label_pads = snap.get("label_pads", {})
            restored_xlabel_pad = False
            restored_ylabel_pad = False
            if label_pads.get("xlabel") is not None:
                try:
                    ax.xaxis.labelpad = label_pads["xlabel"]
                    restored_xlabel_pad = True
                except Exception:
                    pass
            if label_pads.get("ylabel") is not None:
                try:
                    ax.yaxis.labelpad = label_pads["ylabel"]
                    restored_ylabel_pad = True
                except Exception:
                    pass
            ...
            try:
                ...
                pass
            # Only reposition bottom/left labels if we didn't restore pads from snapshot
            # This prevents label drift when undoing x-range changes
            if not restored_xlabel_pad:
                try:
                    position_bottom_xlabel()
                except Exception:
                    pass
            if not restored_ylabel_pad:
                try:
            ...
            try:
                ...
                while True:
                    ...
                    if sub == 'm':
                        ...
                        print("  1. viridis    - Blue→yellow, colorblind-friendly")
                        print("  2. plasma     - Purple→yellow, high contrast")
                        print("  3. inferno    - Black→yellow, good for dark backgrounds")
                        print("  4. magma      - Black→white, excellent for grayscale")
                        print("  5. batlow     - Scientific multi-hue palette")
                        if extras:
                            print("\nOther available: " + ", ".join(extras))
                        print(colorize_inline_commands("Example: 1-4 viridis  or: all 2  or: 1-3,5 plasma  or: 1-3 1_r (reversed)"))
                        line = input("Enter range(s) and palette name/number (e.g., '1-3 viridis' or '1-3 2') or q: ").strip()
                        if not line or line.lower() == 'q':
                            print("Canceled.")
                        else:
                            parts = line.split()
                            if len(parts) < 2:
                                ...
                                if palette_name.endswith('_r'):
                                    base_name = palette_name[:-2]
                                    if base_name in palette_map:
                                        palette_name = palette_map[base_name] + '_r'
                                    # else keep original name (e.g., "viridis_r")
                                elif palette_name in palette_map:
                                    palette_name = palette_map[palette_name]
            ...
            try:
                ...
                while True:
                    ...
                    if mode == '':
                        continue
                    if mode == 'c':
                        # Allow renaming multiple curves in one session
                        while True:
                            ...
                            fig.canvas.draw()
                            # Continue to allow renaming another curve
                    elif mode == 't':
                        cts = getattr(_bp, 'cif_tick_series', None) if _bp is not None else None
                        if not cts:
