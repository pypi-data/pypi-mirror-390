                ...
                if _is_noninteractive:
                    ...
                    try:
                        ...
                        if operando_ec_interactive_menu is not None:
                            # Collect file paths from the operando folder
                            operando_files = []
                                ...
                                try:
                                    ...
                                    for fname in _os.listdir(folder):
                                        ...
                                        if _os.path.isfile(fpath):
                                            operando_files.append(fpath)
                                except Exception:
                                    pass
                            operando_ec_interactive_menu(fig, ax, im, cbar, ec_ax, file_paths=operando_files)
                        else:
    ...
    if len(args.files) == 1 and args.files[0].lower().endswith('.pkl'):
        ...
        if isinstance(sess, dict) and sess.get('kind') == 'ec_gc':
            try:
                ...
                try:
                    ...
                    pass
                try:
                    electrochem_interactive_menu(fig, ax, cycle_lines, file_path=None)
                except Exception as _ie:
                    print(f"Interactive menu failed: {_ie}")
                _plt.show()
                exit()
        ...
        if isinstance(sess, dict) and sess.get('kind') == 'operando_ec':
            try:
                ...
                try:
                    if operando_ec_interactive_menu is not None:
                        # For loaded sessions, we don't have original file paths
                        operando_ec_interactive_menu(fig2, ax2, im2, cbar2, ec_ax2, file_paths=None)
                except Exception as _ie:
                    print(f"Interactive menu failed: {_ie}")
                _plt.show()
                exit()
