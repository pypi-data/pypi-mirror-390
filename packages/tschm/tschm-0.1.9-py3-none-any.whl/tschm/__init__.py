...
class ENTRY(object):
    ...
    def list(self):
        """
        列出 windows 系统中 任务计划程序当中的 \\tschm 目录
        """
    ...
    def list_from_tschm(self):
        ...
        try:
            ...
            for task in tasks:
                ...
                print(f"  状态: {state} | 启用: {'是' if task.Enabled else '否'} | 隐藏: {'是' if is_hidden else '否'}")
    ...
    def add_task(self, name: str, script: str, schedule: str = "DAILY", time: str = "12:00", hidden: bool = True):
        """
        在任务计划程序的 \\tschm 目录下创建一个新的定时任务
        任务名称为 name
        执行的脚本为 script, 现在当前目录当中 寻找 script 文件
            如果当前目录当中存在 script文件 需要先把文件复制到 self.config._config_path 目录下 然后再添加 任务 并且支持 唤醒计算器去运行设置的任务
                文件复制过程中如果发现 self.config._config_path 目录下已经存在同名文件 则需要抛出文件重复提示 并结束操作
            如果当前目录当中不存在 script文件 则在 self.config._config_path 目录下寻找 script 文件
            如果找不到文件则抛出 找不到文件提示并结束操作
        ...
        try:
            ...
            if schedule not in ["DAILY", "WEEKLY", "MONTHLY"]:
                print(f"错误: schedule 参数无效 '{schedule}'，可选值为 DAILY, WEEKLY, MONTHLY")
                return False
            ...
            settings.StartWhenAvailable = True  # 如果错过了计划时间，则在下次可用时立即运行
            ...
            if script_ext == '.ps1':
                ...
                action.Arguments = f'-ExecutionPolicy Bypass -File "{final_script_path}"'
            elif script_ext in ['.bat', '.cmd']:
                action.Path = final_script_path
            else:
                # 默认使用 powershell 执行
                action.Path = "powershell.exe"
                action.Arguments = f'-ExecutionPolicy Bypass -File "{final_script_path}"'
    ...
    def disable_task_today(self, name: str):
        """
        禁用任务计划程序中 \\tschm 目录下的指定任务 今天不运行
        任务名称为 name
        """
        try:
            scheduler = win32com.client.Dispatch('Schedule.Service')
            ...
            # 遍历所有触发器，为每个触发器添加今天的结束边界
            for i in range(triggers.Count):
                trigger = triggers[i]
                trigger.EndBoundary = today.strftime("%Y-%m-%dT%H:%M:%S")
