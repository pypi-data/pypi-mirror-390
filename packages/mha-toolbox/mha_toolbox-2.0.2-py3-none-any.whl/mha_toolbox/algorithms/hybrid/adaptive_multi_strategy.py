...
class Adaptive_Multi_Strategy_Hybrid(OptimizationAlgorithm):
    """
    ...
    def optimize(self):
        ...
        velocities = np.random.uniform(-1, 1, (self.population_size, self.dimensions))
        ...
        return self._create_result(global_best, global_best_fitness, convergence_curve)
    ...
    def _sca_update(self, population, global_best, iteration):
        ...
        for i in range(self.population_size):
            r1 = a - iteration * (a / self.max_iterations)
            r2 = 2 * np.pi * np.random.rand(self.dimensions)
            r3 = 2 * np.random.rand(self.dimensions)
            r4 = np.random.rand(self.dimensions)
