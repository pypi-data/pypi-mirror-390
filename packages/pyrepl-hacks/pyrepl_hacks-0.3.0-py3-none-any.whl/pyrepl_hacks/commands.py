...
def previous_paragraph(reader: HistoricalReader) -> None:
    """Move cursor to the start of the previous paragraph (blank line separated block)."""
    x, y = reader.pos2xy()
    lines = reader.get_unicode().splitlines(keepends=True)

    # If we're already on the first line, go to beginning
    if y == 0:
        reader.pos = 0
        reader.error("start of buffer")
        return

    # Start searching from current line
    search_y = y

    # Skip backward through current paragraph (non-blank lines)
    while search_y > 0 and lines[search_y - 1].strip() != "":
        search_y -= 1

    # search_y now points to first line of current paragraph
    # Skip backward past the blank line(s) before current paragraph
    search_y -= 1
    while search_y > 0 and lines[search_y - 1].strip() == "":
        search_y -= 1

    # search_y now points to last line of previous paragraph (or 0 if no previous)
    # Skip backward to find the start of that paragraph
    while search_y > 0 and lines[search_y - 1].strip() != "":
        search_y -= 1

    # Position at the start of the previous paragraph
    reader.pos = sum(len(line) for line in lines[:search_y])


@register_command  # type: ignore[call-overload]
def next_paragraph(reader: HistoricalReader) -> None:
    """Move cursor to the start of the next paragraph (blank line separated block)."""
    x, y = reader.pos2xy()
    lines = reader.get_unicode().splitlines(keepends=True)

    # If we're already on the last line, go to end
    if y >= len(lines) - 1:
    ...
    # If we're currently on a non-blank line, skip to end of current paragraph
    if lines[search_y].strip() != "":
        search_y += 1
        while search_y < len(lines) and lines[search_y].strip() != "":
            search_y += 1

    # Now skip any blank lines to find the next paragraph
    while search_y < len(lines) and lines[search_y].strip() == "":
        search_y += 1

    # Position at the start of the next paragraph or end of buffer
