...
class TestPreviousParagraph(unittest.TestCase, ReaderTestMixin):
    def test_previous_paragraph_basic(self):
        """Test moving to previous paragraph separated by blank line."""
        text = "line1\nline2\n\nline4\nline5"
        reader = self.create_reader(text, pos=20)  # Position in "line5"

        previous_paragraph(reader)

        # Should move to start of "line1" (the previous paragraph)
    ...
    def test_previous_paragraph_multiple_blank_lines(self):
        """Test moving to previous paragraph with multiple blank lines."""
        text = "line1\n\n\n\nline5"
        reader = self.create_reader(text, pos=10)  # Position in "line5"

        previous_paragraph(reader)

        # Should move to start of "line1" (skipping all blank lines)
        self.assertPositionEquals(reader, 0)

    def test_previous_paragraph_from_blank_line(self):
        """Test moving to previous paragraph when starting on blank line."""
        text = "line1\nline2\n\nline4"
    ...
    def test_previous_paragraph_within_paragraph(self):
        """Test moving to previous paragraph when within a paragraph."""
        text = "para1\n\npara2\nline2\n\npara3"
        reader = self.create_reader(text, pos=13)  # Position in "line2" of para2

        previous_paragraph(reader)

        # Should move to start of para1 (skipping entire current paragraph)
        self.assertPositionEquals(reader, 0)

    def test_previous_paragraph_three_blocks(self):
        """Test moving through three paragraphs."""
        text = "block1\n\nblock2\n\nblock3"
        reader = self.create_reader(text, pos=16)  # In "block3"

        previous_paragraph(reader)
        # Should jump to "block1" (skipping entire "block2")
        self.assertPositionEquals(reader, 0)
...
class TestNextParagraph(unittest.TestCase, ReaderTestMixin):
    def test_next_paragraph_basic(self):
        """Test moving to next paragraph separated by blank line."""
        text = "line1\nline2\n\nline4\nline5"
        reader = self.create_reader(text, pos=5)  # Position in "line2"

        next_paragraph(reader)

        # Should move to start of "line4" (after blank line)
    ...
    def test_next_paragraph_multiple_blank_lines(self):
        """Test moving to next paragraph with multiple blank lines."""
        text = "line1\n\n\n\nline5\nline6"
        reader = self.create_reader(text, pos=2)  # Position in "line1"

        next_paragraph(reader)

        # Should move to start of "line5" (after all blank lines)
        self.assertPositionEquals(reader, 9)

    def test_next_paragraph_from_blank_line(self):
        """Test moving to next paragraph when starting on blank line."""
        text = "line1\n\nline3\nline4"
        reader = self.create_reader(text, pos=6)  # Position on blank line

        next_paragraph(reader)

        # Should skip blank lines and move to start of next paragraph
        self.assertPositionEquals(reader, 7)
    ...
    def test_next_paragraph_three_paragraphs(self):
        ...
        next_paragraph(reader)
        self.assertPositionEquals(reader, 7)  # Start of "para2"

        next_paragraph(reader)
        self.assertPositionEquals(reader, 14)  # Start of "para3"
