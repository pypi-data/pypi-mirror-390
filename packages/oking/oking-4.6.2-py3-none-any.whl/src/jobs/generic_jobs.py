...
def get_column_names_from_query(sql: str, db_config: DatabaseConfig) -> List[str]:
    ...
    try:
        ...
        # Remover ORDER BY da query para obter metadados (SQL Server não aceita ORDER BY em subconsultas sem TOP)
        sql_without_order = sql
        if 'order by' in sql.lower():
            # Remove ORDER BY e tudo após ele
            parts = sql.lower().split('order by')
            sql_without_order = sql[:sql.lower().index('order by')]
        
        # Executar query com LIMIT 0 para obter apenas metadados
        # CORREÇÃO (04/11/2025): db_config é objeto, não dict
        if db_config.db_type.lower() == 'oracle':
            query_metadata = f"SELECT * FROM ({sql_without_order}) WHERE ROWNUM = 0"
        elif db_config.db_type.lower() == 'firebird':
            query_metadata = f"SELECT FIRST 0 * FROM ({sql_without_order})"
        else:  # SQL Server
            query_metadata = f"SELECT TOP 0 * FROM ({sql_without_order}) AS subquery"
...
def job_generic(job_config: dict):
    ...
    with lock:
        try:
            ...
            batch_size = job_config.get('tamanho_pacote', 50)
