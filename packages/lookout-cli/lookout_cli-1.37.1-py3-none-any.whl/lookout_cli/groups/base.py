...
def _get_compose_files(
    prod: bool = False, network: Network = Network.HOST, gpu=False, cache: bool = False
) -> List[ValidPath]:
...
def build(
    no_cache: bool, config_dir: str, services: List[str], pull: bool = False, cache: bool = False
):
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(gpu=config.gpu, prod=False, cache=cache),
        compose_project_directory=get_project_root(),
    )
    services_list = list(services) if services else None
...
@config_dir_option
def configure(
    gpu: bool, default: bool, cuda_version: str, include_defaults: bool, config_dir: str
):
    """Configure Lookout"""
    set_initial_env(config_dir)
