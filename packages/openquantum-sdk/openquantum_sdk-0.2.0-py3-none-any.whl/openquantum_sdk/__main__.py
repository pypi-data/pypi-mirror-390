...
def main():
    ...
    try:
        ...
        if organization_id:
            print(f" > Using specified organization: {organization_id}")
        else:
            print("Discovering user organization...")
            orgs = management.list_user_organizations()
            if not orgs.organizations:
                raise ValueError("No organizations found for user. Please specify one with --organization.")
            organization_id = orgs.organizations[0].id
            print(f" > Using auto-discovered organization: {orgs.organizations[0].name} ({organization_id})")

        # Step 2: Use user-provided inputs
        print(f" > Using backend: {args.backend}")
        print(f" > Using subcategory: {args.subcategory}")
        print("---------------------------------------")

        # Step 3: Build config for high-level submission
