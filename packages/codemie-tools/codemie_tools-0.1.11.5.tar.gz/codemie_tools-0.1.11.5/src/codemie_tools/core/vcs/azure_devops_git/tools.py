...
class AzureDevOpsGitTool(CodeMieTool):
    ...
    def _build_full_url(self, relative_url: str, method_arguments: dict) -> tuple[str, dict]:
        ...
        if isinstance(method_args, dict):
            # Handle repositoryId placeholder
            if "repositoryId" in method_args and "{repositoryId}" in relative_url:
                repo_id = str(method_args.pop("repositoryId"))
                relative_url = relative_url.replace("{repositoryId}", repo_id)

            # Handle commitId placeholder
            if "commitId" in method_args and "{commitId}" in relative_url:
                commit_id = str(method_args.pop("commitId"))
                relative_url = relative_url.replace("{commitId}", commit_id)
