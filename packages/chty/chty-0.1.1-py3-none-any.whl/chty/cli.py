    ...
    for sql_file in sql_files:
        try:
            ...
            if not parameters and not result_schema:
                typer.echo(
                    f"  ⚠️  No parameters or result schema available for {sql_file}",
                    err=True,
                )
            ...
            if parameters:
                ...
                info_parts.append(f"params: {param_names}")
            if result_schema:
                info_parts.append(f"results: {len(result_schema)} cols")

            info_str = ", ".join(info_parts) if info_parts else "query only"
            typer.echo(f"  ✓ Generated {output_file} ({info_str})")
