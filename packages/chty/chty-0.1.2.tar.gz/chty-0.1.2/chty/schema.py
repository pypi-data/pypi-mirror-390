...
def _get_default_value_for_type(clickhouse_type: str) -> str:
    ...
    if clickhouse_type.startswith("Int") or clickhouse_type.startswith("UInt"):
        ...
        return "0.0"
    elif clickhouse_type.startswith("String") or clickhouse_type.startswith(
        "FixedString"
    ):
        return "''"
...
def get_result_schema(query: str, db_url: str) -> list[tuple[str, str]]:
    ...
    Args:
        ...
        db_url: ClickHouse connection URL
            - clickhouse://user:pass@host:port
            - https://user:pass@host:port (for ClickHouse Cloud)
    ...
    if parsed.scheme not in ("clickhouse", "clickhouse+native", "https", "http"):
        raise ValueError(
            f"Invalid db_url scheme: {parsed.scheme}. "
            "Expected 'clickhouse://', 'https://', or 'http://'"
        )

    # Use secure connection for https
