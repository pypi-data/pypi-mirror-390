...
class MessagesAPI:
    ...
    def get_user_thread(
        ...
        Args:
            ...
            include_relation: Include relation metadata (`extra=user.relation`)

        Returns:
            Thread object
        """
        params = {"extra": "user.relation"} if include_relation else None
        data = self.http.get(f"/v1/user-thread/{user_id}", params=params)
        print(data)
        if self._state:
