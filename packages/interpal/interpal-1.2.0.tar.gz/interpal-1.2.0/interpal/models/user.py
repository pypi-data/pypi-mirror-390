...
class UserRelation(BaseModel):
    """
    Relationship metadata returned with `extra=user.relation`.
    """

    def __init__(self, state: Optional[Any] = None, data: Optional[Dict[str, Any]] = None):
        self.uid: Optional[str] = None
        self.target_id: Optional[str] = None
        self.friend_status: Optional[str] = None
        self.user_blocks_target: bool = False
        self.target_blocks_user: bool = False
    ...
    def _from_dict(self, data: Dict[str, Any]):
        ...
        self.uid = parse_user_id(data.get("uid"))
        self.target_id = parse_user_id(data.get("target_id"))
        self.friend_status = data.get("friend_status")
        self.user_blocks_target = data.get("user_blocks_target", False)
        self.target_blocks_user = data.get("target_blocks_user", False)
        self.modified = parse_timestamp(data.get("modified"))
...
class User(BaseModel):
    ...
    def __init__(self, state: Optional[Any] = None, data: Optional[Dict[str, Any]] = None):
        ...
        self.relation: Optional[UserRelation] = None
    ...
    def _from_dict(self, data: Dict[str, Any]):
        ...
        if relation_data:
            self.relation = UserRelation(state=self._state, data=relation_data)
...
class Profile(User):
    ...
    def __init__(self, state: Optional[Any] = None, data: Optional[Dict[str, Any]] = None):
        ...
        self.profile_photos_album = None
        self.profile_photos_avatars: List[Any] = []
        self.newest_photo = None
        self.match_settings: Optional[Dict[str, Any]] = None
        self.permissions: Optional[Dict[str, Any]] = None
        self.can_contact: Optional[Dict[str, Any]] = None
        self.lfor: Optional[Dict[str, Any]] = None
        self.about: Optional[Dict[str, Any]] = None
    ...
    def _from_dict(self, data: Dict[str, Any]):
        ...
        album_data = profile_photos.get("album")
        if album_data:
            if self._state:
                self.profile_photos_album = self._state.create_album(album_data)
            else:
                from .media import Album  # Local import to avoid circular dependency
                self.profile_photos_album = Album(state=self._state, data=album_data)

        avatars_data = profile_photos.get("avatars") or []
        self.profile_photos_avatars = []
        if avatars_data:
            if self._state:
                self.profile_photos_avatars = [
                    self._state.create_photo(photo) for photo in avatars_data
                ]
            else:
                from .media import Photo  # Local import to avoid circular dependency
                self.profile_photos_avatars = [
                    Photo(state=self._state, data=photo) for photo in avatars_data
                ]

        newest_photo_data = data.get("newest_photo")
        if newest_photo_data:
            if self._state:
                self.newest_photo = self._state.create_photo(newest_photo_data)
            else:
                from .media import Photo  # Local import to avoid circular dependency
                self.newest_photo = Photo(state=self._state, data=newest_photo_data)
        else:
            self.newest_photo = None

        self.match_settings = data.get("match_settings")
        self.permissions = data.get("permissions")
        self.can_contact = data.get("can_contact")
        self.lfor = data.get("lfor", self.lfor)
