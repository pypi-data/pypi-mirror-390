...
class ProfileViewEvent(BaseModel):
    """
    PROFILE_VIEW WebSocket event model.
    """
    ...
    def __init__(self, state: Optional[Any] = None, data: Optional[Dict[str, Any]] = None):
        ...
        self.counters: Optional[EventCounters] = None
        self.click_url: Optional[str] = None
        self.viewer: Optional[User] = None
        self.sender: Optional[User] = None

        super().__init__(state=state, data=data)

    def _from_dict(self, data: Dict[str, Any]):
        """Parse PROFILE_VIEW event from WebSocket."""
        self.type = data.get('type', 'PROFILE_VIEW')
        ...
        user_data = data.get('sender') or data.get('viewer') or {}
        if user_data:
            if self._state:
                self.viewer = self._state.create_user(user_data)
            else:
                self.viewer = User(state=self._state, data=user_data)
            # Maintain backwards compatibility with raw payload naming
            self.sender = self.viewer

    @property
    def viewer_id(self) -> Optional[str]:
        """Convenience property to get the viewing user's ID."""
        if self.viewer and getattr(self.viewer, "id", None) is not None:
