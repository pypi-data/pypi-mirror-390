...
**ADDED:**
- ✅ `get_user_thread(user_id, include_relation=False)` - `/v1/user-thread/{user_id}` (GET) - get/create thread with specific user, optionally pass `extra=user.relation` when `include_relation=True`
...
#### Posts API (`interpal/api/posts.py`)

**ADDED:**
- ✅ `get_feed(...)` - `/v1/feed` (GET) - fetch user/global feed entries with support for `type`, `owner_id`, `owner_only`, pagination, and extra query params
