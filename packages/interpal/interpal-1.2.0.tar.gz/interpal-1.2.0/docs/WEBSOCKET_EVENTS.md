...
Represents a profile view event triggered when another user visits your profile.

**Properties:**
- `type` (str): Event type ("PROFILE_VIEW")
- `counters` (EventCounters): Updated counter values
- `click_url` (str): Direct URL to the profile views page
- `viewer` (User): User who viewed your profile (also available as `sender` for parity with the raw payload)
- `viewer_id` (str): Convenience property for the viewer's ID

**Example:**
```python
@ws.on('on_profile_view')
async def handle_profile_view(event: ProfileViewEvent):
    viewer = event.viewer
    print(f"{viewer.name} (@{viewer.username}) just viewed your profile!")
    print(f"New profile views: {event.counters.new_views}")
```
