...
class PostsAPI:
    ...
    def get_feed(
        ...
        **kwargs,
        ...
        Args:
            feed_type: Feed type (`user`, `global`, etc.)
            owner_id: Optional feed owner/user ID
            owner_only: When True, only include posts created by the owner
            limit: Page size
            offset: Pagination offset
            **kwargs: Additional query parameters (e.g. after, before, type filters)

        Returns:
            List of feed entries (parsed into Post models when state is available)
        """
        params: Dict[str, Any] = {
            "type": feed_type,
            "limit": limit,
        ...
        data = self.http.get("/v1/feed", params=params)
        ...
        if self._state:
            return [self._state.create_post(item) for item in items]
        return items
