...
class InterpalState:
    ...
    def create_post(self, data: Dict[str, Any]) -> Post:
        """
        Create or update a Post object with LRU caching.

        Args:
            data: Post data from API
        ...
        post_id = data.get('id')
        if post_id is None:
            return Post(state=self, data=data)
        ...
        post = Post(state=self, data=data)
        self._add_to_cache(self._posts, post_id, post, self.max_messages)
        self._stats['objects_created'] += 1
        self._stats['cache_misses'] += 1

        self._call_creation_handlers('post', post)
