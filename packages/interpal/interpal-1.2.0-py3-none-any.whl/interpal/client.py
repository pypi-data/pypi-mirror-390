...
try:  # Support running as a package or as a standalone module
    ...
    from .exceptions import AuthenticationError, WebSocketAuthenticationError
    ...
    from interpal.exceptions import AuthenticationError, WebSocketAuthenticationError
...
class InterpalClient:
    ...
    def get_user_thread(self, user_id: str, include_relation: bool = False):
        """Get or create thread with a user."""
        return self.messages.get_user_thread(user_id, include_relation)
    ...
    def is_websocket_connected(self) -> bool:
        """Return True if the WebSocket client reports an active connection."""
        if self._ws_client:
...
class AsyncInterpalClient:
    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                if auth_retry_performed:
                    raise
                auth_retry_performed = True
                await self._handle_websocket_authentication_error(exc)

    async def _ensure_authenticated(self):
        """Ensure a valid session exists before establishing the WebSocket connection."""
        if self.is_authenticated:
            return
        await self._refresh_authentication("No valid session found; performing login")

    async def _handle_websocket_authentication_error(self, exc: WebSocketAuthenticationError):
        """Recover from WebSocket authentication errors by refreshing the session."""
        reason = f"WebSocket authentication failed: {exc}"
        await self._refresh_authentication(reason)

    async def _refresh_authentication(self, reason: str):
        """Clear stale session state and perform a fresh login."""
        print(f"{reason}. Refreshing authentication…")

        if self._session_manager:
            self._session_manager.clear_session()

        if hasattr(self.auth, "clear_session"):
            self.auth.clear_session()

        if not self._username or not self._password:
            raise WebSocketAuthenticationError(
                f"{reason}. Username/password not available for automatic login."
            )
        ...
        try:
            ...
            raise WebSocketAuthenticationError(
                f"{reason}. Login failed: {auth_error}"
            ) from auth_error
    ...
    def is_websocket_connected(self) -> bool:
        """Return True if the async WebSocket client has an active connection."""
        if self._ws_client:
            return self._ws_client.is_connected
        return False
