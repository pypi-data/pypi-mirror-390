...
class Bot(AsyncInterpalClient):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Track whether we've bound our internal on_message handler
        self._internal_on_message_registered = False
        self._ensure_internal_handlers()
    ...
    def is_websocket_connected(self) -> bool:
        """Compatibility helper that mirrors AsyncInterpalClient API."""
        if self._ws_client:
    ...
    def _initialize_websocket_client(self):
        """Create the underlying WebSocket client."""
        from ..websocket import WebSocketClient
        self._ws_client = WebSocketClient(self.auth)
        self._internal_on_message_registered = False

    def _ensure_internal_handlers(self):
        """Register internal listeners (like on_message) exactly once."""
        if self._ws_client is None:
            self._initialize_websocket_client()

        if not self._internal_on_message_registered:
            self._ws_client.register_event('on_message', self._handle_message)
            self._internal_on_message_registered = True
