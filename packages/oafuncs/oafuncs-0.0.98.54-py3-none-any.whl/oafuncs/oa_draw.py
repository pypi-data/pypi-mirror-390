...
def font_CN():
    ...
    try:
        ...
        # 查找可用中文字体（宽松匹配，允许字体名包含目标）
        for font_name in candidates:
            font_lower = font_name.lower()
            # 检查是否有已安装字体包含当前候选字体名（不区分大小写）
            if any(font_lower in installed.lower() for installed in installed_fonts):
                # 处理字体列表，避免重复并置于首位
                ...
                print(f'ℹ️ 已设置中文字体: {font_name}')
                return font_name

        # 未找到字体时提示
        print(f'⚠️ 未检测到可用中文字体，中文可能显示异常；建议安装 {suggest_fonts}。')
