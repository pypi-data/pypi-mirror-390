...
class User(PrettyModel):
    ...
    attributes: Optional[List[RLSAttribute]] = Field(default_factory=list)
    ...
    def custom_validator(cls, data: dict):
        """Custom deserialization from {'rls.{table}.{field}': [values]}"""
        result = []
        input_attributes: Dict[str, List[str]] | List[RLSAttribute] = data.get('attributes', {})
        if isinstance(input_attributes, list):
            return data
        elif isinstance(input_attributes, dict):
            for k, v in input_attributes.items():
                ...
                if key.startswith("rls."):
                    parts = key.split(".")
                    if len(parts) == 3:
                        result.append(RLSAttribute(table=parts[1], field=parts[2], values=values))
            data['attributes'] = result
        return data

    @field_serializer('attributes', return_type=Dict[str, List[str]])
    def custom_serializer(self, attributes: Optional[List[RLSAttribute]]) -> Dict[str, List[str]]:
        """Custom serialization to format attributes as {'rls.{table}.{field}': [values]}"""
        return {f'rls.{attr.table}.{attr.field}': attr.values for attr in attributes if attr.table and attr.field and attr.values}
