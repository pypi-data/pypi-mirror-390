...
class AsyncUserDAO:
    ...
    async def create(self, user: User) -> OperationResult[User]:
        ...
        try:
            ...
            if created_user.password:
                await self.client.a_set_user_password(user_id, created_user.password, temporary=False)
    ...
    async def update(self, username: str, payload: User) -> OperationResult[User]:
        ...
        try:
            ...
            attributes = payload.attributes.copy()

            payload_data = payload.model_dump(exclude_unset=True)
            payload_data.pop('attributes', None)

            updated_user = user.model_copy(update=payload_data)

            if attributes:
                updated_user.attributes = attributes

            user_data = updated_user.model_dump()
            user_id = user_data.pop('id')
            user_data.pop('username')

            await self.client.a_update_user(user_id, user_data)
