...
def read_og_effective_geographic_basin(simplified=False, verbose=False):
    """Download data of oil and gas effective geographic basins in Brazil.

    This function downloads and returns data of oil and gas effective geographic basins
    (Bacia Efetiva Geográfica) in Brazil as a GeoPandas GeoDataFrame. 
    ...
    -------
    gpd.GeoDataFrame
        A GeoDataFrame containing data on oil and gas effective geographic basins in Brazil.
    ...
    Example
    -------
    >>> from tunned_geobr import read_og_effective_geographic_basin
    >>> 
    >>> # Read the data
    >>> geo_basins = read_og_effective_geographic_basin()
    >>> 
    >>> # Plot the data
    >>> geo_basins.plot()
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            zip_path = os.path.join(temp_dir, "og_effective_geographic_basin.zip")
            if verbose:
            ...
            if verbose:
                print("Finished processing oil and gas effective geographic basins data")
            
            return gdf
    
    except Exception as e:
        raise Exception(f"Failed to download or process oil and gas effective geographic basins data: {str(e)}")
