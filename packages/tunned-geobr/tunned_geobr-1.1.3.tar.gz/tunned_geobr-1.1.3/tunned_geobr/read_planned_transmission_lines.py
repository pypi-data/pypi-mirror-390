...
def read_planned_transmission_lines(simplified=False, verbose=False):
    """Download Planned Transmission Lines data from EPE.
    
    This function downloads and processes planned transmission lines data from EPE 
    (Energy Research Company). The data includes information about planned transmission 
    lines across Brazil.
    ...
    -------
    gpd.GeoDataFrame
        Geodataframe with planned transmission lines data
        
    Example
    -------
    >>> from tunned_geobr import read_planned_transmission_lines
    
    # Read planned transmission lines data
    >>> planned_lines = read_planned_transmission_lines()
    """
    
    # URL for the EPE geoserver
    url = r"https://gisepeprd2.epe.gov.br/arcgis/rest/services/Download_Dados_Webmap_EPE/GPServer/Extract%20Data%20Task/execute?f=json&env%3AoutSR=102100&Layers_to_Clip=%5B%22Linhas%20de%20Transmiss%C3%A3o%20-%20Expans%C3%A3o%20Planejada%22%5D&Area_of_Interest=%7B%22geometryType%22%3A%22esriGeometryPolygon%22%2C%22features%22%3A%5B%7B%22geometry%22%3A%7B%22rings%22%3A%5B%5B%5B-8655251.47456396%2C-4787514.465591563%5D%2C%5B-8655251.47456396%2C1229608.401015912%5D%2C%5B-3508899.2341809804%2C1229608.401015912%5D%2C%5B-3508899.2341809804%2C-4787514.465591563%5D%2C%5B-8655251.47456396%2C-4787514.465591563%5D%5D%5D%2C%22spatialReference%22%3A%7B%22wkid%22%3A102100%7D%7D%7D%5D%2C%22sr%22%3A%7B%22wkid%22%3A102100%7D%7D&Feature_Format=Shapefile%20-%20SHP%20-%20.shp&Raster_Format=Tagged%20Image%20File%20Format%20-%20TIFF%20-%20.tif"
    ...
    try:
        ...
        raise Exception(f"Error downloading or processing planned transmission lines data: {str(e)}")
        
    return gdf

if __name__ == '__main__':
    try:
        lines_data = read_planned_transmission_lines(verbose=True)
        print(f"Downloaded planned transmission lines data with {len(lines_data)} records and {len(lines_data.columns)} columns")
        
        # Test simplified version
        simplified_data = read_planned_transmission_lines(simplified=True)
