...
def read_existent_solar(simplified=False, verbose=False):
    """Download Existing Solar Power Plants data from EPE.
    
    This function downloads and processes existing solar power plants data from EPE 
    (Energy Research Company). The data includes information about operational solar power 
    ...
    -------
    gpd.GeoDataFrame
        Geodataframe with existing solar power plants data
        
    Example
    -------
    >>> from tunned_geobr import read_existent_solar
    
    # Read existing solar power plants data
    >>> existent_solar = read_existent_solar()
    """
    
    # URL for the EPE geoserver WFS service
    url = r"https://gisepeprd2.epe.gov.br/arcgis/rest/services/Download_Dados_Webmap_EPE/GPServer/Extract%20Data%20Task/execute?f=json&env%3AoutSR=102100&Layers_to_Clip=%5B%22UFV%20-%20Base%20Existente%22%5D&Area_of_Interest=%7B%22geometryType%22%3A%22esriGeometryPolygon%22%2C%22features%22%3A%5B%7B%22geometry%22%3A%7B%22rings%22%3A%5B%5B%5B-8202935.359686549%2C-4447637.219321614%5D%2C%5B-8202935.359686549%2C1119424.4247428626%5D%2C%5B-3487076.4626055676%2C1119424.4247428626%5D%2C%5B-3487076.4626055676%2C-4447637.219321614%5D%2C%5B-8202935.359686549%2C-4447637.219321614%5D%5D%5D%2C%22spatialReference%22%3A%7B%22wkid%22%3A102100%7D%7D%7D%5D%2C%22sr%22%3A%7B%22wkid%22%3A102100%7D%7D&Feature_Format=Shapefile%20-%20SHP%20-%20.shp&Raster_Format=Tagged%20Image%20File%20Format%20-%20TIFF%20-%20.tif"
    ...
    try:
        ...
        raise Exception(f"Error downloading or processing existing solar power plants data: {str(e)}")
        
    return gdf

if __name__ == '__main__':
    try:
        solar_data = read_existent_solar(verbose=True)
        print(f"Downloaded existing solar power plants data with {len(solar_data)} records and {len(solar_data.columns)} columns")
        
        # Test simplified version
        simplified_data = read_existent_solar(simplified=True)
