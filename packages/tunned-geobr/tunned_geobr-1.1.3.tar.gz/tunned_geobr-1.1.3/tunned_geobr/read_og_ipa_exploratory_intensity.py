...
def read_og_ipa_exploratory_intensity(simplified=False, verbose=False):
    """Download data of IPA exploratory intensity for oil and gas in Brazil.

    This function downloads and returns data of IPA exploratory intensity
    (IPA Intensidade Exploratória) for oil and gas areas in Brazil as a GeoPandas GeoDataFrame. 
    ...
    -------
    gpd.GeoDataFrame
        A GeoDataFrame containing data on IPA exploratory intensity for oil and gas areas in Brazil.
    ...
    Example
    -------
    >>> from tunned_geobr import read_og_ipa_exploratory_intensity
    >>> 
    >>> # Read the data
    >>> ipa_intensity = read_og_ipa_exploratory_intensity()
    >>> 
    >>> # Plot the data
    >>> ipa_intensity.plot()
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            zip_path = os.path.join(temp_dir, "og_ipa_exploratory_intensity.zip")
            if verbose:
            ...
            if verbose:
                print("Finished processing IPA exploratory intensity data")
