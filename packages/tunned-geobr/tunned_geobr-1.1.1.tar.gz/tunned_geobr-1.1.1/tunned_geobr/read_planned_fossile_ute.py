...
def read_planned_fossile_ute(simplified=False, verbose=False):
    """Download Planned Fossil Thermoelectric Power Plants data from EPE.
    
    This function downloads and processes planned fossil thermoelectric power plants data from EPE 
    (Energy Research Company). The data includes information about planned fossil thermoelectric 
    ...
    -------
    gpd.GeoDataFrame
        Geodataframe with planned fossil thermoelectric power plants data
        
    Example
    -------
    >>> from tunned_geobr import read_planned_fossile_ute
    
    # Read planned fossil thermoelectric power plants data
    >>> planned_fossile_ute = read_planned_fossile_ute()
    """
    
    # URL for the EPE geoserver
    url = r"https://gisepeprd2.epe.gov.br/arcgis/rest/services/Download_Dados_Webmap_EPE/GPServer/Extract%20Data%20Task/execute?f=json&env%3AoutSR=102100&Layers_to_Clip=%5B%22UTE%20F%C3%B3ssil%20-%20Expans%C3%A3o%20Planejada%22%5D&Area_of_Interest=%7B%22geometryType%22%3A%22esriGeometryPolygon%22%2C%22features%22%3A%5B%7B%22geometry%22%3A%7B%22rings%22%3A%5B%5B%5B-8655251.47456396%2C-4787514.465591563%5D%2C%5B-8655251.47456396%2C1229608.401015912%5D%2C%5B-3508899.2341809804%2C1229608.401015912%5D%2C%5B-3508899.2341809804%2C-4787514.465591563%5D%2C%5B-8655251.47456396%2C-4787514.465591563%5D%5D%5D%2C%22spatialReference%22%3A%7B%22wkid%22%3A102100%7D%7D%7D%5D%2C%22sr%22%3A%7B%22wkid%22%3A102100%7D%7D&Feature_Format=Shapefile%20-%20SHP%20-%20.shp&Raster_Format=Tagged%20Image%20File%20Format%20-%20TIFF%20-%20.tif"
    ...
    try:
        ...
        raise Exception(f"Error downloading or processing planned fossil thermoelectric power plants data: {str(e)}")
        
    return gdf

if __name__ == '__main__':
    try:
        fossile_ute_data = read_planned_fossile_ute(verbose=True)
        print(f"Downloaded planned fossil thermoelectric power plants data with {len(fossile_ute_data)} records and {len(fossile_ute_data.columns)} columns")
        
        # Test simplified version
        simplified_data = read_planned_fossile_ute(simplified=True)
