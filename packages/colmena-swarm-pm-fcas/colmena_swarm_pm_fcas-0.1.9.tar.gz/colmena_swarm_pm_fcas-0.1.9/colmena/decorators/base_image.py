...
class BaseImage:
    """Decorator Base Image that specifies the image to build from in the Dockerfile."""

    __slots__ = ["__base_image", "__logger"]
    ...
    def __call__(self, func: Callable) -> Callable:
        if func.__name__ in ("__init__", "logic"):
            ...
            def logic(self_, *args, **kwargs):
                ...
                if parent_class_name == "Role":
                    if hasattr(self_, "base_image"):
                        return ReplicatedDecorator("BaseImage", self_.__class__)
                    self_.base_image = self.__base_image
                else:
                    raise WrongClassForDecoratorException(
                        class_name=parent_class_name, dec_name="BaseImage"
                    )
                return func(self_, *args, **kwargs)

        else:
            raise WrongFunctionForDecoratorException(
                func_name=func.__name__, dec_name="BaseImage"
            )
        ...
        if hasattr(logic, "base_image"):
            return ReplicatedDecorator("BaseImage")
        logic.config["base_image"] = self.__base_image
