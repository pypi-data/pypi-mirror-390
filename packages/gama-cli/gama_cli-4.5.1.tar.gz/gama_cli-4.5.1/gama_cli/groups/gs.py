...
DOCKER_GS_CACHE = docker_compose_path("gs/docker-compose.cache.yaml")
...
def _get_compose_files(
    network: Network = Network.SHARED, prod: bool = False, cache: bool = False
) -> List[ValidPath]:
    ...
    if cache:
        compose_files.append(DOCKER_GS_CACHE)
...
@config_dir_option
def build(pull: bool = False, config_dir: str = "~/.config/greenroom", cache: bool = False):
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(cache=cache),
        compose_project_directory=get_project_root(),
        compose_profiles=_get_compose_profiles(config.ui),
        compose_project_name=config.namespace_groundstation,
    )
