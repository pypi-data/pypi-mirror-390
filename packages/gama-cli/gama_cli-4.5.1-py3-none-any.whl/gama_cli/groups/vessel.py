...
DOCKER_VESSEL_CACHE = docker_compose_path("vessel/docker-compose.cache.yaml")
...
def _get_compose_files(
    ...
    if cache:
        compose_files.append(DOCKER_VESSEL_CACHE)
...
@config_dir_option
def build(service: str, args: List[str], pull: bool = False, config_dir: str = "~/.config/greenroom", no_cache: bool = False, cache: bool = False):  # type: ignore
    """Build the vessel"""
    set_initial_env(config_dir)
    config_io = get_vessel_config_io()
    config = config_io.read()
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(variant=config.variant, prod=False, cache=cache),
        compose_project_directory=get_project_root(),
        compose_project_name=config.namespace_vessel,
    )
...
def test_ros(config_dir: str = "~/.config/greenroom"):  # type: ignore
    ...
    config = config_io.read()
    set_env(config)
    # Set config directory permissions in case tests need to write config files
    make_dir_set_permission(config_io.get_path().parent)
    docker = DockerClient(
        compose_files=_get_compose_files(),
        compose_project_directory=get_project_root(),
        compose_project_name=config.namespace_vessel,
...
def test_variant(missim: bool, vcan: bool, config_dir: str = "~/.config/greenroom"):  # type: ignore
    ...
    # Set config directory permissions for parameter persistence to create new files
    make_dir_set_permission(get_vessel_config_io().get_path().parent)
...
def configure(
    ...
    config.cameras = [Camera(name="bow")]
