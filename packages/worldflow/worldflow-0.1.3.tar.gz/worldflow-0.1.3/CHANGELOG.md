...
### Fixed
- **Critical**: Fixed orchestrator to properly detect permanently failed steps and emit `workflow_failed` events.
- Added check for `STEP_FAILED` events with `will_retry=False` in `execute_step()` to propagate failures.
- Workflows now correctly transition to "failed" status when steps exhaust all retry attempts.

### Added
- Comprehensive test suite with 10 tests covering core functionality (basic workflows, multi-step, parallel, retry, sleep/timers)
- Error handling tests to verify failed step detection
