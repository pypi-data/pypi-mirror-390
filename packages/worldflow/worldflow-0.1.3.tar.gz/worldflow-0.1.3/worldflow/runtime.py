...
class Orchestrator:
    ...
    async def execute_step(
        ...
        # Look for StepFailed event (permanent failure after retries)
        failed = self._find_event(
            EventType.STEP_FAILED, 
            lambda e: e.payload.get("step_id") == step_id and not e.payload.get("will_retry", True)
        )
        if failed:
            # Step failed permanently; fail the workflow
            error_msg = failed.payload.get("error", "Step failed")
            traceback_msg = failed.payload.get("traceback", "")
            raise RuntimeError(f"Step {step_id} failed: {error_msg}\n{traceback_msg}")
