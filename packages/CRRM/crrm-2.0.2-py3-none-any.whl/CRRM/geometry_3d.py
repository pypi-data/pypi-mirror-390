# Keith Briggs 2025-09-24 s -> self
...
class Ray:
    ...
    def __init__(self, tail, dv):
        self.tail = np.array(tail, dtype=float)
        self.dv = np.array(dv, dtype=float)
        self.dv /= np.linalg.norm(self.dv)

    def __repr__(self):
        return f"Ray({self.tail},{self.dv})"

    def intersect_triangle(self, t):
        "convenience function"
        return intersect3D_RayTriangle(self, t)

    def distance_to_plane(ray, plane):
        """
        Ray:   r(t)=r0+t*u
        Plane: points q self.t. (q-p0)@v=0
        Intersection: t self.t. (r0+t*u-p0)@v=0
                             t*u@v+(r0-p0)@v=0
                             t*u@v=-(r0-p0)@v
                             t=-(r0-p0)@v/u@v
                             t=(p0-r0)@v/u@v
    ...
    def reflect_in_plane(self, p):
        r0, u = self.tail, self.dv
    ...
    def plot(self, ax, length=1.0, color="b", alpha=0.5):
        """Plots the ray in 3d"""
        if plt is None:
            return
        tip = self.tail + length * self.dv / np.linalg.norm(self.dv)
        x = (self.tail[0], tip[0])
        y = (self.tail[1], tip[1])
        z = (self.tail[2], tip[2])
...
class Triangle:
    ...
    def __init__(self, p0, p1, p2):
        ...
        self.side0 = self.p1 - self.p0
        self.side1 = self.p2 - self.p0
        self.normal = np.cross(self.side0, self.side1)
        self.plane = Plane(self.p0, self.normal)

    def __repr__(self):
        return f"Triangle({self.p0},{self.p1},{self.p1})"

    def __add__(self, c):
        "return a new Triangle, translated by the vector c"
        return Triangle(self.p0 + c, self.p1 + c, self.p2 + c)

    def plot(self, ax, color="y", alpha=0.5, drawedges=True):
...
def intersect3D_RayTriangle(r, t):
    ...
    if abs(b) < 1e-12:  # ray is  parallel to triangle plane
        if abs(a) < 1e-12:
            return r.tail, 0.0  # ray lies in triangle plane
    ...
    s = (uv * wv - vv * wu) / D
    if s < 0.0 or s > 1.0:
        return None, np.inf  # I is outside T
    z = (uv * wu - uu * wv) / D
    if z < 0.0 or s + z > 1.0:
        return None, np.inf  # I is outside T
...
class Panel:
    ...
    def __repr__(self):
        r = ",".join([str(t) for t in self.triangles])
        return f"Panel({r})"

    def __iter__(self):
        return iter(self.triangles)

    def plot(self, ax, color="b", alpha=0.5, drawedges=True):
...
class Building:
    ...
    def __repr__(self):
        r = ",".join([str(p) for p in self.panels])
        return f"Building({r})"
    ...
    def number_of_panels_cut(self, ray, max_distance, dbg=False):
        k, d, dd = 0, 0.0, 0.0
        d_seen = []
        for panel in self.panels:
            panel_cut = False
            for triangle in panel:
                ...
                if dbg:
                    print(f"# I={I} d={d:.2f}", file=stderr)
                if (
                    I is not None and d > 1e-9
                ):  # this triangle is cut (and is not at the tail of the ray)
                    panel_cut, dd = True, d
                    break  # this panel is cut, so we don't need to check other triangles
            if panel_cut:
                if dbg:
                    print(f"{panel} is cut, dd={dd:.2f}", file=stderr)
                if max_distance < dd < np.inf:
                    return k, set(d_seen)
                if all(abs(dd - d) > 1e-6 for d in d_seen):
                    k += 1  # do not count identical panels
                d_seen.append(dd)
            if dbg:
                print(f"# panel_cut={panel_cut} k={k}", file=stderr)
        return k, set(d_seen)
...
def draw_building_3d(
    ...
    if line_segments:
        ax.plot(*line_segments, color="b", marker="o", ms=1, lw=1, alpha=1.0)
    for dot in dots:
...
def test_01():
    ...
    for i in range(10):
        panel = Panel(
            [
                Triangle((i, 0, 0), (i, 1, 0), (i, 1, 1)),
                Triangle((i, 0, 0), (i, 0, 1), (i, 1, 1)),
            ]
        )
        panels.append(panel)
    b = Building(panels)
...
def cube(a, b, c=(0.0, 0.0, 0.0)):
    ...
    return (
        Panel(
            [
                Triangle((a, a, a), (a, b, a), (b, b, a)) + c,
                Triangle((a, a, a), (b, a, a), (b, b, a)) + c,
            ]
        ),
        Panel(
            [
                Triangle((a, a, b), (a, b, b), (b, b, b)) + c,
                Triangle((a, a, b), (b, a, b), (b, b, b)) + c,
            ]
        ),
        Panel(
            [
                Triangle((a, a, a), (a, a, b), (a, b, b)) + c,
                Triangle((a, a, a), (a, b, a), (a, b, b)) + c,
            ]
        ),
        Panel(
            [
                Triangle((b, a, a), (b, a, b), (b, b, b)) + c,
                Triangle((b, a, a), (b, b, a), (b, b, b)) + c,
            ]
        ),
        Panel(
            [
                Triangle((a, a, a), (b, a, a), (b, a, b)) + c,
                Triangle((a, a, a), (a, a, b), (b, a, b)) + c,
            ]
        ),
        Panel(
            [
                Triangle((a, b, a), (b, b, a), (b, b, b)) + c,
                Triangle((a, b, a), (a, b, b), (b, b, b)) + c,
            ]
        ),
    )
...
def block(c0, c1):
    ...
    return (
        Panel(
            [
                Triangle((a, b, c), (d, b, c), (d, b, f)),
                Triangle((a, b, c), (a, b, f), (d, b, f)),
            ]
        ),  # front
        Panel(
            [
                Triangle((a, e, c), (d, e, c), (d, e, f)),
                Triangle((a, e, c), (a, e, f), (d, e, f)),
            ]
        ),  # back
        Panel(
            [
                Triangle((a, b, c), (a, e, c), (a, e, f)),
                Triangle((a, b, c), (a, b, f), (a, e, f)),
            ]
        ),  # one side
        Panel(
            [
                Triangle((d, b, c), (d, e, c), (d, e, f)),
                Triangle((d, b, c), (d, b, f), (d, e, f)),
            ]
        ),  # opposite side
        Panel(
            [
                Triangle((a, b, c), (d, b, c), (d, e, c)),
                Triangle((a, b, c), (d, e, c), (a, e, c)),
            ]
        ),  # floor
        Panel(
            [
                Triangle((a, b, f), (d, b, f), (d, e, f)),
                Triangle((a, b, f), (d, e, f), (a, e, f)),
            ]
        ),  # ceiling
    )
...
def test_04(dbg=False, fontsize=4):
    ...
    for dist in dists:  # plot intersections...
        ...
        print(dist, x, y, z)
    if dbg:
        print(f"r={r} distance_to_plane={d} number_of_panels_cut={k}")
...
def test_05():
    ...
    draw_building(
        ...
        color="y",
        limits=[(0, 10), (0, 20), (0, 4)],
        labels=["$x$", "$y$", "$z$"],
        fontsize=6,
        show=True,
        pdffn="img/building0.pdf",
        pngfn="img/building0.png",
    )
