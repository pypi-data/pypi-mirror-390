...
class PRaster():
    ...
    def __init__(self, path=""):
        ...
        if path:
            ...
            self._banda = self._raster.GetRasterBand(1)
            self._size = (self._banda.XSize, self._banda.YSize)
            self._geot = self._raster.GetGeoTransform()
            self._proj = self._raster.GetProjection()
    ...
    def getSize(self):
        """
        Returns a tuple with the size of the grid (XSize, YSize)
        """
        return self._size
    
    def getDims(self):
        """
        Returns a tuple with the size of the internal array (nrow, ncol)
        """
        return self._size[::-1]
    ...
    def copyLayout(self, grid):
        ...
        self._geot = grid.getGeot()
        self._proj = grid.getCRS()
    ...
    def indToCell(self, ind):
        ...
        **tuple** : Tuple with (row, col) indices as numpy.ndarrays
        """
        return np.unravel_index(ind, self.getDims()) 
    ...
    def cellToInd(self, row, col):
        ...
        **numpy.array** : Array with linear indexes (row-major, C-style)
        """
        return np.ravel_multi_index((row, col), self.getDims())
...
class Grid(PRaster):
    ...
    def __init__(self, path="", band=1):
        ...
        if path:
            self._nodata = self._banda.GetNoDataValue()
            self._array = self._banda.ReadAsArray()
    ...
    def setNodata(self, value):
        ...
        # And change old nodata values with the new value
            ...
            self._array[self.getNodataPos()] = value
            self._nodata = value
            
    def getNodataPos(self):
        """
...
class DEM(Grid):
        ...
        **tuple** : Tuple (flats, sills) with two landspy.Grid (as_array=False) or numpy.ndarray (as_array=True)
        ...
        if as_array:
            ...
            for arr in [flats, sills]:
                ...
                grid.setNodata(-1)
                grid._tipo = 'int8'
                grid.setArray(arr.astype(np.int8))
    ...
    def fill2(self, four_way=False):
        ...
        filled_dem.setArray(output_array)
        filled_dem.setNodata(self._nodata)
        return filled_dem
...
class Basin(DEM):
    ...
    def __init__(self, dem, basin=None, idx=1):
        ...
        #dem = DEM(dem, 1)
        basin = np.where(basingrid.readArray()==idx, 1, 0)
        
        if basingrid.getSize() != dem.getSize():
        ...
        dem_cl = dem.readArray()[r1:r2, c1:c2]
