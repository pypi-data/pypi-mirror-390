...
class Flow(PRaster):
    ...
    def save(self, path):
        ...
        * Band 3 --> Elevation of givers reshaped to self._dims)
        ...
        no_cells = self.getNCells() - len(self._ix)
        ...
        ix = ix.reshape(self.getDims())
        ixc = ixc.reshape(self.getDims())
        zx = zx.reshape(self.getDims())
    ...
    def flowAccumulation(self, weights=None, nodata=True, asgrid=True):
        """
        ...
        if weights:
            if self._geot == weights._geot and self._size == weights._size:
                ...
                for n in ix_ixc:
                    row, col = self.indToCell(n)
                    x, y = self.cellToXY(row, col)
                    Wcell = weights.xyToCell(x, y)
                    value = weights.getValue(Wcell[0], Wcell[1])
                    facc[n] = value
            else:
    ...
    def streamPoi(self, threshold, kind="heads", coords="CELL"):
        """
        ...
        fac = self.flowAccumulation(nodata=False, asgrid=False)
        w = fac > threshold
        w = w.ravel()
        I   = w[self._ix]
        ix  = self._ix[I]
        ...
        sp_arr = csc_matrix((aux_vals, (ix, ixc)), shape=(self.getNCells(), self.getNCells()))
    ...
    def drainageBasins(self, outlets=None, min_area = 0.005, asgrid=True):
        """
        ...
        Usage:
        =====
        basins = fd.drainageBasins() # Extract all the basins in the Flow object with area larger than 0.05% of the number of pixels
        basins = fd.drainageBasins(min_area=0.0) # Extract all the possible basins in the Flow object
        basins = fd.drainageBasins([520359.7, 4054132.2]) # Extract the basin for the specified outlet
        xi = [520359.7, 519853.5, 510646.5]
        yi = [4054132.2, 4054863.5, 4054643.5]
        outlets = np.array((xi, yi)).T
        basins = fd.drainageBasins(outlets) # Create basins at positions indicated by xi and yi
        ...
        if outlets is None and min_area > 0:
            threshold = int(self.getNCells() * min_area)
            inds = self.streamPoi(threshold, kind="outlets", coords="IND")
            basin_ids = np.arange(inds.size) + 1
        
        elif isinstance(outlets, np.ndarray):
            if not np.all(self.isInside(outlets[:,0], outlets[:,1])):
                raise FlowError("Some outlets coordinates are outside the grid")                                         
            row, col = self.xyToCell(outlets[:,0], outlets[:,1])
            inds = self.cellToInd(row, col)
            ...
            if not self.isInside(outlets[0], outlets[1]):
                raise FlowError("Some outlets coordinates are outside the grid") 
            row, col = self.xyToCell(outlets[0], outlets[1])
            inds = self.cellToInd(row, col)
            basin_ids = np.arange(inds.size) + 1
        ...
        if inds.size == 0:
            ...
            temp_ixc = self._ixc
            nbasins = 0
            basin_arr = np.zeros(self.getNCells(), np.int64)
            nix = len(temp_ix)
        ...
        # Reshape and return
        basin_arr = basin_arr.reshape(self.getDims())  
    ...
    def snapPoints(self, input_points, threshold, kind="channel", remove_duplicates=False):
        ...
        if kind in ['heads', 'confluences', 'outlets']:
            poi = self.streamPoi(threshold, kind, "XY")         
        else:
            fac = self.flowAccumulation(nodata=False, asgrid=False)
...
def sort_pixels(dem, auxtopo=False, filled=False, verbose=False, verb_func=print, order="C"):
    ...
    flats, sills = fill.identifyFlats(as_array=True)
    del(fill)
    if verbose:
