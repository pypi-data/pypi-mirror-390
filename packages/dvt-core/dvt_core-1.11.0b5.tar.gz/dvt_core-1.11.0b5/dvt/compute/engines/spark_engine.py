...
class SparkEngine(BaseComputeEngine):
    ...
    def _register_source(self, profile_name: str, adapter_type: str, relation: Any) -> None:
        ...
        Uses dbt adapter to extract data, converts to Arrow, then loads into Spark:
        1. Get dbt adapter for profile
        2. Execute SELECT * FROM table via adapter → Agate
        3. Convert Agate table to Arrow
        4. Convert Arrow to Pandas DataFrame
        5. Create Spark DataFrame from Pandas
        6. Register as temp view
        ...
        # Convert Arrow → Pandas → Spark DataFrame
        pandas_df = arrow_table.to_pandas()
        spark_df = self.spark.createDataFrame(pandas_df)

        # Register as temp view
        view_name = f"{profile_name}_{relation.identifier}"
        spark_df.createOrReplaceTempView(view_name)

        fire_event(
            Note(
                msg=f"Registered {relation} from {profile_name} as '{view_name}' in Spark "
