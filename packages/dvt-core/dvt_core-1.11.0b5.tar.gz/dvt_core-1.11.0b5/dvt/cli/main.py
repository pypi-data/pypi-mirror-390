...
# dvt profiles register
@profiles.command("register")
@click.argument("profile_name", required=False)
@click.pass_context
@global_flags
@p.profiles_dir
...
@requires.postflight
@requires.preflight
def profiles_register(ctx, profile_name, **kwargs):
    """Interactively register a new database profile"""
    from dvt.task.profile_register import ProfileRegisterTask

    task = ProfileRegisterTask(ctx.obj["flags"], profile_name)
    results = task.run()
    success = task.interpret_results(results)
    return results, success
...
# dvt compute ui
@compute.command("ui")
@click.option("--open", is_flag=True, help="Open UIs in browser")
@click.pass_context
@global_flags
@p.profiles_dir_exists_false
@p.project_dir
@requires.postflight
@requires.preflight
def compute_ui(ctx, open, **kwargs):
    """Check and display compute engine UIs and metrics endpoints"""
    from dvt.task.compute import ComputeUITask

    task = ComputeUITask(ctx.obj["flags"], open_browser=open)
    results = task.run()
    success = task.interpret_results(results)
    return results, success
