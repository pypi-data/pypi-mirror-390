...
class ExecutionRouter:
    ...
    def __init__(
        self,
        multi_adapter_manager: Any = None,
        compute_config: Optional[Dict[str, Any]] = None,
        available_engines: Optional[Dict[str, BaseComputeEngine]] = None,
    ):
        """
        Initialize execution router.

        Args:
            multi_adapter_manager: Manager for multiple dbt adapters (for UnifiedAdapter)
            compute_config: Compute configuration dict or ComputeConfig object
        ...
        # Handle compute_config as dict or object
        if compute_config is None:
            compute_config = {}

        if isinstance(compute_config, dict):
            # Create default config from dict
            from dvt.config.compute_config import ComputeConfig, AutoSelectConfig

            self.compute_config = ComputeConfig(
                default_engine=compute_config.get('default_engine', 'auto'),
                auto_select=AutoSelectConfig(enabled=False, rules=[]),
            )
        else:
            self.compute_config = compute_config

        self.auto_select_config = self.compute_config.auto_select

    def cleanup(self):
        """Clean up all engines and close connections."""
    ...
    def execute(
        ...
        This method is called by UnifiedAdapter after strategy selection.
        It routes the execution to the appropriate compute engine or adapter.

        Args:
            execution_plan: Execution plan with selected strategy and SQL
            sql: Optional SQL query (for backwards compatibility)

        Returns:
            ComputeResult with execution results

        Example:
            >>> router = ExecutionRouter(...)
            >>> plan = QueryExecutionPlan(strategy=ExecutionStrategy.COMPUTE_LAYER, ...)
            >>> result = router.execute(plan)
        """
        # Get SQL from plan or parameter
        query_sql = sql or getattr(execution_plan, 'sql', None)
        if not query_sql:
            return ComputeResult(
                success=False,
                error="No SQL provided for execution",
            )

        # Select strategy if not already selected
        if execution_plan.strategy == ExecutionStrategy.AUTO:
            execution_plan = self.select_strategy(execution_plan)
        ...
        return engine.execute_query(query_sql, execution_plan)
