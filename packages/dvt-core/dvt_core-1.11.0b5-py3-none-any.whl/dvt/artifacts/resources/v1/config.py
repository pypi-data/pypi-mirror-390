...
class NodeConfig(NodeAndTestConfig):
    ...
    # These fields enable DVT's cross-database query and multi-target capabilities

    compute: Optional[str] = field(
        default=None,
        metadata={
            "merge": MergeBehavior.Clobber,
            "description": "Override compute engine for this model. Options: 'duckdb', 'spark'. "
                          "Example: {{ config(compute='spark') }}"
        },
    )
    ...
    target: Optional[str] = field(
        ...
        metadata={
            "merge": MergeBehavior.Clobber,
            "description": "Override target profile for materialization. Allows materializing "
                          "to a different database than the default target. "
                          "Example: {{ config(target='postgres_analytics') }}"
        },
    )
    ...
    pushdown_enabled: Optional[bool] = field(
        ...
        metadata={
            "merge": MergeBehavior.Clobber,
            "description": "Enable/disable query pushdown optimization. When True, DVT attempts "
                          "to execute queries directly on the source database instead of using "
                          "the compute layer."
        },
    )
