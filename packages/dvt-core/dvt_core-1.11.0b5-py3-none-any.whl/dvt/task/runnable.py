...
class GraphRunnableTask(ConfiguredTask):
    ...
    def _init_dvt_execution_layer(self) -> None:
        ...
        - QueryAnalyzer: Analyze SQL queries for routing
        - ExecutionRouter: Route queries to optimal execution engine
        - UnifiedAdapter: Main entry point for query execution
        ...
        from dvt.adapters.unified import create_unified_adapter
        ...
        try:
            ...
            # Fall back to default target if no unified profiles
            self.unified_profiles = {}

        # Get default profile from config
        default_profile = getattr(self.config, 'target_name', 'default')

        # Create UnifiedAdapter (sets up query analyzer, router, multi-adapter manager)
        try:
            self.unified_adapter = create_unified_adapter(
                profiles=self.unified_profiles or {},
                default_profile=default_profile,
                compute_config=load_compute_config(project_path) if project_path.exists() else None,
            )

            # Expose components for backward compatibility
            self.multi_adapter_manager = self.unified_adapter.multi_adapter_manager
            self.execution_router = self.unified_adapter.execution_router
            self.query_analyzer = self.unified_adapter.query_analyzer
        except Exception as e:
            # Fall back to standard dbt adapter if UnifiedAdapter fails
            fire_event(Formatting(msg=f"Warning: Could not initialize UnifiedAdapter: {e}"))
            fire_event(Formatting(msg="Falling back to standard dbt adapter mode"))
