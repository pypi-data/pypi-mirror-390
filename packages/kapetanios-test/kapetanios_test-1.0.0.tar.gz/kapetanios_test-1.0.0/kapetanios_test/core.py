...
class KapetaniosTest:
    ...
    def _estimate_model(
        ...
        k_opt = self._select_lags(y, break_dates, k_max)
        ...
        if k_opt > 0:
            for i in range(1, k_opt + 1):
                if i < len(dy):
                    dy_lag = np.concatenate([np.zeros(i), dy[:-i]])
                    X = np.column_stack([X, dy_lag[:-1]])
        
        # Ensure X and dy have same length
        X = X[:len(dy)]
    ...
    def _select_lags(
        ...
        if self.lag_selection == 't-stat':
            ...
            for k in range(k_max, 0, -1):
                _, t_stat, _ = self._estimate_model(y, break_dates, k)
                # Last coefficient is Δy_{t-k}
                if abs(t_stat) > 1.96:  # 5% critical value
                    return k
            return 0
