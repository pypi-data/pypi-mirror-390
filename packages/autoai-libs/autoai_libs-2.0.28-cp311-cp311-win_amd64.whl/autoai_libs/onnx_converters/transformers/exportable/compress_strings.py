...
def compress_strings_shape_calculator(operator: Operator):
    pass


def compress_strings_converter(scope: Scope, operator: Operator, container: ModelComponentContainer):
...
def compress_strings_parser(
    ...
    raw_operator: CompressStrings = this_operator.raw_operator

    # outputs
    for i, inpt in enumerate(this_operator.inputs):
        if (
            raw_operator.activate_flag
            and raw_operator.dtypes_list[i] == "char_str"
            and raw_operator.compress_type != "string"
        ):
