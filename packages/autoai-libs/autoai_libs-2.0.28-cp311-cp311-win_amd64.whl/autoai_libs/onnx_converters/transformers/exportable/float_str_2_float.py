...
def float_str_2_float_shape_calculator(operator: Operator):
    pass


def float_str_2_float_converter(scope: Scope, operator: Operator, container: ModelComponentContainer):
...
def float_str_2_float_parser(
    ...
    raw_operator: FloatStr2Float = this_operator.raw_operator

    # outputs
    for i, inpt in enumerate(this_operator.inputs):
        if raw_operator.activate_flag and raw_operator.dtypes_list[i] == "float_str":
