...
################################################################################
import numpy as np
from onnxconverter_common import apply_identity, apply_concat
...
def date_transformer_shape_calculator(operator: Operator):
    pass


def date_transformer_converter(scope: Scope, operator: Operator, container: ModelComponentContainer):
...
def date_transformer_parser(
    ...
    raw_operator: DateTransformer = this_operator.raw_operator

    # outputs
    if raw_operator.activate_flag:
        filtered_inputs = [
            inpt
            for i, inpt in enumerate(inputs)
            if not (i in raw_operator.date_column_indices and raw_operator.delete_source_columns)
        ]

        _, l_column_headers_list = apply_date_aggregations(
            X=None,
            date_column_indices=raw_operator.date_column_indices,
            options=raw_operator.options,
            delete_source_columns=raw_operator.delete_source_columns,
            column_headers_list=(
                raw_operator.column_headers_list if raw_operator.column_headers_list else this_operator.input_full_names
            ),
            float32_processing_flag=raw_operator.float32_processing_flag,
        )
        selected_indices = [i for i in range(len(l_column_headers_list))]

        # Column selector part
        if raw_operator.column_headers_list and len(l_column_headers_list) != len(raw_operator.column_headers_list):
            selected_indices = raw_operator.column_selector.columns_indices_list
            new_column_headers_list = [l_column_headers_list[i] for i in selected_indices]

            if len(new_column_headers_list) != len(raw_operator.column_headers_list):
                raw_operator.new_column_headers_list = new_column_headers_list
                raw_operator.columns_added_flag = True
            else:
                raw_operator.new_column_headers_list = raw_operator.column_headers_list
                raw_operator.columns_added_flag = False

        else:
            raw_operator.new_column_headers_list = (
                l_column_headers_list if l_column_headers_list else raw_operator.column_headers_list
            )
            raw_operator.columns_added_flag = False

        end_of_basic_columns = len(this_operator.inputs)
        if raw_operator.delete_source_columns:
            end_of_basic_columns -= len(raw_operator.date_column_indices)
