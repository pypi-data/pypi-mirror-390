...
def get_curve(
    ...
    curve_exponent : exponent on |sin|
    amp_decay_base : amplitude decay factor in log-frequency
    """
    X = np.asarray(X, float)
    harmonic = float(harmonic)

    base = np.sin(harmonic/2 * X + np.pi)  # SHIFT BY π
...
# ):
#     """
#     "Linear" amplitude version of the curve:

#       - same phase and nonlinear |sin|^curve_exponent shaping,
#       - but amplitude decays approximately linearly in log-frequency:

#         amp = 1 - log2(harmonic) / log2(max_harmonic * 2)
#     """
#     X = np.asarray(X, float)
#     harmonic = float(harmonic)
...
def get_curve(
    ...
    base = np.cos(harmonic/2 * X + np.pi)
    shaped = np.sign(base) * np.abs(base) ** curve_exponent
    amp = amp_decay_base ** np.log2(harmonic)
    return height_scale * shaped * amp
...
def get_curve_linear(
    ...
    base = np.sin(harmonic * X + np.pi)
    shaped = np.sign(base) * np.abs(base) ** curve_exponent
    amp = 1.0 - (np.log2(harmonic) / np.log2(max_harmonic * 2.0))
    return height_scale * shaped * amp
...
def cogwheel(
    ...
    curve_mode:
        "nonlinear" (default) → use get_curve + radial region labels.
        "linear"              → use get_curve_linear + visual-center labels, no rotation.
    """
    if radius is None:
        # fall back to rectangular version; curve_mode does not matter there
        return sine_diagram(
            values,
            class_names,
            colors=colors,
    ...
    if curve_mode not in ("nonlinear", "linear"):
        raise ValueError(f"Unsupported curve_mode {curve_mode!r}; use 'nonlinear' or 'linear'.")

    curve_fn = get_curve_linear if curve_mode == "linear" else get_curve
    ...
    x_min, x_max = 0.0, 2.0 * np.pi
    y_min, y_max = -float(height_scale) * 1.25, float(height_scale) * 1.25
