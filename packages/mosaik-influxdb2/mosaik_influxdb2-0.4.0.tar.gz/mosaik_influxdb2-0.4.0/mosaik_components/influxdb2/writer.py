...
META = {
    ...
    "extra_methods": ["register_src_types"],
}
...
class Simulator(mosaik_api.Simulator):
    ...
    _src_sim_tag: str | None = None
    _src_entity_tag: str | None = None
    _src_type_tag: str | None = None
    _src_types: Dict[str, str] = {}
    ...
    def init(
        ...
        start_date=None,
        step_size=900,
        src_sim_tag: str | None = "src_sim",
        src_entity_tag: str | None = "src_entity",
        src_type_tag: str | None = "src_type",
    ):
        ...
        self._src_sim_tag = src_sim_tag
        self._src_entity_tag = src_entity_tag
        self._src_type_tag = src_type_tag
    ...
    def register_src_types(self, src_types: Dict[str, str]):
        self._src_types.update(src_types)

    def step(self, time, inputs, max_advance):
        data = inputs.get(EID, {})
        logger.trace(f"stepping at time {time} with inputs {inputs}")
        ...
        for attr, src_ids in data.items():
            for src_id, val in src_ids.items():
                ...
                record = (
                    ...
                    .field(attr, float(val) if isinstance(val, int) else val)
                    .time(timestamp)
                )
                if self._src_sim_tag:
                    record.tag(self._src_sim_tag, src_sim)
                if self._src_entity_tag:
                    record.tag(self._src_entity_tag, src_entity)
                if self._src_type_tag:
                    record.tag(self._src_type_tag, self._src_types.get(src_id))
