...
class OneHotDecoder:
    ...
    def decode(df: pd.DataFrame) -> (pd.DataFrame, dict[str, list[str]], list[str]):
        """
        ...
        """
        one_hot_encoded_groups_candidates = OneHotDecoder._group_one_hot_fast(df.columns)
        one_hot_encoded_groups: dict[str, list[str]] = {}

        # 1) Detect valid one-hot groups (filter candidates by column-level checks)
        for group_name, column_candidates in one_hot_encoded_groups_candidates.items():
            ...
            if len(group_columns) > 1:
                one_hot_encoded_groups[group_name] = group_columns

        # 2) Transform: replace each detected group with one categorical column
        if not one_hot_encoded_groups:
            return df, one_hot_encoded_groups, []

        result_df = df.copy()
        pseudo_one_hot_encoded_group_names: list[str] = []
        pseudo_one_hot_encoded_features: list[str] = []
        for group_name, group_columns in one_hot_encoded_groups.items():
            ...
            if any(sub.sum(axis=1) > 1):
                pseudo_one_hot_encoded_group_names.append(group_name)
                pseudo_one_hot_encoded_features.extend(group_columns)
        ...
        for group_name in pseudo_one_hot_encoded_group_names:
            del one_hot_encoded_groups[group_name]

        return result_df, one_hot_encoded_groups, pseudo_one_hot_encoded_features

    def decode_with_cached_groups(
        df: pd.DataFrame, one_hot_encoded_groups: dict[str, list[str]], pseudo_one_hot_encoded_features: list[str]
    ) -> pd.DataFrame:
        """
        ...
        if pseudo_one_hot_encoded_features:
            result_df[pseudo_one_hot_encoded_features] = result_df[pseudo_one_hot_encoded_features].astype("string")
        return result_df
