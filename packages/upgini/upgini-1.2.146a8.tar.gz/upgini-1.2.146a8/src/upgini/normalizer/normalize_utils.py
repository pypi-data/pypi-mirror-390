...
class Normalizer:
    ...
    def __init__(
        ...
        self.onehot_encoded_groups: dict[str, list[str]] | None = None
        self.pseudo_one_hot_encoded_features: list[str] | None = None
    ...
    def _convert_one_hot_encoded_columns(self, df: pd.DataFrame):
        if self.onehot_encoded_groups is not None or self.pseudo_one_hot_encoded_features is not None:
            df = OneHotDecoder.decode_with_cached_groups(
                df, self.onehot_encoded_groups, self.pseudo_one_hot_encoded_features
            )
        else:
            df, self.onehot_encoded_groups, self.pseudo_one_hot_encoded_features = OneHotDecoder.decode(df)
        return df
