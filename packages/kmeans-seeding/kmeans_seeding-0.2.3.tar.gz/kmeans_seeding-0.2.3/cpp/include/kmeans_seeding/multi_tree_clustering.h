...
class MultiTreeClustering {
 public:
  ...
  unsigned long long ComputeCostAndOpen(int center, bool open_center);
  ...
  vector<unsigned long long> distance_to_center;

 private:
  void UpdateDistance(pair<int, unsigned long long> update, int left, int right,
                      int binary_tree_id);

  // Recursive function to sample points based on binary trees.
  // Improves the runtime to log n.
  int SampleAPointRecurse(unsigned long long chosen_prob, int left, int right,
                          int binary_tree_id);
  ...
  vector<unsigned long long> binary_tree_value_;
