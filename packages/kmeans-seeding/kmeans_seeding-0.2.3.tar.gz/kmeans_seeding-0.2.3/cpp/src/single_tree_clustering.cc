...
vector<pair<int, unsigned long long>> SingleTreeClustering::ComputeCostAndOpen(
    int center, bool open_center) {
  // The new distances if this center gets opened.
  vector<pair<int, unsigned long long>> updated_distances;
    ...
    for (auto point : tree_.points_in_node[node]) {
      if (updated_nodes.find(point) == updated_nodes.end()) {
        updated_distances.push_back(
            pair<int, unsigned long long>(point, static_cast<unsigned long long>(1) << (2 * i)));
