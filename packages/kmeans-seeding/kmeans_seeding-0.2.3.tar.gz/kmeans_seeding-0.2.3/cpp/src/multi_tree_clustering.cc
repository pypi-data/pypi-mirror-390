...
void MultiTreeClustering::InitializeTree(const vector<vector<double>>& input,
  ...
  distance_to_center =
      vector<unsigned long long>(input.size(), std::numeric_limits<unsigned long long>::max());
  ...
  binary_tree_value_ = vector<unsigned long long>(2 * binary_tree_boundery_,
                                        std::numeric_limits<unsigned long long>::max());
}

unsigned long long MultiTreeClustering::ComputeCostAndOpen(int center, bool open_center) {
  // To keep the previous costs in case open_center is false.
  map<int, unsigned long long> old_costs;
  // The amount of the improment that opening this center gives.
  unsigned long long improvement = 0;
  // This the first time so there is in previous cost so improvement is not
  // defined.
  if (!open_center &&
      distance_to_center[0] == std::numeric_limits<unsigned long long>::max())
    return improvement;
  for (int i = 0; i < single_trees_.size(); i++)
    for (pair<int, unsigned long long>& update :
...
void MultiTreeClustering::UpdateDistance(pair<int, unsigned long long> update, int left,
...
int MultiTreeClustering::SampleAPoint() {
  ...
  if (binary_tree_value_[1] == std::numeric_limits<unsigned long long>::max()) {
    return RandomHandler::eng() % number_of_points_;
  }
  unsigned long long chosen_prob = RandomHandler::eng() % binary_tree_value_[1];
  return SampleAPointRecurse(chosen_prob, 0, binary_tree_boundery_, 1);
}

int MultiTreeClustering::SampleAPointRecurse(unsigned long long chosen_prob, int left,
                                             int right, int binary_tree_id) {
