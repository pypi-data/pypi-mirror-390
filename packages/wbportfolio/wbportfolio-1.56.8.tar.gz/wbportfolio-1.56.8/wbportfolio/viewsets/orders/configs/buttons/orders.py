...
class ExecutionInstructionSerializer(wb_serializers.Serializer):
    execution_instruction_type = wb_serializers.ChoiceField(
        choices=ExecutionInstruction.choices, default=ExecutionInstruction.MARKET_ON_CLOSE.value
    )
    ...
    percentage = wb_serializers.FloatField(
        ...
        depends_on=[
            {
                "field": "execution_instruction_type",
    ...
    def get_custom_list_instance_buttons(self) -> set:
        return {
            bt.ActionButton(
                ...
                label="Change Execute Instruction",
                action_label="Change Execute Instruction",
                description_fields="<p>Change Execution Instruction.</p>",
                serializer=ExecutionInstructionSerializer,
                instance_display=create_simple_display(
                    [
                        ["execution_instruction_type", "execution_instruction_type"],
