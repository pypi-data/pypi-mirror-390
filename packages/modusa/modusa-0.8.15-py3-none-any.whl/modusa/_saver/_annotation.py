...
class AnnotationSaver:
    ...
    def _ctm_label(ann, outfp, segment_id, channel):
        ...
        with open(outfp, "w") as f:
            for s, e, label in ann:
                dur = e - s
                f.write(f"{segment_id} {channel} {s:.6f} {dur:.6f} {label}\n")
    ...
    def _textgrid_label(ann, outfp, tier_name="labels"):
        ...
        xmin = min(s for s, _, _ in ann) if ann else 0.0
        xmax = max(e for _, e, _ in ann) if ann else 0.0
        ...
        with open(outfp, "w") as f:
            ...
            for i, (s, e, label) in enumerate(ann, start=1):
