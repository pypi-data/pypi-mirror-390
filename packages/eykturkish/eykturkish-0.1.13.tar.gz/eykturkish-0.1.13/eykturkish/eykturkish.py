...
def randomsayi(min,max):
    return random.randint(min,max)
def secretrandomsayi(max_deger: int):
    return secrets.randbelow(max_deger + 1)
def listedenrandom(liste: list):
    return random.choice(liste)
def secretlisterandom(liste: list):
    if not liste:
        return None
    index = secrets.randbelow(len(liste))  # 0 ile len(liste)-1 arası güvenli sayı
    return liste[index]
...
def eposta_dogrula(eposta: str) -> bool:
    ...
    """
    # Temel e-posta regex'i
    regex = r'^[a-z0-9]+[\._-]?[a-z0-9]+@[a-z0-9-]+\.[a-z]{2,}$'
    # ASCII kontrolü
    ascii_uyumlu = all(ord(c) < 128 for c in eposta)
    tld_liste = [
        "com", "org", "net", "edu", "gov", "io", "co", "info", "biz", "me",
        "us", "uk", "de", "fr", "jp", "au", "ru", "cn", "in",
        "app", "shop", "tech", "online", "site", "store", "blog", "xyz",
        "life", "club", "world", "space", "ai", "cloud", "digital", "design",
        "news", "tv", "mobi", "pro", "name", "studio", "agency", "solutions",
        "systems", "software", "center", "group", "network", "services", "company",
        "consulting", "media", "events", "education", "school", "academy", "health",
        "fitness", "travel", "restaurant", "cafe", "photography", "graphics", "fashion",
        "music", "film", "video", "games", "social", "wiki", "finance", "bank",
        "insurance", "investments", "marketing", "legal", "law", "security", "chat",
        "community", "technology", "engineering", "science", "bio", "green", "energy",
        "art", "market", "fun", "cool", "today", "tips", "guide", "expert", "guru",
        "reviews", "training", "courses", "career", "portfolio", "gallery", "lab",
        "research", "ventures", "capital", "fund", "exchange", "trading", "crypto",
        "blockchain", "bitcoin", "nft", "token","illegal"
    ]
    return re.match(regex, eposta, re.IGNORECASE) is not None and ascii_uyumlu and "." in eposta and eposta.split("@")[-1].split(".")[-1] in tld_liste
