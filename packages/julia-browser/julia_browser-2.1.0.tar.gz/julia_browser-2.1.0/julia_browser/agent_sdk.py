...
class AgentSDK:
    ...
    def proxy_get_traffic(self, limit: int = 50, filter: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Get recent HTTP traffic logs with optional filtering
        
        Args:
            limit: Maximum number of traffic entries to return
            filter: Optional filter dict, e.g., {"url_contains": "api", "method": "POST"}
            
        Returns:
            Dict with traffic log entries
        """
        try:
            traffic = self.engine.proxy.get_traffic_log(limit=limit, filter_dict=filter)
            return {
                'success': True,
    ...
    def proxy_add_rule(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
        Add a simple declarative proxy rule - super easy for AI agents!
        
        Args:
            rule: Rule configuration as dictionary
                {
                    "name": "auth_injector",
                    "type": "request",  # or "response"
                    "match": {
                        "url_contains": "api",  # optional
                        "method": "GET",  # optional
                        "header_equals": {"Content-Type": "application/json"}  # optional
                    },
                    "actions": {
                        "set_headers": {"Authorization": "Bearer token123"},
                        "set_method": "POST",  # optional
                        "replace_body": "new body"  # optional
                    }
                }
        ...
        """
        try:
            result = self.engine.proxy.add_declarative_rule(rule)
            return result
        except Exception as e:
            return {'success': False, 'error': str(e)}
    
    def proxy_remove_rule(self, rule_name: str) -> Dict[str, Any]:
        """
        Remove a proxy rule by name
        ...
        """
        try:
            result = self.engine.proxy.remove_rule(rule_name)
            return result
        except Exception as e:
            return {'success': False, 'error': str(e)}
    
    def proxy_list_rules(self) -> Dict[str, Any]:
        """
        List all active proxy rules
        
        Returns:
            Dict with list of all rules
        """
        try:
            rules = self.engine.proxy.list_rules()
            return {
                'success': True,
