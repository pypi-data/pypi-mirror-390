...
### Added - Super Simple Declarative Proxy API for AI Agents! 🎯
- **Declarative Rule System**: AI agents can now configure proxy using simple dictionaries - NO Python functions needed!
- **proxy_add_rule()**: Add interception rules with JSON-like configuration
- **proxy_remove_rule()**: Remove rules by name
- **proxy_list_rules()**: List all active rules
- **Traffic Filtering**: `proxy_get_traffic()` now supports filter parameter for querying specific traffic
- **Complete Match Conditions**: url_contains, url_matches, method, header_equals, header_contains, status_code, content_type
- **Complete Actions**: set_headers, remove_headers, set_method, rewrite_url, replace_body, append_body, find_replace, block_request
- **Request Blocking**: block_request action prevents requests from being sent (returns 403)
- **Body Modifications**: find_replace works on both request and response bodies

### Changed
- **Proxy API Redesign**: Callback-based interceptors moved to "advanced" category, declarative rules now recommended for AI agents
- **README Documentation**: Complete rewrite of proxy section with simple, AI-friendly examples
- **Zero Breaking Changes**: All existing proxy functions still work - fully backward compatible

### Technical Details
- DeclarativeRule class handles rule matching and application
- BrowserEngine checks for blocked requests before HTTP dispatch
- Thread-safe rule storage and execution
