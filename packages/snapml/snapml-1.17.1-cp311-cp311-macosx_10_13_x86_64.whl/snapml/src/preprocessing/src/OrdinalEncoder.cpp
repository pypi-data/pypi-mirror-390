...
OrdinalEncoder::OrdinalEncoder(const snapml::OrdinalEncoder::Params _params)
    : params(_params)
{
}

void OrdinalEncoder::transform(snapml::AnyDataset& dataset)

{
    ...
    if (index != params.categories.size())
        throw std::runtime_error("Input has " + std::to_string(index) + " features, but OrdinalEncoder is expecting "
    ...
    for (uint32_t i : index_list) {               // feature
        for (uint32_t j = 0; j < X.size(); j++) { // transaction
            ...
            if (it != params.categories[index].end()) {
                ...
                ENC[j][i].push_back(params.unknown_value);
        }
        index++;
    }
}
