...
class CustomerOrigins:
    ...
    def summary(self):
        ...
        if metadata["marketsize_col"] is None:
            print(f"{helper.DEFAULT_NAME_MARKETSIZE} column         not defined")
        else:
            print(f"{helper.DEFAULT_NAME_MARKETSIZE} column         " + metadata["marketsize_col"])
        
        if metadata["weighting"][0]["func"] is None and metadata["weighting"][0]["param"] is None:
            print(f"{helper.DEFAULT_NAME_TC} weighting    not defined")
        elif metadata["weighting"][0]["func"] in ["power", "exponential"]:
            print(f"{helper.DEFAULT_NAME_TC} weighting   " + str(round(metadata["weighting"][0]["param"],3)) + " (" + helper.PERMITTED_WEIGHTING_FUNCTIONS[metadata["weighting"][0]["func"]]["description"] + ")")
        elif metadata["weighting"][0]["func"] == "logistic":
            print(f"{helper.DEFAULT_NAME_TC} weighting   " + str(round(metadata["weighting"][0]["param"][0],3)) + ", " + str(round(metadata["weighting"][0]["param"][1],3)) + " (" + helper.PERMITTED_WEIGHTING_FUNCTIONS[metadata["weighting"][0]["func"]]["description"] + ")")
...
class SupplyLocations:
    ...
    def summary(self):
        ...
        if metadata["attraction_col"][0] is None or metadata["attraction_col"] == []:
            print(f"{helper.DEFAULT_NAME_ATTRAC} column(s)  not defined")
        else:
            print(f"{helper.DEFAULT_NAME_ATTRAC} column(s)  " + ",".join(metadata["attraction_col"]))
        
        if metadata["weighting"][0]["func"] is None and metadata["weighting"][0]["param"] is None:
            print(f"{helper.DEFAULT_NAME_ATTRAC} weighting  not defined")
        else:
            print(f"{helper.DEFAULT_NAME_ATTRAC} weighting  " + helper.PERMITTED_WEIGHTING_FUNCTIONS[metadata["weighting"][0]["func"]]["description"] + " with gamma = " + str(round(metadata["weighting"][0]["param"],3)))
    ...
    def define_attraction_weighting(
        ...
        if metadata["attraction_col"] is None:
            raise ValueError(f"Error while defining attraction weighting: {helper.DEFAULT_NAME_ATTRAC} column is not yet defined. Use SupplyLocations.define_attraction()")
    ...
    def add_var(
        ...
        if metadata["attraction_col"] is None:
            raise ValueError (f"Error while adding utility variable: {helper.DEFAULT_NAME_ATTRAC} column is not yet defined. Use SupplyLocations.define_attraction()")
...
class InteractionMatrix:
    ...
    def summary(self):
        ...
        if supply_locations_metadata["attraction_col"][0] is None:
            print(f"{helper.DEFAULT_NAME_ATTRAC} column   not defined")
        else:
            print(f"{helper.DEFAULT_NAME_ATTRAC} column   " + supply_locations_metadata["attraction_col"][0])
        print("Customer origins    " + str(customer_origins_metadata["no_points"]))
        if customer_origins_metadata["marketsize_col"] is None:
            print(f"{helper.DEFAULT_NAME_MARKETSIZE} column not defined")
        else:
            print(f"{helper.DEFAULT_NAME_MARKETSIZE} column  " + customer_origins_metadata["marketsize_col"])
        ...
        if interaction_matrix_metadata != {} and "transport_costs" in interaction_matrix_metadata:
            ...
            if interaction_matrix_metadata["transport_costs"]["network"]:
                print(f"{helper.DEFAULT_NAME_TC} type Time")
                print(f"{helper.DEFAULT_NAME_TC} unit " + interaction_matrix_metadata["transport_costs"]["time_unit"])
            else:
                print(f"{helper.DEFAULT_NAME_TC} type Distance")
                print(f"{helper.DEFAULT_NAME_TC} unit " + interaction_matrix_metadata["transport_costs"]["distance_unit"])
        ...
        if supply_locations_metadata["weighting"][0]["func"] is None and supply_locations_metadata["weighting"][0]["param"] is None:
            print(f"{helper.DEFAULT_NAME_ATTRAC}          not defined")
        else:
            if supply_locations_metadata["weighting"][0]["param"] is not None:
                print(f"{helper.DEFAULT_NAME_ATTRAC}          " + str(round(supply_locations_metadata["weighting"][0]["param"],3)) + " (" + supply_locations_metadata["weighting"][0]["func"] + ")")
            else:
                print(f"{helper.DEFAULT_NAME_ATTRAC}          NA" + " (" + supply_locations_metadata["weighting"][0]["func"] + ")")   

        if customer_origins_metadata["weighting"][0]["func"] is None and customer_origins_metadata["weighting"][0]["param"] is None:
            print(f"{helper.DEFAULT_NAME_TC}     not defined")
            ...
            if customer_origins_metadata["weighting"][0]["param"] is not None:
                print(f"{helper.DEFAULT_NAME_TC}     " + str(round(customer_origins_metadata["weighting"][0]["param"],3)) + " (" + customer_origins_metadata["weighting"][0]["func"] + ")")
            else:
                print(f"{helper.DEFAULT_NAME_TC}     NA" + " (" + customer_origins_metadata["weighting"][0]["func"] + ")")
        elif customer_origins_metadata["weighting"][0]["func"] == "logistic":
            if customer_origins_metadata["weighting"][0]["param"] is not None:
                print(f"{helper.DEFAULT_NAME_TC}    " + str(round(customer_origins_metadata["weighting"][0]["param"][0],3)) + ", " + str(round(customer_origins_metadata["weighting"][0]["param"][1],3)) + " (" + customer_origins_metadata["weighting"][0]["func"] + ")")
            else:
                print(f"{helper.DEFAULT_NAME_TC}     NA" + " (" + customer_origins_metadata["weighting"][0]["func"] + ")")
    ...
    def transport_costs(
        ...
        if network:
            ...
            range_type = transport_costs_matrix_config[helper.ORS_ENDPOINTS["Matrix"]["Parameters"]["unit"]["param"]]

            transport_costs_matrix[helper.MATRIX_COL_SOURCE] = transport_costs_matrix[helper.MATRIX_COL_SOURCE].astype(int)
            transport_costs_matrix[helper.MATRIX_COL_SOURCE] = transport_costs_matrix[helper.MATRIX_COL_SOURCE].map(dict(enumerate(customer_origins_ids)))
            
            transport_costs_matrix[helper.MATRIX_COL_DESTINATION] = transport_costs_matrix[helper.MATRIX_COL_DESTINATION].astype(int)
            transport_costs_matrix[helper.MATRIX_COL_DESTINATION] = transport_costs_matrix[helper.MATRIX_COL_DESTINATION].map(dict(enumerate(supply_locations_ids)))
            
            transport_costs_matrix[f"{helper.MATRIX_COL_SOURCE}_{helper.MATRIX_COL_DESTINATION}"] = transport_costs_matrix[helper.MATRIX_COL_SOURCE].astype(str)+helper.MATRIX_OD_SEPARATOR+transport_costs_matrix[helper.MATRIX_COL_DESTINATION].astype(str)
            transport_costs_matrix = transport_costs_matrix[[f"{helper.MATRIX_COL_SOURCE}_{helper.MATRIX_COL_DESTINATION}", range_type]]

            interaction_matrix_df = interaction_matrix_df.merge(
                transport_costs_matrix,
                left_on=helper.DEFAULT_COLNAME_INTERACTION,
                right_on=f"{helper.MATRIX_COL_SOURCE}_{helper.MATRIX_COL_DESTINATION}"
                )
            ...
            interaction_matrix_df = interaction_matrix_df.drop(columns=[f"{helper.MATRIX_COL_SOURCE}_{helper.MATRIX_COL_DESTINATION}", range_type])
        ...
        interaction_matrix_metadata["transport_costs"] = {
            "network": network,
            helper.ORS_ENDPOINTS["Matrix"]["Parameters"]["unit"]["param"]: range_type,
            "time_unit": time_unit,
    ...
    def utility(
        ...
        if interaction_matrix_df[helper.DEFAULT_COLNAME_TC].isna().all():
            raise ValueError(f"Error in utility calculation: {helper.DEFAULT_NAME_TC} variable is not defined")
        if interaction_matrix_df[helper.DEFAULT_COLNAME_ATTRAC].isna().all():
            raise ValueError(f"Error in utility calculation: {helper.DEFAULT_NAME_ATTRAC} variable is not defined")
    ...
    def flows (
        ...
        if interaction_matrix_df[helper.DEFAULT_COLNAME_MARKETSIZE].isna().all():
            raise ValueError(f"Error in flows calculation: {helper.DEFAULT_NAME_MARKETSIZE} column in customer origins not defined. Use CustomerOrigins.define_marketsize()")
    ...
    def hansen(
        ...
        if from_origins:
            ...
            if tc_weighting["func"] == "power":
                ...
                raise ValueError (f"Error in Hansen accessibility calculation: {helper.DEFAULT_NAME_TC} weighting is not defined.")
    ...
    def huff_ml_fit(
        ...
        if len(initial_params) < 2 or len(initial_params) != params_metadata:
            raise ValueError(f"Error in huff_ml_fit: Parameter 'initial_params' must be a list with {str(params_metadata)} entries ({helper.DEFAULT_NAME_ATTRAC}: {str(params_metadata_supply_locations)}, {helper.DEFAULT_NAME_TC}: {str(params_metadata_customer_origins)}).")
    ...
    def update(self):
        ...
        if "transport_costs" not in interaction_matrix_metadata:
            ...
            range_type = interaction_matrix_metadata["transport_costs"][helper.ORS_ENDPOINTS["Matrix"]["Parameters"]["unit"]["param"]]
...
class HuffModel:
    ...
    def ml_fit(
        ...
        if fit_by in ["probabilities", "flows"]:
            ...
            if len(initial_params) < 2 or len(initial_params) != params_metadata:
                raise ValueError(f"Error in ml_fit: Parameter 'initial_params' must be a list with {str(params_metadata)} entries ({helper.DEFAULT_NAME_ATTRAC}: {str(params_metadata_supply_locations)}, {helper.DEFAULT_NAME_TC}: {str(params_metadata_customer_origins)})")
...
class MCIModel:
    ...
    def summary(self):
        ...
        for key, value in coefs.items():
            ...
            if coefficient_name == helper.DEFAULT_COLNAME_ATTRAC:
                coefficient_name = helper.DEFAULT_NAME_ATTRAC
            if coefficient_name == helper.DEFAULT_COLNAME_TC:
                coefficient_name = helper.DEFAULT_NAME_TC
    ...
    def utility(
        ...
        if interaction_matrix_df[helper.DEFAULT_COLNAME_TC].isna().all():
            raise ValueError(f"Error in utility calculation: {helper.DEFAULT_NAME_TC} variable {helper.DEFAULT_COLNAME_TC} is not defined")
        if interaction_matrix_df[helper.DEFAULT_COLNAME_ATTRAC].isna().all():
            raise ValueError(f"Error in utility calculation: {helper.DEFAULT_NAME_ATTRAC} variable {helper.DEFAULT_COLNAME_ATTRAC} is not defined")
...
def create_interaction_matrix(
    ...
    customer_origins_metadata = customer_origins.get_metadata()
    if customer_origins_metadata["marketsize_col"] is None:
        raise ValueError(f"Error while creating interaction matrix: {helper.DEFAULT_NAME_MARKETSIZE} column in customer origins not defined. Use CustomerOrigins.define_marketsize()")
    
    supply_locations_metadata = supply_locations.get_metadata()
    if supply_locations_metadata["attraction_col"][0] is None:
        raise ValueError(f"Error while creating interaction matrix: {helper.DEFAULT_NAME_ATTRAC} column in supply locations not defined. Use SupplyLocations.define_attraction()")
    ...
    interaction_matrix_df[helper.DEFAULT_COLNAME_INTERACTION] = interaction_matrix_df[helper.DEFAULT_COLNAME_CUSTOMER_ORIGINS].astype(str)+helper.MATRIX_OD_SEPARATOR+interaction_matrix_df[helper.DEFAULT_COLNAME_SUPPLY_LOCATIONS].astype(str)
...
def get_isochrones(
    ...
    for x, y in coords:
        ...
        if donut:
            isochrone_gdf = overlay_difference(
                ...
                sort_col = helper.DEFAULT_SEGMENTS_COL
                )
        ...
        if range_type == "time":
            isochrone_gdf["segm_min"] = isochrone_gdf[helper.DEFAULT_SEGMENTS_COL]/60
        if range_type == "distance":
            isochrone_gdf["segm_km"] = isochrone_gdf[helper.DEFAULT_SEGMENTS_COL]/1000        
    ...
    isochrones_gdf[helper.DEFAULT_SEGMENTS_COL] = isochrones_gdf[helper.DEFAULT_SEGMENTS_COL].astype(int)
