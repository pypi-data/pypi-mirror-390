...
def manhattan_distance(
    ...
    lat1, lat2, lon1, lon2 = helper.lonlat_transform(
        source,
        destination,
        transform=False
    )
...
def euclidean_distance(
    ...
    lat1_r, lat2_r, lon1_r, lon2_r = helper.lonlat_transform(
        source,
        destination
        )
...
def distance_matrix(
    ...
    if distance_type not in helper.DISTANCE_TYPES_LIST_FUNC:
        raise ValueError(f"Distance type {distance_type} is unknown. Please choose one of the following: {', '.join(helper.DISTANCE_TYPES_LIST_FUNC)}.")
    
    if len(sources_uid) > 0 and len(sources_uid) != len(sources) and lines_gdf:
    ...
    if len(destinations_uid) > 0 and len(destinations_uid) != len(destinations) and lines_gdf:
    ...
    for i, source in enumerate(sources):
        ...
        for j, destination in enumerate(destinations):
            ...
            if distance_type == helper.DISTANCE_TYPES_LIST_FUNC[0]:
                dist = euclidean_distance(
                    source, 
                    destination, 
                    unit
                    )
            elif distance_type == helper.DISTANCE_TYPES_LIST_FUNC[1]:
            ...
            if lines_gdf:
                ...
                if len(sources_uid) == len(sources) and len(destinations_uid) == len(destinations):
                    ...
                    line_data.append(
                        {
                            helper.MATRIX_COL_SOURCE: source,
                            helper.MATRIX_COL_DESTINATION: destination,
                            "distance": dist,
                            "geometry": line,
                            f"{helper.MATRIX_COL_SOURCE}_uid": sources_uid[i],
                            f"{helper.MATRIX_COL_DESTINATION}_uid": destinations_uid[j],
                            f"{helper.MATRIX_COL_SOURCE}_{helper.MATRIX_COL_DESTINATION}_uid": f"{sources_uid[i]}{helper.MATRIX_OD_SEPARATOR}{destinations_uid[j]}"
                            }
                        )
...
def buffers(
    ...
    for idx, row in point_gdf.iterrows():
        ...
        for distance in distances:
            ...
            point_buffer_gdf = gp.GeoDataFrame(
            {
                ...
                helper.DEFAULT_SEGMENTS_COL: [distance]
                },
                crs=point_gdf.crs
            )
    ...
    if merge_buffers:
        ...
        all_buffers_gdf = gp.GeoDataFrame(
            {
                unique_id_col: ["all"],
                helper.DEFAULT_SEGMENTS_COL: ["merged"],
...
def map_with_basemap(
    ...
    layers_combined_wgs84 = layers_combined.to_crs(crs = helper.WGS84_CRS)
    ...
    if osm_basemap:
        ...
        img = Image.open(helper.DEFAULT_FILENAME_ORS_TMP)
    ...
    for layer in layers:
        ...
        layer_3857 = layer.to_crs(crs = helper.PSEUDO_MERCATOR_CRS)
        ...
        if styles != {}:
            ...
            layer_3857.plot(
                ax=ax, 
                alpha=helper.DEFAULT_LAYER_ALPHA, 
                label=f"{helper.DEFAULT_LAYER_LABEL} {i+1}"
                )
    ...
    extent_geom = gp.GeoSeries([bbox], crs = helper.WGS84_CRS).to_crs(crs = helper.PSEUDO_MERCATOR_CRS).total_bounds
    ...
    if legend and legend_handles:
        ax.legend(
            ...
            loc=helper.DEFAULT_LEGEND_LOC, 
            fontsize=helper.DEFAULT_LEGEND_FONTSIZE, 
            frameon=True
            )
    ...
    if os.path.exists(helper.DEFAULT_FILENAME_ORS_TMP):
        try:
            os.remove(helper.DEFAULT_FILENAME_ORS_TMP)
        except Exception as e:
            error_message = f"Temporary file {helper.DEFAULT_FILENAME_ORS_TMP} can not be removed. Error message: {str(e)}"
            print(error_message)
