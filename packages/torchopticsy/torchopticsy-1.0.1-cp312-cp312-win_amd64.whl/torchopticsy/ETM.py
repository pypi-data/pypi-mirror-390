...
class ETM:
    def __init__(
        ...
        is_surf = tensor(is_surf, dtype=torch.bool, device=device)
        ...
        self.F_u = F_u
        self.F_v = F_v
        self.stack_material_id = tensor(stack_material_id, device=device)
        self.surf_material_id = tensor(surf_material_id, device=device)
        self.surf = tensor(surf, device=device, dtype=dtype_real)
        self.is_surf = is_surf
        ...
        for z in range(self.er_stack.size(0)):
            self.er_stack[z] = self.Material_list[self.stack_material_id[z]].index ** 2
        self.er_surf = torch.ones(
            [self.surf_material_id.numel(), self.lams.numel()],
            dtype=dtype,
            device=device,
        )
        for z in range(self.er_surf.size(0)):
            self.er_surf[z] = self.Material_list[self.surf_material_id[z]].index ** 2
