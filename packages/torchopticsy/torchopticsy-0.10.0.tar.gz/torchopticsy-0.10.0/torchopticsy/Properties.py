...
class Material:
    ...
    def Add_Fit_Plot(name, wavelengths, refractive_indices, poles=1, iteration=1):
        ...
        w = 2 * torch.pi * 3e8 / lams  # c is the speed of light
        ...
        plt.plot(lams, torch.real(torch.sqrt(er)), "-", label="Real Fitted")
        plt.legend()
        plt.show()
        ...
        plt.plot(lams, torch.imag(torch.sqrt(er)), "-", label="Imag Fitted")
        plt.legend()
        plt.show()
    ...
    def __init__(self, name, lams):
        ...
        w = 2 * torch.pi * 3e8 / lams.view(-1, 1)
