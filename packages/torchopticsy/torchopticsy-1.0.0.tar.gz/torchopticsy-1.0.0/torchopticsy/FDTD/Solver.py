...
import oticsyCUDA.FDTD as FDTD_CUDA_Dispersion
import oticsyCUDA.FDTD4 as FDTD_CUDA_MONO
...
class FDTD_Solver(Properties.Geometry):
    ...
    def Initialize(self):  # 待会去掉
        ...
        if self.dispersion:
            er_type = torch.complex64
        else:
            er_type = torch.float32
        self.er = torch.zeros(
            [len(self.Material_list), self.lams.numel()],
            dtype=er_type,
            device=self.device,
        )
        self.f_co = torch.zeros_like(self.er)
        self.er_mean = torch.zeros(
            [len(self.Material_list)], dtype=er_type, device=self.device
        )
    ...
    def Reset_structure_MONO(self, start):
        ...
        if start:
            ...
            FDTD_CUDA_MONO.Set_ER_average(
                self.ERx, self.pri_weightx, self.pri_mat_idx, self.sec_mat_idx, self.er
            )
            FDTD_CUDA_MONO.Set_ER_average(
                self.ERy, self.pri_weighty, self.pri_mat_idy, self.sec_mat_idy, self.er
            )
            FDTD_CUDA_MONO.Set_ER_average(
                self.ERz, self.pri_weightz, self.pri_mat_idz, self.sec_mat_idz, self.er
            )
