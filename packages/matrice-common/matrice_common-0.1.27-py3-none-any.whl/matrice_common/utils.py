...
# Default: 15 minutes TTL, configurable via MATRICE_ERROR_CACHE_TTL_SECONDS
_ERROR_CACHE_TTL = int(os.environ.get("MATRICE_ERROR_CACHE_TTL_SECONDS", 15 * 60))
...
_DEDUPLICATION_ENABLED = os.environ.get("MATRICE_ERROR_DEDUPLICATION_ENABLED", "true").lower() in ("true", "1", "yes")
...
def seen_error(hash_str: str) -> bool:
    ...
    with _error_cache_lock:
        if len(_error_cache) > _ERROR_CACHE_MAX:
            for k, t in list(_error_cache.items()):
                if now - t > _ERROR_CACHE_TTL:
                    del _error_cache[k]
        if hash_str in _error_cache and now - _error_cache[hash_str] <= _ERROR_CACHE_TTL:
            return True
...
def _get_sentry_client(rpc_client=None, access_key=None, secret_key=None, service_name: str = "py_common"):
    ...
    try:
        if getattr(sentry_sdk, "_initialized", False):
...
def send_sentry_log(
    ...
    Note: Deduplication is handled at the caller level (decorator or send_error_log).
    This function should only be called after deduplication check.
    """
    try:
...
def send_error_log(
    ...
    # Check for duplicate errors if deduplication is enabled
    if _DEDUPLICATION_ENABLED:
        dup_key = generate_error_dedup_key(error_type, filename, function_name, service_name)
        if seen_error(dup_key):
            logging.debug(f"Skipping duplicate error log (Kafka): {dup_key}")
            return
...
def process_error_log(
    ...
    logging.info(f"Processing error from {func_file}:{lineno}, function '{func_name}': {str(error)}")
    ...
    error_msg = f"Exception in {func_file}:{lineno}, function '{func_name}': {str(error)}"
    ...
    if log_error:
        ...
        if _DEDUPLICATION_ENABLED:
            ...
            if seen_error(dup_key):
                logging.debug(f"Skipping duplicate error log: {dup_key}")
                if raise_exception:
                    raise AppError(
                        error_type=error_type,
                        error=error,
