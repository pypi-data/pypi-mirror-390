...
class TimeInterval:
    ...
    def is_t_included_in_original_limits(
        self,
        t: Number
    ) -> bool:
        """
        Check if the `t` time moment provided is included in
        in this time interval instance original limits or not.
        This means that the `t` provided is between the
        original `start` and `end` time moment values.

        The `end` is not included.
        """
        return self.original_start <= t < self.original_end
