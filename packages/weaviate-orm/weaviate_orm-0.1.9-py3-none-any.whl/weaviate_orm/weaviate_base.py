...
class Base_Model(ABC, metaclass=Weaviate_Meta):
    ...
    def _cast_from_response(cls, response: ObjectSingleReturn) -> "Base_Model":
        ...
        # Set vector (default) if it exists
        if hasattr(inst, 'vector') and response.vector is not None:
            inst.vector = response.vector.get('default', None)
    ...
    @classmethod
    @with_client(require_schema_creation=False)
    def near_vector(cls, vector:list[float], include_references, include_vector, client:Optional[WeaviateClient]=None, top_n = 5) -> Any:
        """
        Perform a near-vector query.
        """

        collection = cls.get_collection(client=client)
        result = collection.query.near_vector(vector, limit=top_n)
    ...
    @classmethod
    @with_client(require_schema_creation=False)
    def near_text(cls, query_str:str, include_references, include_vector, client:Optional[WeaviateClient]=None, top_n = 5) -> Any:
        """
        Perform a near-text query.
        """

        collection = cls.get_collection(client=client)
        result = collection.query.near_text(query_str, limit=top_n)
    ...
    def _save_properties(self, update:bool=False, include_references:bool=False, recursive:bool=False, vector:Optional[list[float]]=None, *, client:Optional[WeaviateClient]=None, named_vectors:Optional[dict[str, list[float]]]=None, **callback_ctx) -> dict[str, List[DataReferences]]:
        ...
        if self.instance_exists(self.get_uuid(), client=client):
            if not update:
                ...
                self.update(include_references=include_references, client=client, callback_ctx=ctx)
        else:
            #Add the object
            ...
            for ref_name, ref in {k:v for k,v in self.__dict__.items() if k in ref_names}.items():
                ...
                for r in ref:
                    ...
                    if not r.exists:
                        ...
                        recursive_references = r._save_properties(client=client, include_references=include_references, recursive=recursive, callback_ctx=ctx)
