...
from pytorch_lightning_enterprise.loggers.logger import Logger, rank_zero_experiment
from pytorch_lightning_enterprise.utils.imports import _COMET_AVAILABLE
from pytorch_lightning_enterprise.utils.logger import _convert_params
...
class CometLogger(Logger):
    ...
    **Access the Comet Experiment object:**
    ...
    Args:
        ...
        flush_on_ckpt: If True, the data will be flushed to Comet server when a checkpoint is saved.
            Default is ``True``.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._flush_on_ckpt: bool = flush_on_ckpt
        ...
        # use the rank_zero_only state as PL sets it to the global rank through lightning_utilities
        if rank_zero_only.rank is not None and rank_zero_only.rank != 0:
            return
    ...
    @override
    @rank_zero_only
    def after_save_checkpoint(self, checkpoint_callback: ModelCheckpointProtocol) -> None:
        """Optionally flush the experiment data to the Comet server.

        Args:
            checkpoint_callback: The checkpoint callback that was used to save the checkpoint.
