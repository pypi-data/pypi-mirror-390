...
from pytorch_lightning_enterprise.utils.imports import _BITSANDBYTES_AVAILABLE
...
class BitsandbytesPrecision(LicenseValidator):
...
def _import_bitsandbytes() -> ModuleType:
    ...
    class _Linear8bitLt(bnb.nn.Linear8bitLt):
        ...
        def quantize_(self, weight: Optional[torch.Tensor] = None, device: Optional[torch.device] = None) -> None:
            ...
            if weight.data.dtype == torch.int8:
                # already quantized
                return
            if not isinstance(self.weight, bnb.nn.Int8Params):
                raise TypeError(f"Expected `self.weight` to be a `bnb.nn.Int8Params`, but got {type(self.weight)}")
        ...
        def to_empty(self, *, device: Union[torch.device, str, int], recurse: bool = True) -> Self:
    ...
    class _Linear4bit(bnb.nn.Linear4bit):
        ...
        def __init__(self, *args: Any, device: Optional[Union[torch.device, str, int]] = None, **kwargs: Any) -> None:
        ...
        def quantize_(self, weight: Optional[torch.Tensor] = None, device: Optional[torch.device] = None) -> None:
            ...
            if not isinstance(self.weight, bnb.nn.Params4bit):
                raise TypeError(f"Expected `self.weight` to be a `bnb.nn.Params4bit`, but got {type(self.weight)}")
            self.weight = self.quantize(self.weight, weight, device)
