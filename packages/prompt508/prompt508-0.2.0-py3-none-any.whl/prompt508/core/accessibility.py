...
class AccessibilityInjector:
    ...
    def _build_instructions(self, content_type: Optional[str] = None) -> List[str]:
        ...
        if self.include_captions and "multimedia" in self.accessibility_hints:
            instructions.extend(
                self._format_hints("Multimedia", self.accessibility_hints["multimedia"])
            )

        if self.include_structure and "documents" in self.accessibility_hints:
            instructions.extend(
                self._format_hints("Document Structure", self.accessibility_hints["documents"])
            )
        ...
        if self.strict_mode and "language" in self.accessibility_hints:
            instructions.extend(
                self._format_hints("Language", self.accessibility_hints["language"])
            )
    ...
    def _format_injected_prompt(self, original_prompt: str, instructions: List[str]) -> str:
        """
