...
def run_input_encoding_recon(console: Console, model_wrapper: LLMModelWrapper, final_args: dict[str, Any]):
    ...
    input_encoding_recon_service = ReconInputEncodingClient(API_BASE, access_token)
    ...
    def input_encoding() -> None:
        nonlocal result
        recon_id = input_encoding_command.start_input_encoding(final_args["target"])
    ...
    console.print("\nSome signs of input encoding support found.", style="bold blue")
    console.print(
        f"\nFull results can be found here: {DASHBOARD_URL}/results/targets/{result.target_name}/tests/null/recon\n"
    )
