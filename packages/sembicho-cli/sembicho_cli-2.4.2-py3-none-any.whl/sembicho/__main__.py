#!/usr/bin/env python3
"""SemBicho CLI v2.2.2 - Production Ready with Auth & Quality"""
...
QUICK START:
  sembicho cicd --path ./src --upload  # 🚀 Complete CI/CD analysis
    ...
    if args.command == 'scan':
        ...
        if args.upload:
            ...
            try:
                ...
                pipeline_id = args.pipeline_id or f"scan-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        ...
        if args.fail_on:
            ...
            min_threshold = min([severity_order.get(s, 0) for s in fail_severities])
            
            found_blocking = False
            for severity, count in result.severity_counts.items():
                if count > 0 and severity_order.get(severity, 0) >= min_threshold:
                    found_blocking = True
                    break
            
            if found_blocking:
                print(f"\n❌ Build FAILED: Found vulnerabilities at or above '{args.fail_on}' severity level")
                sys.exit(1)
            else:
                print(f"\n✅ Build PASSED: No vulnerabilities at or above '{args.fail_on}' severity level")
        ...
        print(f"\n{'='*70}")
        print(f"  🚀 SemBicho CI/CD Complete Analysis v{__version__}")
        print(f"{'='*70}\n")
        ...
        # === 1. SECURITY SCAN ===
        print(f"{'─'*70}")
        print("🔒 Step 1/3: Security Vulnerability Scan")
        print(f"{'─'*70}")
        scanner = SemBichoScanner()
        ...
        # === 2. QUALITY ANALYSIS ===
        print(f"{'─'*70}")
        print("📊 Step 2/3: Code Quality Analysis (Lint + Complexity)")
        print(f"{'─'*70}")
        ...
        if args.upload:
            print(f"{'─'*70}")
            print("📤 Step 3/3: Uploading to Backend")
            print(f"{'─'*70}")
        ...
        if args.fail_on:
            ...
            if should_fail:
                print(f"❌ Build FAILED: Found vulnerabilities at or above '{args.fail_on}' severity level")
                sys.exit(1)
            else:
                print(f"✅ Build PASSED: No vulnerabilities at or above '{args.fail_on}' severity level")
