...
def _roi_plotter_mp(data: Tuple[int, np.ndarray, np.ndarray, int, int],
                    loc_dict: dict,
                    font_size: float,
                    circle_sizes: list,
    ...
    group_cnt, data_df, batch_frames, start_frm, end_frm = data[0], data[1], data[2], data[3], data[4]
    ...
    # Process the batch of frames that was passed from main process
    for frame_idx, img in enumerate(batch_frames):
        current_frm = start_frm + frame_idx
        if current_frm > end_frm:
            break
...
class ROIPlotMultiprocess(ConfigReader):
    ...
    def run(self):
        ...
        data_chunks = np.array_split(self.data_df, self.core_cnt)
        data_chunks = [(i, j) for i, j in enumerate(data_chunks)]
        
        # Get frame ranges for each chunk
        frame_ranges = []
        for chunk_idx, (group_cnt, data_df) in enumerate(data_chunks):
            df_frm_range = data_df.index.tolist()
            start_frm, end_frm = df_frm_range[0], df_frm_range[-1]
            frame_ranges.append((start_frm, end_frm))
        ...
        # Use async frame reader in main process
        frm_reader = AsyncVideoFrameReader(
        ...
        with ctx.Pool(self.core_cnt, maxtasksperchild=self.maxtasksperchild) as pool:
            ...
            # Process batches and distribute to workers
            batch_count = 0
            for batch_cnt in range(frm_reader.batch_cnt):
                batch_start, batch_end, batch_frames = get_async_frame_batch(frm_reader, timeout=20)
                
                # Find which data chunk this batch belongs to
                for chunk_idx, (group_cnt, data_df) in enumerate(data_chunks):
                    chunk_start, chunk_end = frame_ranges[chunk_idx]
                    
                    # Check if this batch overlaps with this chunk
                    if batch_start <= chunk_end and batch_end >= chunk_start:
                        # Calculate overlap
                        overlap_start = max(batch_start, chunk_start)
                        overlap_end = min(batch_end, chunk_end)
                        
                        if overlap_start <= overlap_end:
                            # Extract frames for this chunk
                            start_idx = max(0, overlap_start - batch_start)
                            end_idx = min(len(batch_frames), overlap_end - batch_start + 1)
                            chunk_frames = batch_frames[start_idx:end_idx]
                            
                            # Create worker data
                            worker_data = (group_cnt, data_df, chunk_frames, overlap_start, overlap_end)
                            
                            # Submit to pool
                            result = pool.apply_async(constants, (worker_data,))
                            batch_count += 1
                            
                            if self.verbose:
                                print(f'Submitted batch {batch_count} for processing...')
