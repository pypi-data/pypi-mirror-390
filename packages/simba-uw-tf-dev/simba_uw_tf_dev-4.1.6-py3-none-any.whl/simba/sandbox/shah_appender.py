...
def shah_appender(labels_dir: Union[str, os.PathLike],
    ...
    lbls_paths = find_files_of_filetypes_in_directory(directory=labels_dir, extensions=['.txt'], as_dict=True, raise_error=True)
    x_paths = find_files_of_filetypes_in_directory(directory=features_dir, extensions=['.csv'], as_dict=True, raise_error=True)
    ...
    for file_cnt, (file_name, file_path) in enumerate(lbls_paths.items()):
        ...
        with open(file_path, 'r') as f: file_data = f.read()
        video_name = get_video_name(data=file_data, file_path=file_path)
        if video_name not in x_paths.keys():
            raise NoDataError(f'Could not find file for video {video_name} in directory {features_dir}', source=shah_appender.__name__)
        ...
        for clf in clf_names:
            ...
            if len(start_frames) != len(stop_frames):
                raise FrameRangeError(msg=f'File {file_path} has {len(start_frames)} start events and {len(stop_frames)} stop events for behavior {clf}')
            behavior_frames = {behavior: [frame for start, stop in zip(
                ...
                clf_data[clf_data[BEHAVIOR] == behavior].sort_values(FRAME)[clf_data[EVENT] == 'stop'][FRAME].values)
                                          for frame in range(start, stop + 1)] for behavior in clf_data[BEHAVIOR].unique()}[clf]
