...
wazo automatically detects your system's runtime version and uses matching Docker images:
...
These images are used automatically by wazo:

```bash
# wazo detects language and uses appropriate image
wazo build .
...
# Or use user mapping (handled by wazo)
...
- Run `wazo doctor` for diagnostics
