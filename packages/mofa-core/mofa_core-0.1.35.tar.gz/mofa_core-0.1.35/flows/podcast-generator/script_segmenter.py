...
def main():
    ...
    # Text segmentation configuration (copied from dora-text-segmenter)
    ...
    chars_per_second = float(os.getenv("TTS_CHARS_PER_SECOND", "4.5"))  # Conservative estimate for Chinese
    ...
    send_log(node, "INFO", "Script Segmenter started", log_level)
    send_log(
        node,
        "INFO",
    ...
    try:
        ...
        send_log(node, "INFO", f"Loaded {len(segments)} segments from {args.input_file}", log_level)
    except Exception as e:
        send_log(node, "ERROR", f"Failed to parse script: {e}", log_level)
        return

    if not segments:
        send_log(node, "ERROR", "No segments found in script!", log_level)
        return

    # Apply text segmentation to split long segments
    # This expands the segment list with sub-segments
    expanded_segments = []
    for character, text in segments:
        ...
        if len(text) > max_segment_length:
            ...
            chunks = split_long_text(text, max_segment_length, punctuation_marks, node, log_level)
            send_log(
                node,
                "INFO",
    ...
    send_log(node, "INFO", f"Sending segment 1/{len(segments)} to {character}: '{text[:50]}...' (len={len(text)})", log_level)
    node.send_output(f'{character}_text', pa.array([text]))
    waiting_for = character
    current_index = 1
    send_log(node, "INFO", f"Waiting for {waiting_for}_segment_complete...", log_level)

    # Event loop - wait for segment_complete and send next segments
    send_log(node, "INFO", "Entering event loop", log_level)
    for event in node:
        send_log(node, "DEBUG", f"Received event type: {event['type']}", log_level)

        if event["type"] == "INPUT":
            event_id = event["id"]
            send_log(node, "DEBUG", f"Received INPUT event: {event_id}", log_level)

            if event_id == f"{waiting_for}_segment_complete":
                send_log(node, "INFO", f"Received {event_id}", log_level)

                if current_index < len(segments):
                    character, text = segments[current_index]
                    send_log(node, "INFO", f"Sending segment {current_index+1}/{len(segments)} to {character}: '{text[:50]}...' (len={len(text)})", log_level)
                    node.send_output(f'{character}_text', pa.array([text]))
                    waiting_for = character
                    current_index += 1
                else:
                    send_log(node, "INFO", "All segments processed. Sending script_complete.", log_level)
                    node.send_output('script_complete', pa.array([b'']))
                    send_log(node, "INFO", "Script segmenter finished", log_level)
                    break
    ...
    send_log(node, "INFO", "Event loop exited", log_level)
