        ...
        **kwargs,
        ...
        try:
            ...
            for i in range(0, len(data), batch_size):
                ...
                # Handle multiple formats (check data field FIRST for accurate rowcount):
                # - New: data is dict with rows_affected: {"rows_affected": N}
                # - New: data is list with rows_affected: [{"rows_affected": N}]
                # - Old: row_count at result level (fallback)
                rows_affected = 0
                if result and "result" in result:
                    result_data = result["result"]
                    # Try data field first (new DML format - most accurate)
                    if "data" in result_data:
                        data = result_data["data"]
                        # Handle dict format: {"rows_affected": N}
                        if isinstance(data, dict) and "rows_affected" in data:
                            rows_affected = data["rows_affected"]
                        # Handle list format: [{"rows_affected": N}]
                        elif (
                        ...
                        # Fall back to row_count if data doesn't have rows_affected
                        elif "row_count" in result_data:
                            rows_affected = result_data.get("row_count", 0)
                    # Fall back to row_count (old INSERT format)
                    elif "row_count" in result_data:
                        rows_affected = result_data.get("row_count", 0)
