            ...
            async def async_run(self, **kwargs) -> Dict[str, Any]:
                ...
                if operation == "create":
                    ...
                    if database_type.lower() == "postgresql":
                        ...
                        # Uses xmax = 0 flag to detect INSERT vs UPDATE:
                        # - xmax = 0: New transaction (INSERT occurred)
                        # - xmax > 0: Existing transaction modified (UPDATE occurred)
                        # This works with any primary key type including strings.
                        insert_columns = list(insert_data.keys())
                        insert_values = list(insert_data.values())

                        # Phase 2.1: Use conflict_on if provided, otherwise default to where.keys()
                        conflict_columns = kwargs_fixed.get("conflict_on") or list(
                            where.keys()
                        )

                        # Update columns (exclude conflict columns and auto-managed fields)
                        update_columns = [
                            col
                        ...
                        placeholders = ", ".join(
                            [f"${i+1}" for i in range(len(insert_values))]
                        )
                        conflict_str = ", ".join(conflict_columns)

                        # Build SET clause with parameter indices offset by insert values
                        update_set_clauses = []
                        update_params = []
                        param_index = len(insert_values) + 1

                        for col in update_columns:
                            update_set_clauses.append(f"{col} = ${param_index}")
                            update_params.append(update_data[col])
                            param_index += 1
                        ...
                        # Add auto-timestamp for updated_at
                        # Note: DataFlow automatically adds created_at and updated_at columns to all tables,
                        # even if they're not in the model definition. These columns are always present.
                        update_set_str += ", updated_at = CURRENT_TIMESTAMP"
                        ...
                        params = insert_values + update_params
                        ...
                        # SQLite: Pre-check query + atomic upsert pattern
                        #
                        # Why pre-check is necessary:
                        # SQLite's last_insert_rowid() only works with INTEGER PRIMARY KEY
                        # autoincrement columns. Since DataFlow v0.4.7+ supports string IDs,
                        # we cannot rely on last_insert_rowid() to detect INSERT vs UPDATE.
                        #
                        # Solution: Query table first to check if row exists, then perform
                        # atomic upsert. The pre-check result tells us if INSERT or UPDATE
                        # will occur, which we return in the 'created' flag.
                        #
                        # Performance: 2 queries total (still 60% faster than SwitchNode
                        # pattern which requires 5+ nodes and multiple queries).
                        sql_node = self.dataflow_instance._get_or_create_async_sql_node(
                            database_type
                        )

                        # Pre-check: Does row exist?
                        # Phase 2.1: Use conflict_on if provided, otherwise default to where.keys()
                        conflict_columns = kwargs_fixed.get("conflict_on") or list(
                            where.keys()
                        )
                        ...
                        # SQLite upsert query (no RETURNING needed for created flag)
                        query = f"""
                            INSERT INTO {table_name} ({columns_str})
                            VALUES ({placeholders})
                            ON CONFLICT ({conflict_str})
