...
class AuthHandler:
    ...
    def _fetch_from_auth_me(self) -> Optional[Dict]:
        """
        Fetch user info from /.auth/me endpoint (WebSocket fallback).

        Returns:
            Dict with user info, or None if fetch fails
        """
        try:
            import requests
            from requests.cookies import RequestsCookieJar

            # Get cookies from streamlit context
                ...
                if cookie_header:
                    ...
                    for cookie in cookie_header.split('; '):
                        if '=' in cookie:
                            ...
                            cookie_jar.set_cookie(requests.cookies.create_cookie(name, value))

            # Fetch from /.auth/me endpoint
            # Note: This should be configured per-app, but we need a default
            auth_me_url = os.getenv('AZURE_AUTH_ME_URL', '/.auth/me')
            if not auth_me_url.startswith('http'):
                # Relative URL - need to construct full URL
                # Get from streamlit context if available
                if hasattr(st, 'context') and hasattr(st.context, 'headers'):
                    host = st.context.headers.get('Host', '')
                    if host:
                        auth_me_url = f"https://{host}/.auth/me"

            response = requests.get(auth_me_url, cookies=cookie_jar, timeout=5)
            ...
            if response.status_code == 200:
                ...
                if data and len(data) > 0:
                    ...
                    # Extract user claims
                    user_claims = auth_data.get('user_claims', [])
                    ...
                    for claim in user_claims:
                        ...
                        elif 'emailaddress' in claim_type:
                            email = claim_value
                        elif claim_type == 'name':
                            name = claim_value

                    # Get role from groups
                    role = self._get_user_role_from_groups(groups)
    ...
    def get_current_user(self) -> Optional[Dict]:
        ...
        Uses Streamlit session state to cache auth data. Falls back to /.auth/me endpoint
        when headers are not available (WebSocket requests).
        ...
        # Azure production mode - try headers first, then /.auth/me fallback
        try:
            # Try to parse headers (fast path)
            ...
            # Cache in session state for subsequent WebSocket requests
            st.session_state.auth_user_info = user_info
            return user_info

        except RuntimeError as e:
            # Headers not available (WebSocket request) - fall back to /.auth/me
            if "Missing X-MS-CLIENT-PRINCIPAL header" in str(e):
                return self._fetch_from_auth_me()
