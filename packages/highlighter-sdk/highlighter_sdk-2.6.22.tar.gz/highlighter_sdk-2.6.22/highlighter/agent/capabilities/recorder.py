...
class Recorder:
    ...
    def __next__(self) -> DataSample:
        ...
        if recording_active:
            ...
            # Duration threshold
            elif self._seconds_per_file is not None and self._buffer:
                # Only check duration if buffer is not empty
    ...
    def _start_worker(self):
        ...
        self._worker.start()
        self.logger.info(f"Started worker thread (restart count: {self._worker_restart_count})")
    ...
    def _check_worker_health(self):
        ...
        if self._worker_exception:
            self.logger.warning(f"Clearing previous worker exception: {self._worker_exception}")
        ...
        match self._record_mode:
            ...
            case RecordMode.CLOUD:
                ...
                try:
                    ...
                    # Handle specific cloud upload errors (like missing fileData)
                    error_msg = str(e)
                    if "missing required arguments: fileData" in error_msg:
                        try:
                            file_path = data_file.path_to_content_file
                        except (ValueError, AttributeError):
                            file_path = "<unknown path>"
                        self.logger.error(
                            f"Cloud upload failed for DataFile {data_file.file_id} - missing fileData. "
                            f"File may be empty or corrupted: {file_path}"
                        )
                        ...
                        # Re-raise for other types of ValueError
                        raise
                ...
                try:
                    ...
                    if "missing required arguments: fileData" in error_msg:
                        ...
                        self.logger.error(
                            f"Cloud upload failed for DataFile {data_file.file_id} - missing fileData. "
                            f"File may be empty or corrupted: {file_path}. "
                            f"Local file preserved."
                        )
                        # Keep local file and continue processing
                    else:
                        self.logger.error(
                            f"Cloud upload failed for DataFile {data_file.file_id}: {error_msg}"
                        )
