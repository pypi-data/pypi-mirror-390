...
class TGimport:
    ...
    def prepare_imex(
        self, filenames: list[str], get_uris: bool = False, xpath: str = ''
    ) -> ImportStatus:
        """Prepare an imex file for given aggregation(s).

        Args:
            filenames (list[str]): list of files on disk.
            get_uris (bool): wether to get textgrid uris for found objects
            xpath (str): xpath to extraxt textgrid URI from TEI
        ...
        if len(filenames) > 0:
            self.collect_filenames_to_imex(filenames, get_uris, xpath)
    ...
    def upload(self, filenames: list[str], threaded: bool = False) -> ImportStatus:
        ...
        self.prepare_imex(filenames, True)
    ...
    def collect_filenames_to_imex(
        self, filenames: list[str], get_uris: bool = True, xpath: str = ''
    ) -> str:
    ...
    def get_uri_from_tei(self, xpath: str):
        ...
        for file in self._new_objects:
            if file.name.endswith('.xml'):
                ...
                uri_elem = tei_root.find(xpath, TEI_NAMESPACES)
                if uri_elem.text:
                    uri = uri_elem.text
