...
pub struct PenalizedLogisticRegressionCore {
    #[serde(flatten)]
    pub base: BaseModel,
    pub penalties: Option<Vec<PenaltySpecification>>
}
...
impl PenalizedLogisticRegressionCore {
    ...
    pub fn fit(
        ...
        let penalty_matrix: Array2<f64> = self.make_penalty_matrix(&self.penalties, &x_col_names)?;
        ...
        while i <= max_iterations && delta > convergence_delta {
            ...
            let log_likelihood_new: f64 = ndarray::Zip::from(&y_array_view)
                ...
                .map_collect(|&y, &p| {
                    ...
                    let likelihood: f64 = y * p_clipped.ln() + (1.0 - y) * (1.0 - p_clipped).ln();
                    let penalty: f64 = 0.0;
                    // let penalty: f64 = ndarray::Zip::from(&beta_new).and()
                    likelihood - penalty
                })
                .sum();
...
impl PenalizedLogisticRegressionCore { // Non-python methods
    pub fn make_penalty_matrix(
        &self,
        penalties: &Option<Vec<PenaltySpecification>>,
        x_col_names: &Vec<String>
    ) -> Result<Array2<f64>, PenalizedLogisticRegressionError> {
        let n_features: usize = x_col_names.len();
        let mut penalty_values: Array1<f64> = Array1::zeros(n_features);

        if let Some(penalty_specs) = penalties {
            for (i, col_name) in x_col_names.iter().enumerate() {
                ...
                penalty_values[i] = best_spec.lambda_param;
            }
        }

        Ok(Array2::from_diag(&penalty_values))
    }
}
