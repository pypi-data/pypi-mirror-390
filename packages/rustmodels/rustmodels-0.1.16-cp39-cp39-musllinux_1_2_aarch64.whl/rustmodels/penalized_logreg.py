...
from ._rustmodels import PenalizedLogisticRegressionCore, PenaltySpecification, EncodingType
import polars as pl
import numpy as np
...
class PenalizedLogisticRegression:
    """
    A class meant to hold the results and metadata of a penalized logistic regression.
    ...
    Methods:
        ...
        load_from_file(filepath: str): A method that will load a PenalizedLogisticRegression object from a JSON file.
    """

    def __init__(self):
        self._model = PenalizedLogisticRegressionCore()
    ...
    def fit(
        ...
        level_encoding_type: EncodingType = EncodingType.OneHot
    ) -> None:
        """
        Method to fit a penalized logistic regression model.
        ...
        Args:
            ...
            penalty_specs (Optional[list[PenaltySpecification]]): An optional list of PenaltySpecification objects 
                used to fit the model. This is what differentiates it from a traditional LogReg.
        ...
        model_matrix = mm.get_model_matrix(df, parsed_formula, level_encoding_type=level_encoding_type)

        # Ensure that y is binary
        _validate_y_for_logistic(model_matrix.y_matrix)
        
        # Matrix math
        model_instance = PenalizedLogisticRegressionCore()
    ...
    def save_to_file(self, filepath: str) -> None:
        """
        A method that will save a PenalizedLogisticRegression object to a JSON file. This file will then
    ...
    def load_from_file(self, filepath: str) -> None:
        """
        A method that will load a PenalizedLogisticRegression object from a JSON file.
...
@validate_call(config=ConfigDict(arbitrary_types_allowed=True))
def fit_penalized_logistic_regression(
    formula: str, 
    df: pl.DataFrame, 
    penalty_specs: list[PenaltySpecification] | None = None,
    level_encoding_type: EncodingType = EncodingType.OneHot
    ...
    """
    Function that performs a penalized logistic regression fit.
    ...
    Returns:
        PenalizedLogisticRegression: A 'PenalizedLogisticRegression' object containing the regression results.
    """
    model_instance = PenalizedLogisticRegression()
    model_instance.fit(formula, df, penalty_specs=penalty_specs, level_encoding_type=level_encoding_type)
...
def _validate_y_for_logistic(y_matrix: NDArray[np.float64] | None) -> None:
    ...
    if not is_binary:
        raise mm.EncodingError(
            "For penalized logistic regression, the dependent variable must be binary (0s and 1s)."
        )
