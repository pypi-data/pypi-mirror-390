...
def start_hsta_job(node_index, nic_idx, ls_in_wh):
    ...
    if enable_valgrind and node_index == 0:
        args = ["valgrind", "--track-origins=yes", "--leak-check=yes", str(hsta_binary)]
    elif enable_perf and node_index == 0:
        args = ["perf", "record", "-F", "99", "-g", "--user-callchains", "--all-user", "-v", str(hsta_binary)]
...
def send_startup_info(la_channels_info, this_node_idx, nic_idx, hsta_handle):
    ...
    for node_idx in range(num_nodes):
        ...
        ip_addr = la_channels_info.nodes_desc[node_idx_str].ip_addrs[nic_idx].encode()
        ip_addr_len = len(ip_addr)

        LOGGER.debug(f"{node_idx=}: {host_id=}, {ip_addr=}, {ip_addr_len=}, {fabric_ep_addrs_available=}")
...
def hsta(node_index: str, ch_in_sdesc: bytes):
    ...
    if num_nodes > 1:
        ...
        num_nics = int(num_gws / dfacts.NUM_GW_TYPES)

        for nic_idx in range(num_nics):
            handle, pid = start_hsta_job(int(node_index), nic_idx, ls_in_wh)
            procs.append((handle, pid))

            # Start logging thread
            log_thread = threading.Thread(target=logger_thread, args=(handle,))
            log_thread.start()

            send_startup_info(la_channels_info, int(node_index), nic_idx, handle)
...
def main(args=None):
    ...
    from ...utils import B64, set_procname, strtobool
