...
from bear_dereth.query import where_map
...
def test_wal_update_recovery_single_record(tmp_path: Path):
    ...
    """
    db_path = tmp_path / "update_bug.json"
    wal_dir = tmp_path
    ...
    assert len(users) == 5
    user1 = users.get(id=1).first()
    user2 = users.get(id=2).first()
    user3 = users.get(id=3).first()
    user4 = users.get(id=4).first()
    user5 = users.get(id=5).first()
