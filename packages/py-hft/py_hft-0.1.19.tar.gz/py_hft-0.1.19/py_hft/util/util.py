...
@timer
def MPRun(func, args, dicts={}, n_pool= max(multiprocessing.cpu_count()-2, 1), verbose = False):
  ...
  res = []
  pbar = tqdm(total=len(args), desc=ColorString('MPRun %s'%(func.__name__), color='blue'), colour="cyan", disable = verbose)
