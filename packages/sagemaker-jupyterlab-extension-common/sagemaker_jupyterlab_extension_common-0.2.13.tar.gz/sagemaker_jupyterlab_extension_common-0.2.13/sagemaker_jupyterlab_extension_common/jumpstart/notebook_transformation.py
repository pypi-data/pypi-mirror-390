...
def _get_substitute_cell(
    ...
    # For hyperpod start-job pass the extracted recipe path
        ...
        if model_name is None:
            raise ValueError(
                "model_name cannot be None for openSourceTrainingJobNotebookHeaderMarkdown alteration"
            )
        current_cell["source"] = [
        ...
        if model_name is None:
            raise ValueError(
                "model_name cannot be None for openSourceHyperpodNotebookHeaderMarkdown alteration"
            )
        current_cell["source"] = [
...
class OpenSourceNotebook(Notebook):
    def transform(
        ...
        # Serverless SMTJ mode
            ...
            if not (
                baseModelName
                and customizationTechnique
                and modelPackageGroupName
                and dataSetName
                and dataSetVersion
            ):
                raise ValueError(
                    "Must provide either HyperPod parameters (clusterId + recipePath) or "
                    "Serverless SMTJ parameters (baseModelName + customizationTechnique + modelPackageGroupName + dataSetName + dataSetVersion)"
                )
    ...
    def _transform_serverless_smtj(
        ...
        alterations = [
            JumpStartModelNotebookAlterationType.openSourceTrainingJobNotebookHeaderMarkdown,
            JumpStartModelNotebookAlterationType.trainerSelection,
        ]

        # Serverless substitutions
        substitutions = [
            JumpStartModelNotebookSubstitution(
                JumpStartModelNotebookSubstitutionTarget.baseModelName,
                f'base_model_name = "{baseModelName}"',
                True,
            ),
            JumpStartModelNotebookSubstitution(
