...
def test_base_model_name_substitution(basic_notebook_content):
    ...
    options = UpdateHubNotebookUpdateOptions(substitutions, [], [])
    basic_notebook_content["cells"][0]["source"] = ['base_model_name = ""']
    content_str = json.dumps(basic_notebook_content)
    result = json.loads(update_notebook(content_str, None, options))
    assert 'base_model_name = "llama-3-1-8b"' == result["cells"][0]["source"][0]
...
def test_dataset_arn_substitution(basic_notebook_content):
    ...
    substitutions = [
        JumpStartModelNotebookSubstitution(
            ...
            'dataset_arn = "arn:aws:sagemaker:us-west-2:123456789012:hub-content/AIRegistry/DataSet/my-dataset/v1"',
            True,
        )
    ]
    options = UpdateHubNotebookUpdateOptions(substitutions, [], [])
    basic_notebook_content["cells"][0]["source"] = ['dataset_arn = ""']
    content_str = json.dumps(basic_notebook_content)
    result = json.loads(update_notebook(content_str, None, options))
    assert (
        'dataset_arn = "arn:aws:sagemaker:us-west-2:123456789012:hub-content/AIRegistry/DataSet/my-dataset/v1"'
...
def test_open_source_notebook_serverless_e2e_transformation(mock_aws_account):
    ...
    assert (
        'base_model_name = "meta-llama-3-2-1b-instruct"'
    ...
    assert (
        'dataset_arn = "arn:aws:sagemaker:us-west-2:123456789012:hub-content/AIRegistry/DataSet/dpo-preference-dataset/2.1"'
